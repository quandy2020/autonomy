...
bool OnnxEngine::LoadFromMemory(const void* model_data, size_t size) {
    ...
    try {
        impl_->session =
            Ort::Session(impl_->env, model_data, size, impl_->options);
...
static bool ConcretizeShape(std::vector<int64_t>* shape, size_t data_size) {
    ...
    int64_t inferred =
        static_cast<int64_t>(data_size / static_cast<size_t>(product));
    for (int64_t& s : *shape) {
        if (s == -1) {
            s = inferred;
            break;
...
bool OnnxEngine::Infer(
    ...
    for (size_t i = 0; i < impl_->input_names.size(); ++i) {
        ...
        if (expected != 0 && data.size() != expected) {
            ...
            oss << "Input \"" << name << "\" size mismatch: got " << data.size()
                << ", expected " << expected;
            last_error_ = oss.str();
            AERROR << last_error_;
        ...
        if (shape_copy.empty() && !data.empty()) {
            ...
            if (!ConcretizeShape(&shape_copy, data.size())) {
                last_error_ = "Input \"" + name +
                              "\": cannot infer dynamic shape from size " +
    ...
    for (size_t i = 0; i < input_storage.size(); ++i) {
        Ort::Value value = Ort::Value::CreateTensor<float>(
            memory_info, input_storage[i].data(), input_storage[i].size(),
    ...
    try {
        ...
        auto output_values = impl_->session.Run(
            run_options, input_names_ptr.data(), input_values.data(),
            input_names_ptr.size(), output_names_ptr.data(),
        ...
        for (size_t i = 0; i < output_values.size(); ++i) {
            ...
            if (elem_type != ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT) {
                last_error_ =
                    "Unsupported output element type (only float32 supported "
                    "for Infer)";
                return false;
            }
...
bool OnnxEngine::Infer(const std::vector<std::string>& input_names,
