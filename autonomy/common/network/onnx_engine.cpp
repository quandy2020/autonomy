...
void OnnxEngine::ImplDeleter::operator()(Impl* p) const { delete p; }
...
bool OnnxEngine::IsLoaded() const { return impl_ != nullptr && impl_->session != nullptr; }

size_t OnnxEngine::GetInputCount() const { return impl_ ? impl_->input_names.size() : 0; }

size_t OnnxEngine::GetOutputCount() const { return impl_ ? impl_->output_names.size() : 0; }
...
bool OnnxEngine::Infer(const std::unordered_map<std::string, std::vector<float>>& inputs,
  ...
  for (size_t i = 0; i < impl_->input_names.size(); ++i) {
    ...
    if (shape_copy.empty() && !data.empty()) {
      ...
      if (!ConcretizeShape(&shape_copy, data.size())) {
        last_error_ = "Input \"" + name + "\": cannot infer dynamic shape from size " + std::to_string(data.size());
        return false;
      }
    }
  ...
  for (size_t i = 0; i < input_storage.size(); ++i) {
    Ort::Value value = Ort::Value::CreateTensor<float>(memory_info, input_storage[i].data(), input_storage[i].size(),
  ...
  try {
    ...
    auto output_values = impl_->session.Run(run_options, input_names_ptr.data(), input_values.data(),
                                            input_names_ptr.size(), output_names_ptr.data(), output_names_ptr.size());
