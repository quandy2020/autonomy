 ...
 *
 * Shared by @ref Preprocess and @ref RunPipeline. Policy enums and @ref
 * PreprocessOptions are part of the public API.
 */
...
struct Sample {
    ...
    std::vector<float>
        vector_features;      //!< Optional 1-D features for vector inputs
...
enum class ResizePolicy {
    ...
    kUpperBound,  //!< Scale longest side to bound_resize_target, then fit to
                  //!< model H×W
};
...
/**
 * @brief Geometry metadata for mapping network coordinates back to the source
 * image
 */
struct TransformMeta {
    ResizePolicy resize_policy =
        ResizePolicy::kLetterbox;  //!< Resize mode used in preprocess
    double scale_gain =
        1.0;  //!< Uniform scale from source to letterbox/crop space
...
struct PreprocessOptions {
    ...
    NormalizePolicy normalize =
        NormalizePolicy::kZeroOne;       //!< Pixel normalization policy
    ...
    bool swap_red_blue =
        true;             //!< If true, swap BGR to RGB before mean subtraction
    ...
    int bound_resize_target =
        0;  //!< Longest-side target for kUpperBound (0 = use model H/W)
    int align_multiple =
        0;  //!< If >0, round H/W up to this multiple (e.g. patch 14)
};

/**
 * @brief Parsed spatial size and layout from a 4-D or 5-D image input
 * descriptor
 */
