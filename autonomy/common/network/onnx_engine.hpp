...
class OnnxEngine {
 public:
  ...
  // Non-copyable, movable (defined in .cpp so Impl stays incomplete in headers).
   ...
   * @return true on success; false on failure (e.g. file not found, invalid ONNX).
   */
   ...
   * @param outputs On success, same order as output_names, each a vector of float.
   * @return true on success; false on failure.
   */
  bool Infer(const std::vector<std::string>& input_names, const std::vector<std::vector<float>>& input_data,
