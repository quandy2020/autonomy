...
#ifdef AUTONOMY_BUILD_NETWORK_TENSORRT
#include "autonomy/common/network/tensorrt.hpp"
#endif

#ifdef AUTONOMY_BUILD_NETWORK_RKNN
...
#ifdef AUTONOMY_BUILD_NETWORK_RKNN
template <>
struct NetworkBackendTraits<RknnBackend> {
    static constexpr const char* kId = "rknn";
};
#endif
...
#endif
#ifdef AUTONOMY_BUILD_NETWORK_RKNN
    ok = RegisterNetworkBackend<RknnBackend>(factory) && ok;
    if (!ok) {
        LOG(ERROR) << "Failed to register network backend \"rknn\".";
    }
#endif
    return ok;
}
