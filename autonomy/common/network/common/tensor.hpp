 ...
 *
 * - @ref ElementType — element type enum (aligned with ONNX / TensorRT metadata)
 ...
 * - Preprocess emits float32; backends pass FP16 / BF16 / INT8 unchanged when matched
 * - For postprocess float views, prefer @ref Tensor::TryViewFloat32, else @ref Tensor::ToFloat32
 */
 ...
 *
 * Holds raw bytes in `std::vector<uint8_t>`; does not encode layout beyond element type.
 * Shape validation uses @ref ResolveShapeForElementCount and backends at Run time.
 */
class Tensor {
public:
    ...
    uint8_t* mutable_bytes() { return bytes_.data(); }

    /** @return Mutable underlying storage */
    std::vector<uint8_t>& mutable_storage() { return bytes_; }

    /** @return Const underlying storage */
    const std::vector<uint8_t>& storage() const { return bytes_; }
     ...
     * @param element_type Element type
     * @param bytes Buffer; length must be a multiple of @ref ElementTypeByteSize
     */
    static Tensor FromBytes(ElementType element_type, std::vector<uint8_t> bytes);
     ...
     */
    bool TryViewFloat32(const float** data, size_t* count, std::string* error = nullptr) const;
 ...
 *
 * Dynamic axes may appear as <= 0 placeholders; @ref ProductPositiveDims may be 0 then.
 */
class TensorShape {
public:
    TensorShape() = default;
    ...
    std::vector<int64_t>& MutableDims() { return dims_; }
 ...
 *
 * Returned by @ref Engine::GetInputInfos / GetOutputInfos for preprocess and postprocess.
 */
struct ModelTensorInfo {
    std::string name;                              //!< ONNX/TRT tensor name
    TensorShape shape;                             //!< Shape (may include dynamic dims)
    ElementType element_type = ElementType::kFloat32;  //!< Declared element type
};
...
/**
 * @brief Validates element count against static shape and fills resolved dimensions
 *
 * When the model has dynamic axes but the buffer size is fixed, unknown dims are
 * inferred from @p element_count.
 *
 ...
 */
bool FromFloatTensorMap(const FloatTensorMap& inputs, TensorMap* out, std::string* error = nullptr);

/**
 * @brief Converts @ref TensorMap to float32 convenience map (per-tensor conversion)
 *
 * @param outputs Backend typed outputs
 * @param[out] out Float map
 * @param[out] error Message when an output cannot be converted
 */
bool ToFloatTensorMap(const TensorMap& outputs, FloatTensorMap* out, std::string* error = nullptr);
 ...
 * Quantized INT8 models should use @ref Sample::named_tensors with calibrated values.
 *
