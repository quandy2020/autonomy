...
/**
 * @class autonomy::common::SimpleActionServer
...
class SimpleActionServer {
 public:
   ...
   */
  AUTONOMY_SMART_PTR_DEFINITIONS(SimpleActionServer)
   ...
   */
  using GoalCallback = std::function<autolink::action::GoalResponse(const autolink::action::GoalUUID&,
   ...
   */
  explicit SimpleActionServer(std::shared_ptr<autolink::Node> node, const std::string& action_name,
    ...
    action_server_ = autolink::action::CreateServer<ActionT>(
        node_, action_name_,
        std::bind(&SimpleActionServer::HandleGoal, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&SimpleActionServer::HandleCancel, this, std::placeholders::_1),
        std::bind(&SimpleActionServer::HandleAccepted, this, std::placeholders::_1));
  }
   ...
   */
  autolink::action::GoalResponse HandleGoal(const autolink::action::GoalUUID& uuid,
   ...
   */
  void SetGoalCallback(GoalCallback cb) { goal_callback_ = std::move(cb); }
  ...
  autolink::action::CancelResponse HandleCancel(
      const std::shared_ptr<autolink::action::ServerGoalHandle<ActionT>> handle) {
    std::lock_guard<std::recursive_mutex> lock(update_mutex_);
   ...
   */
  void HandleAccepted(const std::shared_ptr<autolink::action::ServerGoalHandle<ActionT>> handle) {
    std::lock_guard<std::recursive_mutex> lock(update_mutex_);
   ...
   */
  void Work() {
    while (autolink::OK() && !stop_execution_ && IsActive(current_handle_)) {
   ...
   */
  void TerminateAll(std::shared_ptr<typename ActionT::Result> result = std::make_shared<typename ActionT::Result>()) {
    std::lock_guard<std::recursive_mutex> lock(update_mutex_);
 ...
 protected:
  ...
  std::shared_ptr<autolink::action::ServerGoalHandle<ActionT>> current_handle_;
  std::shared_ptr<autolink::action::ServerGoalHandle<ActionT>> pending_handle_;
   ...
   */
  constexpr auto EmptyResult() const { return std::make_shared<typename ActionT::Result>(); }
   ...
   */
  bool IsActive(const std::shared_ptr<autolink::action::ServerGoalHandle<ActionT>> handle) const {
  ...
  void LogErrorDetailsIfAvailable(const T& result) {
    if constexpr (has_error_code<typename ActionT::Result>::value && has_error_msg<typename ActionT::Result>::value) {
      WarnMsg("Aborting handle. error_code:" + std::to_string(result->error_code) + ", error_msg:'" +
              result->error_msg + "'.");
    } else if constexpr (has_error_code<typename ActionT::Result>::value) {
      WarnMsg("Aborting handle. error_code:" + std::to_string(result->error_code) + ".");
    } else {
   ...
   */
  void Terminate(std::shared_ptr<autolink::action::ServerGoalHandle<ActionT>>& handle,
                 std::shared_ptr<typename ActionT::Result> result = std::make_shared<typename ActionT::Result>()) {
    if (!handle) {
      return;
    }
   ...
   */
  void InfoMsg(const std::string& msg) const { AINFO << action_name_ << "[ActionServer] " << msg; }

  /**
   * @brief Debug logging
   * @param msg The message to log
   */
  void DebugMsg(const std::string& msg) const { ADEBUG << action_name_ << "[ActionServer] " << msg; }

  /**
   * @brief Error logging
   * @param msg The message to log
   */
  void ErrorMsg(const std::string& msg) const { AERROR << action_name_ << "[ActionServer] " << msg; }

  /**
   * @brief Warn logging
   * @param msg The message to log
   */
  void WarnMsg(const std::string& msg) const { AWARN << action_name_ << "[ActionServer] " << msg; }
};
