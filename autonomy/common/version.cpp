...
// 256色模式（扩展颜色）
#define COLOR_256_INFO(text, code) \
    "\033[38;5;" << code << "m" << text << "\033[0m"
#define BG_COLOR_256_INFO(text, code) \
    "\033[48;5;" << code << "m" << text << "\033[0m"

// RGB颜色（真彩色，如果终端支持）
#define RGB_INFO(text, r, g, b) \
    "\033[38;2;" << r << ";" << g << ";" << b << "m" << text << "\033[0m"
#define BG_RGB_INFO(text, r, g, b) \
...
const std::string GIT_COMMIT_DATE =
    ...
    "2026-03-29 23:08:24"
    "";
const std::string GIT_VERSION = "5d16d49-dirty";
const std::string GIT_BRANCH = "main";

// 构建信息
const std::string BUILD_TIMESTAMP = "2026-03-29 15:14:43";
...
#endif
    return absl::StrCat("Autonomy develop info: Commit ", AUTONOMY_COMMIT_ID,
                        " on ", AUTONOMY_COMMIT_DATE, " ", cuda_info);
}

std::string GetGitCommitID() {
    return absl::StrCat("Autonomy git commit ID: ", AUTONOMY_COMMIT_ID, " on ",
                        AUTONOMY_COMMIT_DATE);
}
...
void ShowVersion() {
    ...
    LOG(INFO) << GREEN_INFO(
        absl::StrCat("  [ version ]: ", common::GetVersionInfo().c_str()));
    LOG(INFO) << GREEN_INFO(
        absl::StrCat("  [ develop ]: ", common::GetGitCommitID().c_str()));
    LOG(INFO) << GREEN_INFO(
        absl::StrCat("  [ cuda    ]: ", common::GetCudaInfo().c_str()));
    LOG(INFO) << GREEN_INFO("  [ contact ]: "
                            << "quandy2020@126.com");
