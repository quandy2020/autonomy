...
#include "autonomy/common/math/mpc_osqp.hpp"
...
MpcOsqp::MpcOsqp(const Eigen::MatrixXd& matrix_a, const Eigen::MatrixXd& matrix_b, const Eigen::MatrixXd& matrix_q,
    ...
    DLOG(INFO) << "state_dim" << state_dim_;
    DLOG(INFO) << "control_dim_" << control_dim_;
    num_param_ = state_dim_ * (horizon_ + 1) + control_dim_ * horizon_;
}

void MpcOsqp::CalculateKernel(std::vector<c_float>* P_data, std::vector<c_int>* P_indices,
...
void MpcOsqp::CalculateGradient() {
    ...
    DLOG(INFO) << "Gradient_mat";
    DLOG(INFO) << gradient_;
}

// equality constraints x(k+1) = A*x(k)
void MpcOsqp::CalculateEqualityConstraint(std::vector<c_float>* A_data, std::vector<c_int>* A_indices,
    ...
    DLOG(INFO) << "matrix_constraint with A";
    DLOG(INFO) << matrix_constraint;
    ...
    DLOG(INFO) << "matrix_constraint with B";
    DLOG(INFO) << matrix_constraint;
    ...
    DLOG(INFO) << "matrix_constraint with I";
    DLOG(INFO) << matrix_constraint;
...
bool MpcOsqp::Solve(std::vector<double>* control_cmd) {
    DLOG(INFO) << "Before Calc Gradient";
    CalculateGradient();
    DLOG(INFO) << "After Calc Gradient";
    CalculateConstraintVectors();
    DLOG(INFO) << "MPC2Matrix";

    OSQPData* data = Data();
    DLOG(INFO) << "OSQP data done";
    DLOG(INFO) << "OSQP data n" << data->n;
    DLOG(INFO) << "OSQP data m" << data->m;
    for (int i = 0; i < data->n; ++i) {
        DLOG(INFO) << "OSQP data q" << i << ":" << (data->q)[i];
    }
    DLOG(INFO) << "OSQP data l" << data->l;
    for (int i = 0; i < data->m; ++i) {
        DLOG(INFO) << "OSQP data l" << i << ":" << (data->l)[i];
    }
    DLOG(INFO) << "OSQP data u" << data->u;
    for (int i = 0; i < data->m; ++i) {
        DLOG(INFO) << "OSQP data u" << i << ":" << (data->u)[i];
    }

    OSQPSettings* settings = Settings();
    DLOG(INFO) << "OSQP setting done";
    OSQPWorkspace* osqp_workspace = nullptr;
    c_int exitflag = osqp_setup(&osqp_workspace, data, settings);
    if (exitflag != 0) {
        LOG(ERROR) << "OSQP setup failed with exit flag: " << exitflag;
        FreeData(data);
        c_free(settings);
        return false;
    }
    DLOG(INFO) << "OSQP workspace ready";
    ...
    if (status < 0 || (status != 1 && status != 2)) {
        LOG(ERROR) << "failed optimization status:\t" << osqp_workspace->info->status;
    ...
    for (size_t i = 0; i < control_dim_; ++i) {
        ...
        DLOG(INFO) << "control_cmd:" << i << ":" << control_cmd->at(i);
    }
