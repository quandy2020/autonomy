...
class StateEstimationInterface : public common::helper_functions::crtp<Derived>
{
public:
    ///
    ...
    ///
    /// @brief      Reset the internal state of the vector to the given state
    /// and covariance.
    ///
    /// @param[in]  state       The new state
    /// @param[in]  covariance  The new covariance
    ///
    /// @tparam     StateT      The state type. It must match the one already
    /// stored in the filter.
    ///
    template <typename StateT>
    void reset(const StateT& state, const typename StateT::Matrix& covariance) {
        static_assert(std::is_same<typename Derived::State, StateT>::value,
                      "\n\nProvided type StateT must match the filter "
