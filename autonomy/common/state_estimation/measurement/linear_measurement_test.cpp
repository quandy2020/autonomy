...
TEST(TestLinearMeasurement, Create) {
    ...
    auto expected_covariance =
        (MeasurementState::Matrix{} << 9.0F, 0.0F, 0.0F, 16.0F).finished();
    EXPECT_TRUE(expected_covariance.isApprox(measurement.covariance()))
    ...
    measurement =
        LinearMeasurement<MeasurementState>{{1.0F, 2.0F}, expected_covariance};
    EXPECT_TRUE(expected_covariance.isApprox(measurement.covariance()))
        << measurement.covariance();
}
...
TEST(TestLinearMeasurement, CreateAndMapFromOtherState) {
    ...
    const auto expected_matrix = (Eigen::Matrix<float, 2, 4>{} << 1.0F, 0.0F,
                                  0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F)
                                     .finished();
    EXPECT_TRUE(mapping_matrix.isApprox(expected_matrix));
    State state{{1.0F, 2.0F, 3.0F, 4.0F}};
    const auto measurement_mapped_state =
        measurement.create_new_instance_from(state);
