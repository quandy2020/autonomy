...
TEST(TestLinearMeasurement, Create) {
    ...
    // When the standard deviation constructor is used, the covariance matrix is
    // diagonal
    auto measurement = LinearMeasurement<MeasurementState>::create_with_stddev({1.0F, 2.0F}, {3.0F, 4.0F});
    EXPECT_EQ((MeasurementState{{1.0F, 2.0F}}), measurement.state());
    auto expected_covariance = (MeasurementState::Matrix{} << 9.0F, 0.0F, 0.0F, 16.0F).finished();
    EXPECT_TRUE(expected_covariance.isApprox(measurement.covariance())) << measurement.covariance();
    // Covariance is preserved when the covariance constructor is used,
...
TEST(TestLinearMeasurement, CreateAndMapFromOtherState) {
    ...
    using MeasurementState = FloatState<X, Y>;
    auto measurement = LinearMeasurement<MeasurementState>::create_with_stddev({42.0F, 23.0F}, {23.0F, 42.0F});
    ...
    const auto expected_matrix =
        (Eigen::Matrix<float, 2, 4>{} << 1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F).finished();
...
TEST(TestLinearMeasurement, Equality) {
    using MeasurementState = FloatState<X, Y>;
    auto measurement_1 = LinearMeasurement<MeasurementState>::create_with_stddev({42.42F, 23.23F}, {23.42F, 42.23F});
    auto measurement_2 = LinearMeasurement<MeasurementState>::create_with_stddev({42.42F, 23.23F}, {23.42F, 42.23F});
