...
#ifndef AUTONOMY_COMMON_STATE_ESTIMATION_MEASUREMENT_LINEAR_MEASUREMENT_HPP_
#define AUTONOMY_COMMON_STATE_ESTIMATION_MEASUREMENT_LINEAR_MEASUREMENT_HPP_
...
///
/// @details    A linear measurement is one that measures directly variables
/// from some unknown real
///             state, thus requiring no measurement Jacobian computation. The
///             mapping matrix will only have 1s and 0s in it.
///
...
public:
    ...
    ///
    /// @brief      Construct a new instance of the measurement from a state
    /// vector.
    ///
    ...
    ///             deviations.
    ///
    /// @details    This results in a diagonal covariance matrix, with the
    /// squared standard
    ///             deviations as entries. In a diagonal covariance matrix, the
...
protected:
    ...
    /// @brief      Get state vector function that is to be called by the CRTP
    /// interface.
    inline StateT& crtp_state() noexcept {
        return m_measurement;
    }
    /// @brief      Get state vector function that is to be called by the CRTP
    /// interface.
    inline const StateT& crtp_state() const noexcept {
        return m_measurement;
    }
    /// @brief      Get covariance function that is to be called by the CRTP
    /// interface.
    inline typename StateT::Matrix& crtp_covariance() noexcept {
        return m_covariance;
    }
    /// @brief      Get covariance function that is to be called by the CRTP
    /// interface.
    inline const typename StateT::Matrix& crtp_covariance() const noexcept {
        return m_covariance;
    }
    ///
    /// @brief      Create a new instance of the measurement from another state.
    ///
    /// @details    This is a linear measurement, i.e., it directly observes the
    /// variables of another
    ///             state, thus this mapping function is linear and just copied
    ///             values for the matching variables in both states.
    ///
    /// @note       It is expected that the measurement StateT is a direct
    /// sub-state of the
    ///             OtherStateT.
    ///
    /// @param[in]  other_state  The other state
    ///
    /// @tparam     OtherStateT  State to which we map the internal state
    /// StateT.
    ///
    /// @return     A vector that represents a mapping of another state to this
    /// measurement's frame.
    ///
    template <typename OtherStateT>
    ...
    ///
    /// @note       Values present in other_state but not in this measurement
    /// will stay intact.
    ///
    /// @param[in]  other_state  The other state
    ///
    ...
    ///
    /// @brief      Get the mapping matrix between the two states called by the
    /// CRTP interface.
    ///
    /// @details    As this is a linear measurement (one that directly measures
    /// variables of some
    ///             state) this function only fills the entries of the mapping
    ///             matrix with 1s and 0s.
    ///
    /// @note       The OtherStateT input parameter is not used in this
    /// implementation. It is required
    ///             for other implementation that must map into the measurement
    ///             state space in a non-linear way.
    ///
    /// @tparam     OtherStateT  State to which we map the internal state
    /// StateT.
    ///
...
#endif  // AUTONOMY_COMMON_STATE_ESTIMATION_MEASUREMENT_LINEAR_MEASUREMENT_HPP_
