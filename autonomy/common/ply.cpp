...
std::vector<PlyPoint> ReadPly(const std::string& path) {
  ...
  while (std::getline(file, line)) {
    ...
    if (line_elems.size() >= 3 && line_elems[0] == "property") {
      ...
      } else if (line == "property uchar r" || line == "property uchar red" || line == "property uchar diffuse_red" ||
                 line == "property uchar ambient_red" || line == "property uchar specular_red") {
        R_index = index;
      ...
      } else if (line == "property uchar b" || line == "property uchar blue" || line == "property uchar diffuse_blue" ||
                 line == "property uchar ambient_blue" || line == "property uchar specular_blue") {
        B_index = index;
  ...
  THROW_CHECK(X_index != -1 && Y_index != -1 && Z_index != -1) << "Invalid PLY file format: x, y, z properties missing";
...
void WriteTextPlyPoints(const std::string& path, const std::vector<PlyPoint>& points, const bool write_normal,
  ...
  for (const auto& point : points) {
    ...
    if (write_rgb) {
      file << " " << static_cast<int>(point.r) << " " << static_cast<int>(point.g) << " " << static_cast<int>(point.b);
    }
