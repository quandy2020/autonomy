...
class HeterEventQueueBase
    : public HeterEventDispatcherBase<EventType_, PrototypeList_, Policies_,
                                      HeterEventQueueBase<EventType_, PrototypeList_, Policies_> > {
 private:
  using super = HeterEventDispatcherBase<EventType_, PrototypeList_, Policies_,
                                         HeterEventQueueBase<EventType_, PrototypeList_, Policies_> >;
  ...
  using QueuedItemSizer =
      QueuedItem<std::tuple<typename std::remove_cv<typename std::remove_reference<Args>::type>::type...> >;
 ...
 public:
  ...
  HeterEventQueueBase(HeterEventQueueBase&& other) noexcept : super(std::move(other)) {}
 ...
 private:
  ...
  void doDispatchQueuedEvent(const QueuedItemBase& item) { item.dispatcher(this, item); }
  ...
  static void doDispatchItem(const HeterEventQueueBase* self, const QueuedItemBase& baseItem) {
    ...
    self->doDispatchQueuedItem<PrototypeInfo>(
        item, typename MakeIndexSequence<std::tuple_size<typename PrototypeInfo::ArgsTuple>::value>::Type());
  }
  ...
  auto doProcessIf(F&& func) -> typename std::enable_if<(PrototypeInfo::index >= 0), bool>::type {
    BufferedItemList tempList;
    ...
    if (!tempList.empty()) {
      for (auto it = tempList.begin(); it != tempList.end();) {
        ...
        if (doInvokeFuncWithQueuedEvent(func, item,
  ...
  auto doEnqueue(T&& first, Args&&... args) ->
    ...
    static_assert(std::tuple_size<typename PrototypeInfo::ArgsTuple>::value == 1 + sizeof...(Args),
                  "Arguments count mismatch.");

    doEnqueueItem(
        QueuedItemType(PrototypeInfo::index, GetEvent::getEvent(std::forward<T>(first), args...),
  ...
  auto doEnqueue(T&& first, Args&&... args) ->
    ...
    static_assert(PrototypeInfo::index >= 0, "Can't find invoker for the given argument types.");
    static_assert(std::tuple_size<typename PrototypeInfo::ArgsTuple>::value == sizeof...(Args),
                  "Arguments count mismatch.");

    doEnqueueItem(QueuedItemType(PrototypeInfo::index, GetEvent::getEvent(std::forward<T>(first), args...),
                                 &HeterEventQueueBase::doDispatchItem<PrototypeInfo>,
