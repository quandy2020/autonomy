...
class HeterEventQueueBase : public HeterEventDispatcherBase<EventType_, PrototypeList_, Policies_,
        ...
        if (!tempList.empty()) {
            for (auto it = tempList.begin(); it != tempList.end();) {
                ...
                if (doInvokeFuncWithQueuedEvent(
                        func, item, typename MakeIndexSequence<std::tuple_size<ArgsTuple>::value>::Type())) {
