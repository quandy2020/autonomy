...
#include "autonomy/common/eventpp/hetereventdispatcher.hpp"

// temp, should move the internal code in eventqueue to separated file
#include "autonomy/common/eventpp/eventqueue.hpp"
...
class HeterEventQueueBase
    : public HeterEventDispatcherBase<
          EventType_, PrototypeList_, Policies_,
          HeterEventQueueBase<EventType_, PrototypeList_, Policies_> >
{
private:
    ...
    using ItemDispatcher = void (*)(const HeterEventQueueBase*,
                                    const QueuedItemBase&);

    struct QueuedItemBase {
        QueuedItemBase(const int callableIndex, const EventType_& event,
                       const ItemDispatcher dispatcher)
    ...
    struct QueuedItem : public QueuedItemBase {
        QueuedItem(const int callableIndex, const EventType_& event,
                   const ItemDispatcher dispatcher, T&& arguments)
            : QueuedItemBase(callableIndex, event, dispatcher),
    ...
    using QueuedItemSizer = QueuedItem<std::tuple<typename std::remove_cv<
        typename std::remove_reference<Args>::type>::type...> >;

    using BufferedQueuedItem = BufferedUnion<
        GetCallablePrototypeMaxSize<PrototypeList_, QueuedItemSizer>::value>;
...
public:
    ...
    HeterEventQueueBase(const HeterEventQueueBase& other) : super(other) {}
    ...
    void enqueue(T&& first, Args&&... args) {
        doEnqueue<ArgumentPassingMode>(std::forward<T>(first),
                                       std::forward<Args>(args)...);
    }

    bool emptyQueue() const {
        return queueList.empty() &&
               (queueEmptyCounter.load(std::memory_order_acquire) == 0);
    }
    ...
    bool process() {
        if (!queueList.empty()) {
            ...
            CounterGuard<decltype(queueEmptyCounter)> counterGuard(
                queueEmptyCounter);
    ...
    bool processOne() {
        if (!queueList.empty()) {
...

            {
                std::lock_guard<Mutex> queueListLock(queueListMutex);
                if (!queueList.empty()) {
                    tempList.splice(tempList.end(), queueList,
                                    queueList.begin());
                }
            }
...
private:
    ...
    static void doDispatchItem(const HeterEventQueueBase* self,
                               const QueuedItemBase& baseItem) {
        const auto& item =
            static_cast<const QueuedItem<typename PrototypeInfo::ArgsTuple>&>(
                baseItem);
        self->doDispatchQueuedItem<PrototypeInfo>(
            item, typename MakeIndexSequence<std::tuple_size<
                      typename PrototypeInfo::ArgsTuple>::value>::Type());
    }
    ...
    auto doProcessIf(F&& func) ->
            ...
            for (auto it = tempList.begin(); it != tempList.end();) {
                ...
                if (doInvokeFuncWithQueuedEvent(
                        ...
                        typename MakeIndexSequence<
                            std::tuple_size<ArgsTuple>::value>::Type())) {
        ...
        using NextPrototypeInfo =
            FindPrototypeByCallableFromIndex<PrototypeInfo::index + 1,
                                             PrototypeList, F>;
    ...
    bool doInvokeFuncWithQueuedEvent(F&& func, T&& item,
                                     IndexSequence<Indexes...>) const {
        return doInvokeFuncWithQueuedEventHelper(
            std::forward<F>(func), std::get<Indexes>(item.arguments)...);
    }
    ...
    auto doEnqueue(T&& first, Args&&... args) -> typename std::enable_if<
        ...
        static_assert(PrototypeInfo::index >= 0,
                      "Can't find invoker for the given argument types.");
        static_assert(
            std::tuple_size<typename PrototypeInfo::ArgsTuple>::value ==
                1 + sizeof...(Args),
        ...
        doEnqueueItem(QueuedItemType(
            ...
            typename PrototypeInfo::ArgsTuple(std::forward<T>(first),
                                              std::forward<Args>(args)...)));
