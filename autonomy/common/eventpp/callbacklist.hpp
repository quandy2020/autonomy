...
class CallbackListBase<ReturnType(Args...), PoliciesType> {
 ...
 public:
  ...
  CallbackListBase(const CallbackListBase& other) : CallbackListBase() { cloneFrom(other.head); }

  CallbackListBase(CallbackListBase&& other) noexcept : CallbackListBase() { swap(other); }
  ...
  bool empty() const {
    ...
    // !head still works even when the underlying raw pointer is garbled
    // (for other thread is writting to head) And empty() doesn't guarantee
    // the list is still empty after the function returned.
  ...
  bool remove(const Handle& handle) {
    ...
    // but that doesn't work in multi-threading and cause related unit tests
    // fail.
    std::lock_guard<Mutex> lockGuard(mutex);
  ...
  void operator()(Args... args) const {
    forEachIf([&args...](Callback& callback) -> bool {
      ...
      // for arg that is passed by value, and the callback prototype
      // accepts it by value, std::forward will move it and may cause the
      // original value invalid. That happens on any value-to-value
      // passing, no matter the callback moves it or not.
  ...
  // This is a compromised patch for GCC 4, it may be not maintained or
  // updated unless there are bugs. We don't use the patch as main code
  // because the patch generates longer code, and duplicated with doForEachIf.
  void operator()(Args... args) const {
    NodePtr node;
 ...
 private:
  ...
  NodePtr doAllocateNode(const Callback& callback) { return std::make_shared<Node>(callback, getNextCounter()); }
  ...
  void doFreeNode(NodePtr& node) {
    ...
    // Mark it as deleted, this must be before the assignment of head and
    // tail below, because node can be a reference to head or tail, and
    // after the assignment, node can be null pointer.
...
class CallbackList : public internal_::CallbackListBase<Prototype_, Policies_>, public TagCallbackList {
 ...
 public:
  ...
  friend void swap(CallbackList& first, CallbackList& second) noexcept { first.swap(second); }
};
