...
struct HasTypeArgumentPassingMode {
    template <typename C>
    static std::true_type test(typename C::ArgumentPassingMode*);
    template <typename C>
...
struct SelectArgumentPassingMode {
    using Type = typename T::ArgumentPassingMode;
};
template <typename T, typename Default>
struct SelectArgumentPassingMode<T, false, Default> {
    using Type = Default;
};
...
struct HasTypeThreading {
    template <typename C>
    static std::true_type test(typename C::Threading*);
    template <typename C>
    static std::false_type test(...);
...
struct SelectThreading {
    using Type = typename T::Threading;
};
template <typename T>
struct SelectThreading<T, false> {
    using Type = MultipleThreading;
};
...
template <typename T, bool, typename D>
struct SelectCallback {
    using Type = typename T::Callback;
};
template <typename T, typename D>
struct SelectCallback<T, false, D> {
    using Type = D;
};
...
struct HasFunctionGetEvent {
    template <typename C>
    static std::true_type test(decltype(C::getEvent(std::declval<Args>()...))*);
    template <typename C>
    static std::false_type test(...);
...
struct SelectGetEvent {
    using Type = T;
};
template <typename T, typename Key>
struct SelectGetEvent<T, Key, false> {
    using Type = DefaultGetEvent<Key>;
};
...
template <typename T, bool>
struct SelectCanContinueInvoking {
    using Type = T;
};
template <typename T>
struct SelectCanContinueInvoking<T, false> {
    using Type = DefaultCanContinueInvoking;
};

template <typename T>
struct HasTemplateMap {
    template <typename C>
    static std::true_type test(typename C::template Map<int, int>*);
    template <typename C>
    static std::false_type test(...);
...
{
    template <typename C>
    static std::true_type test(decltype(std::hash<C>()(std::declval<C>()))*);
    template <typename C>
    static std::false_type test(...);
...
struct HasTypeMixins {
    template <typename C>
    static std::true_type test(typename C::Mixins*);
    template <typename C>
    static std::false_type test(...);
...
template <typename T, bool>
struct SelectMixins {
    using Type = typename T::Mixins;
};
template <typename T>
struct SelectMixins<T, false> {
