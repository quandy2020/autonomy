...
template <int N, typename RT, typename... Args, typename... Others,
          typename Callable, template <typename> class ArgTransformer, int M>
struct FindPrototypeByCallableFromIndex<
    N, HeterTuple<RT(Args...), Others...>, Callable, ArgTransformer, M,
    typename std::enable_if<(N < M)>::type> {
    enum {
        canInvoke =
            CanInvoke<Callable, typename ArgTransformer<Args>::type...>::value
    };

    using Prototype =
        typename std::conditional<canInvoke, RT(Args...),
                                  typename FindPrototypeByCallableFromIndex<
                                      N + 1, HeterTuple<Others...>, Callable,
    ...
    using ArgsTuple = typename std::conditional<
        ...
        typename FindPrototypeByCallableFromIndex<N + 1, HeterTuple<Others...>,
                                                  Callable, ArgTransformer,
                                                  M>::ArgsTuple>::type;
    ...
    enum {
        index =
            canInvoke
                ? N
                : FindPrototypeByCallableFromIndex<N + 1, HeterTuple<Others...>,
                                                   Callable, ArgTransformer,
                                                   M>::index
    };
};
...
struct FindPrototypeByCallableFromIndex<
    N, PrototypeList_, Callable, ArgTransformer, M,
    typename std::enable_if<(N >= M)>::type> {
...
template <int N, typename RT, typename... Args, typename... Others,
          typename... InArgs>
struct FindPrototypeByArgsFromIndex<N, HeterTuple<RT(Args...), Others...>,
                                    InArgs...> {
    enum { canInvoke = CanInvoke<RT(Args...), InArgs...>::value };

    using Prototype = typename std::conditional<
        canInvoke, RT(Args...),
        typename FindPrototypeByArgsFromIndex<N + 1, HeterTuple<Others...>,
                                              InArgs...>::Prototype>::type;
    ...
    using ArgsTuple = typename std::conditional<
        ...
        std::tuple<typename std::remove_cv<
            typename std::remove_reference<Args>::type>::type...>,
        typename FindPrototypeByArgsFromIndex<N + 1, HeterTuple<Others...>,
                                              InArgs...>::ArgsTuple>::type;

    enum {
        index = canInvoke
                    ? N
                    : FindPrototypeByArgsFromIndex<N + 1, HeterTuple<Others...>,
                                                   InArgs...>::index
    };
};
...
struct FindPrototypeByIndexHelper<I, N, HeterTuple<RT(Args...), Others...>> {
    using NextType =
        FindPrototypeByIndexHelper<I + 1, N, HeterTuple<Others...>>;
...
struct FindPrototypeByIndexHelper<N, N, HeterTuple<RT(Args...), Others...>> {
    ...
    using ArgsTuple = HeterTuple<typename std::remove_cv<
        typename std::remove_reference<Args>::type>::type...>;
...
struct ReplaceReturnTypeList<HeterTuple<RT(Args...), Others...>, Replacement> {
    using Type = typename PrependHeterTuple<
        typename ReplaceReturnTypeList<HeterTuple<Others...>,
                                       Replacement>::Type,
