...
bool JsonUtil::GetJsonByPath(const nlohmann::json &json,
  ...
  for (auto &field : paths) {
    if (field.empty()) {
      LOG(ERROR) << "Invalid path: " << field;
      return false;
    }
    const auto iter = upper_layer_json.find(field);
    if (iter == upper_layer_json.end()) {
      LOG(ERROR) << "The json has no such key: " << field;
      return false;
    }
    if (!iter->is_object()) {
      LOG(ERROR) << "Required json but not,invalid type.";
      return false;
    }
...
bool JsonUtil::GetBoolean(const nlohmann::json &json, const std::string &key,
  ...
  if (!iter->is_boolean()) {
    LOG(ERROR) << "The value of json[" << key << "] is not a boolean";
    return false;
  }
  *value = *iter;
  return true;
