...
#ifndef AUTONOMY_COMMON_KALMAN_FILTER_KALMAN_FILTER_HPP_
#define AUTONOMY_COMMON_KALMAN_FILTER_KALMAN_FILTER_HPP_
...
public:
    ...
    /**
     * @brief calculate kalman filter state and covariance by prediction model
     * with A, B, Q matrix. This is mainly for EKF with variable matrix.
     ...
     * A, Q matrix. This is mainly for EKF with variable matrix.
    ...
    /**
     * @brief calculate kalman filter state by prediction model with A, B and Q
     * being class member variables.
    ...
    /**
     * @brief calculate kalman filter state by measurement model with y_pred, C
     * and R matrix. This is mainly for EKF with variable matrix.
     ...
     * matrix. This is mainly for EKF with variable matrix.
     * @param y measured values
     * @param C coefficient matrix of x for measurement model
     * @param R covariance matrix for measurement model
     * @return bool to check matrix operations are being performed properly
...
protected:
    ...
    Eigen::MatrixXd A_;  //!< @brief coefficient matrix of x for process model
                         //!< x[k+1] = A*x[k] + B*u[k]
    Eigen::MatrixXd B_;  //!< @brief coefficient matrix of u for process model
                         //!< x[k+1] = A*x[k] + B*u[k]
    Eigen::MatrixXd C_;  //!< @brief coefficient matrix of x for measurement
                         //!< model y[k] = C * x[k]
...
#endif  // AUTONOMY_COMMON_KALMAN_FILTER_KALMAN_FILTER_HPP_
