...
void CheckForLuaErrors(lua_State* L, int status) { CHECK_EQ(status, 0) << lua_tostring(L, -1); }
...
void PushValue(lua_State* L, const std::string& key) { lua_pushstring(L, key.c_str()); }
...
bool LuaParameterDictionary::HasKey(const std::string& key) const { return HasKeyOfType(L_, key); }
...
int LuaParameterDictionary::LuaInclude(lua_State* L) {
  ...
  if (std::find(parameter_dictionary->included_files_.begin(), parameter_dictionary->included_files_.end(), filename) !=
      parameter_dictionary->included_files_.end()) {
    std::string error_msg = "Tried to include " + filename + " twice. Already included files in order of inclusion: ";
