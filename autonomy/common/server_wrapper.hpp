...
class ServiceWrapper {
 public:
   ...
   */
  bool IsCancelRequested() const { return true; }
   ...
   */
  const std::shared_ptr<const typename ServiceT::Request> GetCurrentRequest() const { return nullptr; }

  /**
   * @brief Get the pending request object
   * @return Request Ptr to the request that's pending
   */
  const std::shared_ptr<const typename ServiceT::Request> GetPendingRequest() const { return nullptr; }

 protected:
  /**
   * @brief Info logging
   */
  void InfoMsg(const std::string& msg) const { LOG(INFO) << service_name_.c_str() << "[ActionServer] " << msg.c_str(); }
   ...
   */
  constexpr auto EmptyResult() const { return std::make_shared<typename ServiceT::Response>(); }
