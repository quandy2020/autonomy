 ...
 *     s.t
 *       x0_lower  <= x0 <= x0_upper            (bounds on variable-set x0 \in
 * R^2)
 *
 ...
 * ConstraintSet::FillJacobianBlock() and are then used to build the complete
 * Jacobian for the cost and constraints.
 *
...
public:
     ...
     * @details ipopt uses 10e-8 for their derivative check, but setting here to
     * more precise
    ...
    /**
     * @brief Each constraint value g(x) for current optimization variables @c
     * x.
     */
