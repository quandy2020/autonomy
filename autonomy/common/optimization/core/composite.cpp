...
void Component::Print(double tol, int& index) const {
    ...
    oss.precision(2);
    oss << std::fixed << std::left << std::setw(30) << name_ << std::right
...
Composite::Jacobian Composite::GetJacobian()
    const {  // set number of variables only the first time this function is
             // called,
    // since number doesn't change during the optimization. Improves efficiency.
    if (n_var == -1)
        n_var =
            components_.empty() ? 0 : components_.front()->GetJacobian().cols();
