...
public:
     ...
     * constraint, whereas every column refers to a single optimization
     * variable.
     *
    ...
    /**
     * @brief Set individual Jacobians corresponding to each decision variable
     * set.
     ...
     * iteration, that changes the sparsity pattern. A more robust way is to
     * directly set as follows (which can also be set =0.0 without erros):
...
private:
     ...
     *
     * Sometimes the number of constraints depends on the variable
     * representation, or shorthands to specific variable sets want to be saved
