 ...
 *
 *  @brief Example to generate a solver-independent formulation for the problem,
 * taken from the IPOPT cpp_example.
 *
...
public:
    ...
    // Attention: see the parent class function for important information on
    // sparsity pattern.
    void FillJacobianBlock(std::string var_set,
                           Jacobian& jac_block) const override {
        // must fill only that submatrix of the overall Jacobian that relates
        // to this constraint and "var_set1". even if more constraints or
        // variables classes are added, this submatrix will always start at row
        // 0 and column 0, thereby being independent from the overall problem.
        if (var_set == "var_set1") {
            Vector2d x = GetVariables()->GetComponent("var_set1")->GetValues();
...
public:
    ...
    void FillJacobianBlock(std::string var_set, Jacobian& jac) const override {
        if (var_set == "var_set1") {
            ...
            jac.coeffRef(0, 1) =
                -2.0 * (x(1) - 2.0);  // derivative of cost w.r.t x1
        }
    }
};
