...
#ifndef AUTONOMY_COMMON_STATE_VECTOR_GENERIC_STATE_HPP_
#define AUTONOMY_COMMON_STATE_VECTOR_GENERIC_STATE_HPP_

#include "autonomy/common/helper_functions/angle_utils.hpp"
#include "autonomy/common/helper_functions/type_name.hpp"
#include "autonomy/common/helper_functions/type_traits.hpp"
#include "autonomy/common/helper_functions/types.hpp"
#include "autonomy/common/state_vector/common_variables.hpp"
...
///
/// @brief      A representation of a generic state vectors with specified
/// variables.
///
...
public:
    ...
    ///
    explicit GenericState(const Vector& vector) : m_state{vector} {}
    ...
    ///
    inline const Vector& vector() const noexcept {
        return m_state;
    }
    ...
    ///
    inline ScalarT& operator[](const Eigen::Index idx) {
        return m_state[idx];
    }
    ///
    /// @brief      Get an element of the vector.
    ...
    /// @return     A reference to the vector element.
    ///
    inline const ScalarT& operator[](const Eigen::Index idx) const {
        return m_state[idx];
    }
    ...
    ///
    /// @details    This function copies (a subset of) variables from the
    /// current state into another
    ///             state. If `other_state` is provided, it is used as a "donor"
    ///             state, i.e., the values already in this `other_state` are
    ///             unchanged unless overwritten by the ones present in the
    ///             current state. If no `other_state` is given, a new
    ///             zero-initialized instance of OtherStateT will be created and
    ///             this state will be used for the further update using the
    ///             variables from the current state. Please see tests for
    ///             examples of usage.
    ///
    /// @note       This function also handles a rearrangement of the variables
    /// if the OtherStateT has
    ///             the variables in a different order from the current state.
    ///
    /// @note       This function copies only variables that are present in both
    /// states. It silently
    ///             does nothing if there are no common variables between the
    ///             states.
    ///
    ...
    ///
    inline constexpr static Eigen::Index size() noexcept {
        return kSize;
    }

    /// @brief      Get a variables tuple used for iteration over all variables.
    inline constexpr static Variables variables() noexcept {
        return Variables{};
    }
    ...
    ///
    /// @warning    This can be unsafe if the order of the variables in the
    /// Eigen vector is different
    ...
    ///
    /// @tparam     T  Either a state vector type or an underlying Eigen vector
    /// type.
    ///
...
#endif  // AUTONOMY_COMMON_STATE_VECTOR_GENERIC_STATE_HPP_
