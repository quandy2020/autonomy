...
TEST(FactoryTest, OneArgConstructor) {
  ...
  EXPECT_TRUE(factory.Register("arg_1", [](const std::string& arg) { return new ArgConstructor(arg); }));
...
TEST(FactoryTest, TwoArgConstructor) {
  ...
  EXPECT_TRUE(
      factory.Register("arg_2", [](const std::string& arg, int value) { return new ArgConstructor(arg, value); }));
