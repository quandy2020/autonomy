...
// ---------------------------------------------------------------------------
// glog-based logging macros (AINFO, AWARN, …)
// ---------------------------------------------------------------------------
#define LEFT_BRACKET "["
#define RIGHT_BRACKET "]"

#ifndef MODULE_NAME
#define MODULE_NAME "autonomy"
#endif
...
// ---------------------------------------------------------------------------
// CHECK_OPTION / THROW_CHECK helpers
// ---------------------------------------------------------------------------
...
#define CHECK_OPTION_IMPL(expr)                                            \
    autonomy::common::utils::__CheckOptionImpl(__FILE__, __LINE__, (expr), \
                                               #expr)
...
#define CHECK_OPTION_OP(name, op, val1, val2)                             \
    if (!autonomy::common::utils::__CheckOptionOpImpl(                    \
            __FILE__, __LINE__, (val1 op val2), val1, val2, #val1, #val2, \
            #op)) {                                                       \
        return false;                                                     \
    }
...
#define LOG_FATAL_THROW(exception)                                        \
    autonomy::common::LogMessageFatalThrow<exception>(__FILE__, __LINE__) \
        .stream()
...
#define THROW_CHECK_OP(name, op, val1, val2) \
    CHECK_OP_LOG(name, op, val1, val2,       \
                 autonomy::common::LogMessageFatalThrowDefault)
...
#define THROW_CHECK_NOTNULL(val)                            \
    autonomy::common::ThrowCheckNotNull(__FILE__, __LINE__, \
...
bool __CheckOptionImpl(const char* file, int line, bool result,
                       const char* expr_str);
...
bool __CheckOptionOpImpl(const char* file, const int line, const bool result,
    ...
    if (result) {
        ...
        LOG(ERROR) << StringPrintf(
            "[%s:%d] Check failed: %s %s %s (%s vs. %s)",
            __GetConstFileBaseName(file), line, val1_str, op_str, val2_str,
            std::to_string(val1).c_str(), std::to_string(val2).c_str());
        return false;
    }
}
