...
#define CHECK_OPTION_IMPL(expr) autonomy::common::utils::__CheckOptionImpl(__FILE__, __LINE__, (expr), #expr)
...
#define CHECK_OPTION_OP(name, op, val1, val2)                                                                       \
    if (!autonomy::common::utils::__CheckOptionOpImpl(__FILE__, __LINE__, (val1 op val2), val1, val2, #val1, #val2, \
                                                      #op)) {                                                       \
        return false;                                                                                               \
    }
...
// level FATAL_THROW.
#define COMPACT_GOOGLE_LOG_FATAL_THROW autonomy::common::LogMessageFatalThrowDefault(__FILE__, __LINE__)

#define LOG_TO_STRING_FATAL_THROW(message) autonomy::common::LogMessageFatalThrowDefault(__FILE__, __LINE__, message)

#define LOG_FATAL_THROW(exception) autonomy::common::LogMessageFatalThrow<exception>(__FILE__, __LINE__).stream()

#define THROW_CHECK(condition) \
    LOG_IF(FATAL_THROW, GOOGLE_PREDICT_BRANCH_NOT_TAKEN(!(condition))) << "Check failed: " #condition " "
...
#define THROW_CHECK_NOTNULL(val) \
    autonomy::common::ThrowCheckNotNull(__FILE__, __LINE__, "'" #val "' Must be non NULL", (val))
...
bool __CheckOptionImpl(const char* file, int line, bool result, const char* expr_str);

template <typename T1, typename T2>
bool __CheckOptionOpImpl(const char* file, const int line, const bool result, const T1& val1, const T2& val2,
                         const char* val1_str, const char* val2_str, const char* op_str) {
    if (result) {
        return true;
    } else {
        LOG(ERROR) << StringPrintf("[%s:%d] Check failed: %s %s %s (%s vs. %s)", __GetConstFileBaseName(file), line,
                                   val1_str, op_str, val2_str, std::to_string(val1).c_str(),
...
inline std::string __MakeExceptionPrefix(const char* file, int line) {
    return "[" + std::string(__GetConstFileBaseName(file)) + ":" + std::to_string(line) + "] ";
}
...
public:
    LogMessageFatalThrow(const char* file, int line)
        : google::LogMessage(file, line, google::GLOG_ERROR, &message_), prefix_(__MakeExceptionPrefix(file, line)) {}
    ...
    LogMessageFatalThrow(const char* file, int line,
#if defined(GLOG_VERSION_MAJOR) && (GLOG_VERSION_MAJOR > 0 || GLOG_VERSION_MINOR >= 7)
