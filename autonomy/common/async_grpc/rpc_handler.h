...
#include "grpc++/grpc++.h"
#if BUILD_TRACING
#include "autonomy/common/async_grpc/opencensus_span.h"
#endif
...
class RpcHandler : public RpcHandlerInterface {
 public:
  ...
  void SetExecutionContext(ExecutionContext* execution_context) override { execution_context_ = execution_context; }
  ...
  void Send(std::unique_ptr<ResponseType> response) { rpc_->Write(std::move(response)); }
  template <typename T>
