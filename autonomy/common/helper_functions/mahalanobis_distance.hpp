...
#ifndef AUTONOMY_COMMON_HELPER_FUNCTIONS_MAHALANOBIS_DISTANCE_HPP_
#define AUTONOMY_COMMON_HELPER_FUNCTIONS_MAHALANOBIS_DISTANCE_HPP_
...
/// \param sample Single column matrix containing sample whose distance needs to
/// be computed
...
types::float32_t calculate_squared_mahalanobis_distance(
    ...
    // diff.T
    // * C.inv() * diff Instead of the covariance matrix C we have its
    ...
    // this allows us to efficiently find the squared Mahalanobis distance using
    // (L.inv() * diff), which can be found as a solution to: L * x = diff.
...
#endif  // AUTONOMY_COMMON_HELPER_FUNCTIONS_MAHALANOBIS_DISTANCE_HPP_
