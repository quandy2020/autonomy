...
struct index<HeadT, std::tuple<HeadT, Tail...>> : std::integral_constant<std::int32_t, 0> {};
...
inline constexpr typename std::enable_if_t<I == sizeof...(TypesT)> visit(std::tuple<TypesT...>&, Callable) noexcept {}
/// @brief      Same as the previous specialization but for const tuple.
template <std::size_t I = 0UL, typename Callable, typename... TypesT>
inline constexpr typename std::enable_if_t<I == sizeof...(TypesT)> visit(const std::tuple<TypesT...>&,
                                                                         Callable) noexcept {}
...
#endif  // AUTONOMY_COMMON_HELPER_FUNCTIONS_TYPE_TRAITS_HPP_
