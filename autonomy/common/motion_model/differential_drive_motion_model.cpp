...
typename CvtrMotionModel<ScalarT>::State cvtr_predict(
    ...
    if (common::helper_functions::abs_eq_zero(w,
                                              static_cast<ScalarT>(kEpsilon))) {
        new_state.template at<X>() = x + t * v * cos(theta);
        new_state.template at<Y>() = y + t * v * sin(theta);
    } else {
        new_state.template at<X>() =
            x + v * (-sin(theta) + sin(t * w + theta)) / w;
        new_state.template at<Y>() =
            y + v * (cos(theta) - cos(t * w + theta)) / w;
    }
...
typename CatrMotionModel<ScalarT>::State catr_predict(
    ...
    if (common::helper_functions::abs_eq_zero(w,
                                              static_cast<ScalarT>(kEpsilon))) {
        new_state.template at<X>() =
            x + half_t_squared_a * cos(theta) + t * v * cos(theta);
        new_state.template at<Y>() =
            y + half_t_squared_a * sin(theta) + t * v * sin(theta);
    } else {
        const auto t_w_plus_theta = t * w + theta;
        const auto inv_w = static_cast<ScalarT>(1.0) / w;
        const auto inv_w_2 = inv_w * inv_w;
        new_state.template at<X>() =
            x + inv_w * t * a * sin(t_w_plus_theta) - inv_w * v * sin(theta) +
            inv_w * v * sin(t_w_plus_theta) +
            inv_w_2 * a * (-cos(theta) + cos(t_w_plus_theta));
        new_state.template at<Y>() =
            y - inv_w * t * a * cos(t_w_plus_theta) + inv_w * v * cos(theta) -
...
typename CatrMotionModel<ScalarT>::State::Matrix catr_jacobian(
    ...
    if (common::helper_functions::abs_eq_zero(w,
        ...
        jacobian(x_index, theta_index) =
            -half * t * t * a * sin(theta) - t * v * sin(theta);
        jacobian(x_index, v_index) = t * cos(theta);
        jacobian(x_index, w_index) =
            -half * t * t * t * a * sin(theta) - t * t * v * sin(theta);
        jacobian(x_index, a_index) = half * t * t * cos(theta);
        // Y row non-identity entries
        jacobian(y_index, theta_index) =
            half * t * t * a * cos(theta) + t * v * cos(theta);
        jacobian(y_index, v_index) = t * sin(theta);
        jacobian(y_index, w_index) =
            half * t * t * t * a * cos(theta) + t * t * v * cos(theta);
        ...
        // X row non-identity entries
        jacobian(x_index, theta_index) = t * a * inv_w * cos_next_theta +
                                         v_cos_theta_diff_inv_w -
                                         a * inv_w * sin_theta_diff_inv_w;
        jacobian(x_index, v_index) = sin_theta_diff_inv_w;
        jacobian(x_index, w_index) =
        ...
        jacobian(x_index, a_index) =
            t * inv_w * sin_next_theta + inv_w * cos_theta_diff_inv_w;
        // Y row non-identity entries.
        jacobian(y_index, theta_index) = t * a * inv_w * sin_next_theta +
                                         v_sin_theta_diff_inv_w +
                                         a * inv_w * cos_theta_diff_inv_w;
        jacobian(y_index, v_index) = -cos_theta_diff_inv_w;
        jacobian(y_index, w_index) =
        ...
        jacobian(y_index, a_index) =
            -t * inv_w * cos_next_theta + inv_w * sin_theta_diff_inv_w;
        // Velocity row non-identity entries.
        jacobian(v_index, a_index) = t;
