...
#include "autonomy/common/motion_model/differential_drive_motion_model.hpp"
...
using autonomy::common::state_vector::variable::XY_ACCELERATION;
using autonomy::common::state_vector::variable::XY_VELOCITY;
...
typename CvtrMotionModel<ScalarT>::State cvtr_predict(const typename CvtrMotionModel<ScalarT>::State& state,
  ...
  if (common::helper_functions::abs_eq_zero(w, static_cast<ScalarT>(kEpsilon))) {
...
template <typename ScalarT>
typename CvtrMotionModel<ScalarT>::State::Matrix cvtr_jacobian(const typename CvtrMotionModel<ScalarT>::State& state,
                                                               const std::chrono::nanoseconds& dt) {
...
template <typename ScalarT>
typename CatrMotionModel<ScalarT>::State catr_predict(const typename CatrMotionModel<ScalarT>::State& state,
                                                      const std::chrono::nanoseconds& dt) {
  ...
  if (common::helper_functions::abs_eq_zero(w, static_cast<ScalarT>(kEpsilon))) {
    ...
    new_state.template at<X>() = x + inv_w * t * a * sin(t_w_plus_theta) - inv_w * v * sin(theta) +
                                 inv_w * v * sin(t_w_plus_theta) + inv_w_2 * a * (-cos(theta) + cos(t_w_plus_theta));
    new_state.template at<Y>() = y - inv_w * t * a * cos(t_w_plus_theta) + inv_w * v * cos(theta) -
                                 inv_w * v * cos(t_w_plus_theta) + inv_w_2 * a * (-sin(theta) + sin(t_w_plus_theta));
  }
...
template <typename ScalarT>
typename CatrMotionModel<ScalarT>::State::Matrix catr_jacobian(const typename CatrMotionModel<ScalarT>::State& state,
                                                               const std::chrono::nanoseconds& dt) {
  ...
  if (common::helper_functions::abs_eq_zero(w, static_cast<ScalarT>(kEpsilon))) {
    ...
    jacobian(x_index, v_index) = sin_theta_diff_inv_w;
    jacobian(x_index, w_index) = t * t * a * inv_w * cos_next_theta + t * v * inv_w * cos_next_theta -
                                 two * t * a * inv_w * inv_w * sin_next_theta - inv_w * v_sin_theta_diff_inv_w -
    ...
    jacobian(y_index, v_index) = -cos_theta_diff_inv_w;
    jacobian(y_index, w_index) = t * t * a * inv_w * sin_next_theta + t * v * inv_w * sin_next_theta +
                                 two * t * a * inv_w * inv_w * cos_next_theta + inv_w * v_cos_theta_diff_inv_w -
