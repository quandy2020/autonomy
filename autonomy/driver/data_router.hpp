...
class DataRouter {
 public:
   ...
   * @return true 成功，false 失败
   */
  bool PublishPointCloud2(const std::string& topic, const std::shared_ptr<commsgs::sensor_msgs::PointCloud2>& message);
 ...
 private:
  ...
  // 目标处理器映射表（目标名称 -> 处理器函数）
  std::map<std::string, std::function<void(const std::string&, const std::shared_ptr<sensor::Data>&)>> target_handlers_;
