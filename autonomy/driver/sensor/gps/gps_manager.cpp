...
#include "autonomy/driver/sensor/gps/gps_manager.hpp"
...
GpsManager::~GpsManager() { Cleanup(); }

bool GpsManager::LoadDriverPlugin(const std::string& library_path) {
...
bool GpsManager::UnloadDriverPlugin(const std::string& library_path) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  AINFO << "Unloaded GPS driver plugin: " << library_path;
  return true;
}

std::vector<std::string> GpsManager::GetAvailableDriverClasses() const {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool GpsManager::RegisterDriver(const std::string& driver_name, GpsBase::SharedPtr driver) {
  ...
  AINFO << "Registered GPS driver: " << driver_name;
  return true;
}

void GpsManager::UnregisterDriver(const std::string& driver_name) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (it != drivers_.end()) {
    ...
    drivers_.erase(it);
    AINFO << "Unregistered GPS driver: " << driver_name;
  }
}

GpsBase::SharedPtr GpsManager::GetDriver(const std::string& driver_name) const {
  std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<std::string> GpsManager::GetRegisteredDriverNames() const {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool GpsManager::ConfigureFromOptions(const proto::DriverOptions& options) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  for (const auto& gps_option : options.gps_sensors()) {
    ...
    std::string driver_name = "gps_driver_" + gps_option.sensor_id();
...
void GpsManager::StartAllDrivers() {
  std::lock_guard<std::mutex> lock(mutex_);

  for (auto& pair : drivers_) {
    if (pair.second) {
      if (!pair.second->IsConnected()) {
        AWARN << "GPS driver not connected: " << pair.first;
        continue;
      }

      if (!pair.second->Initialize()) {
        AERROR << "Failed to initialize GPS driver: " << pair.first;
        continue;
      }

      pair.second->Start();
      AINFO << "Started GPS driver: " << pair.first;
    }
  }
}

void GpsManager::StopAllDrivers() {
  std::lock_guard<std::mutex> lock(mutex_);

  for (auto& pair : drivers_) {
    if (pair.second) {
      pair.second->Stop();
      AINFO << "Stopped GPS driver: " << pair.first;
    }
  }
}
