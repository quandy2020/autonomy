...
/**
 * @class GpsManager
 * @brief GPS/导航卫星驱动管理器，支持动态库插件加载、配置化和热插拔
 *
 * GpsManager 提供：
 * 1. 动态加载GPS驱动插件（.so 文件）
 * 2. 根据配置创建和配置驱动实例
...
public:
    /**
     * Define GpsManager::SharedPtr type
     */
    AUTONOMY_SMART_PTR_DEFINITIONS(GpsManager)
    ...
    /**
     * @brief 创建驱动实例
     * @param driver_class_name 驱动类名（如 "UbloxM8nDriver",
     * "QuectelL76Driver"）
     * @param driver_name 驱动实例名称
     * @return 驱动实例指针，失败返回 nullptr
     */
    GpsBase::SharedPtr CreateDriver(const std::string& driver_class_name, const std::string& driver_name);
     ...
     * @return true 成功，false 失败
     */
    bool RegisterDriver(const std::string& driver_name, GpsBase::SharedPtr driver);
     ...
     */
    GpsBase::SharedPtr GetDriver(const std::string& driver_name) const;
...
private:
     ...
     * @return 驱动实例指针
     */
    GpsBase::SharedPtr CreateDriverFromPlugin(const std::string& library_path, const std::string& driver_class_name);

    // 插件库映射表（library_path -> PluginLibrary）
    std::map<std::string, PluginLibrary> plugin_libraries_;

    // 驱动实例映射表（driver_name -> GpsBase::SharedPtr）
    std::map<std::string, GpsBase::SharedPtr> drivers_;
