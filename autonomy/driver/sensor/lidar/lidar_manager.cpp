...
#include "autonomy/driver/sensor/lidar/lidar_manager.hpp"
...
LidarManager::~LidarManager() { Cleanup(); }

bool LidarManager::LoadDriverPlugin(const std::string& library_path) {
...
bool LidarManager::UnloadDriverPlugin(const std::string& library_path) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  AINFO << "Unloaded lidar driver plugin: " << library_path;
  return true;
}

std::vector<std::string> LidarManager::GetAvailableDriverClasses() const {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool LidarManager::RegisterDriver(const std::string& driver_name, LidarBase::SharedPtr driver) {
  ...
  AINFO << "Registered lidar driver: " << driver_name;
  return true;
}

void LidarManager::UnregisterDriver(const std::string& driver_name) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (it != drivers_.end()) {
    ...
    drivers_.erase(it);
    AINFO << "Unregistered lidar driver: " << driver_name;
  }
}
...
std::vector<std::string> LidarManager::GetRegisteredDriverNames() const {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool LidarManager::ConfigureFromOptions(const proto::DriverOptions& options) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  for (const auto& lidar_option : options.lidars()) {
    ...
    std::string driver_name = "lidar_driver_" + lidar_option.sensor_id();

    // 检查驱动是否已存在
    if (drivers_.find(driver_name) == drivers_.end()) {
      AWARN << "Lidar driver configuration found but driver creation not "
               "yet "
               "implemented. "
...
void LidarManager::StartAllDrivers() {
  std::lock_guard<std::mutex> lock(mutex_);

  for (auto& pair : drivers_) {
    if (pair.second) {
      if (!pair.second->IsConnected()) {
        AWARN << "Lidar driver not connected: " << pair.first;
        continue;
      }

      if (!pair.second->Initialize()) {
        AERROR << "Failed to initialize lidar driver: " << pair.first;
        continue;
      }

      pair.second->Start();
      AINFO << "Started lidar driver: " << pair.first;
    }
  }
}

void LidarManager::StopAllDrivers() {
  std::lock_guard<std::mutex> lock(mutex_);

  for (auto& pair : drivers_) {
    if (pair.second) {
      pair.second->Stop();
      AINFO << "Stopped lidar driver: " << pair.first;
    }
  }
}

void LidarManager::Cleanup() {
  std::lock_guard<std::mutex> lock(mutex_);
