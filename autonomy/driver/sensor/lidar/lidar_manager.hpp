...
/**
 * @class LidarManager
 * @brief 激光雷达驱动管理器，支持动态库插件加载、配置化和热插拔
 *
 * LidarManager 提供：
 * 1. 动态加载激光雷达驱动插件（.so 文件）
 * 2. 根据配置创建和配置驱动实例
...
class LidarManager {
 public:
  /**
   * Define LidarManager::SharedPtr type
   */
  AUTONOMY_SMART_PTR_DEFINITIONS(LidarManager)
  ...
  /**
   * @brief 创建驱动实例
   * @param driver_class_name 驱动类名（如 "RplidarA1Driver",
   * "VelodyneVlp16Driver"）
   ...
   */
  LidarBase::SharedPtr GetDriver(const std::string& driver_name) const;
 ...
 private:
  ...
  // 驱动实例映射表（driver_name -> LidarBase::SharedPtr）
  std::map<std::string, LidarBase::SharedPtr> drivers_;
