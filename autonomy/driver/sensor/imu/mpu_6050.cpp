...
Mpu6050Driver::~Mpu6050Driver() { Cleanup(); }

std::string Mpu6050Driver::GetHardwareModel() const { return "MPU6050"; }

std::string Mpu6050Driver::GetVersion() const { return "1.0.0"; }
...
bool Mpu6050Driver::InitializeHardware() {
  ...
  if (ioctl(i2c_fd_, I2C_SLAVE, i2c_address_) < 0) {
    AERROR << "Failed to set I2C slave address: " << static_cast<int>(i2c_address_) << " (error: " << strerror(errno)
           << ")";
    close(i2c_fd_);
    i2c_fd_ = -1;
    return false;
