...
#include "autonomy/driver/sensor/imu/imu_manager.hpp"
...
ImuManager::~ImuManager() { Cleanup(); }

bool ImuManager::LoadDriverPlugin(const std::string& library_path) {
...
bool ImuManager::UnloadDriverPlugin(const std::string& library_path) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  AINFO << "Unloaded IMU driver plugin: " << library_path;
  return true;
}

std::vector<std::string> ImuManager::GetAvailableDriverClasses() const {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool ImuManager::RegisterDriver(const std::string& driver_name, ImuBase::SharedPtr driver) {
  ...
  AINFO << "Registered IMU driver: " << driver_name;
  return true;
}

void ImuManager::UnregisterDriver(const std::string& driver_name) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (it != drivers_.end()) {
    ...
    drivers_.erase(it);
    AINFO << "Unregistered IMU driver: " << driver_name;
  }
}

ImuBase::SharedPtr ImuManager::GetDriver(const std::string& driver_name) const {
  std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<std::string> ImuManager::GetRegisteredDriverNames() const {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool ImuManager::ConfigureFromOptions(const proto::DriverOptions& options) {
  std::lock_guard<std::mutex> lock(mutex_);

  // 根据配置中的 IMU 选项创建和配置驱动
  ...
  for (const auto& imu_option : options.imus()) {
    ...
    // 这里需要扩展 ImuOptions 来包含这些信息
    // 或者从其他配置源获取

    // 示例：假设配置中有 driver_class 和 plugin_library 字段
    // std::string driver_class = imu_option.driver_class();
    // std::string plugin_library = imu_option.plugin_library();

    // 临时实现：使用默认值
    std::string driver_name = "imu_driver_" + imu_option.sensor_id();
...
void ImuManager::StartAllDrivers() {
  std::lock_guard<std::mutex> lock(mutex_);

  for (auto& pair : drivers_) {
    if (pair.second) {
      if (!pair.second->IsConnected()) {
        AWARN << "IMU driver not connected: " << pair.first;
        continue;
      }

      if (!pair.second->Initialize()) {
        AERROR << "Failed to initialize IMU driver: " << pair.first;
        continue;
      }

      pair.second->Start();
      AINFO << "Started IMU driver: " << pair.first;
    }
  }
}

void ImuManager::StopAllDrivers() {
  std::lock_guard<std::mutex> lock(mutex_);

  for (auto& pair : drivers_) {
    if (pair.second) {
      pair.second->Stop();
      AINFO << "Stopped IMU driver: " << pair.first;
    }
  }
}

void ImuManager::Cleanup() {
  std::lock_guard<std::mutex> lock(mutex_);
