...
/**
 * @class ImuManager
 * @brief IMU 驱动管理器，支持动态库插件加载、配置化和热插拔
 *
 * ImuManager 提供：
 * 1. 动态加载 IMU 驱动插件（.so 文件）
 * 2. 根据配置创建和配置驱动实例
...
class ImuManager {
 public:
  /**
   * Define ImuManager::SharedPtr type
   */
  AUTONOMY_SMART_PTR_DEFINITIONS(ImuManager)
  ...
  /**
   * @brief 创建驱动实例
   * @param driver_class_name 驱动类名（如 "Mpu6050Driver", "Lsm6ds3Driver"）
   * @param driver_name 驱动实例名称
   * @return 驱动实例指针，失败返回 nullptr
   */
  ImuBase::SharedPtr CreateDriver(const std::string& driver_class_name, const std::string& driver_name);
   ...
   * @return true 成功，false 失败
   */
  bool RegisterDriver(const std::string& driver_name, ImuBase::SharedPtr driver);
   ...
   */
  ImuBase::SharedPtr GetDriver(const std::string& driver_name) const;
 ...
 private:
   ...
   * @return 驱动实例指针
   */
  ImuBase::SharedPtr CreateDriverFromPlugin(const std::string& library_path, const std::string& driver_class_name);

  // 插件库映射表（library_path -> PluginLibrary）
  std::map<std::string, PluginLibrary> plugin_libraries_;

  // 驱动实例映射表（driver_name -> ImuBase::SharedPtr）
  std::map<std::string, ImuBase::SharedPtr> drivers_;
