...
#include "autonomy/driver/proto/range_options.pb.h"
...
/**
 * @class RangeBase
 * @brief 测距传感器驱动基类，继承 DriverInterface，提供测距传感器驱动的通用功能
 *
 * RangeBase 提供：
 * 1. 测距数据读取和转换的统一接口
 * 2. 配置管理和生命周期管理
 ...
 *
 * 子类需要实现：
 * - ReadRangeData() - 从硬件读取测距数据
 * - GetHardwareModel() - 返回硬件型号名称
 */
class RangeBase : public common::DriverInterface {
 public:
  /**
   * Define RangeBase::SharedPtr type
   */
  AUTONOMY_SMART_PTR_DEFINITIONS(RangeBase)
  ...
  /**
   * @brief 获取硬件型号名称（子类实现）
   * @return 硬件型号名称，如 "HC-SR04", "VL53L0X", "GP2Y0A21" 等
   */
 ...
 protected:
  /**
   * @brief 从硬件读取测距数据（子类实现）
   * @param sensor_id 传感器ID
   * @return 测距数据消息，如果读取失败返回 nullptr
   */
  virtual std::shared_ptr<commsgs::sensor_msgs::Range> ReadRangeData(const std::string& sensor_id) = 0;

  /**
   * @brief 处理读取到的测距数据并转发
   * @param sensor_id 传感器ID
   * @param range_msg 测距消息
   */
  void ProcessRangeData(const std::string& sensor_id, const std::shared_ptr<commsgs::sensor_msgs::Range>& range_msg);
  ...
  // 测距传感器配置选项映射表（sensor_id -> RangeOptions）
  std::map<std::string, proto::RangeOptions> range_configs_;
