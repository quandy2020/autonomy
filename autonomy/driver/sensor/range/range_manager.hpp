...
/**
 * @class RangeManager
 * @brief 测距传感器驱动管理器，支持动态库插件加载、配置化和热插拔
 *
 * RangeManager 提供：
 * 1. 动态加载测距传感器驱动插件（.so 文件）
 * 2. 根据配置创建和配置驱动实例
...
public:
    /**
     * Define RangeManager::SharedPtr type
     */
    AUTONOMY_SMART_PTR_DEFINITIONS(RangeManager)
    ...
    /**
     * @brief 创建驱动实例
     * @param driver_class_name 驱动类名（如 "HcSr04Driver", "Vl53l0xDriver"）
     * @param driver_name 驱动实例名称
     * @return 驱动实例指针，失败返回 nullptr
     */
    RangeBase::SharedPtr CreateDriver(const std::string& driver_class_name, const std::string& driver_name);
     ...
     * @return true 成功，false 失败
     */
    bool RegisterDriver(const std::string& driver_name, RangeBase::SharedPtr driver);
     ...
     */
    RangeBase::SharedPtr GetDriver(const std::string& driver_name) const;
...
private:
     ...
     * @return 驱动实例指针
     */
    RangeBase::SharedPtr CreateDriverFromPlugin(const std::string& library_path, const std::string& driver_class_name);

    // 插件库映射表（library_path -> PluginLibrary）
    std::map<std::string, PluginLibrary> plugin_libraries_;

    // 驱动实例映射表（driver_name -> RangeBase::SharedPtr）
    std::map<std::string, RangeBase::SharedPtr> drivers_;
