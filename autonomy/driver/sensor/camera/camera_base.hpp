...
#include "autonomy/driver/proto/camera_options.pb.h"
...
/**
 * @class CameraBase
 * @brief 相机驱动基类，继承 DriverInterface，提供相机驱动的通用功能
 *
 * CameraBase 提供：
 * 1. 相机数据读取和转换的统一接口
 * 2. 配置管理和生命周期管理
 ...
 *
 * 子类需要实现：
 * - ReadImageData() - 从硬件读取图像数据
 * - GetHardwareModel() - 返回硬件型号名称
 */
class CameraBase : public common::DriverInterface
{
public:
    /**
     * Define CameraBase::SharedPtr type
     */
    AUTONOMY_SMART_PTR_DEFINITIONS(CameraBase)
    ...
    /**
     * @brief 获取硬件型号名称（子类实现）
     * @return 硬件型号名称，如 "RealSense D435i", "USB Camera", "Basler" 等
     */
...
protected:
    /**
     * @brief 从硬件读取图像数据（子类实现）
     * @param sensor_id 传感器ID
     * @return 图像数据消息，如果读取失败返回 nullptr
     */
    virtual std::shared_ptr<commsgs::sensor_msgs::Image> ReadImageData(const std::string& sensor_id) = 0;

    /**
     * @brief 从硬件读取深度图像数据（子类实现，仅用于RGBD相机）
     * @param sensor_id 传感器ID
     * @return 深度图像数据消息，如果读取失败返回 nullptr
     */
    virtual std::shared_ptr<commsgs::sensor_msgs::Image> ReadDepthData(const std::string& sensor_id) {
        (void)sensor_id;
        return nullptr;
    }
     ...
     */
    void ProcessImageData(const std::string& sensor_id, const std::shared_ptr<commsgs::sensor_msgs::Image>& image_msg);
    ...
    // 相机配置选项映射表（sensor_id -> CameraOptions）
    std::map<std::string, proto::CameraOptions> camera_configs_;
