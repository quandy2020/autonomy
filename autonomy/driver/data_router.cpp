...
bool DataRouter::Initialize(const proto::DriverOptions& options) {
  ...
  for (const auto& sensor_id : sensor_ids) {
    ...
    AINFO << "DataRouter: sensor " << sensor_id << " data source: " << (source == DataSource::ROS2 ? "ROS2" : "DRIVER");
  }
...
bool DataRouter::PublishPointCloud2(const std::string& topic,
  ...
  auto writer =
      std::static_pointer_cast<::autolink::Writer<commsgs::sensor_msgs::PointCloud2>>(sensor_publishers_[topic].writer);
  return writer->Write(message);
}
...
bool DataRouter::PublishPointCloud(const std::string& topic,
  ...
  auto writer =
      std::static_pointer_cast<::autolink::Writer<commsgs::sensor_msgs::PointCloud>>(sensor_publishers_[topic].writer);
  return writer->Write(message);
}
...
bool DataRouter::PublishOdometry(const std::string& topic,
  ...
  auto writer =
      std::static_pointer_cast<::autolink::Writer<commsgs::planning_msgs::Odometry>>(sensor_publishers_[topic].writer);
  return writer->Write(message);
}
