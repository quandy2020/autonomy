...
class Matrix3x3 {
 ...
 public:
  ...
  Matrix3x3(const tf2Scalar& yaw, const tf2Scalar& pitch, const tf2Scalar&
  roll)
  {
  ...
  */
  ...
  TF2SIMD_FORCE_INLINE Vector3 getColumn(int i) const { return Vector3(m_el[0][i], m_el[1][i], m_el[2][i]); }
  ...
  void setValue(const tf2Scalar& xx, const tf2Scalar& xy, const tf2Scalar& xz, const tf2Scalar& yx, const tf2Scalar& yy,
                const tf2Scalar& yz, const tf2Scalar& zx, const tf2Scalar& zy, const tf2Scalar& zz) {
   ...
   *
   **/
  void setRPY(tf2Scalar roll, tf2Scalar pitch, tf2Scalar yaw) { setEulerYPR(yaw, pitch, roll); }
   ...
   * @param iteration The iteration stops when all off-diagonal elements are
   * less than the threshold multiplied by the sum of the absolute values of
   * the diagonal, or when maxSteps have been executed.
   *
   ...
   * See http://en.wikipedia.org/wiki/Cofactor_(linear_algebra) for more
   * details
   */
...
TF2SIMD_FORCE_INLINE tf2Scalar Matrix3x3::determinant() const { return tf2Triple((*this)[0], (*this)[1], (*this)[2]); }
...
TF2SIMD_FORCE_INLINE Matrix3x3 operator*(const Matrix3x3& m1, const Matrix3x3& m2) {
  return Matrix3x3(m2.tdotx(m1[0]), m2.tdoty(m1[0]), m2.tdotz(m1[0]), m2.tdotx(m1[1]), m2.tdoty(m1[1]), m2.tdotz(m1[1]),
                   m2.tdotx(m1[2]), m2.tdoty(m1[2]), m2.tdotz(m1[2]));
}

/*
TF2SIMD_FORCE_INLINE Matrix3x3 tf2MultTransposeLeft(const Matrix3x3& m1, const
Matrix3x3& m2) { return Matrix3x3( m1[0][0] * m2[0][0] + m1[1][0] * m2[1][0] +
m1[2][0] * m2[2][0], m1[0][0] * m2[0][1] + m1[1][0] * m2[1][1] + m1[2][0] *
m2[2][1], m1[0][0] * m2[0][2] + m1[1][0] * m2[1][2] + m1[2][0] * m2[2][2],
