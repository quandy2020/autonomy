...
//			#pragma warning(disable:4530) // Disable the exception
// disable but used in MSCV Stl warning. 			#pragma
// warning(disable:4996) //Turn off warnings about deprecated C routines
...
/// The tf2Scalar type abstracts floating point numbers, to easily switch
/// between double and single floating point precision.
...
TF2SIMD_FORCE_INLINE tf2Scalar tf2Acos(tf2Scalar x) {
  if (x < tf2Scalar(-1)) x = tf2Scalar(-1);
  if (x > tf2Scalar(1)) x = tf2Scalar(1);
  return acos(x);
}
...
TF2SIMD_FORCE_INLINE bool tf2Equal(tf2Scalar a, tf2Scalar eps) { return (((a) <= eps) && !((a) < -eps)); }
TF2SIMD_FORCE_INLINE bool tf2GreaterEqual(tf2Scalar a, tf2Scalar eps) { return (!((a) <= eps)); }

TF2SIMD_FORCE_INLINE int tf2IsNegative(tf2Scalar x) { return x < tf2Scalar(0.0) ? 1 : 0; }
...
#ifndef tf2Fsel
TF2SIMD_FORCE_INLINE tf2Scalar tf2Fsel(tf2Scalar a, tf2Scalar b, tf2Scalar c) { return a >= 0 ? b : c; }
#endif
...
TF2SIMD_FORCE_INLINE unsigned tf2Select(unsigned condition, unsigned valueIfConditionNonZero,
  ...
  // bit off Use arithmetic shift right, shifting the sign bit through all 32
  // bits
...
TF2SIMD_FORCE_INLINE unsigned tf2SwapEndian(int val) { return tf2SwapEndian((unsigned)val); }

TF2SIMD_FORCE_INLINE unsigned short tf2SwapEndian(short val) { return tf2SwapEndian((unsigned short)val); }
...
/// might 'correct' invalid floating point values Not all values of
/// sign/exponent/mantissa are valid floating point numbers according to IEEE
/// 754. When a floating point unit is faced with an invalid value, it may
/// actually change the value, or worse, throw an exception. In most systems,
/// running user mode code, you wouldn't get an exception, but instead the
/// hardware/os/runtime will 'fix' the number for you. so instead of returning a
/// float/double, we return integer/long long integer
...
struct tf2TypedObject {
  tf2TypedObject(int objectType) : m_objectType(objectType) {}
  int m_objectType;
  inline int getObjectType() const { return m_objectType; }
};
