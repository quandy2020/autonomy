...
#define TF2_DECLARE_ALIGNED_ALLOCATOR()                             \
    TF2SIMD_FORCE_INLINE void* operator new(size_t sizeInBytes) {   \
        return tf2AlignedAlloc(sizeInBytes, 16);                    \
    }                                                               \
    TF2SIMD_FORCE_INLINE void operator delete(void* ptr) {          \
        tf2AlignedFree(ptr);                                        \
    }                                                               \
    TF2SIMD_FORCE_INLINE void* operator new(size_t, void* ptr) {    \
        return ptr;                                                 \
    }                                                               \
...
TF2SIMD_FORCE_INLINE bool tf2FuzzyZero(tf2Scalar x) {
    return tf2Fabs(x) < TF2SIMD_EPSILON;
}
...
TF2SIMD_FORCE_INLINE tf2Scalar tf2Radians(tf2Scalar x) {
    return x * TF2SIMD_RADS_PER_DEG;
}
TF2SIMD_FORCE_INLINE tf2Scalar tf2Degrees(tf2Scalar x) {
    return x * TF2SIMD_DEGS_PER_RAD;
}
