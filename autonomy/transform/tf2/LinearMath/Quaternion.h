...
class Quaternion : public QuadWord {
 public:
  ...
  Quaternion(const tf2Scalar& x, const tf2Scalar& y, const tf2Scalar& z, const tf2Scalar& w) : QuadWord(x, y, z, w) {}
  /**@brief Axis angle Constructor
   * @param axis The axis which the rotation is around
   * @param angle The magnitude of the rotation around the angle (Radians) */
  Quaternion(const Vector3& axis, const tf2Scalar& angle) { setRotation(axis, angle); }
  ...
  void setRotation(const Vector3& axis, const tf2Scalar& angle) {
    ...
    setValue(axis.x() * s, axis.y() * s, axis.z() * s, tf2Cos(angle * tf2Scalar(0.5)));
  }
  ...
  tf2Scalar length2() const { return dot(*this); }

  /**@brief Return the length of the quaternion */
  tf2Scalar length() const { return tf2Sqrt(length2()); }
  ...
  tf2Scalar angleShortestPath(const Quaternion& q) const {
    tf2Scalar s = tf2Sqrt(length2() * q.length2());
    tf2Assert(s != tf2Scalar(0.0));
    if (dot(q) < 0)  // Take care of long angle case see
  ...
  Quaternion inverse() const { return Quaternion(-m_floats[0], -m_floats[1], -m_floats[2], m_floats[3]); }
  ...
  TF2SIMD_FORCE_INLINE Quaternion farthest(const Quaternion& qd) const {
    ...
    diff = *this - qd;
    sum = *this + qd;
    if (diff.dot(diff) > sum.dot(sum)) return qd;
    return (-qd);
  }
   ...
   * result is this, if t=1 the result is q. Slerp interpolates assuming
   * constant velocity.  */
  Quaternion slerp(const Quaternion& q, const tf2Scalar& t) const {
    ...
    if (theta != tf2Scalar(0.0)) {
      ...
      if (dot(q) < 0)  // Take care of long angle case see
                       // http://en.wikipedia.org/wiki/Slerp
        return Quaternion((m_floats[0] * s0 + -q.x() * s1) * d, (m_floats[1] * s0 + -q.y() * s1) * d,
                          (m_floats[2] * s0 + -q.z() * s1) * d, (m_floats[3] * s0 + -q.m_floats[3] * s1) * d);
      else
        return Quaternion((m_floats[0] * s0 + q.x() * s1) * d, (m_floats[1] * s0 + q.y() * s1) * d,
                          (m_floats[2] * s0 + q.z() * s1) * d, (m_floats[3] * s0 + q.m_floats[3] * s1) * d);
...
TF2SIMD_FORCE_INLINE Quaternion operator-(const Quaternion& q) { return Quaternion(-q.x(), -q.y(), -q.z(), -q.w()); }
...
TF2SIMD_FORCE_INLINE Quaternion operator*(const Quaternion& q, const Vector3& w) {
  return Quaternion(q.w() * w.x() + q.y() * w.z() - q.z() * w.y(), q.w() * w.y() + q.z() * w.x() - q.x() * w.z(),
...
TF2SIMD_FORCE_INLINE Quaternion operator*(const Vector3& w, const Quaternion& q) {
  return Quaternion(w.x() * q.w() + w.y() * q.z() - w.z() * q.y(), w.y() * q.w() + w.z() * q.x() - w.x() * q.z(),
                    w.z() * q.w() + w.x() * q.y() - w.y() * q.x(), -w.x() * q.x() - w.y() * q.y() - w.z() * q.z());
}

/**@brief Calculate the dot product between two quaternions */
TF2SIMD_FORCE_INLINE tf2Scalar dot(const Quaternion& q1, const Quaternion& q2) { return q1.dot(q2); }

/**@brief Return the length of a quaternion */
TF2SIMD_FORCE_INLINE tf2Scalar length(const Quaternion& q) { return q.length(); }

/**@brief Return the ***half*** angle between two quaternions*/
TF2SIMD_FORCE_INLINE tf2Scalar angle(const Quaternion& q1, const Quaternion& q2) { return q1.angle(q2); }
...
TF2SIMD_FORCE_INLINE Quaternion inverse(const Quaternion& q) { return q.inverse(); }
...
shortestArcQuat(const Vector3& v0,
                const Vector3& v1)  // Game Programming Gems 2.10. make sure v0,v1 are normalized
{
  ...
  if (d < -1.0 + TF2SIMD_EPSILON) {
    ...
    return Quaternion(n.x(), n.y(), n.z(),
                      0.0f);  // just pick any vector that is orthogonal to v0
  }
