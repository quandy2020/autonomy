  ...
  try {
    ...
    for (const auto& tf_node : transforms_node) {
      ...
      if (tf_node["translation"]) {
        ...
        if (trans["x"]) transform.transform.translation.x = trans["x"].as<double>();
        if (trans["y"]) transform.transform.translation.y = trans["y"].as<double>();
        if (trans["z"]) transform.transform.translation.z = trans["z"].as<double>();
      }

      // rotation
      if (tf_node["rotation"]) {
        const auto& rot = tf_node["rotation"];
        if (rot["x"]) transform.transform.rotation.x = rot["x"].as<double>();
        if (rot["y"]) transform.transform.rotation.y = rot["y"].as<double>();
        if (rot["z"]) transform.transform.rotation.z = rot["z"].as<double>();
        if (rot["w"]) transform.transform.rotation.w = rot["w"].as<double>();
      } else {
