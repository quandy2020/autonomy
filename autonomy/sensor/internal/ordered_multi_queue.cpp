...
inline std::ostream& operator<<(std::ostream& out, const QueueKey& key) { return out << '(' << key.sensor_id << ')'; }
...
void OrderedMultiQueue::Dispatch() {
  ...
  //     // If we haven't dispatched any data for this trajectory yet, fast
  //     forward
  //     // all queues of this trajectory until a common start time has been
  //     reached. const Time common_start_time =
  //         GetCommonStartTime(next_queue_key);
  ...
  //       next_queue->callback(next_queue->queue.Pop());
  //     } else {
  //       // We take a peek at the time after next data. If it also is not
  //       beyond
  //       // 'common_start_time' we drop 'next_data', otherwise we just found
  //       the
...
Time OrderedMultiQueue::GetCommonStartTime() {
  //   auto emplace_result = common_start_time_.emplace(Time{});
  //   Time& common_start_time = emplace_result[0];
  //   if (emplace_result.second) {
  //     for (auto& entry : queues_) {
  //       if (entry.first.trajectory_id == trajectory_id) {
  //         common_start_time = std::max(
  //             common_start_time,
  //             entry.second.queue.Peek<Data>()->GetTime());
  //       }
  //     }
