...
AutoDiscovery::AutoDiscovery(std::shared_ptr<FoxgloveBridge> bridge)
    ...
    node_ = autolink::CreateNode("visualization_auto_discovery_" +
...
bool AutoDiscovery::SubscribeByTypeTraits(const std::string& topic_name,
    ...
    if (type_lower.find("geometry_msgs") != std::string::npos) {
        ...
        if (short_type == "twist") {
            return SubscribeTopicTyped<commsgs::proto::geometry_msgs::Twist>(
                topic_name);
        }
        if (short_type == "twiststamped") {
            return SubscribeTopicTyped<
                commsgs::proto::geometry_msgs::TwistStamped>(topic_name);
        }
        return false;
    }
    ...
    if (short_type == "twiststamped") {
        return SubscribeTopicTyped<commsgs::proto::geometry_msgs::TwistStamped>(
            topic_name);
    }
...
bool AutoDiscovery::SubscribeTopicTyped(const std::string& topic_name) {
    ...
    auto reader = node_->CreateReader<MsgType>(
        reader_cfg, [this, topic_name](const std::shared_ptr<MsgType>& msg) {
            AINFO_EVERY(200)
                << "Reader callback triggered for topic: " << topic_name;
...
void AutoDiscovery::OnProtoMessage(const std::string& topic_name,
    ...
    AINFO_EVERY(200) << "Publishing message to Foxglove for topic: "
                     << topic_name;
    if (!channel->Publish(*msg)) {
        ...
        AINFO_EVERY(200)
            << "Successfully published message to Foxglove for topic: "
            << topic_name;
    }
}
...
template bool AutoDiscovery::SubscribeTopicTyped<
    commsgs::proto::geometry_msgs::Twist>(const std::string&);
template bool AutoDiscovery::SubscribeTopicTyped<
    commsgs::proto::geometry_msgs::TwistStamped>(const std::string&);
template bool AutoDiscovery::SubscribeTopicTyped<
