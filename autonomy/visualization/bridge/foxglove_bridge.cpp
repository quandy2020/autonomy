...
FoxgloveBridge::~FoxgloveBridge() { Stop(); }
...
bool FoxgloveBridge::Start() {
  ...
  if (options_.enable_client_publish) {
    capabilities = capabilities | ::foxglove::WebSocketServerCapabilities::ClientPublish;
  }
  if (options_.enable_connection_graph) {
    capabilities = capabilities | ::foxglove::WebSocketServerCapabilities::ConnectionGraph;
  }
  if (options_.enable_parameters) {
    capabilities = capabilities | ::foxglove::WebSocketServerCapabilities::Parameters;
  }
  if (options_.enable_time) {
    capabilities = capabilities | ::foxglove::WebSocketServerCapabilities::Time;
  }
  if (options_.enable_services) {
    capabilities = capabilities | ::foxglove::WebSocketServerCapabilities::Services;
  }
  ...
  // Create server (retry ports if needed; common failure is "address already in use")
  ...
  for (int i = 0; i < kMaxPortTries; ++i) {
    // create() takes an rvalue; keep ws_options_ intact and move a copy each try
    ...
    if (server_result.has_value()) {
      ...
      if (i > 0) {
        AWARN << "Visualization port " << base_port << " unavailable, using " << (base_port + i);
      }
      break;
    }
  ...
  // Start AutoDiscovery
  discovery_ = std::make_unique<transport::AutoDiscovery>(std::static_pointer_cast<FoxgloveBridge>(shared_from_this()));
  if (discovery_) {
    discovery_->Start();
  ...
  AINFO << "Foxglove WebSocket server started successfully on " << options_.host << ":" << server_->port();
...
void FoxgloveBridge::Stop() {
  ...
  if (server_) {
    ...
    if (error != ::foxglove::FoxgloveError::Ok) {
      AERROR << "Error stopping Foxglove WebSocket server: " << static_cast<int>(error);
    }
    server_.reset();
  }
...
void FoxgloveBridge::SetupCallbacks(::foxglove::WebSocketServerCallbacks& callbacks) {
