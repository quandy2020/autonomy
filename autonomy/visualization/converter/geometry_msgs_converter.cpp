...
// 将 Arrow 的 +X 轴对齐到给定向量（简化版：优先用 XY 平面的 yaw；纯 Z
// 方向做特判）
foxglove::schemas::Quaternion QuaternionFromVector(double x, double y,
                                                   double z) {
...
foxglove::schemas::Pose MakeArrowPose(double px, double py, double pz,
...
foxglove::schemas::SceneUpdate ToFoxgloveImpl(
    ...
    foxglove::schemas::SceneEntity entity;
    SetEntityHeader(entity, /*timestamp=*/std::nullopt,
    ...
    if (message.has_linear()) {
        ...
        if (speed > 0.01) {
            ...
            // Foxglove 的 ArrowPrimitive 默认沿 pose 的 +X 轴延伸，所以用
            // pose.orientation 表达方向
            vel_arrow.pose =
                MakeArrowPose(/*px=*/0.0, /*py=*/0.0, /*pz=*/0.0, vx, vy, vz);
    ...
    if (message.has_angular()) {
        ...
        if (ang_speed > 0.01) {
            ...
            ang_arrow.pose =
                MakeArrowPose(/*px=*/0.0, /*py=*/0.0, /*pz=*/0.5, wx, wy, wz);
...
foxglove::schemas::SceneUpdate ToFoxgloveImpl(
    ...
    if (message.has_twist()) {
        ...
        if (twist.has_linear()) {
            ...
            if (speed > 0.01) {
                foxglove::schemas::ArrowPrimitive vel_arrow;
                vel_arrow.pose = MakeArrowPose(/*px=*/0.0, /*py=*/0.0,
                                               /*pz=*/0.0, vx, vy, vz);
        ...
        if (twist.has_angular()) {
            ...
            if (ang_speed > 0.01) {
                foxglove::schemas::ArrowPrimitive ang_arrow;
                ang_arrow.pose = MakeArrowPose(/*px=*/0.0, /*py=*/0.0,
                                               /*pz=*/0.5, wx, wy, wz);
    ...
    AINFO << "Converted TwistStamped to SceneUpdate (arrows="
          << entity.arrows.size() << ")";
    return scene_update;
}
