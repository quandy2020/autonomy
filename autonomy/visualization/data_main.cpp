...
void PublishPointCloud2(std::shared_ptr<autolink::Writer<autonomy::commsgs::proto::sensor_msgs::PointCloud2>> writer,
  ...
  *msg->mutable_header() = CreateHeader("map");
  ...
  for (int i = 0; i < WIDTH; ++i) {
    ...
    for (int j = 0; j < 4; ++j) msg->mutable_data()->Add(static_cast<uint32_t>(x_bytes[j]));
    for (int j = 0; j < 4; ++j) msg->mutable_data()->Add(static_cast<uint32_t>(y_bytes[j]));
    for (int j = 0; j < 4; ++j) msg->mutable_data()->Add(static_cast<uint32_t>(z_bytes[j]));
    for (int j = 0; j < 4; ++j) msg->mutable_data()->Add(static_cast<uint32_t>(i_bytes[j]));
  }
...
void Run() {
  ...
  auto pointcloud2_writer = g_node->CreateWriter<autonomy::commsgs::proto::sensor_msgs::PointCloud2>(pointcloud2_attr);
