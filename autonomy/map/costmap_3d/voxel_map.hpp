...
class VoxelMap : public common::MapInterface {
 public:
   ...
   */
  Eigen::Vector3i GetSize(void) const { return map_size_; }
   ...
   * @brief Get value
   */
  Eigen::Vector3d GetOrigin() const { return o_; }

  /**
   * @brief Get value
   */
  inline Eigen::Vector3d GetCorner() const { return map_size_.cast<double>() * scale_ + o_; }

  /**
   * @brief Get value
   */
  const std::vector<uint8>& GetVoxels() const { return voxels_; }
   ...
   * @brief Get
   */
  inline Eigen::Vector3d Cast(const Eigen::Vector3i& id) const { return id.cast<double>() * scale_ + oc_; }

  /**
   * @brief Get
   */
  inline Eigen::Vector3i Cast(const Eigen::Vector3d& pos) const { return ((pos - o_) / scale_).cast<int>(); }
