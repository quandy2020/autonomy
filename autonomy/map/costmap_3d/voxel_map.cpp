...
void VoxelMap::SetOccupied(const Eigen::Vector3d& pos) {
  ...
  if (id(0) >= 0 && id(1) >= 0 && id(2) >= 0 && id(0) < map_size_(0) && id(1) < map_size_(1) && id(2) < map_size_(2)) {
    ...
    for (int y = 0; y <= bounds_(1); y += step_(1)) {
      for (int z = 0; z <= bounds_(2); z += step_(2)) {
        if (voxels_[x + y + z] == kConstantOccupied) {
          VOXEL_DILATER(i, j, k, x, y, z, step_(1), step_(2), bounds_(0), bounds_(1), bounds_(2), check, voxels_, idx,
                        kConstantDilated, cvec)
        }
      }
    }
  }
