    ...
    for (const auto& layer : gridMapSourceCopy.getLayers()) {
        ...
        if (hasNaN) {
            result = GridMapCvConverter::toImage<unsigned short, 4>(
                gridMapSourceCopy, layer, CV_16UC4, minValue, maxValue,
                imageSource);
        } else {
            result = GridMapCvConverter::toImage<unsigned short, 1>(
                gridMapSourceCopy, layer, CV_16UC1, minValue, maxValue,
                imageSource);
        }
        if (!result)
            return false;
        cv::resize(imageSource, imageResult, cv::Size(0.0, 0.0), sizeFactor,
                   sizeFactor, interpolationAlgorithm);
        if (firstLayer) {
        ...
        if (hasNaN) {
            result = GridMapCvConverter::addLayerFromImage<unsigned short, 4>(
                imageResult, layer, gridMapResult, minValue, maxValue);
        } else {
...
GridMap GridMapCvProcessing::getTransformedMap(
    GridMap&& gridMapSource, const Eigen::Isometry3d& transform,
    ...
    for (const auto& layer : gridMapSource.getLayers()) {
        ...
        if (firstLayer) {
            ...
            boundingBox = cv::RotatedRect(cv::Point2f(0, 0), imageSource.size(),
                                          rotationAngle)
                              .boundingRect2f();
