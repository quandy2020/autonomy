...
TEST(CubicInterpolation, FlatWorld) {
    ...
    auto map =
        gmt::createMap(gm::Length(2.0, 2.0), 0.1, gm::Position(0.0, 0.0));
    auto trueValues = gmt::createFlatWorld(&map);
    const auto queryPoints = gmt::uniformlyDitributedPointsWithinMap(map, 100);

    verifyValuesAtQueryPointsAreClose(map, trueValues, queryPoints,
                                      gm::InterpolationMethods::INTER_CUBIC);

    if (::testing::Test::HasFailure()) {
        std::cout << "\n Test CubicInterpolation, FlatWorld failed with seed: "
                  << seed << std::endl;
    }
}
...
TEST(CubicInterpolation, RationalFunctionWorld) {
    ...
    gmt::rndGenerator.seed(seed);
    auto map =
        gmt::createMap(gm::Length(3.0, 3.0), 0.01, gm::Position(0.0, 0.0));
    ...
    if (::testing::Test::HasFailure()) {
        std::cout << "\n Test CubicInterpolation, RationalFunctionWorld failed "
                     "with seed: "
                  << seed << std::endl;
    }
}
...
TEST(CubicInterpolation, SaddleWorld) {
    ...
    gmt::rndGenerator.seed(seed);
    auto map =
        gmt::createMap(gm::Length(3.0, 3.0), 0.1, gm::Position(0.0, 0.0));
    ...
    if (::testing::Test::HasFailure()) {
        std::cout
            << "\n Test CubicInterpolation, SaddleWorld failed with seed: "
            << seed << std::endl;
    }
}
...
TEST(CubicInterpolation, SecondOrderPolyWorld) {
    ...
    if (::testing::Test::HasFailure()) {
        std::cout << "\n Test CubicInterpolation, SecondOrderPolyWorld failed "
                     "with seed: "
                  << seed << std::endl;
    }
}
...
TEST(CubicInterpolation, SineWorld) {
    ...
    if (::testing::Test::HasFailure()) {
        std::cout << "\n Test CubicInterpolation, SineWorld failed with seed: "
                  << seed << std::endl;
    }
}
...
TEST(CubicInterpolation, TanhWorld) {
    ...
    gmt::rndGenerator.seed(seed);
    auto map =
        gmt::createMap(gm::Length(2.5, 2.5), 0.02, gm::Position(0.0, 0.0));
    ...
    if (::testing::Test::HasFailure()) {
        std::cout << "\n Test CubicInterpolation, TanhWorld failed with seed: "
                  << seed << std::endl;
    }
}
...
TEST(CubicInterpolation, GaussianWorld) {
    ...
    gmt::rndGenerator.seed(seed);
    auto map =
        gmt::createMap(gm::Length(3.0, 3.0), 0.02, gm::Position(0.0, 0.0));
    ...
    if (::testing::Test::HasFailure()) {
        std::cout
            << "\n Test CubicInterpolation, GaussianWorld failed with seed: "
            << seed << std::endl;
    }
}
