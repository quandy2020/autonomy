...
class GridMap {
 public:
  // Type traits for use with template methods/classes using GridMap as a
  // template parameter.
  ...
  /*!
   * Constructor.
   * @param layers a vector of strings containing the definition/description
   * of the data layer.
   */
  ...
  /*!
   * Add a new data layer (if the layer already exists, overwrite its data,
   * otherwise add layer and data).
   * @param layer the name of the layer.
  ...
  /*!
   * Set the basic layers that need to be valid for a cell to be considered as
   * valid. Also, the basic layers are set to NAN when clearing the cells with
   * `clearBasic()`. By default the list of basic layers is empty.
   * @param basicLayers the list of layers that are the basic layers of the
   * map.
   */
   ...
   * @return the data of the cell.
   * @throw std::out_of_range if no map layer with name `layer` is present.
   * @throw std::runtime_error if the specified interpolation method is not
   * implemented.
   */
  float atPosition(const std::string& layer, const Position& position,
  ...
  /*!
   * Gets the 2d position of cell specified by the index (x, y of cell
   * position) in the grid map frame.
   ...
   * i.e. if all basic types are finite. Returns `false` if no basic types are
   * defined.
   ...
   * as z) in the grid map frame. This is useful for data layers such as
   * elevation.
   * @param layer the name of the layer to be accessed.
   ...
   * requested location and length. Note: The returned submap may not have the
   * requested length due to the borders of the map and discretization.
   * @param[in] position the requested position of the submap (usually the
   * center).
   ...
   * center).
   * @param[in] length the requested length of the submap.
   * @param[out] indexInSubmap the index of the requested position in the
   * submap.
   * @param[out] isSuccess true if successful, false otherwise.
   * @return submap (is empty if success is false).
   */
   ...
   * returns the transformed map. Note: The returned map may not have the same
   * length since it's geometric description contains Note: The transformation
   * will only be applied to the height layer of the grid map, other layers
   * will remain untouched. the original map.
   ...
   * @param[in] sampleRatio if zero or negative, no in-painting is used to
   * fill missing points due to sparsity of the map. Otherwise, four points
   * are sampled around each grid cell to make sure that at least one of those
   * points map to a new grid cell. A sampleRatio of 1 corresponds to the the
  ...
  /*!
   * Relocates the region captured by grid map w.r.t. to the static grid map
   * frame. Use this to move the grid map boundaries without relocating the
   * grid map data. Takes care of all the data handling, such that the grid
   * map data is stationary in the grid map frame.
   * - Data in the overlapping region before and after the position change
   * remains stored.
   ...
   *  The data storage is implemented as two-dimensional circular buffer to
   * minimize computational effort.
   *
   *  Note: Due to the circular buffer structure, neighbouring indices might
   * not fall close in the map frame. This assumption only holds for indices
   * obtained by getUnwrappedIndex().
   *
   ...
   * @param newRegions the regions of the newly covered / previously uncovered
  ...
  /*!
   * Move the grid map w.r.t. to the grid map frame. Use this to move the grid
   * map boundaries without moving the grid map data. Takes care of all the
   * data handling, such that the grid map data is stationary in the grid map
   * frame.
   * @param position the new location of the grid map in the map frame.
   * @return true if map has been moved, false otherwise.
   */
   ...
   * @param extendMap if true the grid map is resized that the other map fits
   * within.
   * @param overwriteData if true the new data replaces the old values, else
   * only invalid cells are updated.
   ...
   * If basic layers are used, clearBasic() is preferred as it is more
   * efficient. Header information (geometry etc.) remains valid.
   */
  void clearAll();
   ...
   * @param[in] position position that should be approached as close as
   * possible.
 ...
 private:
  ...
  /*!
   * Get cell data at requested position, linearly interpolated from 2x2
   * cells.
   * @param layer the name of the layer to be accessed.
   * @param position the requested position.
  ...
  //! This means that for a cell to be valid, all basic layers need to be
  //! valid. Also, the basic layers are set to NAN when clearing the map with
  //! `clear()`.
