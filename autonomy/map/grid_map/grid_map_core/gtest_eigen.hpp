 ...
 * @brief  Code to simplify Eigen integration into GTest. Pretty basic but the
...
void assertEqual(const M1& A, const M2& B, std::string const& message = "") {
    ASSERT_EQ((size_t)A.rows(), (size_t)B.rows())
        << message << "\nMatrix A:\n"
        << A << "\nand matrix B\n"
        << B << "\nare not the same\n";
    ASSERT_EQ((size_t)A.cols(), (size_t)B.cols())
        << message << "\nMatrix A:\n"
        << A << "\nand matrix B\n"
        << B << "\nare not the same\n";
...
void assertNear(const M1& A, const M2& B, T tolerance,
    ...
    for (int r = 0; r < A.rows(); r++) {
        for (int c = 0; c < A.cols(); c++) {
            if (std::isnan(A(r, c))) {
                ...
                ASSERT_NEAR(A(r, c), B(r, c), tolerance)
...
void expectNear(const M1& A, const M2& B, T tolerance,
                std::string const& message = "") {
    EXPECT_EQ((size_t)A.rows(), (size_t)B.rows())
        << message << "\nMatrix A:\n"
        << A << "\nand matrix B\n"
        << B << "\nare not the same\n";
    EXPECT_EQ((size_t)A.cols(), (size_t)B.cols())
        << message << "\nMatrix A:\n"
        << A << "\nand matrix B\n"
        << B << "\nare not the same\n";
...
void assertFinite(const M1& A, std::string const& /*message*/ = "") {
    ...
    for (int r = 0; r < A.rows(); r++) {
        for (int c = 0; c < A.cols(); c++) {
            ASSERT_TRUE(std::isfinite(A(r, c)))
                << std::endl
                << "Check for finite values failed at A(" << r << "," << c
                << "). Matrix A:" << std::endl
                << A << std::endl;
        }
    }
...
inline bool compareRelative(double a, double b, double percentTolerance,
...
#define ASSERT_DOUBLE_MX_EQ(A, B, PERCENT_TOLERANCE, MSG)                \
    ...
    for (int r = 0; r < (A).rows(); r++) {                               \
        for (int c = 0; c < (A).cols(); c++) {                           \
            ...
            ASSERT_TRUE(grid_map::compareRelative(                       \
                (A)(r, c), (B)(r, c), PERCENT_TOLERANCE, &percentError)) \
                << (MSG) << "\nComparing:\n"                             \
                << #A << "(" << r << "," << c << ") = " << (A)(r, c)     \
                << std::endl                                             \
                << #B << "(" << r << "," << c << ") = " << (B)(r, c)     \
                << std::endl                                             \
                << "Error was " << percentError << "% > "                \
                << (PERCENT_TOLERANCE) << "%\n"                          \
                << "\nMatrix " << #A << ":\n"                            \
                << (A) << "\nand matrix " << #B << "\n"                  \
                << (B);                                                  \
...
#define ASSERT_DOUBLE_SPARSE_MX_EQ(A, B, PERCENT_TOLERANCE, MSG)             \
    ...
    for (int r = 0; r < (A).rows(); r++) {                                   \
        for (int c = 0; c < (A).cols(); c++) {                               \
            double percentError = 0.0;                                       \
            ASSERT_TRUE(                                                     \
                grid_map::compareRelative((A).coeff(r, c), (B).coeff(r, c),  \
                                          PERCENT_TOLERANCE, &percentError)) \
                << (MSG) << "\nComparing:\n"                                 \
                << #A << "(" << r << "," << c << ") = " << (A).coeff(r, c)   \
                << std::endl                                                 \
                << #B << "(" << r << "," << c << ") = " << (B).coeff(r, c)   \
