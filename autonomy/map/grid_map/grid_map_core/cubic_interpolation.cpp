...
#include "autonomy/map/grid_map/grid_map_core/cubic_interpolation.hpp"
...
double getLayerValue(const Matrix& layerMat, unsigned int rowReq,
                     unsigned int colReq) {
...
bool evaluateBicubicConvolutionInterpolation(const GridMap& gridMap,
    ...
    if (!assembleFunctionValueMatrix(gridMap, layer, queriedPosition,
                                     &functionValues)) {
        return false;
    }

    Position normalizedCoordinate;
    if (!getNormalizedCoordinates(gridMap, queriedPosition,
...
double convolve1D(double t, const Eigen::Vector4d& functionValues) {
    ...
    const Eigen::Vector4d temp =
        cubicInterpolationConvolutionMatrix * functionValues;
...
bool assembleFunctionValueMatrix(const GridMap& gridMap,
     ...
     * increasing f's indices is flipped w.r.t. to the above since in the
     * article they use a coordinate frame centered around (i,j). Therefore:
     ...
     */
    *data << f(i + 1, j + 1), f(i, j + 1), f(i - 1, j + 1), f(i - 2, j + 1),
        f(i + 1, j), f(i, j), f(i - 1, j), f(i - 2, j), f(i + 1, j - 1),
        f(i, j - 1), f(i - 1, j - 1), f(i - 2, j - 1), f(i + 1, j - 2),
        f(i, j - 2), f(i - 1, j - 2), f(i - 2, j - 2);
...
bool getNormalizedCoordinates(const GridMap& gridMap,
    ...
    position->x() =
        (queriedPosition.x() - middleKnot.x()) / gridMap.getResolution();
    position->y() =
        (queriedPosition.y() - middleKnot.y()) / gridMap.getResolution();
...
bool evaluateBicubicInterpolation(const GridMap& gridMap,
    ...
    if (!getUnitSquareCornerIndices(gridMap, queriedPosition,
                                    &unitSquareCornerIndices)) {
        return false;
    }
    ...
    DataMatrix dfx;
    if (!getFirstOrderDerivatives(layerMat, unitSquareCornerIndices, Dim2D::X,
                                  resolution, &dfx)) {
        return false;
    }
    ...
    DataMatrix dfy;
    if (!getFirstOrderDerivatives(layerMat, unitSquareCornerIndices, Dim2D::Y,
                                  resolution, &dfy)) {
        return false;
    }
    // mixed derivatives in x y
    DataMatrix ddfxy;
    if (!getMixedSecondOrderDerivatives(layerMat, unitSquareCornerIndices,
                                        resolution, &ddfxy)) {
        return false;
    }
    ...
    *interpolatedValue = evaluatePolynomial(
        functionValues, normalizedCoordinates.x(), normalizedCoordinates.y());
...
bool computeNormalizedCoordinates(const GridMap& gridMap,
    ...
    normalizedCoordinates->x() =
        (queriedPosition.x() - origin.x()) / gridMap.getResolution();
    normalizedCoordinates->y() =
        (queriedPosition.y() - origin.y()) / gridMap.getResolution();
...
bool getFunctionValues(const Matrix& layerData, const IndicesMatrix& indices,
    ...
    data->bottomLeft_ =
        layerData(indices.bottomLeft_.x(), indices.bottomLeft_.y());
    data->bottomRight_ =
        layerData(indices.bottomRight_.x(), indices.bottomRight_.y());
    return true;
}
...
void bindIndicesToRange(const GridMap& gridMap, IndicesMatrix* indices) {
    ...
    // top left
    {
        const unsigned int iBoundToRange =
            bindIndexToRange(indices->topLeft_.x(), numRow);
        const unsigned int jBoundToRange =
            bindIndexToRange(indices->topLeft_.y(), numCol);
    ...
    {
        const unsigned int iBoundToRange =
            bindIndexToRange(indices->topRight_.x(), numRow);
        const unsigned int jBoundToRange =
            bindIndexToRange(indices->topRight_.y(), numCol);
    ...
    {
        const unsigned int iBoundToRange =
            bindIndexToRange(indices->bottomLeft_.x(), numRow);
        const unsigned int jBoundToRange =
            bindIndexToRange(indices->bottomLeft_.y(), numCol);
    ...
    {
        const unsigned int iBoundToRange =
            bindIndexToRange(indices->bottomRight_.x(), numRow);
        const unsigned int jBoundToRange =
            bindIndexToRange(indices->bottomRight_.y(), numCol);
...
bool getFirstOrderDerivatives(const Matrix& layerData,
    ...
    derivatives->topLeft_ =
        firstOrderDerivativeAt(layerData, indices.topLeft_, dim, resolution);
    derivatives->topRight_ =
        firstOrderDerivativeAt(layerData, indices.topRight_, dim, resolution);
    derivatives->bottomLeft_ =
        firstOrderDerivativeAt(layerData, indices.bottomLeft_, dim, resolution);
    derivatives->bottomRight_ = firstOrderDerivativeAt(
        layerData, indices.bottomRight_, dim, resolution);
    return true;
}
...
double firstOrderDerivativeAt(const Matrix& layerData, const Index& index,
    ...
    switch (dim) {
        case Dim2D::X: {
            left =
                layerData(bindIndexToRange(index.x() + 1, numRow), index.y());
            right =
                layerData(bindIndexToRange(index.x() - 1, numRow), index.y());
            break;
        }
        case Dim2D::Y: {
            left =
                layerData(index.x(), bindIndexToRange(index.y() + 1, numCol));
            right =
                layerData(index.x(), bindIndexToRange(index.y() - 1, numCol));
            break;
        }
...
double mixedSecondOrderDerivativeAt(const Matrix& layerData, const Index& index,
    ...
    return (f11 - f1m1 - fm11 + fm1m1) / (4.0 * perturbation * perturbation) *
...
bool getMixedSecondOrderDerivatives(const Matrix& layerData,
    ...
    derivatives->topLeft_ =
        mixedSecondOrderDerivativeAt(layerData, indices.topLeft_, resolution);
    derivatives->topRight_ =
        mixedSecondOrderDerivativeAt(layerData, indices.topRight_, resolution);
    derivatives->bottomLeft_ = mixedSecondOrderDerivativeAt(
        layerData, indices.bottomLeft_, resolution);
    derivatives->bottomRight_ = mixedSecondOrderDerivativeAt(
        layerData, indices.bottomRight_, resolution);
    return true;
}
...
double evaluatePolynomial(const FunctionValueMatrix& functionValues, double tx,
    ...
    const Eigen::Matrix4d tempMat =
        functionValues * bicubicInterpolationMatrix.transpose();
    const Eigen::Matrix4d polynomialCoeffMatrix =
        bicubicInterpolationMatrix * tempMat;
