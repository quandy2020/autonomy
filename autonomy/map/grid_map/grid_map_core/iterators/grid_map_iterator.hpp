...
public:
     ...
     * @return the regular index (2-dim.) of the cell on which the iterator is
     * pointing.
     */
    Index operator*() const;

    /*!
     * Returns the the linear (1-dim.) index of the cell the iterator is
     * pointing at. Note: Use this access for improved efficiency when working
     * with large maps. Example: See `runGridMapIteratorVersion3()` of
     * `grid_map_demos/src/iterator_benchmark.cpp`.
     * @return the 1d linear index.
     */
    const size_t& getLinearIndex() const;

    /*!
     * Retrieve the index as unwrapped index, i.e., as the corresponding index
     * of a grid map with no circular buffer offset.
     */
     ...
     * @return true if iterator is out of scope, false if end has not been
     * reached.
     */
