...
static void assertMatrixesEqualWithNan(Eigen::Ref<const Eigen::MatrixXf> first,
                                       ...
                                       std::string secondName, int line) {
    ...
    for (Eigen::Index row = 0; row < first.rows() && matricesAreEqual; ++row) {
        for (Eigen::Index col = 0; col < first.cols() && matricesAreEqual;
             ++col) {
            bool ifRealThenValid = first.block<1, 1>(row, col).isApprox(
                second.block<1, 1>(row, col));
            bool bothNaN =
                std::isnan(first(row, col)) && std::isnan(second(row, col));
    ...
    ASSERT_TRUE(matricesAreEqual)  // NO LINT
        ...
        << ": Matrices are not equal"             // NO LINT
        << "\n"                                   // NO LINT
        << firstName << "\n"                      // NO LINT
