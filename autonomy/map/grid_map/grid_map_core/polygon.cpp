...
#include "autonomy/map/grid_map/grid_map_core/polygon.hpp"
...
Polygon::Polygon() : timestamp_(0) {}

Polygon::Polygon(std::vector<Position> vertices) : Polygon() { vertices_ = vertices; }
...
void Polygon::addVertex(const Position& vertex) { vertices_.push_back(vertex); }

const Position& Polygon::getVertex(const size_t index) const { return vertices_.at(index); }

void Polygon::removeVertices() { vertices_.clear(); }

const Position& Polygon::operator[](const size_t index) const { return getVertex(index); }

const std::vector<Position>& Polygon::getVertices() const { return vertices_; }

size_t Polygon::nVertices() const { return vertices_.size(); }

const std::string& Polygon::getFrameId() const { return frameId_; }

void Polygon::setFrameId(const std::string& frameId) { frameId_ = frameId; }

uint64_t Polygon::getTimestamp() const { return timestamp_; }

void Polygon::setTimestamp(const uint64_t timestamp) { timestamp_ = timestamp; }

void Polygon::resetTimestamp() { timestamp_ = 0.0; }
...
bool Polygon::convertToInequalityConstraints(Eigen::MatrixXd& A, Eigen::VectorXd& b) const {
  ...
  for (unsigned int i = 0; i < nVertices(); ++i) V.row(i) = vertices_[i];
  ...
  Eigen::MatrixXi k;
  k.resizeLike(V);
  for (unsigned int i = 0; i < V.rows(); ++i) k.row(i) << i, (i + 1) % V.rows();
...
std::vector<Polygon> Polygon::triangulate(const TriangulationMethods& /*method*/) const {
  ...
  if (vertices_.size() < 3) return polygons;
