...
#include "autonomy/map/grid_map/grid_map_octomap/grid_map_octomap_oonverter.hpp"
...
bool GridMapOctomapConverter::fromOctomap(const octomap::OcTree& octomap, const std::string& layer,
    ...
    // Copy octomap in order to expand any pruned occupied cells and maintain
    // constness of input.
    octomap::OcTree octomapCopy(octomap);

    // Iterate through leaf nodes and project occupied cells to elevation map.
    // On the first pass, expand all occupied cells that are not at maximum
    // depth.
    ...
    do {
        ...
        // std::cout << "Expanded " << collapsed_occ_nodes.size() << " nodes" <<
        // std::endl;
    ...
    // std::endl; std::cout << "Position: [" << position(0) << ", " <<
    // position(1)
    // << "]" << std::endl; std::cout << "Resolution: " << resolution <<
    // std::endl;
    ...
    // For each voxel, if its elevation is higher than the existing value for
