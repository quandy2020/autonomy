...
bool CurvatureFilter::configure() {
  if (!FilterBase::getParam(std::string("input_layer"), inputLayer_)) {
    AERROR << "Curvature filter did not find parameter `input_layer`.";
    return false;
  }
  ADEBUG << "Curvature filter input layer is = " << (inputLayer_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("output_layer"), outputLayer_)) {
    AERROR << "Curvature filter did not find parameter `output_layer`.";
    return false;
  }
  ADEBUG << "Curvature filter output_layer = " << (outputLayer_.c_str()) << ".";
