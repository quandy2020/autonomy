...
bool NormalColorMapFilter::configure() {
  if (!FilterBase::getParam(std::string("input_layers_prefix"), inputLayersPrefix_)) {
    AERROR << "Normal color map filter did not find parameter `input_layers_prefix`.";
    return false;
  }
  ADEBUG << "Normal color map filter input layers prefix is = " << (inputLayersPrefix_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("output_layer"), outputLayer_)) {
    AERROR << "Normal color map filter did not find parameter `output_layer`.";
    return false;
  }
  ADEBUG << "Normal color map filter output_layer = " << (outputLayer_.c_str()) << ".";
  return true;
}
