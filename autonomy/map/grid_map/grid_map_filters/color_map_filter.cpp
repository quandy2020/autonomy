...
bool ColorMapFilter::configure() {
  if (!FilterBase::getParam(std::string("input_layer"), inputLayer_)) {
    AERROR << "Color map filter did not find parameter `input_layer`.";
    return false;
  }
  ADEBUG << "Color map filter input_layer = " << (inputLayer_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("output_layer"), outputLayer_)) {
    AERROR << "Color map filter did not find parameter `output_layer`.";
    return false;
  }
  ADEBUG << "Color map filter output_layer = " << (outputLayer_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("min/value"), min_)) {
    AERROR << "Color map filter did not find parameter `min/value`.";
    return false;
  }
  if (!FilterBase::getParam(std::string("max/value"), max_)) {
    AERROR << "Color map filter did not find parameter `max/value`.";
    return false;
  }

  std::vector<double> minColor;
  if (!FilterBase::getParam(std::string("min/color"), minColor)) {
    AERROR << "Color map filter did not find parameter `min/color`.";
    return false;
  }
  if (minColor.size() != 3) {
    AERROR << "Color map filter parameter `min/color` needs to be of size 3.";
    return false;
  }
  ...
  if (!FilterBase::getParam(std::string("max/color"), maxColor)) {
    AERROR << "Color map filter did not find parameter `max/color`.";
    return false;
  }
  if (maxColor.size() != 3) {
    AERROR << "Color map filter parameter `max/color` needs to be of size 3.";
    return false;
  }
