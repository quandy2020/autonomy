...
bool ThresholdFilter::configure() {
  ...
  if (FilterBase::getParam(std::string("lower_threshold"), lowerThreshold_)) {
    ...
    ADEBUG << "lower threshold = " << lowerThreshold_;
  }

  if (FilterBase::getParam(std::string("upper_threshold"), upperThreshold_)) {
    useUpperThreshold_ = true;
    ADEBUG << "upper threshold = " << upperThreshold_;
  }

  if (!useLowerThreshold_ && !useUpperThreshold_) {
    AERROR << "ThresholdFilter did not find parameter 'lower_threshold' or 'upper_threshold',";
    return false;
  }

  if (useLowerThreshold_ && useUpperThreshold_) {
    AERROR << "Set either 'lower_threshold' or 'upper_threshold'! Only one threshold can be used!";
    return false;
  }

  if (!FilterBase::getParam(std::string("set_to"), setTo_)) {
    AERROR << "ThresholdFilter did not find parameter 'set_to'.";
    return false;
  }

  if (!FilterBase::getParam(std::string("condition_layer"), conditionLayer_)) {
    AERROR << "ThresholdFilter did not find parameter 'condition_layer'.";
    return false;
  }

  if (!FilterBase::getParam(std::string("output_layer"), outputLayer_)) {
    AERROR << "ThresholdFilter did not find parameter 'ouput_layer'.";
    return false;
  }
...
bool ThresholdFilter::update(const GridMap& mapIn, GridMap& mapOut) {
  ...
  if (!mapOut.exists(conditionLayer_)) {
    AERROR << "Check your condition_layer! Layer " << (conditionLayer_.c_str()) << " does not exist";
    return false;
  }

  if (!mapOut.exists(outputLayer_)) {
    AERROR << "Check your output_layer! Layer " << (outputLayer_.c_str()) << " does not exist";
    return false;
  }
