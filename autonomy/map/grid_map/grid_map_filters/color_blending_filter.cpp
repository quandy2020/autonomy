...
bool ColorBlendingFilter::configure() {
  if (!FilterBase::getParam(std::string("background_layer"), backgroundLayer_)) {
    AERROR << "Color blending filter did not find parameter `background_layer`.";
    return false;
  }
  ADEBUG << "Color blending filter background layer is = " << (backgroundLayer_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("foreground_layer"), foregroundLayer_)) {
    AERROR << "Color blending filter did not find parameter `foreground_layer`.";
    return false;
  }
  ADEBUG << "Color blending filter foreground layer is = " << (foregroundLayer_.c_str()) << ".";

  std::string blendMode;
  if (!FilterBase::getParam(std::string("blend_mode"), blendMode)) {
    blendMode = "normal";
  }
  ADEBUG << "Color blending filter blend mode is = " << (blendMode.c_str()) << ".";
  if (blendMode == "normal") {
    ...
    AERROR << "Color blending filter blend mode `" << (blendMode.c_str()) << "` does not exist.";
    return false;
  }

  if (!FilterBase::getParam(std::string("opacity"), opacity_)) {
    AERROR << "Color blending filter did not find parameter `opacity`.";
    return false;
  }
  ADEBUG << "Color blending filter opacity is = " << (opacity_) << ".";

  if (!FilterBase::getParam(std::string("output_layer"), outputLayer_)) {
    AERROR << "Color blending filter did not find parameter `output_layer`.";
    return false;
  }
  ADEBUG << "Color blending filter output_layer = " << (outputLayer_.c_str()) << ".";
  return true;
}
