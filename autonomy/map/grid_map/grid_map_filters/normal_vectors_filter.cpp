...
bool NormalVectorsFilter::configure() {
  ...
  if (!FilterBase::getParam(std::string("algorithm"), algorithm)) {
    AWARN << "Could not find the parameter: `algorithm`. Setting to default value: 'area'.";
    // Default value.
    algorithm = "area";
  }
  ...
  if (algorithm != "raster") {
    ...
    if (!FilterBase::getParam(std::string("radius"), estimationRadius_)) {
      AWARN << "Could not find the parameter: `radius`. Switching to raster method.";
      algorithm = "raster";
    }
    ADEBUG << "Normal vectors estimation radius = " << estimationRadius_;
    // If radius not positive switch to raster method.
    if (estimationRadius_ <= 0) {
      AWARN << "Parameter `radius` is not positive. Switching to raster method.";
      algorithm = "raster";
    }
  }
  ...
  if (!FilterBase::getParam(std::string("parallelization_enabled"), parallelizationEnabled_)) {
    AWARN << "Could not find the parameter: `parallelization_enabled`. Setting to default value: 'false'.";
    parallelizationEnabled_ = false;
  }
  ADEBUG << "Parallelization_enabled = " << parallelizationEnabled_;
  ...
  if (!FilterBase::getParam(std::string("thread_number"), threadCount_)) {
    AWARN << "Could not find the parameter: `thread_number`. Setting to default value: 'automatic'.";
    threadCount_ = -1;  // automatic: do not cap TBB parallelism
  }
  ADEBUG << "Thread_number = " << threadCount_;
  ...
  if (algorithm == "raster") {
    ...
    if (parallelizationEnabled_) {
      ...
      ADEBUG << "Method RasterParallel";
    } else {
      method_ = Method::RasterSerial;
      ADEBUG << "Method RasterSerial";
    }
  } else {
    // If parallelizationEnabled_=true, use the parallel method, otherwise serial.
    if (parallelizationEnabled_) {
      method_ = Method::AreaParallel;
      ADEBUG << "Method AreaParallel";
    } else {
      method_ = Method::AreaSerial;
      ADEBUG << "Method AreaSerial";
    }
    ADEBUG << "estimationRadius_ = " << estimationRadius_;
  }
  ...
  if (!FilterBase::getParam(std::string("normal_vector_positive_axis"), normalVectorPositiveAxis)) {
    AERROR << "Normal vectors filter did not find parameter `normal_vector_positive_axis`.";
    return false;
  }
  if (normalVectorPositiveAxis == "z") {
    ...
    AERROR << "The normal vector positive axis '" << (normalVectorPositiveAxis.c_str()) << "' is not valid.";
    return false;
  }

  // Read input_layer, to define input grid map layer.
  if (!FilterBase::getParam(std::string("input_layer"), inputLayer_)) {
    AERROR << "Normal vectors filter did not find parameter `input_layer`.";
    return false;
  }
  ADEBUG << "Normal vectors filter input layer is = " << (inputLayer_.c_str()) << ".";
  ...
  if (!FilterBase::getParam(std::string("output_layers_prefix"), outputLayersPrefix_)) {
    AERROR << "Normal vectors filter did not find parameter `output_layers_prefix`.";
    return false;
  }
  ADEBUG << "Normal vectors filter output_layer = " << (outputLayersPrefix_.c_str()) << ".";
...
void NormalVectorsFilter::computeWithAreaSerial(GridMap& map, const std::string& inputLayer, const std::string& outputLayersPrefix) {
  ...
  ADEBUG << "NORMAL COMPUTATION TIME = " << std::chrono::duration<double>(end - start).count();
}
...
void NormalVectorsFilter::computeWithAreaParallel(GridMap& map, const std::string& inputLayer, const std::string& outputLayersPrefix) {
  ...
  if (threadCount_ > 0) {
    tbbControl = std::make_unique<tbb::global_control>(
        tbb::global_control::max_allowed_parallelism, static_cast<size_t>(threadCount_));
  }
...
void NormalVectorsFilter::areaSingleNormalComputation(GridMap& map, const std::string& inputLayer, const std::string& outputLayersPrefix,
  ...
  if (estimationRadius_ <= minAllowedEstimationRadius) {
    AWARN << "Estimation radius is smaller than allowed by the map resolution (" << (estimationRadius_) << " < " << (minAllowedEstimationRadius) << ")";
  }
  ...
  if (nPoints < 3) {
    ADEBUG << "Not enough points to establish normal direction (nPoints = " << (static_cast<int>(nPoints)) << ")";
    ...
    if (solver.eigenvalues()(1) > 1e-8) {
      ...
      ADEBUG << "Covariance matrix needed for eigen decomposition is degenerated.";
      ADEBUG << "Expected cause: data is on a straight line (nPoints = " << (static_cast<int>(nPoints)) << ")";
      unitaryNormalVector = Vector3::UnitZ();
    }
  }
...
void NormalVectorsFilter::computeWithRasterParallel(GridMap& map, const std::string& inputLayer, const std::string& outputLayersPrefix) {
  ...
  if (threadCount_ > 0) {
    ...
    AERROR << "Grid map size is too small for normal raster computation";
  }
