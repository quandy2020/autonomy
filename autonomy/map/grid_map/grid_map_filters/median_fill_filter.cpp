...
bool MedianFillFilter::configure() {
  if (!FilterBase::getParam(std::string("fill_hole_radius"), fillHoleRadius_)) {
    AERROR << "Median filter did not find parameter fill_hole_radius.";
    return false;
  }

  if (fillHoleRadius_ < 0.0) {
    AERROR << "fill_hole_radius must be greater than zero.";
    return false;
  }

  ADEBUG << "fill_hole_radius = " << (fillHoleRadius_) << ".";

  if (!FilterBase::getParam(std::string("filter_existing_values"), filterExistingValues_)) {
    AINFO << "Median filter did not find parameter filter_existing_values. Not filtering existing values.";
    filterExistingValues_ = false;
  }

  ADEBUG << "Filter_existing_values = " << (filterExistingValues_ ? "true" : "false") << ".";

  if (!FilterBase::getParam(std::string("input_layer"), inputLayer_)) {
    AERROR << "Median filter did not find parameter `input_layer`.";
    return false;
  }

  if (!FilterBase::getParam(std::string("num_erode_dilation_iterations"), numErodeDilationIterations_)) {
    AERROR << "Median filter did not find parameter `num_erode_dilation_iterations`.";
    return false;
  }

  if (filterExistingValues_) {
    if (!FilterBase::getParam(std::string("existing_value_radius"), existingValueRadius_)) {
      AERROR << "Median filter did not find parameter existing_value_radius.";
      return false;
    }

    if (existingValueRadius_ < 0.0) {
      AERROR << "existing_value_radius must be greater than zero.";
      return false;
    }

    ADEBUG << "existing_value_radius = " << (existingValueRadius_) << ".";
  }

  ADEBUG << "Median input layer is = " << (inputLayer_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("output_layer"), outputLayer_)) {
    AERROR << "Median filter did not find parameter `output_layer`.";
    return false;
  }

  ADEBUG << "Median output layer = " << (outputLayer_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("fill_mask_layer"), fillMaskLayer_)) {
    AERROR << "Median filter did not find parameter `fill_mask_layer`.";
    return false;
  }

  ADEBUG << "Median fill mask layer = " << (fillMaskLayer_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("debug"), debug_)) {
    AINFO << "Median filter did not find parameter debug. Disabling debug output.";
    debug_ = false;
  }

  ADEBUG << "Debug mode= " << (debug_ ? "true" : "false") << ".";

  if (debug_ && !FilterBase::getParam(std::string("debug_infill_mask_layer"), debugInfillMaskLayer_)) {
    AERROR << "Median filter did not find parameter `debug_infill_mask_layer`.";
    return false;
  }

  ADEBUG << "Median debug infill mask layer = " << (debugInfillMaskLayer_.c_str()) << ".";
...
bool MedianFillFilter::update(const GridMap& mapIn, GridMap& mapOut) {
  ...
  ADEBUG << "Median fill filter " << this->getName() << " observed " << numNans << " Nans in input layer!";
