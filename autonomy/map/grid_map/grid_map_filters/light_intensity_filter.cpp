...
bool LightIntensityFilter::configure() {
  if (!FilterBase::getParam(std::string("input_layers_prefix"), inputLayersPrefix_)) {
    AERROR << "Light intensity filter did not find parameter `input_layers_prefix`.";
    return false;
  }
  ADEBUG << "Light intensity filter input layers prefix is = " << (inputLayersPrefix_.c_str()) << ".";

  if (!FilterBase::getParam(std::string("output_layer"), outputLayer_)) {
    AERROR << "Light intensity filter did not find parameter `output_layer`.";
    return false;
  }
  ADEBUG << "Light intensity filter output_layer = " << (outputLayer_.c_str()) << ".";

  std::vector<double> lightDirection;
  if (!FilterBase::getParam(std::string("light_direction"), lightDirection)) {
    AERROR << "Light intensity filter did not find parameter `light_direction`.";
    return false;
  }
  if (lightDirection.size() != 3) {
    AERROR << "Light intensity filter parameter `light_direction` needs to be of size 3.";
    return false;
  }
  lightDirection_ << lightDirection[0], lightDirection[1], lightDirection[2];
  lightDirection_.normalize();
  ADEBUG << "Light intensity filter light_direction: " << lightDirection_.transpose() << ".";
