...
#include "autonomy/map/grid_map/grid_map_demos/interpolation_demo.hpp"
...
AnalyticalFunctions createGaussianWorld(grid_map::GridMap *map)
{
  ...
  std::array<double, numGaussians> scales = {
      {-2.0, -1.0, 2.0, 1.0, 3.0, 4.0, 1.0}};
...
InterpolationDemo::InterpolationDemo(const Options& options)
{
  interpolationMethod_ = options.interpolationMethod;
  world_ = options.world;
  groundTruthResolution_ = options.groundTruthResolution;
  dataResolution_ = options.dataResolution;
  interpolatedResolution_ = options.interpolatedResolution;
  worldLength_ = options.worldLength;
  worldWidth_ = options.worldWidth;
...
void InterpolationDemo::runDemo()
{
  ...
  grid_map::grid_map_demos::saveLayerAsPng(groundTruthMap_, demoLayer,
                                 std::filesystem::path(outDir_) / "ground_truth.png");
  grid_map::grid_map_demos::saveLayerAsPng(dataSparseMap_, demoLayer,
                                 std::filesystem::path(outDir_) / "data_sparse.png");
  grid_map::grid_map_demos::saveLayerAsPng(interpolatedMap_, demoLayer,
                                 std::filesystem::path(outDir_) / "interpolated.png");
...
int main(int argc, char** argv) {
  grid_map::grid_map_demos::InterpolationDemo::Options options;
  options.world = grid_map::grid_map_demos::requireArg(argc, argv, 1, "Sine");
  options.interpolationMethod =
      grid_map::grid_map_demos::requireArg(argc, argv, 2, "Nearest");
  options.outDir = grid_map::grid_map_demos::requireArg(
      argc, argv, 3, "/tmp/grid_map_demos/interpolation");
  grid_map::grid_map_demos::InterpolationDemo demo(options);
  return 0;
}
