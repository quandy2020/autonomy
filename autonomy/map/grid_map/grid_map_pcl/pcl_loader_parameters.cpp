...
#include "autonomy/map/grid_map/grid_map_pcl/pcl_loader_parameters.hpp"
...
bool PclLoaderParameters::loadParameters(const std::string& filename) {
  ...
  if (loadingFailed) {
    AERROR << "PclLoaderParameters: Reading from file failed";
    return false;
  }
  ...
  try {
    ...
    AERROR << "PclLoaderParameters: Loading parameters failed: " << exception.what();
    return false;
  }
