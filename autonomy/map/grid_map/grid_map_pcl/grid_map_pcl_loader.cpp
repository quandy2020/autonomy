...
void GridMapPclLoader::preProcessInputCloud() {
  ...
  AINFO << "Preprocessing of the pointcloud started";
  ...
  AINFO << "Preprocessing and filtering finished";
}
...
void GridMapPclLoader::initializeGridMapGeometryFromInputCloud() {
  ...
  AINFO << "Grid map dimensions: " << workingGridMap_.getLength()(0) << " x "
        << workingGridMap_.getLength()(1);
  AINFO << "Grid map resolution: " << workingGridMap_.getResolution();
  AINFO << "Grid map num cells: " << workingGridMap_.getSize()(0) << " x "
        << workingGridMap_.getSize()(1);
  AINFO << "Initialized map geometry";
}

void GridMapPclLoader::addLayerFromInputCloud(const std::string& layer) {
  AINFO << "Started adding layer: " << layer;
  // Preprocess: allocate memory in the internal data structure
  preprocessGridMapCells();
  AINFO << "Finished preprocessing";
  ...
  AWARN << "OpemMP not found, defaulting to single threaded implementation";
#else
...
#pragma omp parallel for schedule(dynamic, 10)
  ...
  AINFO << "Finished adding layer: " << layer;
}
...
void GridMapPclLoader::processGridMapCell(const unsigned int linearGridMapIndex, grid_map::Matrix* gridMapData) {
  ...
  if (isTooFewPointsInCell) {
    AWARN_EVERY(50) << "Less than " << params_.get().gridMap_.minCloudPointsPerCell_ << " points in a cell. Skipping.";
    return;
  }
  if (pointsInsideCellBorder->size() > params_.get().gridMap_.maxCloudPointsPerCell_) {
    AWARN_EVERY(50) << "More than " << params_.get().gridMap_.maxCloudPointsPerCell_ << " points in a cell. Skipping.";
    return;
  }
...
void GridMapPclLoader::calculateElevationFromPointsInsideGridMapCell(Pointcloud::ConstPtr cloud, std::vector<float>& heights) const {
  ...
  if (isNoClustersFound) {
    AWARN_EVERY(50) << "No clusters found in the grid map cell";
    return;
  }
