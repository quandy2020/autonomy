...
//  * -> All values smaller or equal to freeSpace are considered free space
//  (except noInformation).
//  * -> All values bigger or equal to lethalObstacle are considered a lethal
//  obstacle (except noInformation).
//  * -> All values between inscribedInflatedObstacle and lethalObstacle are
//  considered an inscribed inflated obstacle (except noInformation).
//  * -> All other values are interpolated between freeSpace and
//  inscribedInflatedObstacle
//  * @tparam noInformation                Gridmap value that represents no
//  information.
//  * @tparam lethalObstacle               Gridmap value (lower bound) for
//  lethal obstacles.
//  * @tparam inscribedInflatedObstacle    Gridmap value (lower bound) for
//  inscribed inflated obstacles.
//  * @tparam freeSpace                    Gridmap value (upper bound) for free
//  space.
//  */
// template <int64_t noInformation, int64_t lethalObstacle, int64_t
// inscribedInflatedObstacle, int64_t freeSpace> class Costmap2DTranslationTable
// {
//   // Check required pre-conditions of template arguments
//   static_assert(freeSpace < inscribedInflatedObstacle,
//                 "[Costmap2DTranslationTable] Condition violated: freeSpace <
//                 inscribedInflatedObstacle.");
...
//                 "[Costmap2DTranslationTable] Condition violated:
//                 noInformation < freeSpace || noInformation >
//                 lethalObstacle.");
...
//   /**
//    * Creates a translation table from costmap to grid map. (for backwards
//    compatibility)
//    * @tparam DataType Data type of the grid map.
//    * @param costTranslationTable Translation table mapping from costmap value
//    to grid map value.
//    */
...
//       costTranslationTable[i] =
//       fromCostmap<DataType>(static_cast<uint8_t>(i));
//     }
//   }
...
//     // Map costmap map interval to gridmap interval for values between free
//     space and inflated obstacle constexpr DataType costmapIntervalStart =
//     costmap_2d::FREE_SPACE; constexpr DataType costmapIntervalWidth =
//     costmap_2d::INSCRIBED_INFLATED_OBSTACLE - costmapIntervalStart; constexpr
//     DataType gridmapIntervalStart = freeSpace; constexpr DataType
//     gridmapIntervalWidth = inscribedInflatedObstacle - gridmapIntervalStart;
//     const DataType interpolatedValue =
//         gridmapIntervalStart + (costmapValue - costmapIntervalStart) *
//         gridmapIntervalWidth / costmapIntervalWidth;
...
//     } else if (gridmapValue >=
//     static_cast<DataType>(inscribedInflatedObstacle)) {
//       return costmap_2d::INSCRIBED_INFLATED_OBSTACLE;
//     }

//     // Map gridmap interval to costmap interval for values between free space
//     and inflated obstacle constexpr DataType costmapIntervalStart =
//     costmap_2d::FREE_SPACE; constexpr DataType costmapIntervalWidth =
//     costmap_2d::INSCRIBED_INFLATED_OBSTACLE - costmapIntervalStart; constexpr
//     DataType gridmapIntervalStart = freeSpace; constexpr DataType
//     gridmapIntervalWidth = inscribedInflatedObstacle - gridmapIntervalStart;
//     const DataType interpolatedValue =
//         costmapIntervalStart + (gridmapValue - gridmapIntervalStart) *
//         costmapIntervalWidth / gridmapIntervalWidth;
...
//  * This maps the cost directly, simply casting between the underlying
//  unsigned char and float fields.
//  */
// using Costmap2DDirectTranslationTable =
// Costmap2DTranslationTable<costmap_2d::NO_INFORMATION,
...
//  * This maps the cost onto float fields between 0.0 and 100.0 with reserved
//  values allocated for the costmap_2d.
//  */
// using Costmap2DCenturyTranslationTable = Costmap2DTranslationTable<-1, 100,
// 99, costmap_2d::FREE_SPACE>;

// template <typename DataType>
// class Costmap2DDefaultTranslationTable : public
// Costmap2DDirectTranslationTable {};

// template <>
// class Costmap2DDefaultTranslationTable<float> : public
...
//  * @tparam TranslationTable : class that creates a cost -> new type
//  conversion table
//  *
//  * @sa Costmap2DDirectTranslationTable, Costmap2DCenturyTranslationTable
//  */
// template <typename MapType, typename TranslationTable =
// Costmap2DDefaultTranslationTable<typename MapType::DataType>> class
// Costmap2DConverter {
//  public:
...
//    */
//   void initializeFromGridMap(const MapType& gridMap, costmap_2d::Costmap2D&
//   outputCostmap) {
//     // Different origin position
//     const Position position = gridMap.getPosition() - Position(0.5 *
//     gridMap.getLength()); const Size sizeXY = gridMap.getSize();
...
//    */
//   bool setCostmap2DFromGridMap(const MapType& gridMap, const std::string&
//   layer, costmap_2d::Costmap2D& outputCostmap) {
//     // Check compliance.
//     Size size(outputCostmap.getSizeInCellsX(),
//     outputCostmap.getSizeInCellsY()); if ((gridMap.getSize() != size).any())
//     {
...
//       outputCostmap.getCharMap()[j] = TranslationTable::template
//       toCostmap<DataType>(gridMap.get(layer).data()[i]);
//     }
//     return true;
//   }

//   void initializeFromCostmap2D(costmap_2d::Costmap2DROS& costmap2d, MapType&
//   outputMap) {
//     initializeFromCostmap2D(*(costmap2d.getCostmap()), outputMap);
//     outputMap.setFrameId(costmap2d.getGlobalFrameID());
//   }

//   void initializeFromCostmap2D(const costmap_2d::Costmap2D& costmap2d,
//   MapType& outputMap) {
//     const double resolution = costmap2d.getResolution();
//     Length length(costmap2d.getSizeInCellsX() * resolution,
//     costmap2d.getSizeInCellsY() * resolution); Position
//     position(costmap2d.getOriginX(), costmap2d.getOriginY());
...
//    * @warning This does not lock the costmap2d object, you should take care
//    to do so from outside this function.
...
//    * @return true if successful, false otherwise.
//    */
//   bool addLayerFromCostmap2D(const costmap_2d::Costmap2D& costmap2d, const
//   std::string& layer, MapType& outputMap) {
//     // Check compliance.
...
//    * @return true if successful, false otherwise.
//    */
//   bool addLayerFromCostmap2D(costmap_2d::Costmap2DROS& costmap2d, const
//   std::string& layer, MapType& outputMap) {
//     return addLayerFromCostmap2D(*(costmap2d.getCostmap()), layer,
//     outputMap);
//   }

//   /*!
//    * Initialize the output map at the robot pose of the underlying
//    Costmap2DROS object.
//    *
//    * Note: This needs some beyond just fixing the center of the output map to
//    the robot pose
//    * itself. To avoid introducing error, you want to shift the center so the
//    * underlying costmap2d cells align with the newly created map object. This
//    does
//    * that here.
//    *
//    * @warning this does not lock the costmap2d object, you should take care
//    to do so from
//    * outside this function in scope with any addLayerFromCostmap2DAtRobotPose
...
//    * @param outputMap : initialise the output map with the required
//    parameters
//    */
//   bool initializeFromCostmap2DAtRobotPose(costmap_2d::Costmap2DROS&
//   costmap2d, const Length& length, MapType& outputMap) {
...
//     Position rosMapOrigin(costmap2d.getCostmap()->getOriginX(),
//     costmap2d.getCostmap()->getOriginY()); Position newCostMapOrigin;
...
//     // Float versions of the cell co-ordinates, use static_cast<int> to get
//     the indices. Position robotCellPosition = (robotPosition - rosMapOrigin)
//     / resolution;
...
//       newCostMapOrigin(0) = std::floor(robotCellPosition.x()) * resolution +
//       resolution / 2.0 + rosMapOrigin.x();
//     } else {
//       newCostMapOrigin(0) = std::round(robotCellPosition.x()) * resolution +
//       rosMapOrigin.x();
//     }
//     if (numberOfCellsY % 2) {  // odd
//       newCostMapOrigin(1) = std::floor(robotCellPosition.y()) * resolution +
//       resolution / 2.0 + rosMapOrigin.y();
//     } else {
//       newCostMapOrigin(1) = std::round(robotCellPosition.y()) * resolution +
//       rosMapOrigin.y();
//     }
...
//   /**
//    * Fill a layer in the output map with data from the subwindow centered at
//    the robot pose.
//    * @warning This does not lock the costmap2d object, you should take care
//    to do so from
//    * outside this function in scope with any
//    initializeFromCostmap2DAtRobotPose calls
//    * you wish to make.
//    * @param costmap2d the underlying Costmap2DROS object.
...
//       return false;
//     }
//     // 1) would be nice to check the output map centre has been initialised
//     where it should be
//     //      i.e. the robot pose didn't move since or the initializeFrom
//     wasn't called
//     //    but this would mean listening to tf's again and anyway, it gets
//     shifted to make sure
//     //    the costmaps align, so these wouldn't be exactly the same anyway
//     // 2) check the geometry fits inside the costmap2d subwindow is done
//     below
...
//                                                        geometry.y());
//     if (!isValidWindow) {
//       // handle differently - e.g. copy the internal part only and lethal
//       elsewhere, but other parts would have to handle being outside too
