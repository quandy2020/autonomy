...
#include "autonomy/map/utils/decompose/geometric_utils.hpp"
 ...
 protected:
  ...
  typename std::enable_if<U == 2>::type find_ellipsoid(double offset_x) {
    ...
    while (!obs_inside.empty()) {
      ...
      if (p(0) < axes(0)) axes(1) = std::abs(p(1)) / std::sqrt(1 - std::pow(p(0) / axes(0), 2));
      ...
      for (const auto& it : obs_inside) {
        if (1 - E.dist(it) > epsilon_) obs_new.push_back(it);
      }
      obs_inside = obs_new;
    }
  ...
  typename std::enable_if<U == 3>::type find_ellipsoid(double offset_x) {
    ...
    while (!obs_inside.empty()) {
      ...
      if (dd > epsilon_) axes(2) = std::abs(p(2)) / std::sqrt(dd);
      Matf<Dim, Dim> new_C = Matf<Dim, Dim>::Identity();
      new_C(0, 0) = axes(0);
      new_C(1, 1) = axes(1);
      new_C(2, 2) = axes(2);
