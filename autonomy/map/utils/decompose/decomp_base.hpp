...
class DecompBase {
 public:
   ...
   * are not w.r.t the world coordinate system, but instead, x-axis is
   * parallel to the line, y-axis is perpendicular to the line and world
   * z-axis, z-axis is perpendiculat to the line and y-axis
   */
  ...
  void find_polyhedron() {
    ...
    while (!obs_remain.empty()) {
      ...
      for (const auto& it : obs_remain) {
        if (v.signed_dist(it) < 0) obs_tmp.push_back(it);
      }
      obs_remain = obs_tmp;
      /*
