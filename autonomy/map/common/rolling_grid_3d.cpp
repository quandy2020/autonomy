/**
 * @file rolling_grid_3d.cpp
 */

#include "autonomy/map/common/rolling_grid_3d.hpp"

namespace rolling_grid_ns {

RollingGrid::RollingGrid(const Eigen::Vector3i& size)
    : size_(size), which_grid_(true) {
    array_ind_to_ind_.resize(size_.x() * size_.y() * size_.z());
    grid0_ = std::make_shared<grid_ns::Grid<int>>(size_, 0);
    grid1_ = std::make_shared<grid_ns::Grid<int>>(size_, 0);
...
void RollingGrid::RollHelper(
    const std::shared_ptr<grid_ns::Grid<int>>& grid_in,
    const std::shared_ptr<grid_ns::Grid<int>>& grid_out,
    Eigen::Vector3i roll_dir) {
    Eigen::Vector3i grid_in_size = grid_in->GetSize();
    Eigen::Vector3i grid_out_size = grid_out->GetSize();
    assert(grid_in_size.x() == grid_out_size.x() &&
           grid_in_size.x() == size_.x());
    assert(grid_in_size.y() == grid_out_size.y() &&
           grid_in_size.y() == size_.y());
    assert(grid_in_size.z() == grid_out_size.z() &&
           grid_in_size.z() == size_.z());
    roll_dir.x() %= size_.x();
    roll_dir.y() %= size_.y();
    roll_dir.z() %= size_.z();
    Eigen::Vector3i dir;
    ...
    for (int ind = 0; ind < cell_num; ind++) {
        ...
        grid_out->GetCell(ind) =
            grid_in->GetCellValue(from_x, from_y, from_z);
    }
}
...
void RollingGrid::GetRolledInIndices(const Eigen::Vector3i& roll_dir) {
    ...
    if (dir.z() > 0) {
        ...
        GetIndices(updated_indices_,
                   Eigen::Vector3i(x_start, y_start, start_idx.z()),
                   Eigen::Vector3i(x_end, y_end, end_idx.z()));
    }
}

void RollingGrid::GetRolledOutIndices(
    const Eigen::Vector3i& roll_dir, std::vector<int>& rolled_out_indices) {
    Eigen::Vector3i rolled_out_start_idx, rolled_out_end_idx;
    rolled_out_start_idx.x() =
        roll_dir.x() >= 0 ? size_.x() - roll_dir.x() : 0;
    rolled_out_start_idx.y() =
        roll_dir.y() >= 0 ? size_.y() - roll_dir.y() : 0;
    rolled_out_start_idx.z() =
        roll_dir.z() >= 0 ? size_.z() - roll_dir.z() : 0;

    rolled_out_end_idx.x() =
        roll_dir.x() >= 0 ? size_.x() - 1 : -roll_dir.x() - 1;
    rolled_out_end_idx.y() =
        roll_dir.y() >= 0 ? size_.y() - 1 : -roll_dir.y() - 1;
    rolled_out_end_idx.z() =
        roll_dir.z() >= 0 ? size_.z() - 1 : -roll_dir.z() - 1;
    ...
    if (dir.x() > 0) {
        GetIndices(rolled_out_indices,
                   Eigen::Vector3i(rolled_out_start_idx.x(), 0, 0),
                   Eigen::Vector3i(rolled_out_end_idx.x(), size_.y() - 1,
                                   size_.z() - 1));
    }
    if (dir.y() > 0) {
        int x_start = 0;
        ...
        GetIndices(rolled_out_indices,
                   Eigen::Vector3i(x_start, rolled_out_start_idx.y(), 0),
    ...
    if (dir.z() > 0) {
        ...
        GetIndices(rolled_out_indices,
                   Eigen::Vector3i(x_start, y_start, rolled_out_start_idx.z()),
                   Eigen::Vector3i(x_end, y_end, rolled_out_end_idx.z()));
    }
}

void RollingGrid::GetIndices(std::vector<int>& indices, Eigen::Vector3i start_idx,
...
void RollingGrid::GetUpdatedIndices(std::vector<int>& updated_indices) const {
    ...
    std::copy(updated_indices_.begin(), updated_indices_.end(),
              updated_indices.begin());
}

void RollingGrid::GetUpdatedArrayIndices(
    std::vector<int>& updated_array_indices) const {
