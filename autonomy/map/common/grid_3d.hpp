/**
 * @file grid_3d.hpp
 * @brief Fixed-size 3D grid used by exploration planners and rolling maps.
 */
...
class Grid {
public:
    explicit Grid(const Eigen::Vector3i& size, _T init_value,
    ...
    Eigen::Vector3d GetResolutionInv() const { return resolution_inv_; }
    ...
    Eigen::Vector3i Ind2Sub(int ind) const { return subs_[ind]; }
    ...
    Eigen::Vector3i Pos2Sub(const Eigen::Vector3d& pos) const {
        ...
        for (int i = 0; i < dimension_; i++) {
            sub(i) = pos(i) - origin_(i) > 0
                         ? static_cast<int>((pos(i) - origin_(i)) *
                                            resolution_inv_(i))
                         : -1;
        }
        return sub;
    ...
    _T& GetCell(int x, int y, int z) { return GetCell(Eigen::Vector3i(x, y, z)); }
    ...
    _T& GetCell(int index) { return cells_[index]; }
    ...
    _T GetCellValue(int index) const { return cells_[index]; }
    ...
    void SetCellValue(int index, const _T& value) { cells_[index] = value; }
