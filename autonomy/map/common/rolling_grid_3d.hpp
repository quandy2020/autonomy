/**
 * @file rolling_grid_3d.hpp
 * @brief Rolling 3D grid with stable array-index mapping.
 */
...
#include "autonomy/map/common/grid_3d.hpp"
...
public:
    ...
    bool InRange(Eigen::Vector3i sub) const { return grid0_->InRange(sub); }
    bool InRange(int ind) const { return grid0_->InRange(ind); }
    Eigen::Vector3i Ind2Sub(int ind) const { return grid0_->Ind2Sub(ind); }
    int Sub2Ind(Eigen::Vector3i sub) const { return grid0_->Sub2Ind(sub); }
...
private:
    ...
    inline int GetFromIdx(int cur_idx, int roll_step, int max_idx) const {
        return cur_idx <= roll_step - 1 ? max_idx - roll_step + cur_idx
                                        : cur_idx - roll_step;
    }
