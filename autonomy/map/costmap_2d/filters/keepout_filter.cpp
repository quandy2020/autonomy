...
void KeepoutFilter::initializeFilter(const std::string& filter_info_topic) {
  ...
  //     std::bind(&KeepoutFilter::filterInfoCallback, this,
  //     std::placeholders::_1));
...
void KeepoutFilter::filterInfoCallback(const commsgs::planning_msgs::CostmapFilterInfo::SharedPtr msg) {
  ...
  //     RCLCPP_INFO(
  //     logger_,
  //     "KeepoutFilter: Received filter info from %s topic.",
  //     filter_info_topic_.c_str());
  // } else {
  //     RCLCPP_WARN(
  //     logger_,
  //     "KeepoutFilter: New costmap filter info arrived from %s topic.
  //     Updating old filter info.", filter_info_topic_.c_str());
  ...
  //     (%f and %f)", BASE_DEFAULT, MULTIPLIER_DEFAULT);
  // }
  ...
  //     mask_topic_,
  //     rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable(),
  //     std::bind(&KeepoutFilter::maskCallback, this,
  //     std::placeholders::_1));
}
...
void KeepoutFilter::maskCallback(const commsgs::map_msgs::OccupancyGrid::SharedPtr msg) {
  ...
  //     RCLCPP_INFO(
  //         logger_,
  //         "KeepoutFilter: Received filter mask from %s topic.",
  //         mask_topic_.c_str());
  // } else {
  //     RCLCPP_WARN(
  //         logger_,
  //         "KeepoutFilter: New filter mask arrived from %s topic. Updating
  //         old filter mask.",
  //     mask_topic_.c_str());
  //     filter_mask_.reset();
  // }
...
void KeepoutFilter::process(Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j,
  ...
  if (mask_frame != global_frame_) {
    ...
    // Calculating bounds corresponding to bottom-left overlapping (1)
    // corner filter_mask_ -> master_grid indexes conversion
  ...
  for (i = mg_min_x_u; i < mg_max_x_u; i++) {
    for (j = mg_min_y_u; j < mg_max_y_u; j++) {
      ...
      if (mask_frame != global_frame_) {
        ...
        // In this case master_grid and filter-mask are in the same
        // frame
        msk_wx = gl_wx;
        msk_wy = gl_wy;
      }
      // Get mask coordinates corresponding to (i, j) point at
      // filter_mask_
      if (worldToMask(filter_mask_, msk_wx, msk_wy, mx, my)) {
        data = getMaskCost(filter_mask_, mx, my);
        // Update if mask_ data is valid and greater than existing
        // master_grid's one
