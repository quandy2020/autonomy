...
void SpeedFilter::initializeFilter(const std::string& filter_info_topic) {
    ...
    // declareParameter("speed_limit_topic",
    // rclcpp::ParameterValue("speed_limit")); node->get_parameter(name_ + "." +
    ...
    // filter_info_sub_ =
    // node->create_subscription<nav2_msgs::msg::CostmapFilterInfo>(
    //     filter_info_topic_,
    //     rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable(),
    //     std::bind(&SpeedFilter::filterInfoCallback, this,
    //     std::placeholders::_1));
...
// void SpeedFilter::filterInfoCallback(const
// commmsgs::msg::CostmapFilterInfo::SharedPtr msg)
// {
//     std::lock_guard<CostmapFilter::mutex_t> guard(*getMutex());
...
//         RCLCPP_INFO(
//         logger_,
//         "SpeedFilter: Received filter info from %s topic.",
//         filter_info_topic_.c_str());
//     } else {
//         RCLCPP_WARN(
//         logger_,
//         "SpeedFilter: New costmap filter info arrived from %s topic. Updating
//         old filter info.", filter_info_topic_.c_str());
//         // Resetting previous subscriber each time when new costmap filter
//         information arrives mask_sub_.reset();
//     }
...
//         RCLCPP_INFO(
//         logger_,
//         "SpeedFilter: Using absolute speed_limit = %f + filter_mask_data *
//         %f", base_, multiplier_);
//     } else {
    ...
    if (speed_mask_data == SPEED_MASK_NO_LIMIT) {
        ...
        if (percentage_) {
            if (speed_limit_ < 0.0 || speed_limit_ > 100.0) {
                ...
                //     " "out of bounds of [0, 100]. Setting it to no-limit
                //     value.", mask_robot_i, mask_robot_j, speed_limit_);
                speed_limit_ = NO_SPEED_LIMIT;
            }
        } else {
            if (speed_limit_ < 0.0) {
                // RCLCPP_WARN(logger_,
                // "SpeedFilter: Speed limit in filter_mask[%i, %i] is less than
                // 0 m/s, " "which can not be true. Setting it to no-limit
                // value.", mask_robot_i, mask_robot_j);
                speed_limit_ = NO_SPEED_LIMIT;
            }
        }
    }
