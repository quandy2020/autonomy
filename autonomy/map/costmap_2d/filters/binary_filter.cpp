...
void BinaryFilter::initializeFilter(const std::string& filter_info_topic) {
    ...
    // declareParameter("binary_state_topic",
    // rclcpp::ParameterValue("binary_state")); node->get_parameter(name_ + "."
    ...
    //     std::bind(&BinaryFilter::filterInfoCallback, this,
    //     std::placeholders::_1));
...
void BinaryFilter::filterInfoCallback(
    ...
    //     RCLCPP_INFO(
    //     logger_,
    //     "BinaryFilter: Received filter info from %s topic.",
    //     filter_info_topic_.c_str());
    // } else {
    //     RCLCPP_WARN(
    //     logger_,
    ...
    //     RCLCPP_ERROR(logger_, "BinaryFilter: Mode %i is not supported",
    //     msg->type); return;
    // }
...
void BinaryFilter::maskCallback(
    ...
    if (!filter_mask_) {
        // RCLCPP_INFO(
        // logger_,
        // "BinaryFilter: Received filter mask from %s topic.",
        // mask_topic_.c_str());
    } else {
        // RCLCPP_WARN(
        // logger_,
...
void BinaryFilter::process(Costmap2D& /*master_grid*/, int /*min_i*/,
    ...
    if (!worldToMask(filter_mask_, mask_pose.x, mask_pose.y, mask_robot_i,
        ...
        // RCLCPP_WARN(
        // logger_,
        // "BinaryFilter: Robot is outside of filter mask. Resetting binary
        // state to default."); changeState(default_state_);
        return;
    }
...
void BinaryFilter::changeState(const bool state) {
    ...
    // std::unique_ptr<commsgs::std_msgs::Bool> msg =
    // std::make_unique<commsgs::std_msgs::Bool>(); msg->data = state;
