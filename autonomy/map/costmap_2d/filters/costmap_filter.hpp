...
class CostmapFilter : public Layer {
 public:
  ...
  /** CostmapFilter API **/
  /**
   * @brief: Initializes costmap filter. Creates subscriptions to
   ...
   * Costmap2D with calculated data and makes an action based on processed
   * data
  ...
  * @return True if the conversion was successful (legal bounds) false
  otherwise
  */
  bool worldToMask(commsgs::map_msgs::OccupancyGrid::ConstSharedPtr filter_mask, double wx, double wy, unsigned int& mx,
