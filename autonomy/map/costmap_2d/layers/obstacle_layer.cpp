...
void ObstacleLayer::onInitialize() {
  ...
  // rclcpp::ParameterValue(true)); declareParameter("min_obstacle_height",
  // rclcpp::ParameterValue(0.0)); declareParameter("max_obstacle_height",
  // rclcpp::ParameterValue(2.0)); declareParameter("combination_method",
  // rclcpp::ParameterValue(1)); declareParameter("observation_sources",
  ...
  // footprint_clearing_enabled_); node->get_parameter(name_ + "." +
  // "min_obstacle_height", min_obstacle_height_); node->get_parameter(name_ +
  ...
  // node->get_parameter(name_ + "." + "combination_method",
  // combination_method_param); combination_method_ =
  // combination_method_from_int(combination_method_param);
  ...
  // stringstream for std::stringstream ss(topics_string);

  // std::string source;
  // while (ss >> source) {
  //     // get the parameters for the specific topic
  //     double observation_keep_time, expected_update_rate,
  //     min_obstacle_height, max_obstacle_height; std::string topic,
  //     sensor_frame, data_type; bool inf_is_valid, clearing, marking;

  //     declareParameter(source + "." + "topic",
  //     rclcpp::ParameterValue(source)); declareParameter(source + "." +
  //     "sensor_frame", rclcpp::ParameterValue(std::string("")));
  //     declareParameter(source + "." + "observation_persistence",
  //     rclcpp::ParameterValue(0.0)); declareParameter(source + "." +
  ...
  //     rclcpp::ParameterValue(false)); declareParameter(source + "." +
  //     "marking", rclcpp::ParameterValue(true)); declareParameter(source +
  //     "."
  //     + "clearing", rclcpp::ParameterValue(false)); declareParameter(source
  //     +
  ...
  //     sensor_frame); node->get_parameter( name_ + "." + source + "." +
  //     "observation_persistence", observation_keep_time);
  //     node->get_parameter( name_ + "." + source + "." +
  //     "expected_update_rate", expected_update_rate);
  //     node->get_parameter(name_ + "." + source + "." + "data_type",
  //     data_type); node->get_parameter(name_ + "." + source + "." +
  //     "min_obstacle_height", min_obstacle_height);
  //     node->get_parameter(name_
  //     + "." + source + "." + "max_obstacle_height", max_obstacle_height);
  //     node->get_parameter(name_ + "." + source + "." + "inf_is_valid",
  //     inf_is_valid); node->get_parameter(name_ + "." + source + "." +
  //     "marking", marking); node->get_parameter(name_ + "." + source + "." +
  //     "clearing", clearing);
  ...
  //     node->get_parameter(name_ + "." + source + "." +
  //     "obstacle_max_range", obstacle_max_range); node->get_parameter(name_
  //     + "." + source + "." + "obstacle_min_range", obstacle_min_range);

  //     // get the raytrace ranges for the sensor
  //     double raytrace_max_range, raytrace_min_range;
  //     node->get_parameter(name_ + "." + source + "." +
  //     "raytrace_min_range", raytrace_min_range); node->get_parameter(name_
  //     + "." + source + "." + "raytrace_max_range", raytrace_max_range);
  ...
  //         min_obstacle_height,
  //         max_obstacle_height, obstacle_max_range, obstacle_min_range,
  ...
  //     // check if we'll also add this buffer to our clearing observation
  //     buffers if (clearing) {
  ...
  //     %s, " "expected update rate: %.2f, observation persistence: %.2f",
  //     source.c_str(), topic.c_str(),
  ...
  //     auto sub =
  //     std::make_shared<message_filters::Subscriber<sensor_msgs::msg::LaserScan,
  //         rclcpp_lifecycle::LifecycleNode>>(node, topic,
  //         custom_qos_profile, sub_opt);
  //     sub->unsubscribe();
  ...
  //         std::bind(
  //             &ObstacleLayer::laserScanValidInfCallback, this,
  //             std::placeholders::_1, observation_buffers_.back()));

  //     } else {
  //         filter->registerCallback(
  //         std::bind(
  //             &ObstacleLayer::laserScanCallback, this,
  //             std::placeholders::_1, observation_buffers_.back()));
  //     }
...
void ObstacleLayer::laserScanCallback(commsgs::sensor_msgs::LaserScan::ConstSharedPtr message,
  ...
  //     RCLCPP_WARN(
  //     logger_,
  //     "High fidelity enabled, but TF returned a transform exception to
  //     frame %s: %s", global_frame_.c_str(), ex.what());
  //     projector_.projectLaser(*message, cloud);
  // } catch (std::runtime_error & ex) {
  //     RCLCPP_WARN(
  //     logger_,
  //     "transformLaserScanToPointCloud error, it seems the message from
  //     laser is malformed." " Ignore this message. what(): %s", ex.what());
  //     return;
  // }
...
void ObstacleLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x, double* min_y,
  ...
  // if (rolling_window_) {
  //     updateOrigin(robot_x - getSizeInMetersX() / 2, robot_y -
  //     getSizeInMetersY() / 2);
  // }
  // if (!enabled_) {
  //     return;
  ...
  //     raytraceFreespace(clearing_observations[i], min_x, min_y, max_x,
  //     max_y);
  // }

  // // place the new obstacles into a priority queue... each with a priority
  // of zero to begin with for (std::vector<Observation>::const_iterator it =
  // observations.begin();
  ...
  //     double sq_obstacle_max_range = obs.obstacle_max_range_ *
  //     obs.obstacle_max_range_; double sq_obstacle_min_range =
  ...
  //     // if the obstacle is too high or too far away from the robot we
  //     won't add it if (pz > max_obstacle_height_) {
  //         RCLCPP_DEBUG(logger_, "The point is too high");
  //         continue;
  //     }

  //     // compute the squared distance from the hitpoint to the pointcloud's
  //     origin double sq_dist =
  //         (px -
  //         obs.origin_.x) * (px - obs.origin_.x) + (py - obs.origin_.y) *
  //         (py
  //         - obs.origin_.y) + (pz - obs.origin_.z) * (pz - obs.origin_.z);
...
void ObstacleLayer::raytraceFreespace(const Observation& clearing_observation, double* min_x, double* min_y,
  ...
  //     RCLCPP_WARN(
  //     logger_,
  //     "Sensor origin at (%.2f, %.2f) is out of map bounds (%.2f, %.2f) to
  //     (%.2f, %.2f). " "The costmap cannot raytrace for it.", ox, oy,
  //     origin_x_, origin_y_,
  //     origin_x_ + getSizeInMetersX(), origin_y_ + getSizeInMetersY());
  //     return;
  // }

  // // we can pre-compute the enpoints of the map outside of the inner
  // loop... we'll need these later double origin_x = origin_x_, origin_y =
  // origin_y_; double map_end_x = origin_x + size_x_ * resolution_; double
  // map_end_y = origin_y + size_y_ * resolution_;
  ...
  //     coordinates of its endpoint unsigned int x1, y1;
  ...
  //     cellDistance(clearing_observation.raytrace_max_range_); unsigned int
  //     cell_raytrace_min_range =
  //     cellDistance(clearing_observation.raytrace_min_range_); MarkCell
  //     marker(costmap_, FREE_SPACE);
  //     // and finally... we can execute our trace to clear obstacles along
  //     that line raytraceLine(marker, x0, y0, x1, y1,
