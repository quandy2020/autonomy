...
class RangeSensorLayer : public CostmapLayer {
 ...
 protected:
   ...
   */
  inline double to_prob(unsigned char c) { return static_cast<double>(c) / LETHAL_OBSTACLE; }

  /**
   * @brief Find cost value of a probability
   */
  inline unsigned char to_cost(double p) { return static_cast<unsigned char>(p * LETHAL_OBSTACLE); }
   ...
   */
  int orient2d(int Ax, int Ay, int Bx, int By, int Cx, int Cy) { return (Bx - Ax) * (Cy - Ay) - (By - Ay) * (Cx - Ax); }
};
