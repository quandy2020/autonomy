...
void VoxelLayer::onInitialize() {
  ...
  // rclcpp::ParameterValue(true)); declareParameter("max_obstacle_height",
  // rclcpp::ParameterValue(2.0)); declareParameter("z_voxels",
  // rclcpp::ParameterValue(10)); declareParameter("origin_z",
  // rclcpp::ParameterValue(0.0)); declareParameter("z_resolution",
  // rclcpp::ParameterValue(0.2)); declareParameter("unknown_threshold",
  // rclcpp::ParameterValue(15)); declareParameter("mark_threshold",
  // rclcpp::ParameterValue(0)); declareParameter("combination_method",
  // rclcpp::ParameterValue(1)); declareParameter("publish_voxel_map",
  // rclcpp::ParameterValue(false));

  // node->get_parameter(name_ + "." + "enabled", enabled_);
  // node->get_parameter(name_ + "." + "footprint_clearing_enabled",
  // footprint_clearing_enabled_); node->get_parameter(name_ + "." +
  // "max_obstacle_height", max_obstacle_height_); node->get_parameter(name_ +
  // "." + "z_voxels", size_z_); node->get_parameter(name_ + "." + "origin_z",
  // origin_z_); node->get_parameter(name_ + "." + "z_resolution",
  // z_resolution_); node->get_parameter(name_ + "." + "unknown_threshold",
  // unknown_threshold_); node->get_parameter(name_ + "." + "mark_threshold",
  // mark_threshold_); node->get_parameter(name_ + "." + "publish_voxel_map",
  // publish_voxel_);
  ...
  // node->create_publisher<sensor_msgs::msg::PointCloud2>("clearing_endpoints",
  // custom_qos); clearing_endpoints_pub_->on_activate();
...
void VoxelLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x, double* min_y,
  ...
  // of zero to begin with for (auto it = observations.begin(); it !=
  // observations.end(); ++it) {
  ...
  //     won't add it if (*iter_z > max_obstacle_height_) {
  //         continue;
  //     }

  //     // compute the squared distance from the hitpoint to the pointcloud's
  //     origin double sq_dist = (*iter_x - obs.origin_.x) * (*iter_x -
  //     obs.origin_.x) +
  ...
  //     // mark the cell in the voxel grid and check if we should also mark
  //     it in the costmap if (voxel_grid_.markVoxelInMap(mx, my, mz,
  //     mark_threshold_)) {
  ...
  //     memcpy(&grid_msg->data[0], voxel_grid_.getData(), size *
  //     sizeof(unsigned int));
...
void VoxelLayer::raytraceFreespace(const Observation& clearing_observation, double* min_x, double* min_y, double* max_x,
  ...
  //     }
  //     publish_clearing_points =
  //     (node->count_subscribers("clearing_endpoints") > 0);
  // }
  ...
  // clearing_endpoints_iter_x(*clearing_endpoints_, "x");
  // sensor_msgs::PointCloud2Iterator<float>
  // clearing_endpoints_iter_y(*clearing_endpoints_, "y");
  // sensor_msgs::PointCloud2Iterator<float>
  // clearing_endpoints_iter_z(*clearing_endpoints_, "z");

  // // we can pre-compute the enpoints of the map outside of the inner
  // loop... we'll need these later double map_end_x = origin_x_ +
  // getSizeInMetersX(); double map_end_y = origin_y_ + getSizeInMetersY();
  // double map_end_z = origin_z_ + getSizeInMetersZ();

  // sensor_msgs::PointCloud2ConstIterator<float>
  // iter_x(*(clearing_observation.cloud_), "x");
  // sensor_msgs::PointCloud2ConstIterator<float>
  // iter_y(*(clearing_observation.cloud_), "y");
  // sensor_msgs::PointCloud2ConstIterator<float>
  // iter_z(*(clearing_observation.cloud_), "z");
  ...
  //     scaling_fact = std::max(std::min(scaling_fact, (distance - 2 *
  //     resolution_) / distance), 0.0); wpx = scaling_fact * (wpx - ox) + ox;
  ...
  //     point_y, point_z); voxel_grid_.clearVoxelLineInMap(
...
void VoxelLayer::updateOrigin(double new_origin_x, double new_origin_y) {
  ...
  // copyMapRegion(
  //     voxel_map, lower_left_x, lower_left_y, size_x_, local_voxel_map, 0,
  ...
  // // now we want to copy the overlapping information back into the map, but
  // in its new location copyMapRegion(
  //     local_map, 0, 0, cell_size_x, costmap_, start_x, start_y, size_x_,
  //     cell_size_x, cell_size_y);
  // copyMapRegion(
  //     local_voxel_map, 0, 0, cell_size_x, voxel_map, start_x, start_y,
