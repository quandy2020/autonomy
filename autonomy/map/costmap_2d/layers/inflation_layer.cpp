...
InflationLayer::~InflationLayer() { delete access_; }
...
void InflationLayer::onInitialize() {
  ...
  if (options_ && options_->has_inflation_layer()) {
    ...
    inflate_unknown_ = inflation_opts.inflate_unknown();
    inflate_around_unknown_ = inflation_opts.inflate_around_unknown();
  } else {
    // 使用默认值
    enabled_ = true;
    inflation_radius_ = 0.55;
    cost_scaling_factor_ = 10.0;
    inflate_unknown_ = false;
    inflate_around_unknown_ = false;
  }

  AINFO << "InflationLayer initialized: enabled=" << enabled_ << ", inflation_radius=" << inflation_radius_
        << ", cost_scaling_factor=" << cost_scaling_factor_ << ", inflate_unknown=" << inflate_unknown_
...
void InflationLayer::updateCosts(Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j) {
  ...
  // make sure the inflation list is empty at the beginning of the cycle
  // (should always be true)
  for (auto& dist : inflation_cells_) {
    // RCLCPP_FATAL_EXPRESSION(
    // logger_,
    // !dist.empty(), "The inflation list must be empty at the beginning of
    // inflation");
  }
  ...
  if (seen_.size() != size_x * size_y) {
    LOG(WARNING) << "InflationLayer::updateCosts(): seen_ vector size is wrong";
  ...
  for (auto& dist_bin : inflation_cells_) {
    ...
    for (std::size_t i = 0; i < dist_bin.size(); ++i) {
      ...
      // assign the cost associated with the distance from an obstacle to
      // the cell
      ...
      if (static_cast<int>(mx) >= base_min_i && static_cast<int>(my) >= base_min_j &&
      ...
      // attempt to put the neighbors of the current cell onto the
      // inflation list
      if (mx > 0) {
    ...
    // This level of inflation_cells_ is not needed anymore. We can free the
    // memory Note that dist_bin.clear() is not enough, because it won't
    // free the memory
 ...
 */
void InflationLayer::enqueue(unsigned int index, unsigned int mx, unsigned int my, unsigned int src_x,
                             unsigned int src_y) {
  if (!seen_[index]) {
...
int InflationLayer::generateIntegerDistances() {
  ...
  std::sort(points.begin(), points.end(), [](const std::pair<int, int>& a, const std::pair<int, int>& b) -> bool {
...
CLASS_LOADER_REGISTER_CLASS(autonomy::map::costmap_2d::InflationLayer, autonomy::map::costmap_2d::Layer)