...
#include "autonomy/map/costmap_2d/denoise/image_processing.hpp"
...
{
    friend class DenoiseLayerTester;  // For test some private methods using
                                      // gtest
...
private:
     ...
     * - as obstacles. Replaces groups of obstacles smaller than
     * minimal_group_size_ to free space. Groups connectivity type is determined
     * by the connectivity parameter.
     *
     * If minimal_group_size_ is 1 or 0, it does nothing
     * (all standalone obstacles will be preserved, since it satisfies this
     * condition). If minimal_group_size_ equals 2, performs fast filtering
     * based on the dilation operation. Otherwise, it performs a slower
     ...
     * Works similarly to removeGroups with minimal_group_size_ = 2, but about
     * 10x faster
     * @throw std::logic_error in case inner logic errors
     * @warning If image.empty() the behavior is undefined
     */
