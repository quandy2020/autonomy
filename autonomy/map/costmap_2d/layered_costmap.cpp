...
void LayeredCostmap::updateMap(double robot_x, double robot_y,
                               double robot_yaw) {
    ...
    if (rolling_window_) {
        double new_origin_x =
            robot_x - combined_costmap_.getSizeInMetersX() / 2;
        double new_origin_y =
            robot_y - combined_costmap_.getSizeInMetersY() / 2;
    ...
    if (isOutofBounds(robot_x, robot_y)) {
        // Pre-map default window is often origin=(0,0) with configured size
        // while the robot already sits in negative map-frame coords. Throttle
        // (and soften) until StaticLayer resizes the master grid.
        const bool awaiting_static_map =
            !rolling_window_ &&
            combined_costmap_.getOriginX() == 0.0 &&
            combined_costmap_.getOriginY() == 0.0;
        AWARN_EVERY(awaiting_static_map ? 200 : 50)
            << "Robot is out of bounds of the costmap! pose=(" << robot_x
            << ", " << robot_y << ") origin=("
            ...
            << (awaiting_static_map ? " (awaiting static map)" : "");
    }
    ...
    for (auto plugin = plugins_.begin(); plugin != plugins_.end(); ++plugin) {
        ...
        (*plugin)->updateBounds(robot_x, robot_y, robot_yaw, &minx_, &miny_,
                                &maxx_, &maxy_);
        if (minx_ > prev_minx || miny_ > prev_miny || maxx_ < prev_maxx ||
            maxy_ < prev_maxy) {
            // RCLCPP_WARN(
            //     rclcpp::get_logger(
            //     "nav2_costmap_2d"), "Illegal bounds change, was [tl: (%f,
            //     %f), br:
            //     (%f, %f)], but " "is now [tl: (%f, %f), br: (%f, %f)]. The
            //     offending layer is %s", prev_minx, prev_miny, prev_maxx,
            //     prev_maxy, minx_, miny_, maxx_, maxy_,
    ...
    for (auto filter = filters_.begin(); filter != filters_.end(); ++filter) {
        ...
        double prev_maxx = maxx_;
        double prev_maxy = maxy_;
        (*filter)->updateBounds(robot_x, robot_y, robot_yaw, &minx_, &miny_,
                                &maxx_, &maxy_);
        if (minx_ > prev_minx || miny_ > prev_miny || maxx_ < prev_maxx ||
            maxy_ < prev_maxy) {
            // RCLCPP_WARN(
            ...
            //     %f), br:
            //     (%f, %f)], but " "is now [tl: (%f, %f), br: (%f, %f)]. The
            //     offending filter is %s", prev_minx, prev_miny, prev_maxx,
            //     prev_maxy, minx_, miny_, maxx_, maxy_,
    ...
    x0 = std::max(0, x0);
    xn =
        std::min(static_cast<int>(combined_costmap_.getSizeInCellsX()), xn + 1);
    y0 = std::max(0, y0);
    yn =
        std::min(static_cast<int>(combined_costmap_.getSizeInCellsY()), yn + 1);
    ...
    if (filters_.size() == 0) {
        // If there are no filters enabled just update costmap sequentially by
        // each plugin
        combined_costmap_.resetMap(x0, y0, xn, yn);
        for (auto plugin = plugins_.begin(); plugin != plugins_.end();
             ++plugin) {
        ...
        if (!combined_costmap_.copyWindow(primary_costmap_, x0, y0, xn, yn, x0,
                                          y0)) {
            // RCLCPP_ERROR(
        ...
        for (auto filter = filters_.begin(); filter != filters_.end();
             ++filter) {
...
bool LayeredCostmap::isCurrent() {
    ...
    for (auto plugin = plugins_.begin(); plugin != plugins_.end(); ++plugin) {
        current_ =
            current_ && ((*plugin)->isCurrent() || !(*plugin)->isEnabled());
    }
    for (auto filter = filters_.begin(); filter != filters_.end(); ++filter) {
        current_ =
            current_ && ((*filter)->isCurrent() || !(*filter)->isEnabled());
    }
    return current_;
}
