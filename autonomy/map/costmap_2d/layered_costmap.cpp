...
LayeredCostmap::LayeredCostmap(std::string global_frame, bool rolling_window, bool track_unknown)
    : primary_costmap_(),
      ...
      footprint_(std::make_shared<std::vector<commsgs::geometry_msgs::Point>>()) {
    if (track_unknown) {
...
void LayeredCostmap::setFootprint(const std::vector<commsgs::geometry_msgs::Point>& footprint_spec) {
    std::pair<double, double> inside_outside = calculateMinAndMaxDistances(footprint_spec);
    // use atomic store here since footprint is used by various
    // planners/controllers and not otherwise locked
    std::atomic_store(&footprint_, std::make_shared<std::vector<commsgs::geometry_msgs::Point>>(footprint_spec));
