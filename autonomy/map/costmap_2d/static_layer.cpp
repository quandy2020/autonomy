...
#include "autonomy/map/costmap_2d/static_layer.hpp"
...
{
    LOG(INFO) << "StaticLayer: Process map";
...
void StaticLayer::updateFootprint(
  double robot_x, double robot_y, double robot_yaw,
...
void StaticLayer::updateCosts(Costmap2D & master_grid, int min_i, int min_j, int max_i, int max_j)
{
    ...
    if (!map_received_in_update_bounds_) {
        ...
        if (++count == 10) {
            LOG(WARNING) << "Can't update static costmap layer, no map received";
            count = 0;
        }
        return;
    }
    ...
    if (!layered_costmap_->isRolling()) {
        ...
        // try {
        //     transform = tf_->lookupTransform(map_frame_, global_frame_, transform::tf2::TimePointZero, transform_tolerance_);
        // } catch (transform::tf2::TransformException & ex) {
        //     LOG(ERROR) << "StaticLayer: " << ex.what();
        //     return;
        // }
        ...
        //         transform::tf2::Vector3 p(wx, wy, 0);
        //         p = tf2_transform * p;
