...
public:
     ...
     * For calculation of new target value the operation can use source value
     * and an optionally current target value. This function call
     * operation(this[i, j], target[i, j]) for each pixel where target[i, j] is
     * mutable
