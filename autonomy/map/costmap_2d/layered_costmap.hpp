...
class LayeredCostmap {
 public:
   ...
   * If you want to update the map outside of the update loop that runs, you
   * can call this.
   */
  ...
  /**
   * @brief If the costmap is current, e.g. are all the layers processing
   * recent data and not stale information for a good state.
   */
  bool isCurrent();
   ...
   */
  bool isTrackingUnknown() { return combined_costmap_.getDefaultValue() == NO_INFORMATION; }
  ...
  const std::vector<commsgs::geometry_msgs::Point>& getFootprint() { return *std::atomic_load(&footprint_); }
