...
class Costmap2D {
  ...
  */
  bool copyWindow(const Costmap2D& source, unsigned int sx0, unsigned int sy0, unsigned int sxn, unsigned int syn,
  ...
  /**
   * @brief  Convert from world coordinates to map coordinates without
   * checking for legal bounds
   * @param  wx The x world coordinate
   * @param  wy The y world coordinate
   * @param  mx Will be set to the associated map x coordinate
   * @param  my Will be set to the associated map y coordinate
   ...
   * @param  mx Will be set to the associated map x coordinate
   * @param  my Will be set to the associated map y coordinate
   * @note   The returned map coordinates are guaranteed to lie within the
   * map.
   */
  ...
  /**
   * @brief  Will return a pointer to the underlying unsigned char array used
   * as the costmap
   * @return A pointer to the underlying unsigned char array storing cost
   * values
   */
   ...
   */
  bool setConvexPolygonCost(const std::vector<commsgs::geometry_msgs::Point>& polygon, unsigned char cost_value);
 ...
 protected:
  ...
  inline void raytraceLine(ActionType at, unsigned int x0, unsigned int y0, unsigned int x1, unsigned int y1,
    ...
    if (dist > 0.0) {
      // Adjust starting point and offset to start from min_length
      // distance
