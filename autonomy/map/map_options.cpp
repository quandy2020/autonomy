...
proto::MapOptions LoadOptions(::autonomy::common::LuaParameterDictionary* const parameter_dictionary) {
    ...
    options.set_publish_frequency(parameter_dictionary->GetDouble("publish_frequency"));
...
proto::Costmap2DOptions CreateCostmap2DOptions(::autonomy::common::LuaParameterDictionary* const parameter_dictionary) {
    ...
    options.set_update_frequency(parameter_dictionary->GetDouble("update_frequency"));
    options.set_robot_radius(parameter_dictionary->GetDouble("robot_radius"));
    options.set_always_send_full_costmap(parameter_dictionary->GetBool("always_send_full_costmap"));
    ...
    if (parameter_dictionary->HasKey("static_layer")) {
        auto static_layer_dict = parameter_dictionary->GetDictionary("static_layer");
        ...
        static_layer->set_subscribe_to_updates(static_layer_dict->GetBool("subscribe_to_updates"));
        static_layer->set_transform_tolerance(static_layer_dict->GetDouble("transform_tolerance"));
        static_layer->set_footprint_clearing_enabled(static_layer_dict->GetBool("footprint_clearing_enabled"));
        static_layer->set_map_topic(static_layer_dict->GetString("map_topic"));
    }

    if (parameter_dictionary->HasKey("denoise_layer")) {
        auto denoise_layer_dict = parameter_dictionary->GetDictionary("denoise_layer");
        ...
        denoise_layer->set_denoise_radius(denoise_layer_dict->GetDouble("denoise_radius"));
    }

    if (parameter_dictionary->HasKey("inflation_layer")) {
        auto inflation_layer_dict = parameter_dictionary->GetDictionary("inflation_layer");
        ...
        inflation_layer->set_cost_scaling_factor(inflation_layer_dict->GetDouble("cost_scaling_factor"));
        inflation_layer->set_inflation_radius(inflation_layer_dict->GetDouble("inflation_radius"));
        inflation_layer->set_inflate_unknown(inflation_layer_dict->GetBool("inflate_unknown"));
        inflation_layer->set_inflate_around_unknown(inflation_layer_dict->GetBool("inflate_around_unknown"));
    }
