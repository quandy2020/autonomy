...
struct FeedbackUtils {
  ...
  /** Resolved default BT XML path for this navigator (optional, set by BtNavigator). */
 ...
 public:
  ...
  void StartNavigating(const std::string& navigator_name) {
    ...
    if (!current_navigator_.empty()) {
      AERROR << "NavigatorMuxer: navigation requested while another navigation task is in "
                "progress.";
    }
  ...
  void StopNavigating(const std::string& navigator_name) {
    ...
    if (current_navigator_ != navigator_name) {
      AERROR << "NavigatorMuxer: stop from " << navigator_name << " while current is " << current_navigator_;
    } else {
 ...
 * @brief Base class for all BT-based Navigator action plugins. Implements NavigatorBase;
 *        final methods (Configure/Activate/Deactivate/Cleanup) delegate to BtActionServer
 *        and to virtual configure/activate/deactivate/cleanup. Subclasses implement
 *        GoalReceived, OnLoop, OnPreempt, GoalCompleted, GetName, GetDefaultBTFilepath,
 *        and optionally configure, cleanup. Aligned with nav2_core::BehaviorTreeNavigator.
 */
template <class ActionT>
class BehaviorTreeNavigator : public NavigatorBase {
 public:
  /** Default constructor for plugin loading; object must be initialized before use. */
  BehaviorTreeNavigator() : NavigatorBase(), plugin_muxer_(nullptr) {}

  BehaviorTreeNavigator(std::shared_ptr<autolink::Node> node, const std::vector<std::string>& plugin_lib_names,
                        const FeedbackUtils& feedback_utils, const std::shared_ptr<NavigatorMuxer>& plugin_muxer,
                        std::shared_ptr<OdomSmoother> odom_smoother)
      : NavigatorBase(), plugin_muxer_(plugin_muxer.get()) {
    feedback_utils_ = feedback_utils;
    std::string default_bt_xml = feedback_utils.default_bt_xml_filename.empty()
 ...
 protected:
  bool OnGoalReceived(std::shared_ptr<const typename ActionT::Goal> goal) {
    if (plugin_muxer_ && plugin_muxer_->IsNavigating()) {
      AERROR << "Requested navigation from " << GetName() << " while another navigator is processing, rejecting.";
      return false;
    }
  ...
  void OnCompletion(std::shared_ptr<typename ActionT::Result> result, const BtStatus final_bt_status) {
    if (plugin_muxer_) plugin_muxer_->StopNavigating(GetName());
    GoalCompleted(result, final_bt_status);
  }

  virtual bool GoalReceived(std::shared_ptr<const typename ActionT::Goal> goal) = 0;
  virtual void OnLoop() = 0;
  virtual void OnPreempt(std::shared_ptr<const typename ActionT::Goal> goal) = 0;
  virtual void GoalCompleted(std::shared_ptr<typename ActionT::Result> result, const BtStatus final_bt_status) = 0;
