...
BtActionServer<ActionT>::BtActionServer(
    const std::shared_ptr<autolink::Node>& parent,
    ...
    blackboard_->set("wait_for_service_timeout",
                     wait_for_service_timeout_);  // NOLINT

    // Action server: execute callback runs BT; optional goal callback rejects
    // by content (nav2 does this inside executeCallback)
    action_server_ = std::make_shared<ActionServer>(
        node_, action_name_, [this]() { ExecuteCallback(); }, nullptr,
        std::chrono::milliseconds(500), false);
    action_server_->SetGoalCallback(
        [this](const autolink::action::GoalUUID& /*uuid*/,
               std::shared_ptr<const typename ActionT::Goal> goal) {
            if (!goal)
                return autolink::action::GoalResponse::REJECT;
            bool accept = on_goal_received_callback_(goal);
            return accept ? autolink::action::GoalResponse::ACCEPT_AND_EXECUTE
                          : autolink::action::GoalResponse::REJECT;
        });
}
...
void BtActionServer<ActionT>::ExecuteCallback() {
    // Aligned with nav2: give server a chance to reject goal (we also reject
    // via SetGoalCallback)
    if (action_server_ &&
        !on_goal_received_callback_(action_server_->GetCurrentGoal())) {
    ...
    // Ensure BT is not in running state from previous execution (nav2:
    // haltAllActions)
    if (tree_.rootNode()) {
        tree_.haltTree();
    }
