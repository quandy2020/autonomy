...
BT::NodeStatus RemoveInCollisionGoals::on_completion(
    std::shared_ptr<proto::GetCosts::Response> response) {
    if (!response->success()) {
    ...
    for (size_t i = 0; i < response->costs_size(); ++i) {
        ...
        if ((cost == 255 && !consider_unknown_as_obstacle_) ||
            cost < cost_threshold_) {
            ...
            int cur_waypoint_index = findNextMatchingGoalInWaypointStatuses(
