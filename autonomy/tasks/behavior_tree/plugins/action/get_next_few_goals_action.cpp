...
GetNextFewGoals::GetNextFewGoals(const std::string& name,
...
inline BT::NodeStatus GetNextFewGoals::tick() {
    ...
    for (unsigned int i = 0; i < num_goals && i < input_goals.goals.size();
         ++i) {
...
BT_REGISTER_NODES(factory) {
    factory.registerNodeType<
        autonomy::tasks::behavior_tree::plugins::action::GetNextFewGoals>(
        "GetNextFewGoals");
}
