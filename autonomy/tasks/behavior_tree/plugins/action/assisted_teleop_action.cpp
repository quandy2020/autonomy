...
AssistedTeleopAction::AssistedTeleopAction(const std::string& xml_tag_name, const std::string& action_name,
...
BT::NodeStatus AssistedTeleopAction::on_success() {
    setOutput("error_code_id", static_cast<int32_t>(proto::AssistedTeleopErrorCode::ASSISTED_TELEOP_ERROR_NONE));
...
BT::NodeStatus AssistedTeleopAction::on_aborted() {
    if (result_.result) {
        ...
        setOutput("error_code_id", static_cast<int32_t>(proto::AssistedTeleopErrorCode::ASSISTED_TELEOP_ERROR_UNKNOWN));
...
void AssistedTeleopAction::on_timeout() {
    setOutput("error_code_id", static_cast<int32_t>(proto::AssistedTeleopErrorCode::ASSISTED_TELEOP_ERROR_TIMEOUT));
...
BT_REGISTER_NODES(factory) {
    BT::NodeBuilder builder = [](const std::string& name, const BT::NodeConfiguration& config) {
        return std::make_unique<autonomy::tasks::behavior_tree::plugins::action::AssistedTeleopAction>(
            name, "assisted_teleop", config);
    };

    factory.registerBuilder<autonomy::tasks::behavior_tree::plugins::action::AssistedTeleopAction>("AssistedTeleop",
                                                                                                   builder);
}
