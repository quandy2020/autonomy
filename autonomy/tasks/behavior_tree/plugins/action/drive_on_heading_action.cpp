...
bool SetRecoveryTargetFromDistance(const BT::NodeConfiguration& conf,
                                   ...
                                   task_proto::DriveOnHeadingAction_Goal& goal) {
...
bool BuildDriveCommand(
    const BT::Blackboard::Ptr& blackboard,
    const task_proto::DriveOnHeadingAction_Goal& goal,
...
class DriveOnHeadingAction : public BtActionNode<DriveOnHeadingActionTraits>
{
public:
    DriveOnHeadingAction(const std::string& name, const BT::NodeConfiguration& conf)
        : BtActionNode(name, kDriveOnHeadingActionName, conf) {}
    ...
    BT::NodeStatus tick() override {
        ...
        if (!BT::isStatusActive(status())) {
            ...
            control::ControllerServer::RecoveryMotionCommand cmd;
            if (!BuildDriveCommand(config().blackboard, goal_, cmd) ||
...
protected:
    void OnTick() override {
        ...
        if (!SetRecoveryTargetFromDistance(config(), dist, false, goal_)) {
    ...
    BT::NodeStatus OnSuccess() override {
        setOutput("error_code_id",
                  static_cast<uint16_t>(task_proto::DRIVE_ON_HEADING_NONE));
        return BT::NodeStatus::SUCCESS;
    }

    BT::NodeStatus OnAborted() override {
        setOutput("error_code_id",
                  static_cast<uint16_t>(task_proto::DRIVE_ON_HEADING_UNKNOWN));
        return BT::NodeStatus::FAILURE;
    }
