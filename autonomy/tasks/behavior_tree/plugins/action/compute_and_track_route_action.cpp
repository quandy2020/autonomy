...
BT::NodeStatus ComputeAndTrackRouteAction::on_success() {
    ...
    setOutput("error_code_id",
              static_cast<int32_t>(proto::ComputeAndTrackRouteErrorCode::
                                       COMPUTE_AND_TRACK_ROUTE_ERROR_NONE));
...
BT::NodeStatus ComputeAndTrackRouteAction::on_aborted() {
    ...
    if (result_.result) {
        ...
        setOutput(
            "error_code_id",
            static_cast<int32_t>(proto::ComputeAndTrackRouteErrorCode::
                                     COMPUTE_AND_TRACK_ROUTE_ERROR_UNKNOWN));
...
void ComputeAndTrackRouteAction::on_timeout() {
    setOutput("error_code_id",
              static_cast<int32_t>(proto::ComputeAndTrackRouteErrorCode::
                                       COMPUTE_AND_TRACK_ROUTE_ERROR_TIMEOUT));
    setOutput("error_msg",
    ...
    if (use_poses) {
        ...
        if (!goal_.has_goal() ||
            goal_.goal().pose().position().x() !=
                proto_goal.pose().position().x() ||
            goal_.goal().pose().position().y() !=
                proto_goal.pose().position().y()) {
            goal_updated_ = true;
        }
        ...
        if (use_start) {
            ...
            if (!goal_.has_start() ||
                goal_.start().pose().position().x() !=
                    proto_start.pose().position().x() ||
                goal_.start().pose().position().y() !=
                    proto_start.pose().position().y()) {
                goal_updated_ = true;
            }
        }
    } else {
