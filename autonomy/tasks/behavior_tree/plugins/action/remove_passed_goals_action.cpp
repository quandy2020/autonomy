...
BT::NodeStatus RemovePassedGoals::tick() {
    ...
    if (!autonomy::tasks::utils::getCurrentPose(current_pose, tf_, goal_poses.goals[0].header.frame_id,
    ...
    while (goal_poses.goals.size() > 1) {
        dist_to_goal =
            autonomy::map::costmap_2d::utils::euclidean_distance(goal_poses.goals[0].pose, current_pose.pose);
