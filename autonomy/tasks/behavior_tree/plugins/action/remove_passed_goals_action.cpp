...
BT::NodeStatus RemovePassedGoals::tick() {
  ...
  if (!autonomy::tasks::utils::getCurrentPose(current_pose, tf_, goal_poses.goals[0].header.frame_id, robot_base_frame_,
  ...
  while (goal_poses.goals.size() > 1) {
    dist_to_goal = autonomy::map::costmap_2d::utils::euclidean_distance(goal_poses.goals[0].pose, current_pose.pose);
    ...
    if (waypoint_statuses_get_res) {
      auto cur_waypoint_index = findNextMatchingGoalInWaypointStatuses(waypoint_statuses, goal_poses.goals[0]);
      if (cur_waypoint_index == -1) {
        AERROR << "Failed to find matching goal in waypoint_statuses";
        return BT::NodeStatus::FAILURE;
      }
      waypoint_statuses[cur_waypoint_index].set_waypoint_status(proto::WaypointStatusType::WAYPOINT_STATUS_COMPLETED);
    }
...
BT_REGISTER_NODES(factory) {
  factory.registerNodeType<autonomy::tasks::behavior_tree::plugins::action::RemovePassedGoals>("RemovePassedGoals");
}
