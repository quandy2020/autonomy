...
ComputeRouteAction::ComputeRouteAction(const std::string& xml_tag_name,
...
BT::NodeStatus ComputeRouteAction::on_success() {
    if (result_.result) {
        if (result_.result->has_path()) {
            ...
            for (const auto& proto_pose : proto_path.poses()) {
                path.poses.push_back(
                    commsgs::geometry_msgs::FromProto(proto_pose));
            }
            setOutput("path", path);
        } else {
        ...
        if (result_.result->has_planning_time()) {
            ...
            commsgs::builtin_interfaces::Duration duration =
                commsgs::builtin_interfaces::FromProto(proto_duration);
    ...
    setOutput(
        "error_code_id",
        static_cast<int32_t>(
            proto::ComputeRouteActionErrorCode::COMPUTE_ROUTE_ERROR_NONE));
...
BT::NodeStatus ComputeRouteAction::on_aborted() {
    ...
    if (result_.result) {
        ...
        setOutput("error_code_id",
                  static_cast<int32_t>(proto::ComputeRouteActionErrorCode::
                                           COMPUTE_ROUTE_ERROR_UNKNOWN));
...
void ComputeRouteAction::on_timeout() {
    ...
    setOutput(
        "error_code_id",
        static_cast<int32_t>(
            proto::ComputeRouteActionErrorCode::COMPUTE_ROUTE_ERROR_TIMEOUT));
    setOutput("error_msg",
...
BT_REGISTER_NODES(factory) {
    ...
    factory.registerBuilder<
        autonomy::tasks::behavior_tree::plugins::action::ComputeRouteAction>(
