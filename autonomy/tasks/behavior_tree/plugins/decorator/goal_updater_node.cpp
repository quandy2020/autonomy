...
void GoalUpdater::initialize() { createROSInterfaces(); }
...
inline BT::NodeStatus GoalUpdater::tick() {
  ...
  if (last_goal_received_set_) {
    ...
    bool has_timestamp = !(last_goal_received_.header.stamp.sec == 0 && last_goal_received_.header.stamp.nanosec == 0);
    if (!has_timestamp) {
      ...
      int64_t goal_time_ns =
          static_cast<int64_t>(goal.header.stamp.sec) * 1000000000LL + static_cast<int64_t>(goal.header.stamp.nanosec);
