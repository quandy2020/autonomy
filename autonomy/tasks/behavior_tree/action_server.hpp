...
/**
 * @brief In-process behavior-tree executor (one tree run per navigation goal).
 */
class BehaviorTreeActionServer
{
public:
    ...
    using OnCompletionCallback = std::function<void(RunStatus)>;

    BehaviorTreeActionServer(std::shared_ptr<BehaviorTreeEngine> engine,
