...
inline std::function<bool()> CancelCheckerFromBlackboard(
  const BT::Blackboard::Ptr & blackboard)
{
  std::function<bool()> checker;
  if (blackboard && blackboard->get("cancel_checker", checker)) {
    return checker;
  }
  return []() { return false; };
}

inline std::function<bool()> CancelCheckerFromConfig(
  const BT::NodeConfiguration & config)
{
  return CancelCheckerFromBlackboard(config.blackboard);
}

inline std::string GlobalFrameFromBlackboard(
  const BT::Blackboard::Ptr & blackboard, const std::string & fallback = "map")
{
  std::string frame = fallback;
  if (blackboard) {
    blackboard->get("global_frame", frame);
  }
  return frame;
}

inline std::string RobotBaseFrameFromBlackboard(
  const BT::Blackboard::Ptr & blackboard,
  const std::string & fallback = "base_link")
{
  std::string frame = fallback;
  if (blackboard) {
    blackboard->get("robot_base_frame", frame);
  }
  return frame;
}

inline float TransformToleranceFromBlackboard(
  const BT::Blackboard::Ptr & blackboard, const float fallback = 0.1f)
{
  double tol = fallback;
  if (blackboard) {
    blackboard->get("transform_tolerance", tol);
  }
  return static_cast<float>(tol);
}

inline std::string DefaultPlannerFromBlackboard(
  const BT::Blackboard::Ptr & blackboard,
  const std::string & fallback = "navfn_planner")
{
  std::string id = fallback;
  if (blackboard) {
    blackboard->get("selected_planner", id);
  }
  return id;
}

inline std::string DefaultControllerFromBlackboard(
  const BT::Blackboard::Ptr & blackboard,
  const std::string & fallback = "FollowPath")
{
  std::string id = fallback;
  if (blackboard) {
    blackboard->get("selected_controller", id);
  }
  return id;
}

inline std::string DefaultSmootherFromBlackboard(
  const BT::Blackboard::Ptr & blackboard,
  const std::string & fallback = "simple_smoother")
{
  std::string id = fallback;
  if (blackboard) {
    blackboard->get("selected_smoother", id);
  }
  return id;
}

inline std::string DefaultGoalCheckerFromBlackboard(
  const BT::Blackboard::Ptr & blackboard,
  const std::string & fallback = "")
{
  std::string id = fallback;
  if (blackboard) {
    blackboard->get("selected_goal_checker", id);
  }
  return id;
}

inline std::string DefaultProgressCheckerFromBlackboard(
  const BT::Blackboard::Ptr & blackboard,
  const std::string & fallback = "")
{
  std::string id = fallback;
  if (blackboard) {
    blackboard->get("selected_progress_checker", id);
  }
  return id;
}

inline std::shared_ptr<TaskContext> TaskContextFromBlackboard(
  const BT::Blackboard::Ptr & blackboard)
{
  std::shared_ptr<TaskContext> ctx;
  if (blackboard) {
    blackboard->get("task_context", ctx);  // NOLINT
  }
  return ctx;
}

inline std::shared_ptr<transform::Buffer> TransformBufferFromBlackboard(
  const BT::Blackboard::Ptr & blackboard)
{
  if (auto ctx = TaskContextFromBlackboard(blackboard); ctx && ctx->tf_buffer) {
    return ctx->tf_buffer;
  }
  std::shared_ptr<transform::Buffer> tf;
  if (blackboard) {
    blackboard->get("tf_buffer", tf);  // NOLINT
  }
  return tf;
}
...
/** @brief In-process BT action node: onStart / onRunning / onHalted lifecycle. */
...
protected:
    virtual BT::NodeStatus onStart() { return BT::NodeStatus::RUNNING; }
    ...
    std::shared_ptr<common::TaskContext> taskContext() const {
        return common::TaskContextFromBlackboard(config().blackboard);
    }

    bool isCancelRequested() const {
        return common::CancelCheckerFromConfig(config())();
    }
