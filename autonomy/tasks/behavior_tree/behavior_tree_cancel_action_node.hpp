...
public:
    ...
    BtCancelActionNode(const std::string& xml_tag_name,
        ...
        node_ =
            config()
                .blackboard->template get<std::shared_ptr<::autolink::Node>>(
                    "node");
    ...
    void createCancelClient(const std::string& action_name) {
        ...
        cancel_goal_client_ =
            node_->CreateClient<autolink::action::internal::CancelGoalRequest,
                                autolink::action::internal::CancelGoalResponse>(
                action_name + "/cancel_goal");

        // Make sure the server is actually there before continuing
        ADEBUG << "Waiting for " << action_name.c_str()
               << " cancel_goal service";
        int wait_count = 0;
        int max_wait = wait_for_service_timeout_.count() /
                       100;  // Convert ms to count (assuming 100ms intervals)
        while (!cancel_goal_client_->ServiceIsReady() &&
               wait_count < max_wait) {
        ...
        if (!cancel_goal_client_->ServiceIsReady()) {
            AERROR << "\"" << action_name
                   << "\" cancel_goal service not available after waiting for "
                   << wait_for_service_timeout_.count() / 1000.0 << "s";
            throw std::runtime_error(std::string("Cancel goal service ") +
    ...
    BT::NodeStatus tick() override {
        ...
        // behavior)
        auto request =
            std::make_shared<autolink::action::internal::CancelGoalRequest>();
        // Leave goal_id as zero/empty to cancel all goals

        auto response = cancel_goal_client_->SendRequest(
            request,
            std::chrono::duration_cast<std::chrono::seconds>(server_timeout_));
...
protected:
    ...
    std::shared_ptr<
        autolink::Client<autolink::action::internal::CancelGoalRequest,
                         autolink::action::internal::CancelGoalResponse>>
        cancel_goal_client_;
