...
// 根据 navigator 名称从 TaskOptions 取 plugin 类型；若未配置则用默认约定
// "autonomy_tasks/<id>"
std::string GetPluginType(const std::string& navigator_id,
    ...
    if (navigator_id == "navigate_through_poses" &&
        options.has_navigate_through_poses() &&
    ...
    if (navigator_id == "navigate_to_docking" &&
        options.has_navigate_to_docking() &&
    ...
    if (navigator_id == "explore_to_anywhere" &&
        options.has_explore_to_anywhere() &&
