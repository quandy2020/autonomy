...
bool getCurrentPose(commsgs::geometry_msgs::PoseStamped& global_pose,
                    std::shared_ptr<autonomy::transform::Buffer> tf_buffer, const std::string global_frame,
                    const std::string robot_frame, const float transform_timeout) {
...
bool transformPoseInTargetFrame(const commsgs::geometry_msgs::PoseStamped& input_pose,
  ...
  try {
    transformed_pose = tf_buffer->transform(input_pose, target_frame, transform_timeout);
    return true;
  } catch (const TransformException& ex) {
    AERROR << "Transform error looking up target frame: " << ex.what();
    AERROR << "Failed to transform from " << input_pose.header.frame_id << " to " << target_frame << ": " << ex.what();
...
bool getTransform(const std::string& source_frame_id, const std::string& target_frame_id,
  ...
  try {
    ...
    zero_time.sec = 0;
    zero_time.nanosec = 0;
    transform_msg = tf_buffer->lookupTransform(target_frame_id, source_frame_id, zero_time, transform_tolerance);
    return true;
  } catch (const TransformException& e) {
    AERROR << "Failed to get \"" << source_frame_id << "\"->\"" << target_frame_id
           << "\" frame transform: " << e.what();
    return false;
...
bool getTransform(const std::string& source_frame_id, const std::string& target_frame_id,
  ...
  if (getTransform(source_frame_id, target_frame_id, transform_tolerance, tf_buffer, transform)) {
    ...
    tf2_transform = Transform(
        Quaternion(tf2_transform_msg.rotation.x, tf2_transform_msg.rotation.y, tf2_transform_msg.rotation.z,
                   tf2_transform_msg.rotation.w),
        Vector3(tf2_transform_msg.translation.x, tf2_transform_msg.translation.y, tf2_transform_msg.translation.z));
    return true;
  }
  return false;
}
...
bool getTransform(const std::string& source_frame_id, const commsgs::builtin_interfaces::Time& source_time,
  ...
  try {
    ...
    transform_msg = tf_buffer->lookupTransform(target_frame_id, target_time, source_frame_id, source_time,
                                               fixed_frame_id, transform_tolerance);
    return true;
  } catch (const TransformException& ex) {
    AERROR << "Failed to get \"" << source_frame_id << "\"->\"" << target_frame_id
           << "\" frame transform: " << ex.what();
    return false;
  } catch (const std::exception& ex) {
    AERROR << "Failed to get \"" << source_frame_id << "\"->\"" << target_frame_id
           << "\" frame transform: " << ex.what();
...
bool getTransform(const std::string& source_frame_id, const commsgs::builtin_interfaces::Time& source_time,
                  const std::string& target_frame_id, const commsgs::builtin_interfaces::Time& target_time,
                  const std::string& fixed_frame_id, const float transform_tolerance,
                  std::shared_ptr<autonomy::transform::Buffer> tf_buffer, Transform& tf2_transform) {
  commsgs::geometry_msgs::TransformStamped transform;
  tf2_transform.setIdentity();  // initialize by identical transform
  if (getTransform(source_frame_id, source_time, target_frame_id, target_time, fixed_frame_id, transform_tolerance,
                   tf_buffer, transform)) {
    // Convert TransformStamped to TF2 transform
    geometry_msgs::Transform tf2_transform_msg;
    tf2_transform_msg.translation.x = transform.transform.translation.x;
