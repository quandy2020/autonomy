...
PiecewiseQuinticSpiralPath::PiecewiseQuinticSpiralPath(const double theta, const double kappa, const double dkappa)
    : last_theta_(theta), last_kappa_(kappa), last_dkappa_(dkappa) {
  accumulated_s_.push_back(0.0);
}

void PiecewiseQuinticSpiralPath::Append(const double theta, const double kappa, const double dkappa,
                                        const double delta_s) {
  double s = delta_s + accumulated_s_.back();
  accumulated_s_.push_back(s);

  pieces_.emplace_back(last_theta_, last_kappa_, last_dkappa_, theta, kappa, dkappa, delta_s);
...
double PiecewiseQuinticSpiralPath::Evaluate(const std::uint32_t order, const double s) const {
...
double PiecewiseQuinticSpiralPath::ParamLength() const { return accumulated_s_.back(); }
