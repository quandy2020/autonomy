...
#include "autonomy/common/math/integral.hpp"
#include "autonomy/planning/math/curve1d/quintic_polynomial_curve1d.hpp"
...
class QuinticSpiralPathWithDerivation : public QuinticPolynomialCurve1d {
 public:
  ...
  QuinticSpiralPathWithDerivation(const std::array<double, 3>& start, const std::array<double, 3>& end,
                                  const double delta_s);

  QuinticSpiralPathWithDerivation(const double theta0, const double kappa0, const double dkappa0, const double theta1,
                                  const double kappa1, const double dkappa1, const double delta_s);
...
template <size_t N>
QuinticSpiralPathWithDerivation<N>::QuinticSpiralPathWithDerivation(const double x0, const double dx0,
  ...
  coef_deriv_[5][6] =
      30.0 * x0 / s6 + 12.0 * dx0 / s5 + 1.5 * ddx0 / s4 - 30.0 * x1 / s6 + 12.0 * dx1 / s5 - 1.5 * ddx1 / s4;
  ...
  coef_deriv_[4][6] =
      -60.0 * x0 / s5 - 24.0 * dx0 / s4 - 3.0 * ddx0 / s3 + 60.0 * x1 / s5 - 21.0 * dx1 / s4 + 2.0 * ddx1 / s3;
  ...
  coef_deriv_[3][6] =
      30.0 * x0 / s4 + 12.0 * dx0 / s3 + 1.5 * ddx0 / s2 - 30.0 * x1 / s4 + 8.0 * dx1 / s3 - 0.5 * ddx1 / s2;
...
template <size_t N>
QuinticSpiralPathWithDerivation<N>::QuinticSpiralPathWithDerivation(const std::array<double, 3>& start,
                                                                    const std::array<double, 3>& end,
                                                                    const double delta_s)
    : QuinticSpiralPathWithDerivation<N>(start[0], start[1], start[2], end[0], end[1], end[2], delta_s) {}

template <size_t N>
double QuinticSpiralPathWithDerivation<N>::evaluate(const size_t order, const size_t i, const size_t n) {
...
template <size_t N>
std::pair<double, double> QuinticSpiralPathWithDerivation<N>::DeriveCartesianDeviation(const size_t param_index) {
...
template <size_t N>
double QuinticSpiralPathWithDerivation<N>::DeriveKappaDerivative(const size_t param_index, const int i, const int n) {
  ...
  double derivative = 5.0 * coef_deriv_[5][param_index] * s4 + 4.0 * coef_deriv_[4][param_index] * s3 +
                      3.0 * coef_deriv_[3][param_index] * s2 + 2.0 * coef_deriv_[2][param_index] * s +
                      coef_deriv_[1][param_index];

  if (param_index == DELTA_S) {
    derivative += 20.0 * coef_[5] * s3 * r + 12.0 * coef_[4] * s2 * r + 6.0 * coef_[3] * s * r + 2.0 * coef_[2] * r;
  }
...
template <size_t N>
double QuinticSpiralPathWithDerivation<N>::DeriveDKappaDerivative(const size_t param_index, const int i, const int n) {
  auto key = param_index * INDEX_MAX + i;
  ...
  double derivative = 20.0 * coef_deriv_[5][param_index] * s3 + 12.0 * coef_deriv_[4][param_index] * s2 +
...
template <size_t N>
double QuinticSpiralPathWithDerivation<N>::DeriveThetaDerivative(const size_t param_index, const double r) const {
  double s = param_ * r;
  double s2 = s * s;
  double s3 = s2 * s;
  double s4 = s2 * s2;
  ...
  if (param_index == DELTA_S) {
    derivative += coef_[5] * 5.0 * s4 * r + coef_[4] * 4.0 * s3 * r + coef_[3] * 3.0 * s2 * r + coef_[2] * 2.0 * s * r +
                  coef_[1] * r;
  }
  return derivative;
}
...
template <size_t N>
double QuinticSpiralPathWithDerivation<N>::DeriveCosTheta(const size_t param_index, const double r) const {
  double g = param_ * r;
...
template <size_t N>
double QuinticSpiralPathWithDerivation<N>::DeriveSinTheta(const size_t param_index, const double r) const {
  double g = param_ * r;
  double theta = Evaluate(0, g);
