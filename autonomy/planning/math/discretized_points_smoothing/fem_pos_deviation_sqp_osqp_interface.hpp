...
// gflags will be defined in .cpp file
#include "osqp/osqp.h"
...
class FemPosDeviationSqpOsqpInterface {
 public:
  ...
  void set_ref_points(const std::vector<std::pair<double, double>>& ref_points) { ref_points_ = ref_points; }
  ...
  void set_weight_path_length(const double weight_path_length) { weight_path_length_ = weight_path_length; }

  void set_weight_ref_deviation(const double weight_ref_deviation) { weight_ref_deviation_ = weight_ref_deviation; }
  ...
  void set_curvature_constraint(const double curvature_constraint) { curvature_constraint_ = curvature_constraint; }
 ...
 private:
  ...
  void CalculateAffineConstraint(const std::vector<std::pair<double, double>>& points, std::vector<c_float>* A_data,
 ...
 private:
  ...
  // park generic
 public:
  void set_point_box(std::vector<std::vector<common::math::Vec2d>> point_box) { point_box_ = point_box; }

 private:
  void CalculateOffset(std::vector<std::pair<double, double>> points, std::vector<c_float>* q);
