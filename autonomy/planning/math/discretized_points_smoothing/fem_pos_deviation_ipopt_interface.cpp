...
#include "autonomy/planning/math/discretized_points_smoothing/fem_pos_deviation_ipopt_interface.hpp"
...
bool FemPosDeviationIpoptInterface::get_nlp_info(int& n, int& m, int& nnz_jac_g, int& nnz_h_lag,
    ...
    m = static_cast<int>(num_of_points_ * 2 + num_of_curvature_constr_ + num_of_slack_constr_);
...
bool FemPosDeviationIpoptInterface::get_bounds_info(int n, double* x_l, double* x_u, int m, double* g_l, double* g_u) {
    ...
    double average_delta_s = ref_total_length / static_cast<double>(num_of_points_ - 1);
    double curvature_constr_upper = average_delta_s * average_delta_s * curvature_constraint_;

    for (size_t i = curvature_constr_start_index_; i < curvature_constr_end_index_; ++i) {
        g_l[i] = -1e20;
...
bool FemPosDeviationIpoptInterface::get_starting_point(int n, bool init_x, double* x, bool init_z, double* z_L,
...
bool FemPosDeviationIpoptInterface::eval_f(int n, const double* x, bool new_x, double& obj_value) {
    CHECK_EQ(static_cast<size_t>(n), num_of_variables_);
...
bool FemPosDeviationIpoptInterface::eval_grad_f(int n, const double* x, bool new_x, double* grad_f) {
    CHECK_EQ(static_cast<size_t>(n), num_of_variables_);
...
bool FemPosDeviationIpoptInterface::eval_g(int n, const double* x, bool new_x, int m, double* g) {
    CHECK_EQ(static_cast<size_t>(n), num_of_variables_);
    CHECK_EQ(static_cast<size_t>(m), num_of_constraints_);
...
bool FemPosDeviationIpoptInterface::eval_jac_g(int n, const double* x, bool new_x, int m, int nele_jac, int* iRow,
...
bool FemPosDeviationIpoptInterface::eval_h(int n, const double* x, bool new_x, double obj_factor, int m,
...
void FemPosDeviationIpoptInterface::finalize_solution(Ipopt::SolverReturn status, int n, const double* x,
                                                      const double* z_L, const double* z_U, int m, const double* g,
...
bool FemPosDeviationIpoptInterface::eval_obj(int n, const T* x, T* obj_value) {
    *obj_value = 0.0;
    ...
    for (size_t i = 0; i < num_of_points_; ++i) {
        size_t index = i * 2;
        *obj_value +=
            weight_ref_deviation_ * ((x[index] - ref_points_[i].first) * (x[index] - ref_points_[i].first) +
    ...
    for (size_t i = 0; i + 2 < num_of_points_; ++i) {
        ...
        *obj_value += weight_fem_pos_deviation_ *
                      (((x[findex] + x[lindex]) - 2.0 * x[mindex]) * ((x[findex] + x[lindex]) - 2.0 * x[mindex]) +
    ...
    for (size_t i = 0; i + 1 < num_of_points_; ++i) {
        ...
        *obj_value += weight_path_length_ * ((x[findex] - x[nindex]) * (x[findex] - x[nindex]) +
...
template <class T>
bool FemPosDeviationIpoptInterface::eval_constraints(int n, const T* x, int m, T* g) {
    // a. positional deviation constraints
    for (size_t i = 0; i < num_of_points_; ++i) {
        size_t index = i * 2;
        g[index] = x[index];
...
void FemPosDeviationIpoptInterface::generate_tapes(int n, int m, int* nnz_jac_g, int* nnz_h_lag) {
