...
#include "autonomy/planning/math/discretized_points_smoothing/fem_pos_deviation_sqp_osqp_interface.hpp"
...
// Define required gflags
DEFINE_bool(enable_obstacle_potential_field, false, "Enable obstacle potential field");
...
bool FemPosDeviationSqpOsqpInterface::Solve() {
  ...
  while (pen_itr < sqp_pen_max_iter_) {
    ...
    while (sub_itr < sqp_sub_max_iter_) {
      ...
      if (!iterative_solve_res) {
        AERROR << "iteration at " << sub_itr << ", solving fails with max sub iter " << sqp_sub_max_iter_;
      ...
      if (ftol < sqp_ftol_) {
        ...
        ADEBUG << "merit function value converges to " << cur_fvalue << ", with ftol " << ftol << ", under max_ftol "
               << sqp_ftol_;
        fconverged = true;
        break;
      }
  ...
  ADEBUG << "constraint voilation value drops to " << ctol << ", higher than max_ctol " << sqp_ctol_;
  weight_curvature_constraint_slack_var_ = original_slack_penalty;
  osqp_cleanup(work);
  c_free(data->A);
  c_free(data->P);
...
void FemPosDeviationSqpOsqpInterface::CalculateKernel(std::vector<c_float>* P_data, std::vector<c_int>* P_indices,
                                                      std::vector<c_int>* P_indptr) {
  ...
  for (int col = 2; col < 4; ++col) {
    ...
    columns[col].emplace_back(col, 5.0 * weight_fem_pos_deviation_ + 2.0 * weight_path_length_ + weight_ref_deviation_);
    ++col_num;
  }
  ...
  for (int point_index = 2; point_index < second_point_from_last_index; ++point_index) {
    ...
    for (int col = 0; col < 2; ++col) {
      ...
      columns[col_index].emplace_back(col_index - 2, -4.0 * weight_fem_pos_deviation_ - weight_path_length_);
...
void FemPosDeviationSqpOsqpInterface::CalculateOffset(std::vector<std::pair<double, double>> points,
                                                      std::vector<c_float>* q) {
  ...
  AINFO << "FLAGS_enable_obstacle_potential_field: " << FLAGS_enable_obstacle_potential_field
        << " num_of_points_: " << num_of_points_ << "point_box_: " << point_box_.size();
  if (FLAGS_enable_obstacle_potential_field && point_box_.size() == num_of_points_ - 1) {
    ...
    for (int i = 1; i < num_of_points_ - 1; ++i) {
      ...
      for (int j = 0; j < 4; j++) {
        ...
        Vec2d ego_vec(points[i].first - point_box_[i][j].x(), points[i].second - point_box_[i][j].y());
...
void FemPosDeviationSqpOsqpInterface::CalculateAffineConstraint(
    const std::vector<std::pair<double, double>>& points, std::vector<c_float>* A_data, std::vector<c_int>* A_indices,
    std::vector<c_int>* A_indptr, std::vector<c_float>* lower_bounds, std::vector<c_float>* upper_bounds) {
  ...
  double curvature_constraint_sqr = (interval_sqr * curvature_constraint_) * (interval_sqr * curvature_constraint_);
  for (int i = 0; i < num_of_curvature_constraints_; ++i) {
    (*upper_bounds)[num_of_variable_constraints_ + i] = (curvature_constraint_sqr - lin_cache[i][6]) * scale_factor;
    (*lower_bounds)[num_of_variable_constraints_ + i] = -1e20;
  }
}

void FemPosDeviationSqpOsqpInterface::SetPrimalWarmStart(const std::vector<std::pair<double, double>>& points,
                                                         std::vector<c_float>* primal_warm_start) {
  ...
  for (int i = 1; i < num_of_points_; ++i) {
    ...
    total_length += std::sqrt((pre_point.first - cur_point.first) * (pre_point.first - cur_point.first) +
...
bool FemPosDeviationSqpOsqpInterface::OptimizeWithOsqp(const std::vector<c_float>& primal_warm_start,
                                                       OSQPWorkspace** work) {
