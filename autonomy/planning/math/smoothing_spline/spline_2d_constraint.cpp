...
/**
 * @file : spline_smoother_constraint.cpp
 **/

#include "autonomy/planning/math/smoothing_spline/spline_2d_constraint.hpp"
...
Spline2dConstraint::Spline2dConstraint(const std::vector<double>& t_knots, const uint32_t order)
  ...
  total_param_ = 2 * (spline_order_ + 1) * (static_cast<uint32_t>(t_knots.size()) - 1);
}

// direct method
bool Spline2dConstraint::AddInequalityConstraint(const Eigen::MatrixXd& constraint_matrix,
                                                 const Eigen::MatrixXd& constraint_boundary) {
  return inequality_constraint_.AddConstraint(constraint_matrix, constraint_boundary);
}

bool Spline2dConstraint::AddEqualityConstraint(const Eigen::MatrixXd& constraint_matrix,
                                               const Eigen::MatrixXd& constraint_boundary) {
 ...
 **/
bool Spline2dConstraint::Add2dBoundary(const std::vector<double>& t_coord, const std::vector<double>& angle,
                                       const std::vector<Vec2d>& ref_point,
                                       const std::vector<double>& longitudinal_bound,
                                       const std::vector<double>& lateral_bound) {
  if (t_coord.size() != angle.size() || angle.size() != ref_point.size() || ref_point.size() != lateral_bound.size() ||
      lateral_bound.size() != longitudinal_bound.size()) {
    return false;
  }
  Eigen::MatrixXd affine_inequality = Eigen::MatrixXd::Zero(4 * t_coord.size(), total_param_);
  Eigen::MatrixXd affine_boundary = Eigen::MatrixXd::Zero(4 * t_coord.size(), 1);
...
bool Spline2dConstraint::Add2dDerivativeBoundary(const std::vector<double>& t_coord, const std::vector<double>& angle,
                                                 const std::vector<Vec2d>& ref_point,
                                                 const std::vector<double>& longitudinal_bound,
                                                 const std::vector<double>& lateral_bound) {
  if (t_coord.size() != angle.size() || angle.size() != ref_point.size() || ref_point.size() != lateral_bound.size() ||
  ...
  for (uint32_t i = 0; i < t_coord.size(); ++i) {
    ...
    std::vector<double> longitudinal_coef = AffineDerivativeCoef(angle[i] - M_PI / 2, rel_t);
    for (uint32_t j = 0; j < 2 * (spline_order_ + 1); ++j) {
      // upper longi
      affine_inequality(4 * i, index_offset + j) = longi_coef[j];
      // lower longi
...
bool Spline2dConstraint::Add2dSecondDerivativeBoundary(const std::vector<double>& t_coord,
  ...
  for (uint32_t i = 0; i < t_coord.size(); ++i) {
    ...
    const double rel_t = t_coord[i] - t_knots_[index];
    const uint32_t index_offset = 2 * index * (spline_order_ + 1);
    std::vector<double> longi_coef = AffineSecondDerivativeCoef(angle[i], rel_t);
    std::vector<double> longitudinal_coef = AffineSecondDerivativeCoef(angle[i] - M_PI / 2, rel_t);
    for (uint32_t j = 0; j < 2 * (spline_order_ + 1); ++j) {
      // upper longi
      affine_inequality(4 * i, index_offset + j) = longi_coef[j];
      // lower longi
...
bool Spline2dConstraint::Add2dThirdDerivativeBoundary(const std::vector<double>& t_coord,
                                                      const std::vector<double>& angle,
                                                      const std::vector<Vec2d>& ref_point,
                                                      const std::vector<double>& longitudinal_bound,
                                                      const std::vector<double>& lateral_bound) {
  ...
  for (uint32_t i = 0; i < t_coord.size(); ++i) {
    ...
    std::vector<double> longitudinal_coef = AffineThirdDerivativeCoef(angle[i] - M_PI / 2, rel_t);
    for (uint32_t j = 0; j < 2 * (spline_order_ + 1); ++j) {
      // upper longi
      affine_inequality(4 * i, index_offset + j) = longi_coef[j];
      // lower longi
...
bool Spline2dConstraint::AddPointConstraint(const double t, const double x, const double y) {
...
bool Spline2dConstraint::AddPointSecondDerivativeConstraint(const double t, const double ddx, const double ddy) {
...
bool Spline2dConstraint::AddPointThirdDerivativeConstraint(const double t, const double dddx, const double dddy) {
  const size_t index = FindIndex(t);
  const double rel_t = t - t_knots_[index];
...
bool Spline2dConstraint::AddPointKthOrderDerivativeConstraint(const double t, const double x_kth_derivative,
...
bool Spline2dConstraint::AddPointAngleConstraint(const double t, const double angle) {
  ...
  const Eigen::MatrixXd affine_inequality_boundary = Eigen::MatrixXd::Zero(2, 1);
...
bool Spline2dConstraint::AddSmoothConstraint() {
  ...
  Eigen::MatrixXd affine_equality = Eigen::MatrixXd::Zero(2 * (t_knots_.size() - 2), total_param_);
  Eigen::MatrixXd affine_boundary = Eigen::MatrixXd::Zero(2 * (t_knots_.size() - 2), 1);
...
bool Spline2dConstraint::AddDerivativeSmoothConstraint() {
  ...
  Eigen::MatrixXd affine_equality = Eigen::MatrixXd::Zero(4 * (t_knots_.size() - 2), total_param_);
  Eigen::MatrixXd affine_boundary = Eigen::MatrixXd::Zero(4 * (t_knots_.size() - 2), 1);
  ...
  for (uint32_t i = 0; i + 2 < t_knots_.size(); ++i) {
    ...
    for (uint32_t j = 0; j < num_params; ++j) {
      ...
      affine_equality(4 * i + 3, j + index_offset + num_params) = derivative_t[j];
    }
...
bool Spline2dConstraint::AddThirdDerivativeSmoothConstraint() {
  ...
  Eigen::MatrixXd affine_equality = Eigen::MatrixXd::Zero(8 * (t_knots_.size() - 2), total_param_);
  Eigen::MatrixXd affine_boundary = Eigen::MatrixXd::Zero(8 * (t_knots_.size() - 2), 1);
  ...
  for (uint32_t i = 0; i + 2 < t_knots_.size(); ++i) {
    ...
    for (uint32_t j = 0; j < num_params; ++j) {
      ...
      affine_equality(8 * i + 5, j + index_offset + num_params) = derivative_t[j];
      affine_equality(8 * i + 6, j + index_offset + num_params) = second_derivative_t[j];
      affine_equality(8 * i + 7, j + index_offset + num_params) = third_derivative_t[j];
    }
 ...
 **/
const AffineConstraint& Spline2dConstraint::inequality_constraint() const { return inequality_constraint_; }

const AffineConstraint& Spline2dConstraint::equality_constraint() const { return equality_constraint_; }
...
uint32_t Spline2dConstraint::FindIndex(const double t) const {
  ...
  return std::min(static_cast<uint32_t>(t_knots_.size() - 1), static_cast<uint32_t>(upper_bound - t_knots_.begin())) -
         1;
}
...
std::vector<double> Spline2dConstraint::AffineDerivativeCoef(const double angle, const double t) const {
  const uint32_t num_params = spline_order_ + 1;
  std::vector<double> result(num_params * 2, 0.0);
  double x_coef = -std::sin(angle);
  double y_coef = std::cos(angle);
...
std::vector<double> Spline2dConstraint::AffineSecondDerivativeCoef(const double angle, const double t) const {
  const uint32_t num_params = spline_order_ + 1;
  std::vector<double> result(num_params * 2, 0.0);
  double x_coef = -std::sin(angle);
  double y_coef = std::cos(angle);
...
std::vector<double> Spline2dConstraint::AffineThirdDerivativeCoef(const double angle, const double t) const {
  const uint32_t num_params = spline_order_ + 1;
  std::vector<double> result(num_params * 2, 0.0);
  double x_coef = -std::sin(angle);
  double y_coef = std::cos(angle);
...
std::vector<double> Spline2dConstraint::SecondDerivativeCoef(const double t) const {
  std::vector<double> result(spline_order_ + 1, 0.0);
  std::vector<double> power_t = PolyCoef(t);
...
std::vector<double> Spline2dConstraint::ThirdDerivativeCoef(const double t) const {
  std::vector<double> result(spline_order_ + 1, 0.0);
  std::vector<double> power_t = PolyCoef(t);
