...
Spline2dKernel* OsqpSpline2dSolver::mutable_kernel() {
    return &kernel_;
}

Spline2d* OsqpSpline2dSolver::mutable_spline() {
    return &spline_;
}
...
bool OsqpSpline2dSolver::Solve() {
    ...
    for (int i = 0; i < constraint_num; ++i) {
        if (i < inequality_constraint_boundary.rows()) {
            ...
            l[static_cast<size_t>(i)] =
                equality_constraint_boundary(idx, 0) - kEpsilon;
    ...
    if (data->P == nullptr || data->A == nullptr) {
        AERROR << "OSQP csc_matrix allocation failed. P="
               << (data->P != nullptr) << ", A=" << (data->A != nullptr);
        if (data->A != nullptr) {
            c_free(data->A);
        }
        if (data->P != nullptr) {
...
// extract
const Spline2d& OsqpSpline2dSolver::spline() const {
    return spline_;
}
