...
/**
 * @file spline_2d_kernel.cpp
 **/

#include "autonomy/planning/math/smoothing_spline/spline_2d_kernel.hpp"

#include <algorithm>

#include "autonomy/planning/math/smoothing_spline/spline_seg_kernel.hpp"
...
Spline2dKernel::Spline2dKernel(const std::vector<double>& t_knots, const uint32_t spline_order)
  ...
  total_params_ = (t_knots_.size() > 1 ? 2 * (t_knots_.size() - 1) * (1 + spline_order_) : 0);
...
bool Spline2dKernel::AddKernel(const Eigen::MatrixXd& kernel, const Eigen::MatrixXd& offset, const double weight) {
...
bool Spline2dKernel::AddKernel(const Eigen::MatrixXd& kernel, const double weight) {
  Eigen::MatrixXd offset = Eigen::MatrixXd::Zero(kernel.rows(), 1);
  return AddKernel(kernel, offset, weight);
}

Eigen::MatrixXd* Spline2dKernel::mutable_kernel_matrix() { return &kernel_matrix_; }

Eigen::MatrixXd* Spline2dKernel::mutable_offset() { return &offset_; }

Eigen::MatrixXd Spline2dKernel::kernel_matrix() const { return kernel_matrix_ * 2.0; }
...
void Spline2dKernel::AddNthDerivativeKernelMatrix(const uint32_t n, const double weight) {
  for (uint32_t i = 0; i + 1 < t_knots_.size(); ++i) {
    ...
    Eigen::MatrixXd cur_kernel =
        SplineSegKernel::Instance()->NthDerivativeKernel(n, num_params, t_knots_[i + 1] - t_knots_[i]) * weight;
    kernel_matrix_.block(2 * i * num_params, 2 * i * num_params, num_params, num_params) += cur_kernel;
    kernel_matrix_.block((2 * i + 1) * num_params, (2 * i + 1) * num_params, num_params, num_params) += cur_kernel;
  }
}

void Spline2dKernel::AddDerivativeKernelMatrix(const double weight) { AddNthDerivativeKernelMatrix(1, weight); }

void Spline2dKernel::AddSecondOrderDerivativeMatrix(const double weight) { AddNthDerivativeKernelMatrix(2, weight); }

void Spline2dKernel::AddThirdOrderDerivativeMatrix(const double weight) { AddNthDerivativeKernelMatrix(3, weight); }

// reference line kernel, t_coord in strictly increasing order (for path
// optimizer)
bool Spline2dKernel::AddReferenceLineKernelMatrix(const std::vector<double>& t_coord,
  ...
  for (uint32_t i = 0; i < t_coord.size(); ++i) {
    ...
    kernel_matrix_.block((2 * cur_index) * num_params, (2 * cur_index) * num_params, num_params, num_params) +=
        weight * ref_kernel;
    kernel_matrix_.block((2 * cur_index + 1) * num_params, (2 * cur_index + 1) * num_params, num_params, num_params) +=
        weight * ref_kernel;
  }
  return true;
}
