...
Spline2d::Spline2d(const std::vector<double>& t_knots, const uint32_t order) : t_knots_(t_knots), spline_order_(order) {
  if (t_knots.size() > 1) {
...
const Spline2dSeg& Spline2d::smoothing_spline(const uint32_t index) const { return splines_[index]; }
