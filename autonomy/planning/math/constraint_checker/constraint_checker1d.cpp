...
/**
 * @file constraint_checker1d.cpp
 **/

#include "autonomy/planning/math/constraint_checker/constraint_checker1d.hpp"

#include "autonomy/common/logging.hpp"
#include "gflags/gflags.h"

// Define required gflags (these are already defined in constraint_checker.cpp,
// but we need to declare them here to avoid duplicate definition errors)
// Since gflags handles duplicate definitions internally, we can use DECLARE here
...
bool ConstraintChecker1d::IsValidLateralTrajectory(const Curve1d& lat_trajectory, const Curve1d& lon_trajectory) {
    double t = 0.0;
    while (t < lon_trajectory.ParamLength()) {
        ...
        if (!fuzzy_within(a, -FLAGS_lateral_acceleration_bound, FLAGS_lateral_acceleration_bound)) {
            return false;
        }
