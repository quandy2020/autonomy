...
#include "autonomy/planning/math/piecewise_jerk/piecewise_jerk_path_problem.hpp"
...
void PiecewiseJerkPathProblem::CalculateKernel(std::vector<c_float>* P_data, std::vector<c_int>* P_indices,
  ...
  columns[n - 1].emplace_back(
      n - 1, (weight_x_ + weight_x_ref_vec_[n - 1] + weight_end_state_[0]) / (scale_factor_[0] * scale_factor_[0]));
  ++value_index;

  // x(i)'^2 * w_dx
  for (int i = 0; i < n - 1; ++i) {
    columns[n + i].emplace_back(n + i, weight_dx_ / (scale_factor_[1] * scale_factor_[1]));
    ++value_index;
  }
  ...
  for (int i = 1; i < n - 1; ++i) {
    columns[2 * n + i].emplace_back(
        2 * n + i, (weight_ddx_ + 2.0 * weight_dddx_ / delta_s_square) / (scale_factor_[2] * scale_factor_[2]));
    ++value_index;
  }
...
void PiecewiseJerkPathProblem::CalculateAffineConstraint(std::vector<c_float>* A_data, std::vector<c_int>* A_indices,
  ...
  const int n = static_cast<int>(num_of_knots_);
  const int num_of_extra_constraints = static_cast<int>(extra_constraints_.size());
  const int num_of_vertex_constraints = static_cast<int>(vertex_constraints_.size());
  const int num_of_variables = 3 * n;
  const int num_of_constraints =
      num_of_variables + 3 * (n - 1) + 3 + num_of_extra_constraints + num_of_vertex_constraints;
  ...
  for (int i = 0; i < num_of_extra_constraints; ++i) {
    ...
    variables[left_index].emplace_back(constraint_index, left_weight / scale_factor_[0]);
    variables[right_index].emplace_back(constraint_index, right_weight / scale_factor_[0]);
  ...
  for (int i = 0; i < num_of_vertex_constraints; ++i) {
    ...
    variables[left_index].emplace_back(constraint_index, left_weight / scale_factor_[0]);
    variables[right_index].emplace_back(constraint_index, right_weight / scale_factor_[0]);
    variables[n + left_index].emplace_back(constraint_index,
                                           vertex_constraints_.front_edge_to_center * left_weight / scale_factor_[1]);
    variables[n + right_index].emplace_back(constraint_index,
                                            vertex_constraints_.front_edge_to_center * right_weight / scale_factor_[1]);
...
void PiecewiseJerkPathProblem::CalculateOffset(std::vector<c_float>* q) {
  ...
  if (has_towing_x_ref_) {
    for (int i = 0; i < n; ++i) {
      q->at(i) += -2.0 * weight_towing_x_ref_vec_.at(i) * towing_x_ref_[i] / scale_factor_[0];
    }
  }
}
