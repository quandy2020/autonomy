...
void PiecewiseJerkPathProblem::CalculateAffineConstraint(std::vector<c_float>* A_data, std::vector<c_int>* A_indices,
    ...
    for (int i = 0; i < num_of_vertex_constraints; ++i) {
        ...
        variables[n + left_index].emplace_back(
            constraint_index, vertex_constraints_.front_edge_to_center * left_weight / scale_factor_[1]);
        variables[n + right_index].emplace_back(
            constraint_index, vertex_constraints_.front_edge_to_center * right_weight / scale_factor_[1]);
