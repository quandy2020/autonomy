...
#include "autonomy/planning/math/piecewise_jerk/piecewise_jerk_problem.hpp"
...
bool PiecewiseJerkProblem::FormulateProblem(OSQPData* data) {
    ...
    data->q = CopyData(q);
    data->A = csc_matrix(num_affine_constraint, kernel_dim, A_data.size(), CopyData(A_data), CopyData(A_indices),
                         CopyData(A_indptr));
...
void PiecewiseJerkProblem::set_ddx_bounds(const double ddx_lower_bound, const double ddx_upper_bound) {
...
void PiecewiseJerkProblem::set_x_ref(std::vector<double> weight_x_ref_vec, std::vector<double> x_ref) {
    CHECK_EQ(x_ref.size(), num_of_knots_);
...
void PiecewiseJerkProblem::set_towing_x_ref(const double weight_towing_x_ref, std::vector<double> towing_x_ref) {
    CHECK_EQ(towing_x_ref.size(), num_of_knots_);
    weight_towing_x_ref_vec_ = std::vector<double>(num_of_knots_, weight_towing_x_ref);
    towing_x_ref_ = std::move(towing_x_ref);
    has_towing_x_ref_ = true;
}

void PiecewiseJerkProblem::set_towing_x_ref(std::vector<double> weight_towing_x_ref_vec,
...
bool PiecewiseJerkProblem::CheckLowUpperBound(const std::vector<c_float>& lower, const std::vector<c_float>& upper) {
