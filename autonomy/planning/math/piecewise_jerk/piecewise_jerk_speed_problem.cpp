...
#include "autonomy/planning/math/piecewise_jerk/piecewise_jerk_speed_problem.hpp"
...
PiecewiseJerkSpeedProblem::PiecewiseJerkSpeedProblem(const size_t num_of_knots, const double delta_s,
...
void PiecewiseJerkSpeedProblem::set_dx_ref(const double weight_dx_ref, const double dx_ref) {
  weight_dx_ref_.resize(num_of_knots_, weight_dx_ref);
  dx_ref_.resize(num_of_knots_, dx_ref);
  has_dx_ref_ = true;
}

void PiecewiseJerkSpeedProblem::set_dx_ref(const std::vector<double>& weight_dx_ref,
...
void PiecewiseJerkSpeedProblem::CalculateKernel(std::vector<c_float>* P_data, std::vector<c_int>* P_indices,
  ...
  for (int i = 0; i < n - 1; ++i) {
    columns[n + i].emplace_back(n + i, (weight_dx_ref_[i] + penalty_dx_[i]) / (scale_factor_[1] * scale_factor_[1]));
    ++value_index;
  }
  // x(n-1)'^2 * (w_dx_ref + penalty_dx + w_end_dx)
  columns[2 * n - 1].emplace_back(2 * n - 1, (weight_dx_ref_[n - 1] + penalty_dx_[n - 1] + weight_end_state_[1]) /
