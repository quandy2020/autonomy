...
PolynomialXd::PolynomialXd(const std::uint32_t order) : params_(order + 1, 0.0) { CHECK_GE(order, 0U); }

PolynomialXd::PolynomialXd(const std::vector<double>& params) : params_(params) { ACHECK(!params.empty()); }
