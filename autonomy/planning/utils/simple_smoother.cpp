...
SimpleSmoother::SimpleSmoother(
    ...
    : Smoother(std::move(name), std::move(costmap_wrapper)) {
    ApplyOptions(options);
    ApplyDefaultTuning();
}

void SimpleSmoother::ApplyDefaultTuning() {
...
bool SimpleSmoother::Smooth(commsgs::planning_msgs::Path& path,
    ...
    double max_time_seconds = max_time.count() / 1000.0;
...
using autonomy::planning::utils::SimpleSmoother;
