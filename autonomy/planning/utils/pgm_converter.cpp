...
void initMagickOnce() {
  std::call_once(g_magick_init_flag, []() { Magick::InitializeMagick(nullptr); });
}
...
double costToGray(unsigned char cost) {
  if (cost == autonomy::map::costmap_2d::NO_INFORMATION) return kUnknownGray;
  if (cost == autonomy::map::costmap_2d::FREE_SPACE) return kFreeGray;
  if (cost >= autonomy::map::costmap_2d::LETHAL_OBSTACLE) return kObstacleGray;
  double norm = static_cast<double>(cost) / static_cast<double>(autonomy::map::costmap_2d::LETHAL_OBSTACLE);
  return kFreeGray - norm * kCostNormFactor;
}

Magick::Image createImageFromCostmap(const autonomy::map::costmap_2d::Costmap2D& costmap) {
...
std::vector<Magick::Coordinate> convertPathToImageCoords(const autonomy::map::costmap_2d::Costmap2D& costmap,
                                                         const autonomy::commsgs::planning_msgs::Path& path,
  ...
  for (const auto& pose : path.poses) {
    ...
    if (costmap.worldToMap(pose.pose.position.x, pose.pose.position.y, mx, my)) {
      coords.push_back(Magick::Coordinate(std::min(mx, width - 1), std::min(height - 1 - my, height - 1)));
    }
  }
  return coords;
}

void addCircle(std::list<Magick::Drawable>& draw_list, const Magick::Coordinate& pt, double size,
  ...
  draw_list.push_back(Magick::DrawableCircle(pt.x(), pt.y(), pt.x() + size, pt.y()));
}

void addPathToDrawList(std::list<Magick::Drawable>& draw_list, const std::vector<Magick::Coordinate>& coords,
  ...
  for (size_t i = 0; i < coords.size() - 1; ++i) {
    draw_list.push_back(Magick::DrawableLine(coords[i].x(), coords[i].y(), coords[i + 1].x(), coords[i + 1].y()));
  }
...
std::string ensureExtension(const std::string& path, const std::string& format) {
...
map::costmap_2d::Costmap2D::SharedPtr PgmConverter::loadFromPgm(const std::string& pgm_file_path) {
...
map::costmap_2d::Costmap2D::SharedPtr PgmConverter::loadFromPgm(const std::string& pgm_file_path,
  ...
  try {
    ...
    load_params.origin = {params.origin_x, params.origin_y, params.origin_yaw};
    ...
    auto costmap = std::make_shared<map::costmap_2d::Costmap2D>(occupancy_grid);

    AINFO << "Loaded PGM map: " << pgm_file_path << " -> " << costmap->getSizeInCellsX() << "x"
          << costmap->getSizeInCellsY() << " cells, resolution: " << costmap->getResolution() << " m/pixel";
    return costmap;
  } catch (const std::exception& e) {
    AERROR << "Failed to load PGM map from " << pgm_file_path << ": " << e.what();
    return nullptr;
  }
}

map::costmap_2d::Costmap2D::SharedPtr PgmConverter::loadFromYaml(const std::string& yaml_file_path) {
  try {
    map::costmap_2d::LoadParameters load_params = map::costmap_2d::loadMapYaml(yaml_file_path);
    commsgs::map_msgs::OccupancyGrid occupancy_grid;
    map::costmap_2d::loadMapFromFile(load_params, occupancy_grid);
    auto costmap = std::make_shared<map::costmap_2d::Costmap2D>(occupancy_grid);

    AINFO << "Loaded map from YAML: " << yaml_file_path << " -> " << costmap->getSizeInCellsX() << "x"
          << costmap->getSizeInCellsY() << " cells, resolution: " << costmap->getResolution() << " m/pixel";
    return costmap;
  } catch (const std::exception& e) {
    AERROR << "Failed to load map from YAML " << yaml_file_path << ": " << e.what();
    return nullptr;
  }
}

bool PgmConverter::savePathToImage(const map::costmap_2d::Costmap2D& costmap, const commsgs::planning_msgs::Path& path,
...
bool PgmConverter::savePathToImage(const map::costmap_2d::Costmap2D& costmap, const commsgs::planning_msgs::Path& path,
  ...
  try {
    ...
    if (!path.poses.empty()) {
      std::vector<Magick::Coordinate> coords = convertPathToImageCoords(costmap, path, height, width);
      if (coords.size() >= 2) {
        std::list<Magick::Drawable> draw_list;
        addPathToDrawList(draw_list, coords, params);
        if (params.draw_start_marker) {
          addCircle(draw_list, coords[0], params.start_marker_size, "green");
        }
        if (params.draw_goal_marker) {
          addCircle(draw_list, coords.back(), params.goal_marker_size, "blue");
        }
    ...
    const std::string final_path = ensureExtension(output_file_path, format);
    image.magick(format);
    image.write(final_path);

    AINFO << "Saved map with path to: " << final_path << " (format: " << format << ", points: " << path.poses.size()
          << ")";
    return true;
  } catch (const std::exception& e) {
