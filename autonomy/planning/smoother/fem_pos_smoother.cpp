...
void FemPosSmoother::ApplyOptions(
    const proto::math::FemPosDeviationSmootherConfig& config, double path_bound) {
    config_ = config;
    path_bound_ = path_bound > 0.0 ? path_bound : 0.5;
    solver_ = std::make_unique<math::FemPosDeviationSmoother>(config_);
}

bool FemPosSmoother::Smooth(commsgs::planning_msgs::Path& path,
    ...
    if (!solver_->Solve(raw_points, bounds, &opt_x, &opt_y)) {
        AWARN << "FemPosSmoother failed to optimize path";
        return false;
    }
    if (opt_x.size() != path.poses.size() || opt_y.size() != path.poses.size()) {
        AWARN << "FemPosSmoother returned unexpected point count";
        return false;
    }
...
using autonomy::planning::smoother::FemPosSmoother;

AUTOLINK_PLUGIN_MANAGER_REGISTER_PLUGIN(FemPosSmoother, Smoother);
