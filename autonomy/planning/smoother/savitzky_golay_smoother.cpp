...
void SavitzkyGolaySmoother::CalculateCoefficients() {
    ...
    Eigen::VectorXd v = Eigen::VectorXd::LinSpaced(
        window_size_, -half_window_size_, half_window_size_);
...
bool SavitzkyGolaySmoother::Smooth(commsgs::planning_msgs::Path& path,
        ...
        if (path_segments[i].end - path_segments[i].start > minimum_points) {
            ...
            if (time_remaining <= 0.0) {
                AWARN << "Smoothing time exceeded allowed duration of "
                      << max_time.count() / 1000.0 << " seconds";
                throw common::SmootherTimedOut(
                    "Smoothing time exceed allowed duration");
            }

            // Smooth path segment
            success =
                success && SmoothImpl(curr_path_segment, reversing_segment);
...
bool SavitzkyGolaySmoother::SmoothImpl(commsgs::planning_msgs::Path& path,
                                       bool& reversing_segment) {
    const unsigned int& path_size = path.poses.size();

    // Convert PoseStamped to Eigen
    auto toEigenVec =
        [](const commsgs::geometry_msgs::PoseStamped& pose) -> Eigen::Vector2d {
        return {pose.pose.position.x, pose.pose.position.y};
    };

    auto ApplyFilterOverAxes =
        [&](std::vector<commsgs::geometry_msgs::PoseStamped>& plan_pts,
        ...
        for (unsigned int idx = 1; idx != path_size - 1; idx++) {
            ...
            for (int j = -half_window_size_; j <= half_window_size_; j++) {
                ...
                accum +=
                    sg_coeffs_(j + half_window_size_) * init_plan_pts[path_idx];
            }
    ...
    if (do_refinement_) {
        for (int i = 0; i < refinement_num_; i++) {
            std::vector<Eigen::Vector2d> reined_initial_path_poses(
                path.poses.size());
            std::transform(path.poses.begin(), path.poses.end(),
