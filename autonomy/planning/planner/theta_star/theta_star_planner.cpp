...
void ThetaStarPlanner::Activate() { AINFO << "Activating plugin " << name_ << " of type theta_star_planner"; }

void ThetaStarPlanner::Deactivate() { AINFO << "Deactivating plugin " << name_ << " of type theta_star_planner"; }
...
uint32_t ThetaStarPlanner::CreatePlan(const commsgs::geometry_msgs::PoseStamped& start,
  ...
  if (costmap_ptr->getCost(mx_goal, my_goal) == map::costmap_2d::LETHAL_OBSTACLE) {
    AERROR << "Goal coordinates (" << goal.pose.position.x << ", " << goal.pose.position.y << ") are in lethal cost";
