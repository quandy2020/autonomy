...
struct comp {
  bool operator()(const tree_node* p1, const tree_node* p2) const { return (p1->f) > (p2->f); }
};
...
class ThetaStar {
 public:
   ...
   */
  void setStartAndGoal(const commsgs::geometry_msgs::PoseStamped& start,
                       const commsgs::geometry_msgs::PoseStamped& goal);

  /**
   * @brief checks whether the start and goal points have costmap costs
   * greater than LETHAL_COST
 ...
 protected:
  ...
  /// parent node, and whether or not the node is present in queue_, for all
  /// the nodes searched it is initialised with no elements and its size
  /// increases depending on the number of nodes searched
   ...
   * and then multiply the actual costmap cost by 0.9 to keep the output in
   * the range of [25, 255)
   */
   ...
   * @return the result of the check
   */
  inline bool isGoal(const tree_node& this_node) const { return this_node.x == dst_.x && this_node.y == dst_.y; }

  /**
   * @brief initialises the node_position_ vector by storing -1 as index for
   * all points(x, y) within the limits of the map
   ...
   * ]
   * @param id_this a pointer to the location at which the data of the
   * point(cx, cy) is stored in nodes_data_
   */
   ...
   */
  inline tree_node* getIndex(const int& cx, const int& cy) { return node_position_[size_x_ * cy + cx]; }

  /**
   * @brief this function depending on the size of the nodes_data_ vector
   * allots space to store the data for a node(x, y)
