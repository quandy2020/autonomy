  ...
  if (isROS) {  // ROS-type cost array
    for (int i = 0; i < ny; i++) {
      ...
      for (int j = 0; j < nx; j++, k++, cmap++, cm++) {
        ...
        // COST_OBS                 -> COST_OBS (incoming "lethal
        // obstacle") COST_OBS_ROS             -> COST_OBS (incoming
        // "inscribed inflated obstacle") values in range 0 to 252 ->
        // values from COST_NEUTRAL to COST_OBS_ROS.
        *cm = COST_OBS;
        int v = *cmap;
        if (v < COST_OBS_ROS) {
...
bool NavFn::propNavFnDijkstra(int cycles, std::function<bool()> cancelChecker, bool atStart) {
  ...
  //     "[NavFn] Used %d cycles, %d cells visited (%d%%), priority buf max
...
int NavFn::calcPath(int n, int* st) {
  ...
  for (int i = 0; i < n; i++) {
    // check if near goal
    int nearest_point =
        std::max(0, std::min(nx * ny - 1, stc + static_cast<int>(round(dx)) + static_cast<int>(nx * round(dy))));
    ...
    if (stc < nx || stc > ns - nx) {  // would be out of bounds
      //   RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "[PathCalc] Out of
      //   bounds");
      return 0;
    }
    ...
    if (potarr[stc] >= POT_HIGH || potarr[stc + 1] >= POT_HIGH || potarr[stc - 1] >= POT_HIGH ||
      ...
      //   RCLCPP_DEBUG(
      //     rclcpp::get_logger("rclcpp"),
      //     "[Path] Pot fn boundary, following grid (%0.1f/%d)",
      //     potarr[stc], npath);
      ...
      //   RCLCPP_DEBUG(
      //     // rclcpp::get_logger("rclcpp"), "[Path] Pot: %0.1f  pos:
      //     %0.1f,%0.1f", potarr[stc], pathx[npath - 1], pathy[npath -
      //     1]);

      if (potarr[stc] >= POT_HIGH) {
        // RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "[PathCalc] No
        // path found, high potential"); savemap("navfn_highpot");
        return 0;
      }
      ...
      if (x == 0.0 && y == 0.0) {
        // RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "[PathCalc] Zero
        // gradient");
        return 0;
      }
    ...
    //      ROS_INFO("[Path] Pot: %0.1f  grad: %0.1f,%0.1f  pos:
    //      %0.1f,%0.1f\n", potarr[stc], x, y, pathx[npath-1],
    //      pathy[npath-1]);
  }

  //  return npath;  // out of cycles, return failure
  //   RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "[PathCalc] No path found,
  //   path too long");
