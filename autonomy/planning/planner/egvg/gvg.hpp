...
struct IntPoint {
  ...
  bool operator!=(const IntPoint& other) const { return !(*this == other); }
...
class SimpleTimer {
 public:
  SimpleTimer(const std::string& name = "") : name_(name) { start_ = std::chrono::steady_clock::now(); }
...
struct GraphNode {
  ...
  GraphNode(int x, int y, NODE_TYPE type_) : pos(x, y), type(type_), parent() { node_state = NOT_EXPAND_GVG; }
...
class NodeComparator0 {
 public:
  bool operator()(GraphNode::Ptr node1, GraphNode::Ptr node2) { return node1->f_score > node2->f_score; }
};
...
class NodeHashTable0 {
 ...
 public:
  ...
  bool contains(const IntPoint& idx) const { return data_2d_.find(idx) != data_2d_.end(); }
  void clear() { data_2d_.clear(); }
};
