...
void DynamicVoronoi::exchangeObstacles(std::vector<Point>& points) {
        ...
        for (int dy = -1; dy <= 1; ++dy) {
          ...
          if (nc.obstX != invalidObstData && !nc.needsRaise) {
            if (!isOccupied(nc.obstX, nc.obstY, data[nc.obstX][nc.obstY])) {
              Point p;
              p.x = static_cast<double>(nx);
              p.y = static_cast<double>(ny);
              p.z = 0.0;
              open.push(nc.sqdist, p);
      ...
      for (int dx = -1; dx <= 1; ++dx) {
        ...
        for (int dy = -1; dy <= 1; ++dy) {
          ...
          if (!nc.needsRaise) {
            ...
            if (overwrite) {
              Point np;
              np.x = static_cast<double>(nx);
              np.y = static_cast<double>(ny);
              np.z = 0.0;
              open.push(newSqDistance, np);
              nc.queueing = fwQueued;
              if (updateRealDist) {
...
float DynamicVoronoi::getDistanceNoBoundCheck(int x, int y) const { return data[x][y].dist; }
...
void DynamicVoronoi::commitAndColorize(bool updateRealDist) {
  ...
  for (unsigned int i = 0; i < addList.size(); ++i) {
    ...
    if (c.queueing != fwQueued) {
      ...
      c.voronoi = occupied;
      data[x][y] = c;
      open.push(0, Point{static_cast<double>(x), static_cast<double>(y), 0.0});
    }
  }

  // REMOVE OLD OBSTACLES
  for (unsigned int i = 0; i < removeList.size(); ++i) {
    const Point& p = removeList[i];
    int x = static_cast<int>(p.x);
    int y = static_cast<int>(p.y);
    dataCell c = data[x][y];
...
bool DynamicVoronoi::isOccupied(int& x, int& y, dataCell& c) { return (c.obstX == x && c.obstY == y); }
...
std::vector<Point> DynamicVoronoi::getAllPruneQueuePoints() {
  std::vector<Point> points;
  auto pruneQueue_tmp = pruneQueue;
  while (!pruneQueue_tmp.empty()) {
    const Point& p = pruneQueue_tmp.front();
    points.push_back(p);
    pruneQueue_tmp.pop();
  }
  return points;
...
std::vector<Point> DynamicVoronoi::getAllSortedPruneQueuePoints() {
  std::vector<Point> points;
  auto sortedPrio_tmp = sortedPruneQueue;
  while (!sortedPrio_tmp.empty()) {
    points.push_back(sortedPrio_tmp.pop());
  }
  return points;
}
  ...
  while (!pruneQueue.empty()) {
    ...
    if (x + 2 < sizeX && r.voronoi == occupied) {
      ...
      if (tr.voronoi != occupied && br.voronoi != occupied && data[x + 2][y].voronoi != occupied) {
        ...
        sortedPruneQueue.push(r.sqdist, Point{static_cast<double>(x + 1), static_cast<double>(y), 0.0});
        data[x + 1][y] = r;
      }
    }
    if (x - 2 >= 0 && l.voronoi == occupied) {
      ...
      if (tl.voronoi != occupied && bl.voronoi != occupied && data[x - 2][y].voronoi != occupied) {
        ...
        sortedPruneQueue.push(l.sqdist, Point{static_cast<double>(x - 1), static_cast<double>(y), 0.0});
        data[x - 1][y] = l;
      }
    }
    if (y + 2 < sizeY && t.voronoi == occupied) {
      ...
      if (tr.voronoi != occupied && tl.voronoi != occupied && data[x][y + 2].voronoi != occupied) {
        ...
        sortedPruneQueue.push(t.sqdist, Point{static_cast<double>(x), static_cast<double>(y + 1), 0.0});
        data[x][y + 1] = t;
      }
    }
    if (y - 2 >= 0 && b.voronoi == occupied) {
      ...
      if (br.voronoi != occupied && bl.voronoi != occupied && data[x][y - 2].voronoi != occupied) {
        ...
        sortedPruneQueue.push(b.sqdist, Point{static_cast<double>(x), static_cast<double>(y - 1), 0.0});
        data[x][y - 1] = b;
      }
    }
  }
  for (int i = 1; i < sizeX - 1; ++i)
    for (int j = 1; j < sizeY - 1; ++j)
      if (isVoronoi(i, j)) {
        ...
        sortedPruneQueue.push(data[i][j].sqdist, Point{static_cast<double>(i), static_cast<double>(j), 0.0});
      }

  // sortedPruneQueueBackup = sortedPruneQueueBackup;

  while (!sortedPruneQueue.empty()) {
    Point p = sortedPruneQueue.pop();
    int px = static_cast<int>(p.x);
    int py = static_cast<int>(p.y);
    dataCell c = data[px][py];
    int v = c.voronoi;
    if (v != freeQueued && v != voronoiRetry) {
      continue;
    }

    markerMatchResult r = markerMatch(px, py);
    if (r == pruned)
    ...
    if (sortedPruneQueue.empty()) {
      while (!pruneQueue.empty()) {
        ...
        int px = static_cast<int>(p.x);
        int py = static_cast<int>(p.y);
        sortedPruneQueue.push(data[px][py].sqdist, p);
      }
    }
  }
}
...
void DynamicVoronoi::updateAlternativePrunedDiagram() {
  ...
  std::queue<Point> end_cells;
  BucketPrioQueue<Point> sortedPruneQueue;
  // 从data（原版voronoi图）里面读取数据，并将节点压到队列里面
  for (int x = 1; x < sizeX - 1; ++x) {
    for (int y = 1; y < sizeY - 1; ++y) {
      dataCell& c = data[x][y];
      alternativeDiagram[x][y] = c.voronoi;
      if (c.voronoi <= free) {
        sortedPruneQueue.push(c.sqdist, Point{static_cast<double>(x), static_cast<double>(y), 0.0});
        end_cells.push(Point{static_cast<double>(x), static_cast<double>(y), 0.0});
      }
    }
  }
  ...
  for (int x = 1; x < sizeX - 1; ++x) {
    for (int y = 1; y < sizeY - 1; ++y) {
      ...
      if (getNumVoronoiNeighborsAlternative(x, y) >= 3) {
        ...
        sortedPruneQueue.push(data[x][y].sqdist, Point{static_cast<double>(x), static_cast<double>(y), 0.0});
        end_cells.push(Point{static_cast<double>(x), static_cast<double>(y), 0.0});
      }
    }
  }
  ...
  while (!sortedPruneQueue.empty()) {
    ...
    if (markerMatchAlternative(px, py)) {
      alternativeDiagram[px][py] = voronoiPrune;
    } else {
      if (alternativeDiagram[px][py] != voronoiVertex) {
        alternativeDiagram[px][py] = voronoiKeep;
      }
    }
  }
  ...
  while (!end_cells.empty()) {
    Point p = end_cells.front();
    end_cells.pop();
    int px = static_cast<int>(p.x);
    int py = static_cast<int>(p.y);

    if (isVoronoiAlternative(px, py) && getNumVoronoiNeighborsAlternative(px, py) == 1) {
      if (alternativeDiagram[px][py] == voronoiVertex) continue;
        ...
        for (int dy = -1; dy <= 1; ++dy) {
          ...
          if (isVoronoiAlternative(nx, ny)) {
            if (getNumVoronoiNeighborsAlternative(nx, ny) == 1) {
              end_cells.push(Point{static_cast<double>(nx), static_cast<double>(ny), 0.0});
            }
          }
        }
      }
...
bool DynamicVoronoi::markerMatchAlternative(int x, int y) {
...

  {
    ...
    if ((f[3] && f[4] && !f[1] && !f[6]) || (f[1] && f[6] && !f[3] && !f[4])) return false;
  }
  return true;
}
  ...
  for (int dx = -1; dx <= 1; ++dx) {
    for (int dy = -1; dy <= 1; ++dy) {
      ...
      if (alternativeDiagram[nx][ny] == free || alternativeDiagram[nx][ny] == voronoiKeep ||
          alternativeDiagram[nx][ny] == voronoiVertex) {
        ++count;
      }
    }
  }
