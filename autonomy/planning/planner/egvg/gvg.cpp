...
void GVG::createGraph(const DynamicVoronoi& voronoi) {
    ...
    if (neighbor_count == 4) {
      ...
      if (neighbor_week_count == 2) {
        if (voronoi.getDistance(strong_node_pt->pos.x, strong_node_pt->pos.y) < 2.0 * std::sqrt(cle_thr_sq_low_)) {
          ...
          strong_node_pt->type = GraphNode::Weak;
          // 将邻接的强节点加入队列
          for (int i = 0; i < static_cast<int>(neighbor_strong_idx.size()); ++i) {
          ...
          for (int i = 0; i < static_cast<int>(neighbor_strong_idx.size()); ++i) {
            ...
            IntPoint neighbor_parent =
                IntPoint(voronoi.getObstacleX(neighbor_strong_pt->pos.x, neighbor_strong_pt->pos.y),
                         voronoi.getObstacleY(neighbor_strong_pt->pos.x, neighbor_strong_pt->pos.y));
            IntPoint dir2 = neighbor_parent - strong_node_pt->pos;
            // 计算两个dir的夹角
            float angle_cos = (dir1.x * dir2.x + dir1.y * dir2.y) / (std::sqrt(dir1.x * dir1.x + dir1.y * dir1.y) *
                                                                     std::sqrt(dir2.x * dir2.x + dir2.y * dir2.y));
            if (angle_cos < -0.5) {  // 夹角大于120度
              // 将其标记为弱节点
              strong_node_pt->type = GraphNode::Weak;
              // 将邻接的强节点加入队列
              for (int j = 0; j < static_cast<int>(neighbor_strong_idx.size()); ++j) {
  ...
  for (int i = 0; i < num_clusters; ++i) {
    ...
    for (const auto& nb : center_node->neighbors) {
      auto nb_ptr = nb.lock();
      if (!nb_ptr) continue;
      if (nb_ptr->type != GraphNode::Strong) continue;
    ...
    center_neighbors.erase(std::unique(center_neighbors.begin(), center_neighbors.end()), center_neighbors.end());
    ...
    for (const auto& pt : cluster_points) {
      ...
      if (!node->neighbors.empty()) {
        AERROR << "Error: Non-center cluster node " << pt.x << "," << pt.y << " still has neighbors!";
      }
    }
  }
  ...
  for (int i = 0; i < static_cast<int>(graphs_.size()); ++i) {
    ...
    for (auto& node : graph) {
      ...
      if (node_ptr->type == GraphNode::Strong) voronoi_new[y * sizeX_ + x] = true;
      if (node_ptr->type == GraphNode::Weak && node_ptr->neighbor_paths[0].path_length > 50) {
...
std::vector<std::unordered_map<IntPoint, GraphNode::Ptr>>& GVG::getGraphs() { return graphs_; }

int GVG::getGraphsSize() { return static_cast<int>(graphs_.size()); }

void GVG::set_use_EGVG(bool use_egvg) { use_EGVG_ = use_egvg; }
...
bool GVG::isVoronoi(int x, int y) { return voronoi_new[y * sizeX_ + x]; }

bool GVG::isVoronoi(IntPoint pt) { return voronoi_new[pt.y * sizeX_ + pt.x]; }
...
void GVG::setClearanceThresholdSq(float threshold_low, float threshold_high) {
  ...
  cle_thr_sq_high2_ = (std::sqrt(threshold_high) + 1.0f) * (std::sqrt(threshold_high) + 1.0f);
}

const std::vector<IntPoint>& GVG::getTruncatedPoints() const { return truncated_points_; }

const std::vector<IntPoint>& GVG::getCompleteConnectionPoints() const { return completeCoonction_points_; }
...
void GVG::insertNodeToGraph(const GraphNode::Ptr& node, int graph_id) {
  ...
  if (findNodeInGraphs(node->pos, graph_id)) {
    AERROR << "insertNodeToGraph: node already exists in graph " << graph_id;
    return;
  }
...
std::unordered_map<IntPoint, GraphNode::Ptr> GVG::DFSSearch(const IntPoint& start) {
  ...
  while (!stack.empty()) {
    ...
    if (local_graph.find(current) != local_graph.end()) {
      ...
      // 创建新的节点
      current_node =
          std::make_shared<GraphNode>(current.x, current.y, current_type == 1 ? GraphNode::Strong : GraphNode::Weak);
...
void GVG::DFSSearch2(GraphNode::Ptr& center_node, const std::vector<IntPoint>& cluster_points) {
  ...
  while (!stack.empty()) {
    ...
    for (size_t i = 0; i < cur_node->neighbors.size(); ++i) {
      ...
      new_path.insert(new_path.end(), cur_node->neighbor_paths[i].path.begin(), cur_node->neighbor_paths[i].path.end());
...
void GVG::completeConnection(int x, int y, const DynamicVoronoi& voronoi) {
  ...
  for (int k = 0; k < 8; ++k) {
    ...
    if (isVoronoi) {
      ++voroCount;
      // 四邻域：k==1,3,4,6
      if (k == 1 || k == 3 || k == 4 || k == 6) ++voroCountFour;
    }
    if (isVoronoi && (k == 1 || k == 3 || k == 4 || k == 6)) ++count;
  }
  (void)nx;
  (void)ny;
  (void)dx;
...
void GVG::tryCompleteVoronoi(int x, int y, int idx1, int idx2, const DynamicVoronoi& voronoi) {
...
bool GVG::voronoiAt(int x, int y) const { return voronoi_new[y * sizeX_ + x]; }

bool GVG::voronoiAt(const IntPoint& pt) const { return voronoi_new[pt.y * sizeX_ + pt.x]; }
