...
struct TrajectoryPoint {
  ...
  PathPoint& path_point() { return path_point_; }
};
...
class DiscretizedTrajectory {
 public:
  ...
  size_t NumOfPoints() const { return trajectory_points_.size(); }

  // Get trajectory point at index
  const TrajectoryPoint& TrajectoryPointAt(uint32_t index) const { return trajectory_points_.at(index); }

  // Add trajectory point
  void AppendTrajectoryPoint(const TrajectoryPoint& point) { trajectory_points_.push_back(point); }
