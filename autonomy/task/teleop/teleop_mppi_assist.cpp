...
bool TeleopMppiAssist::Configure(
    ...
    if (transform_buffer_) {
        transform::SeedBenchmarkTfTree(transform_buffer_.get(), "teleop_mppi_assist");
    }
...
bool TeleopMppiAssist::Tick(double joy_linear_x, double joy_angular_z,
    ...
    if (!options_.enabled) {
        FillPassthroughCmd(joy_linear_x, joy_angular_z, command_out);
        return true;
    }
    if (!IsPerceptionOk()) {
        FillPassthroughCmd(0.0, 0.0, command_out);
        return false;
    }
    if (!mppi_ || !costmap_) {
        FillPassthroughCmd(0.0, 0.0, command_out);
