...
{
    BlackboardClientStore<MappingClient>::SetShared(client);
    if (client) {
...
{
    return BlackboardClientStore<MappingClient>::FromBlackboard(blackboard, kMappingClientBlackboardKey);
}

MappingClient::Ptr MappingClient::FromNode(const BT::TreeNode& node)
{
    return BlackboardClientStore<MappingClient>::FromNode(node, kMappingClientBlackboardKey);
}
