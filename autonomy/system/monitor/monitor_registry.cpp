...
MonitorRegistry::MonitorRegistry(MonitorOptions options)
...
void MonitorRegistry::Start() {
    ...
    if (options_.enable_prometheus &&
        !options_.prometheus_bind_address.empty()) {
        prometheus_ = std::make_unique<PrometheusState>();
        prometheus_->registry = std::make_shared<prometheus::Registry>();
        prometheus_->exposer = std::make_unique<prometheus::Exposer>(
            options_.prometheus_bind_address);
...
void MonitorRegistry::StartGperfIfRequested() {
    ...
    if (options_.enable_heap_profile &&
        !options_.heap_profile_filename.empty() &&
