...
bool ParseCpuLine(const std::string& line, CpuTickCounts* out) {
  ...
  iss >> cpu_label;
  if (cpu_label.empty() || cpu_label.find("cpu") != 0) return false;
  uint64_t u, n, s, i, iow, irq, soft, steal, guest, gnice;
  if (!(iss >> u >> n >> s >> i >> iow >> irq >> soft >> steal >> guest >> gnice)) return false;
  out->user = u;
  out->nice = n;
  out->system = s;
  out->idle = i;
...
bool CpuMonitorBase::ReadProcStat(std::vector<CpuTickCounts>* out) {
  ...
  while (std::getline(f, line)) {
    ...
    if (ParseCpuLine(line, &counts)) out->push_back(counts);
  }
  return !out->empty();
}
...
void CpuMonitorBase::Collect() {
  ...
  if (info_.num_cores == 0 && !curr.empty()) info_.num_cores = static_cast<uint32_t>(curr.size());
  prev_ticks_ = std::move(curr);

#if defined(USE_PROMETHEUS) && USE_PROMETHEUS
  if (gauges_) {
    if (gauges_->total_usage) gauges_->total_usage->Set(usage_.total_usage_percent);
...
std::unique_ptr<CpuMonitorBase> CreateCpuMonitor() { return std::make_unique<CpuMonitorBase>(); }
