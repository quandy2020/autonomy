...
/**
 * @brief Top-level autonomy process: map / planner / controller / tasks::Task.
 *
 * Task owns behavior-tree navigation (BtNavigator). AutonomyNode wires servers,
 * applies runtime overrides, and exposes navigation helpers for ROS bridges.
 */
class AutonomyNode
{
public:
    ...
    explicit AutonomyNode(proto::AutonomyOptions options);
    ~AutonomyNode();

    AutonomyNode(const AutonomyNode&) = delete;
    AutonomyNode& operator=(const AutonomyNode&) = delete;

    /** Construct planner / controller / map / TF; does not start Task yet. */
    void Start();

    /** Apply runtime IDs/frames and configure tasks::Task (BT navigators). */
...
/** Factory used by main and autonomy_ros. */
