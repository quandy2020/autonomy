...
bool Time::operator==(const Time& rhs) const { return this->nanosec == rhs.nanosec; }
...
uint32 Time::Nanoseconds() const { return nanosec; }

Time Time::Min() { return Time{std::numeric_limits<int32_t>::min(), 0}; }

Time Time::Max() { return Time(std::numeric_limits<int32_t>::max(), 999999999); }

double Time::Seconds() const { return sec; }
...
Time FromProto(const proto::builtin_interfaces::Time& proto) { return {proto.sec(), proto.nanosec()}; }
...
Duration::Duration(std::chrono::nanoseconds nanoseconds) { duration_ = nanoseconds.count(); }

Duration& Duration::operator=(const Duration& rhs) = default;

bool Duration::operator==(const Duration& rhs) const { return duration_ == duration_; }

bool Duration::operator!=(const Duration& rhs) const { return duration_ != rhs.duration_; }
...
int64_t Duration::Nanoseconds() const { return duration_; }

Duration Duration::Max() { return Duration(std::numeric_limits<int32_t>::max(), 999999999); }

double Duration::Seconds() const { return std::chrono::duration<double>(std::chrono::nanoseconds(duration_)).count(); }

Duration Duration::FromSeconds(double seconds) {
  Duration ret;
  ret.duration_ = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::duration<double>(seconds)).count();
  return ret;
}
