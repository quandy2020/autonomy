...
struct Duration {
     ...
     * Large values for nsecs are wrapped automatically with the remainder added
     * to secs. Both inputs must be integers. Seconds can be negative.
     *
    ...
    /**
     * \warning Depending on sizeof(double) there could be significant precision
     * loss. When an exact time is required use nanoseconds() instead.
     * \return the duration in seconds as a floating point number.
     */
    double Seconds() const;

    /// Create a duration object from a floating point number representing
    /// seconds
