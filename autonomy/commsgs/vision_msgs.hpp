...
// XY matches the sensor_msgs/Image convention: X is positive right and Y is
// positive down.
struct Point2D {
  double x;
  double y;
...
// Represents a 2D pose (coordinates and a radian rotation). Rotation is
...
struct BoundingBox3D {
  ...
  // The total size of the bounding box, in meters, surrounding the object's
  // center
  //   pose.
...
struct ObjectHypothesis {
  ...
  //   this ID, such as its human-readable class name, listeners should
  //   perform a lookup in a metadata database. See vision_msgs/VisionInfo.msg
  //   for more detail.
  std::string class_id;

  // The probability or confidence value of the detected object. By
  // convention,
...
struct Classification {
  ...
  // A list of class probabilities. This list need not provide a probability
  // for
  //   every possible class, just ones that are nonzero, or above some
  //   user-defined threshold.
  std::vector<ObjectHypothesis> results;

  // Source data that generated this classification are not a part of the
  // struct. If you need to access them, use an exact or approximate time
  // synchronizer in your code, as this struct's header should match the
  // header of the source data.
};
...
struct ObjectHypothesisWithPose {
  ...
  //   defined as the pose of some fixed reference point on the object, such
  //   as the geometric center of the bounding box, the center of mass of the
  //   object or the origin of a reference mesh of the object.
  // Note that this pose is not stamped; frame information can be defined by
  //   parent structs.
  // Also note that different classes predicted for the same input data may
  // have
...
struct Detection2D {
  ...
  // of the same object in different detection structs should have the same
  // id. This field may be empty.
  std::string id;
  ...
  // in your code, as this struct's header should match the header of the
  // source data.
};
...
struct Detection2DArray {
  ...
  // A list of the detected proposals. A multi-proposal detector might
  // generate
...
//   return id based classifications back to human readable std::string class
//   names.
struct LabelInfo {
  // Used for sequencing
  std_msgs::Header header;
  ...
  //   maxiumum class id allowed (65535 for uint16 and 255 for uint8) belongs
  //   to the "UNLABELED" class.
...
struct VisionInfo {
  ...
  // Name of the vision pipeline. This should be a value that is meaningful to
  // an
  //   outside user.
  std::string method;

  // Location where the metadata database is stored. The recommended location
  // is
  //   as an XML std::string on the ROS parameter server, but the exact
  //   implementation and information is left up to the user.
  ...
  //   elements could be classes in a pixel segmentation algorithm, object
  //   classes in a detector, different people's faces in a face detection
  //   algorithm, etc. Vision pipelines report results in terms of numeric
  //   IDs, which map into this  database.
  // The information stored in this database is, again, left up to the user.
  // The
  ...
  //   each time the pipeline begins using a new version of the database
  //   (useful in the case of online training or user modifications). The
  //   counter value can be monitored by listeners to ensure that the pipeline
