 ...
 *
 * Extended robot model / state / trajectory types.
 */
...
/** @brief Named joint positions (and optional velocities). */
struct JointState {
...
/** @brief Timed joint-space path as waypoints plus `time_from_start`. */
struct RobotTrajectory {
  std::vector<JointState> waypoints;
  std::vector<double> time_from_start;  // s
};
 ...
 *
 * Provides joint lists, planning groups, and per-joint limits.
 */
class RobotModel {
 public:
 ...
 *
 * Implementations may also support interpolation between states.
 */
class RobotState {
 public:
...
/**
 * @brief Trajectory flattened for messaging: shared joint names + time series.
 */
...
/**
 * @brief Convert a waypoint trajectory to flat joint-name / series form.
 * @param[in] traj Source trajectory.
 * @return Flattened representation suitable for messaging.
 */
FlatTrajectory ToFlat(const RobotTrajectory& traj);

/**
 * @brief Rebuild a waypoint trajectory from flat form.
 * @param[in] flat Flattened trajectory.
 * @return Waypoint + time_from_start trajectory.
 */
