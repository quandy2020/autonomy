...
/**
 * @brief Number of position DOF in a joint state.
 * @param[in] state sensor_msgs/JointState.
 * @return Size of the position field.
 */
inline int GetJointStateDof(const JointState& state) {
 ...
 */
inline void SetJointState(JointState* state, const std::vector<std::string>& names,
 ...
 */
inline void ResizeJointState(JointState* state, int dof) {
  if (!state || dof < 0) {
    return;
  }
 ...
 */
inline void CopyTrajectoryJointNames(const JointTrajectory& traj,
                                     JointState* state) {
  if (!state) {
    return;
  }
 ...
 */
inline JointState MakeJointStateFromPoint(const JointTrajectory& traj,
                                          int point_index) {
  JointState state;
 ...
 */
inline JointTrajectoryPoint* AddTrajectoryPoint(JointTrajectory* traj,
...
/**
 * @brief time_from_start of trajectory point @p i in seconds.
 * @param[in] traj Source trajectory.
 * @param[in] i Point index.
 * @return Seconds, or 0 if @p i is out of range.
 */
inline double GetTrajectoryTime(const JointTrajectory& traj, int i) {
  if (i < 0 || i >= traj.points_size()) {
    return 0.0;
  }
  return GetDurationSeconds(traj.points(i).time_from_start());
}
 ...
 */
inline void SetPose(Pose* pose, double x, double y, double z, double qx,
 ...
 */
inline Pose MakeIdentityPose() {
  Pose pose;
  SetPose(&pose, 0, 0, 0, 0, 0, 0, 1);
  return pose;
}
