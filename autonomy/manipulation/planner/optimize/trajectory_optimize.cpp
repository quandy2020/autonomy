...
bool EstimateTip(const MotionPlanRequest& request, const core::JointState& state,
 ...
 * (or EE proxy when no LinkFkTree).
 */
...
std::vector<double> ObstacleGradientJacobian(const MotionPlanRequest& request,
  ...
  if (request.link_tree) {
    std::unordered_map<std::string, core::Transform> poses0;
    ...
    for (const auto& kv : poses0) {
      add_point(kv.second.x, kv.second.y, kv.second.z,
                [&](std::size_t j, double* jx, double* jy, double* jz) {
                  core::JointState plus = state;
                  core::JointState minus = state;
                  ...
                  std::unordered_map<std::string, core::Transform> pp, pm;
...
double CollisionCost(const MotionPlanRequest& request,
  ...
  for (int i = 0; i + 1 < traj.points_size(); ++i) {
    core::JointState mid = MakeJointStateFromPoint(traj, i);
...
core::RobotTrajectory InterpolateSeedTrajectory(const MotionPlanRequest& request,
                                      int waypoints,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < dof; ++j) {
      const double a = request.start_state.position(j);
      const double b = request.goal_state.position(j);
...
void CopyStatePositionsToPoint(const core::JointState& state,
...
core::RobotTrajectory ChompOptimize(const MotionPlanRequest& request,
  ...
  for (int iter = 0; iter < params.max_iterations; ++iter) {
    ...
    CopyStatePositionsToPoint(request.goal_state,
                              traj.mutable_points(traj.points_size() - 1));
    CopyStatePositionsToPoint(request.start_state, traj.mutable_points(0));
    ClampTrajectory(request, &traj);
...
core::RobotTrajectory StompOptimize(const MotionPlanRequest& request,
  ...
  for (int it = 0; it < params.num_iterations; ++it) {
    ...
    CopyStatePositionsToPoint(request.start_state, best.mutable_points(0));
    CopyStatePositionsToPoint(request.goal_state,
