...
KDL::Frame PoseToFrame(const Pose& pose) {
  return KDL::Frame(
      KDL::Rotation::Quaternion(pose.qx, pose.qy, pose.qz, pose.qw),
      KDL::Vector(pose.x, pose.y, pose.z));
}
...
bool KdlKinematics::MapJoints(const core::JointState& joints,
  ...
  q->resize(n);
  if (joints.positions.size() == n &&
      (joints.names.empty() || joints.names.size() == n)) {
    if (!joints.names.empty()) {
      std::unordered_map<std::string, double> by_name;
      for (std::size_t i = 0; i < joints.names.size(); ++i) {
        by_name[joints.names[i]] = joints.positions[i];
      }
    ...
    for (unsigned int i = 0; i < n; ++i) {
      (*q)(i) = joints.positions[i];
    }
    return true;
  }

  if (joints.names.size() != joints.positions.size()) {
    return false;
  }
  std::unordered_map<std::string, double> by_name;
  for (std::size_t i = 0; i < joints.names.size(); ++i) {
...
ErrorCode KdlKinematics::GetPositionIK(const Pose& tip_pose,
  ...
  for (int attempt = 0; attempt < attempts; ++attempt) {
    ...
    if (SolveOnce(tip_pose, seed_try, options.position_only, &q_out)) {
      solution->names = model_.joint_names;
      solution->positions.resize(model_.joint_names.size());
      solution->velocities.clear();
      for (std::size_t i = 0; i < model_.joint_names.size(); ++i) {
        solution->positions[i] = q_out(i);
      }
