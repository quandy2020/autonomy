...
automsgs::msgs::geometry_msgs::Pose PerturbFk(common::KinematicsInterface* kin, const automsgs::msgs::sensor_msgs::JointState& q,
               int j, double eps) {
  automsgs::msgs::sensor_msgs::JointState qp = q;
...
automsgs::msgs::geometry_msgs::Twist PoseErrorTwist(const automsgs::msgs::geometry_msgs::Pose& cur, const automsgs::msgs::geometry_msgs::Pose& goal, double kp_linear,
                     double kp_angular) {
  automsgs::msgs::geometry_msgs::Twist t;
...
void AssignZeroVelocities(automsgs::msgs::sensor_msgs::JointState* state, int n) {
  if (!state) {
    return;
  }
...
void DampedLeastSquaresCartesianServo::SetKinematics(
    common::KinematicsInterface::SharedPtr kin) {
  std::lock_guard<std::mutex> lock(mutex_);
  kinematics_ = std::move(kin);
}

void DampedLeastSquaresCartesianServo::SetScene(
    std::shared_ptr<scene::PlanningScene> scene) {
  std::lock_guard<std::mutex> lock(mutex_);
  scene_ = std::move(scene);
}

void DampedLeastSquaresCartesianServo::SetModel(
    std::shared_ptr<const model::SimpleRobotModel> model) {
  std::lock_guard<std::mutex> lock(mutex_);
  model_ = std::move(model);
}

void DampedLeastSquaresCartesianServo::SetState(const automsgs::msgs::sensor_msgs::JointState& state) {
  std::lock_guard<std::mutex> lock(mutex_);
  state_ = state;
}

bool DampedLeastSquaresCartesianServo::Init() {
  std::lock_guard<std::mutex> lock(mutex_);
  halted_ = false;
  status_ = CartesianServoStatus::kOk;
  return kinematics_ != nullptr && GetJointStateDegreesOfFreedom(state_) > 0;
}

bool DampedLeastSquaresCartesianServo::NumericJacobian(
    const automsgs::msgs::sensor_msgs::JointState& q, std::vector<std::vector<double>>* J) const {
  if (!kinematics_ || !J || GetJointStateDegreesOfFreedom(q) <= 0) {
    return false;
  }
  automsgs::msgs::geometry_msgs::Pose p0;
  if (!kinematics_->GetPositionFK(q, &p0)) {
    return false;
  }
  const double eps = 1e-4;
  const int n = GetJointStateDegreesOfFreedom(q);
...
double DampedLeastSquaresCartesianServo::ComputeManipulability(
...
automsgs::msgs::geometry_msgs::Twist DampedLeastSquaresCartesianServo::FilterTwist(const automsgs::msgs::geometry_msgs::Twist& twist) {
...
double DampedLeastSquaresCartesianServo::CollisionApproachScale(
    const automsgs::msgs::sensor_msgs::JointState& q) const {
  if (!scene_) {
    return 1.0;
  }
...
bool DampedLeastSquaresCartesianServo::CommitState(const automsgs::msgs::sensor_msgs::JointState& next,
                                          automsgs::msgs::sensor_msgs::JointState* joint_command) {
  if (model_) {
    ...
    for (int j = 0; j < n; ++j) {
      ...
      if (next.position(j) < lim->min_position() ||
        ...
        status_ = CartesianServoStatus::kJointLimit;
  ...
  automsgs::msgs::sensor_msgs::JointState candidate = next;
  ...
  if (scene_) {
    if (scene_->CheckCollision(candidate)) {
      ...
      double lo = 0.0;
      double hi = 1.0;
      automsgs::msgs::sensor_msgs::JointState best = state_;
      const int n = GetJointStateDegreesOfFreedom(candidate);
      for (int it = 0; it < 8; ++it) {
        const double mid = 0.5 * (lo + hi);
        automsgs::msgs::sensor_msgs::JointState trial = state_;
        if (GetJointStateDegreesOfFreedom(trial) != n) {
          break;
        }
      ...
      if (lo < 1e-3) {
        status_ = CartesianServoStatus::kCollision;
        *joint_command = state_;
        return false;
      }
      candidate = best;
  ...
  state_ = candidate;
  status_ = decelerated ? CartesianServoStatus::kDecelerateForCollision
                        : CartesianServoStatus::kOk;
  *joint_command = state_;
  return true;
}

bool DampedLeastSquaresCartesianServo::Update(const automsgs::msgs::geometry_msgs::Twist& twist,
                                     automsgs::msgs::sensor_msgs::JointState* joint_command) {
  if (!joint_command) {
    return false;
  }
  std::lock_guard<std::mutex> lock(mutex_);
  if (!kinematics_ || GetJointStateDegreesOfFreedom(state_) <= 0 || halted_) {
    status_ = halted_ ? CartesianServoStatus::kHalted : CartesianServoStatus::kOk;
    *joint_command = state_;
    return !halted_;
  }

  const automsgs::msgs::geometry_msgs::Twist cmd = FilterTwist(twist);
  std::vector<std::vector<double>> J;
  if (!NumericJacobian(state_, &J)) {
    status_ = CartesianServoStatus::kHalted;
    return false;
  }
  ...
  if (manip < singularity_threshold_) {
    status_ = CartesianServoStatus::kSingularity;
    *joint_command = state_;
    AWARN << "CartesianServo singularity halt manip=" << manip;
    return false;
  }
  ...
  const int n = GetJointStateDegreesOfFreedom(state_);
  ...
  automsgs::msgs::sensor_msgs::JointState next = state_;
  ...
  if (ok && cscale < 1.0 && status_ == CartesianServoStatus::kOk) {
    status_ = CartesianServoStatus::kDecelerateForCollision;
  }
  return ok;
}

bool DampedLeastSquaresCartesianServo::UpdateJointJog(const automsgs::msgs::control_msgs::JointJog& jog,
                                             automsgs::msgs::sensor_msgs::JointState* joint_command) {
  if (!joint_command) {
    return false;
  }
  std::lock_guard<std::mutex> lock(mutex_);
  if (GetJointStateDegreesOfFreedom(state_) <= 0 || halted_) {
    status_ = halted_ ? CartesianServoStatus::kHalted : CartesianServoStatus::kOk;
    *joint_command = state_;
    return !halted_;
  }
  ...
  automsgs::msgs::sensor_msgs::JointState next = state_;
  const int n = GetJointStateDegreesOfFreedom(next);
  AssignZeroVelocities(&next, n);
  const double cscale = CollisionApproachScale(state_);
  if (cscale <= 0.0) {
    status_ = CartesianServoStatus::kCollision;
...
bool DampedLeastSquaresCartesianServo::UpdatePose(const automsgs::msgs::geometry_msgs::PoseStamped& cmd,
                                         automsgs::msgs::sensor_msgs::JointState* joint_command) {
  if (!joint_command) {
    return false;
  }
  automsgs::msgs::geometry_msgs::Pose cur;
  double kp_l = 1.0;
  double kp_a = 1.0;
  {
    std::lock_guard<std::mutex> lock(mutex_);
...
void DampedLeastSquaresCartesianServo::Stop() {
  std::lock_guard<std::mutex> lock(mutex_);
  halted_ = true;
  status_ = CartesianServoStatus::kHalted;
...
void CartesianServoNode::SetServo(std::shared_ptr<CartesianServo> servo) {
  std::lock_guard<std::mutex> lock(mutex_);
  servo_ = std::move(servo);
}

void CartesianServoNode::SetCommandType(CommandType type) {
  std::lock_guard<std::mutex> lock(mutex_);
  command_type_ = type;
}

void CartesianServoNode::SetCommand(const automsgs::msgs::geometry_msgs::Twist& twist) {
  std::lock_guard<std::mutex> lock(mutex_);
  twist_ = twist;
  command_type_ = CommandType::kTwist;
}

void CartesianServoNode::SetJointJog(const automsgs::msgs::control_msgs::JointJog& jog) {
  std::lock_guard<std::mutex> lock(mutex_);
  joint_jog_ = jog;
  command_type_ = CommandType::kJointJog;
}

void CartesianServoNode::SetPose(const automsgs::msgs::geometry_msgs::PoseStamped& pose) {
  std::lock_guard<std::mutex> lock(mutex_);
  pose_ = pose;
  command_type_ = CommandType::kPose;
}

void CartesianServoNode::SetCommandPublisher(CommandPublisher pub) {
  std::lock_guard<std::mutex> lock(mutex_);
  publisher_ = std::move(pub);
}

void CartesianServoNode::SetStatusPublisher(StatusPublisher pub) {
  std::lock_guard<std::mutex> lock(mutex_);
  status_publisher_ = std::move(pub);
}

void CartesianServoNode::Start() {
...
void CartesianServoNode::Loop() {
  while (running_.load()) {
    ...
    std::shared_ptr<CartesianServo> servo;
    ...
    if (servo) {
      ...
      if (status_publisher) {
        status_publisher(servo->GetStatus());
      }
    }
