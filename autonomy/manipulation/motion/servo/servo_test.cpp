...
class JointWallDetector : public collision::CollisionDetector {
 public:
  explicit JointWallDetector(double limit) : limit_(limit) {}

  bool Init(const std::string& /*id*/) override { return true; }

  collision::CollisionResult CheckRobotWorld(
      const JointState& state,
      const scene::PlanningScene& /*scene*/) const override {
    collision::CollisionResult r;
    if (state.position_size() > 0 && state.position(0) >= limit_) {
      r.collision = true;
      r.contact_body_a = "ee";
      r.contact_body_b = "wall";
    }
    return r;
  }

  collision::CollisionResult CheckRobotSelf(
      const JointState& /*state*/,
      const scene::PlanningScene* /*scene*/) const override {
    return {};
  }

  collision::DistanceResult DistanceRobotWorld(
      const JointState& state,
      const scene::PlanningScene& /*scene*/) const override {
    collision::DistanceResult d;
    const double q0 =
        state.position_size() > 0 ? state.position(0) : 0.0;
    d.distance = limit_ - q0;
    d.collision = d.distance <= 0.0;
    if (d.collision) {
      d.distance = 0.0;
    }
    return d;
...
JointState MakeTwoDofState(double q0, double q1) {
  JointState q;
  q.add_name("j1");
  q.add_name("j2");
  q.add_position(q0);
...
TEST(ServoTest, CollisionProximityDeceleratesJointJog) {
  ...
  JointState out;
  ASSERT_TRUE(servo.UpdateJointJog(jog, &out));
  EXPECT_EQ(servo.status(), ServoStatus::kDecelerateForCollision);
...
TEST(ServoTest, CollisionHaltNearWall) {
  ...
  EXPECT_EQ(servo.status(), ServoStatus::kCollision);
