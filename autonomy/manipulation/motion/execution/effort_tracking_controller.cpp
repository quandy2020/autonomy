...
std::vector<double> CorrectEffortCommand(
  ...
  for (std::size_t i = 0; i < n; ++i) {
    cmd[i] = desired[i] + kp * (desired[i] - actual[i]);
  }
...
void EffortTrackingController::SetDesired(const core::JointState& desired) {
  std::lock_guard<std::mutex> lock(mutex_);
...
bool EffortTrackingController::Publish() {
  ...
  const std::vector<double> cmd =
      CorrectEffortCommand(desired, actual, kp, max_abs);
  if (cmd.empty()) {
    return false;
  }
  publisher(cmd);
