...
inline void ComposeQuaternion(double aw, double ax, double ay, double az, double bw,
...
inline void RotateVector(const automsgs::msgs::geometry_msgs::Pose& t, double x,
                      double y, double z, double* ox, double* oy, double* oz) {
  const double qw = t.orientation().w();
  const double qx = t.orientation().x();
  const double qy = t.orientation().y();
  const double qz = t.orientation().z();
 ...
 */
inline automsgs::msgs::geometry_msgs::Pose GetObjectPose(const automsgs::msgs::moveit_msgs::CollisionObject& object) {
...
inline void SetObjectPose(automsgs::msgs::moveit_msgs::CollisionObject* object, const automsgs::msgs::geometry_msgs::Pose& pose) {
  if (!object) {
    return;
  }
  if (object->primitive_poses_size() == 0) {
...
inline bool HasPrimitive(const automsgs::msgs::moveit_msgs::CollisionObject& object) {
  return object.primitives_size() > 0;
}

/** @brief Whether the object carries at least one mesh. */
inline bool HasMesh(const automsgs::msgs::moveit_msgs::CollisionObject& object) {
 ...
 */
inline bool GetPrimitiveSizes(const automsgs::msgs::moveit_msgs::CollisionObject& object, double* size_x,
...
/** @brief Build a box automsgs::msgs::moveit_msgs::CollisionObject at (@p x,@p y,@p z) with extents. */
inline automsgs::msgs::moveit_msgs::CollisionObject MakeBoxObject(const std::string& id, double x, double y,
                                     double z, double sx, double sy, double sz) {
  automsgs::msgs::moveit_msgs::CollisionObject obj;
  obj.set_id(id);
  obj.set_operation(automsgs::msgs::moveit_msgs::CollisionObject::ADD);
...
/** @brief Build a sphere automsgs::msgs::moveit_msgs::CollisionObject (radius @p r). */
inline automsgs::msgs::moveit_msgs::CollisionObject MakeSphereObject(const std::string& id, double x, double y,
                                        double z, double r) {
  automsgs::msgs::moveit_msgs::CollisionObject obj;
  obj.set_id(id);
  obj.set_operation(automsgs::msgs::moveit_msgs::CollisionObject::ADD);
...
inline automsgs::msgs::moveit_msgs::CollisionObject MakeCylinderObject(const std::string& id, double x,
 ...
 */
inline void UpdateMeshAabb(automsgs::msgs::moveit_msgs::CollisionObject* object) {
...
/**
 * @brief automsgs::msgs::geometry_msgs::Pose @p object from link frame into world using @p link_tf.
 * Transforms all primitive_poses / mesh_poses / pose.
 */
inline automsgs::msgs::moveit_msgs::CollisionObject TransformAttached(const automsgs::msgs::moveit_msgs::CollisionObject& object,
                                         const automsgs::msgs::geometry_msgs::Pose& link_tf) {
  automsgs::msgs::moveit_msgs::CollisionObject out = object;
  auto transform_pose = [&](automsgs::msgs::geometry_msgs::Pose* pose) {
    if (!pose) {
      return;
    }
    double wx = 0.0;
    double wy = 0.0;
    double wz = 0.0;
    detail::RotateVector(link_tf, pose->position().x(), pose->position().y(),
                      pose->position().z(), &wx, &wy, &wz);
    pose->mutable_position()->set_x(link_tf.position().x() + wx);
    pose->mutable_position()->set_y(link_tf.position().y() + wy);
    pose->mutable_position()->set_z(link_tf.position().z() + wz);
    double ow = 0.0;
    double ox = 0.0;
    double oy = 0.0;
    double oz = 0.0;
    detail::ComposeQuaternion(link_tf.orientation().w(), link_tf.orientation().x(),
                        link_tf.orientation().y(), link_tf.orientation().z(),
...
inline void TranslateObject(automsgs::msgs::moveit_msgs::CollisionObject* object, double dx, double dy,
                            double dz) {
  if (!object) {
    return;
  }
  auto shift = [&](automsgs::msgs::geometry_msgs::Pose* pose) {
    if (!pose) {
      return;
    }
