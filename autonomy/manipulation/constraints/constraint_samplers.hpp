 ...
 */
inline bool SatisfiesJointConstraints(const planner::MotionPlanRequest& req,
                                      const automsgs::msgs::sensor_msgs::JointState& state) {
  for (const auto& c : req.pb.joint_constraints()) {
    bool found = false;
 ...
 */
inline bool SatisfiesPositionConstraint(const automsgs::msgs::moveit_msgs::PositionConstraint& c,
...
inline bool SatisfiesOrientationConstraint(
    const automsgs::msgs::moveit_msgs::OrientationConstraint& c, const automsgs::msgs::geometry_msgs::Pose& pose) {
 ...
 */
inline bool SatisfiesCartesianConstraints(const planner::MotionPlanRequest& req,
                                          const automsgs::msgs::sensor_msgs::JointState& state) {
  if ((req.pb.position_constraints_size() == 0) && (req.pb.orientation_constraints_size() == 0)) {
    return true;
  }
  if (!req.kinematics) {
    return false;
 ...
 */
inline bool SatisfiesPathConstraints(const planner::MotionPlanRequest& req,
                                     const automsgs::msgs::trajectory_msgs::JointTrajectory& traj) {
  for (int i = 0; i < traj.points_size(); ++i) {
    const automsgs::msgs::sensor_msgs::JointState wp = MakeJointStateFromPoint(traj, i);
 ...
 */
inline bool ClampToJointConstraints(const planner::MotionPlanRequest& req,
...
inline bool ProjectOntoCartesianConstraints(
    const planner::MotionPlanRequest& req, automsgs::msgs::sensor_msgs::JointState* state) {
  ...
  automsgs::msgs::geometry_msgs::Pose target = tip;
  bool need_ik = false;
  ...
  opts.set_timeout(0.05);
  opts.set_max_attempts(8);
  opts.set_position_only((req.pb.orientation_constraints_size() == 0));
 ...
 */
inline bool SampleJointConstrainedState(const planner::MotionPlanRequest& req,
                                        automsgs::msgs::sensor_msgs::JointState* state,
                                        std::mt19937* rng,
 ...
 */
inline bool SampleConstrainedTipPose(const planner::MotionPlanRequest& req,
                                     automsgs::msgs::geometry_msgs::Pose* pose, std::mt19937* rng) {
  if (!pose || !rng) {
    return false;
  }
  *pose = MakeIdentityPose();
  ...
  } else if ((req.pb.position_constraints_size() == 0)) {
    return false;
  }
  return true;
}
 ...
 */
inline bool SampleIkConstrainedState(const planner::MotionPlanRequest& req,
 ...
 * @return ErrorCode::SUCCESS or a constraint-violation code.
 */
inline ErrorCode EvaluateRequestConstraints(
    const planner::MotionPlanRequest& req, std::string* error) {
  if (!SatisfiesJointConstraints(req, req.pb.start_state())) {
    if (error) {
      *error = "start state violates joint constraints";
    }
    return ErrorCode::START_STATE_VIOLATES_PATH_CONSTRAINTS;
  }
  if (req.pb.goal_state().position_size() > 0 &&
      !SatisfiesJointConstraints(req, req.pb.goal_state())) {
    if (error) {
      *error = "goal state violates joint constraints";
    }
    return ErrorCode::GOAL_VIOLATES_PATH_CONSTRAINTS;
  }
  if (!(req.pb.position_constraints_size() == 0) ||
    ...
    if (!SatisfiesCartesianConstraints(req, req.pb.start_state())) {
      if (error) {
    ...
    if (req.pb.goal_state().position_size() > 0 &&
        !SatisfiesCartesianConstraints(req, req.pb.goal_state())) {
      if (error) {
