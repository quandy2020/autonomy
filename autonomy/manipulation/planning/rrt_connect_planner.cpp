...
MotionPlanResponse RrtConnectPlanner::Plan(const MotionPlanRequest& request) {
  ...
  const Vec start = request.start_state.positions;
  const Vec goal = request.goal_state.positions;
  ...
  // Direct connect first (fast path).
  {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      js.names = names;
      js.positions = q;
      traj.waypoints.push_back(js);
      traj.time_from_start.push_back(0.05 * i);
    }
    if (ok) {
  ...
  // Rebuild path (trees may have been swapped; approximate via BFS from ends).
  // Simpler: use tree_a/tree_b as left at exit — swap restores orientation if even iters.
  // Rebuild from start tree and goal tree by walking parents.
  ...
  // After possible swaps, identify which tree holds start.
  ...
  auto append = [&](const Vec& q) {
    ...
    js.positions = q;
    traj.waypoints.push_back(js);
    traj.time_from_start.push_back(0.05 * k++);
  };
