...
bool BuildKdlChainFromUrdfFile(const std::string& urdf_path_in,
                           const std::string& base_link,
                           const std::string& tip_link, KdlChainDescription* out,
                           std::string* error) {
  if (!out || base_link.empty() || tip_link.empty()) {
    if (error) {
      *error = "invalid BuildKdlChainFromUrdfFile arguments";
    }
    return false;
  }
  ...
  out->position_lower_bounds = KDL::JntArray(mins.size());
  out->position_upper_bounds = KDL::JntArray(maxs.size());
  for (std::size_t i = 0; i < mins.size(); ++i) {
    out->position_lower_bounds(i) = mins[i];
    out->position_upper_bounds(i) = maxs[i];
  }
