...
bool LoadPlanningGroupsFromSrdf(const std::string& path_in,
  ...
  for (std::sregex_iterator it(xml.begin(), xml.end(), group_re), end;
    ...
    group.set_name(ExtractAttr((*it)[1].str(), "name"));
    if (group.name().empty()) {
      continue;
    }
    ...
    for (std::sregex_iterator jt(body.begin(), body.end(), joint_re), jend;
      ...
      if (!jn.empty()) {
        group.add_joint_names(jn);
      }
    }
    const std::regex chain_re(R"regex(<chain\b[^>]*/?>)regex");
    std::smatch cm;
    if (std::regex_search(body, cm, chain_re)) {
      group.set_base_frame(ExtractAttr(cm[0].str(), "base_link"));
      group.set_tip_frame(ExtractAttr(cm[0].str(), "tip_link"));
      group.set_is_chain(true);
    }
...
bool LoadEndEffectorsFromSrdf(const std::string& path_in,
  ...
  for (std::sregex_iterator it(xml.begin(), xml.end(), ee_re), end; it != end;
    ...
    const std::string tag = (*it)[0].str();
    SrdfEndEffector ee;
    ee.set_name(ExtractAttr(tag, "name"));
    ee.set_group(ExtractAttr(tag, "group"));
    ee.set_parent_link(ExtractAttr(tag, "parent_link"));
    ee.set_parent_group(ExtractAttr(tag, "parent_group"));
    if (!ee.name().empty()) {
...
bool LoadPassiveJointsFromSrdf(const std::string& path_in,
