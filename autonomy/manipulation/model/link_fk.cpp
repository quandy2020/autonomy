...
void ApplyMimicJoints(const std::vector<LinkFkJoint>& joints, JointState* state) {
  ...
  for (int i = 0; i < state->name_size() && i < state->position_size(); ++i) {
    qmap[state->name(i)] = state->position(i);
  }
  ...
  for (int i = 0; i < state->name_size() && i < state->position_size(); ++i) {
    const auto it = qmap.find(state->name(i));
    if (it != qmap.end()) {
      state->set_position(i, it->second);
    }
  }
  for (const auto& j : joints) {
    ...
    bool found = false;
    for (int k = 0; k < state->name_size(); ++k) {
      if (state->name(k) == j.name) {
        found = true;
        break;
      }
    }
    ...
    state->add_name(j.name);
    state->add_position(qmap[j.name]);
  }
}
...
bool LinkFkTree::Compute(
  ...
  std::unordered_map<std::string, double> qmap;
  for (int i = 0;
       i < expanded.name_size() && i < expanded.position_size(); ++i) {
    qmap[expanded.name(i)] = expanded.position(i);
  }
