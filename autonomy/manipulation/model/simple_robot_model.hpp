...
class SimpleRobotModel : public RobotModel {
 public:
  ...
  /** @brief SRDF end_effector entries. */
  const std::vector<SrdfEndEffector>& EndEffectors() const {
    return end_effectors_;
  }

  /** @brief SRDF passive_joint names. */
  const std::vector<std::string>& PassiveJoints() const {
    return passive_joints_;
  }

  /** @brief URDF path last passed to Load (empty if none). */
  const std::string& UrdfPath() const { return urdf_path_; }

  /** @brief Underlying LinkFkTree after Load (may be empty). */
  const LinkFkTree& LinkTree() const { return link_tree_; }

  /** @brief Ensure LinkFkTree is loaded from @p urdf (or cached path). */
  bool EnsureLinkTree(const std::string& urdf = {}) {
    const std::string path = urdf.empty() ? urdf_path_ : urdf;
    if (path.empty()) {
      return !link_tree_.LinkNames().empty();
    }
    return link_tree_.LoadUrdf(path);
  }
