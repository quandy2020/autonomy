...
bool LoFrontend::Init(const Atla2Config& cfg) {
  cfg_ = cfg;
  Reset();
  return true;
}

void LoFrontend::Reset() {
  result_ = OdometryResult{};
  local_map_.clear();
  initialized_ = false;
}

void LoFrontend::AccumulateMap(const LidarScan& scan) {
...
bool LoFrontend::Process(const SensorData& data) {
  if (!data.has_lidar || data.lidar.points.empty()) {
    return false;
  }
  result_.t = data.t;
  if (!initialized_) {
  ...
  // Registration placeholder: no IMU; hold pose and grow local map.
...
bool LoFrontend::GetResult(OdometryResult* out) {
