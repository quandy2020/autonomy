...
#include "autonomy/localization/cartographer/io/internal/mapping_state_serialization.hpp"

#include "autonomy/localization/cartographer/mapping/proto/serialization.pb.h"
...
mapping::proto::AllTrajectoryBuilderOptions CreateAllTrajectoryBuilderOptionsProto(
    const std::vector<mapping::proto::TrajectoryBuilderOptionsWithSensorIds>& all_options_with_sensor_ids,
  ...
  for (auto id : trajectory_ids_to_serialize) {
    *all_options_proto.add_options_with_sensor_ids() = all_options_with_sensor_ids[id];
  }
  return all_options_proto;
}

// Will return all trajectory ids, that have `state != DELETED`.
std::vector<int> GetValidTrajectoryIds(const std::map<int, PoseGraphInterface::TrajectoryState>& trajectory_states) {
...
SerializedData SerializePoseGraph(const mapping::PoseGraph& pose_graph, bool include_unfinished_submaps) {
...
SerializedData SerializeTrajectoryBuilderOptions(
  ...
  *proto.mutable_all_trajectory_builder_options() =
      CreateAllTrajectoryBuilderOptionsProto(trajectory_builder_options, trajectory_ids_to_serialize);
  return proto;
}

void SerializeSubmaps(const MapById<SubmapId, PoseGraphInterface::SubmapData>& submap_data,
...
void SerializeTrajectoryNodes(const MapById<NodeId, TrajectoryNode>& trajectory_nodes,
  ...
  for (const auto& node_id_data : trajectory_nodes) {
    ...
    node_proto->mutable_node_id()->set_trajectory_id(node_id_data.id.trajectory_id);
...
void SerializeTrajectoryData(const std::map<int, PoseGraphInterface::TrajectoryData>& all_trajectory_data,
                             ProtoStreamWriterInterface* const writer) {
  for (const auto& trajectory_data : all_trajectory_data) {
    ...
    *trajectory_data_proto->mutable_imu_calibration() = transform::ToProto(
        Eigen::Quaterniond(trajectory_data.second.imu_calibration[0], trajectory_data.second.imu_calibration[1],
                           trajectory_data.second.imu_calibration[2], trajectory_data.second.imu_calibration[3]));
...
void SerializeOdometryData(const sensor::MapByTime<sensor::OdometryData>& all_odometry_data,
                           ProtoStreamWriterInterface* const writer) {
...
void SerializeFixedFramePoseData(const sensor::MapByTime<sensor::FixedFramePoseData>& all_fixed_frame_pose_data,
                                 ProtoStreamWriterInterface* const writer) {
  for (const int trajectory_id : all_fixed_frame_pose_data.trajectory_ids()) {
    for (const auto& fixed_frame_pose_data : all_fixed_frame_pose_data.trajectory(trajectory_id)) {
      SerializedData proto;
      auto* const fixed_frame_pose_data_proto = proto.mutable_fixed_frame_pose_data();
      fixed_frame_pose_data_proto->set_trajectory_id(trajectory_id);
      *fixed_frame_pose_data_proto->mutable_fixed_frame_pose_data() = sensor::ToProto(fixed_frame_pose_data);
      writer->WriteProto(proto);
    }
  }
}

void SerializeLandmarkNodes(const std::map<std::string, PoseGraphInterface::LandmarkNode>& all_landmark_nodes,
                            ProtoStreamWriterInterface* const writer) {
  for (const auto& node : all_landmark_nodes) {
    for (const auto& observation : node.second.landmark_observations) {
      ...
      landmark_data_proto->mutable_landmark_data()->set_timestamp(common::ToUniversal(observation.time));
      auto* observation_proto = landmark_data_proto->mutable_landmark_data()->add_landmark_observations();
...
void WritePbStream(const mapping::PoseGraph& pose_graph,
  ...
  writer->WriteProto(SerializeTrajectoryBuilderOptions(trajectory_builder_options,
