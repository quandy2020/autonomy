...
UniqueCairoPtr MakeUniqueCairoPtr(cairo_t* surface) { return UniqueCairoPtr(surface, cairo_destroy); }
...
void Image::WritePng(FileWriter* const file_writer) {
  // TODO(hrapp): cairo_image_surface_create_for_data does not take ownership
  // of the data until the surface is finalized. Once it is finalized though,
  // cairo_surface_write_to_png fails, complaining that the surface is already
  // finalized. This makes it pretty hard to pass back ownership of the image
  // to the caller.
...
const Uint8Color Image::GetPixel(int x, int y) const { return CairoToUint8Color(pixels_[y * width_ + x]); }

void Image::SetPixel(int x, int y, const Uint8Color& color) { pixels_[y * width_ + x] = Uint8ColorToCairo(color); }
