...
void CairoPaintSubmapSlices(
    ...
    for (auto& pair : submaps) {
        ...
        cairo_matrix_init(&matrix, homo(1, 0), homo(0, 0), -homo(1, 1),
                          -homo(0, 1), homo(0, 3), -homo(1, 3));
        ...
        CairoPaintSubmapSlices(1. / resolution, submaps, cr.get(),
                               [&cr](const SubmapSlice& submap_slice) {
                                   cairo_set_source_surface(
                                       cr.get(), submap_slice.surface.get(), 0.,
                                       0.);
                                   cairo_paint(cr.get());
                               });
...
UniqueCairoSurfacePtr DrawTexture(const std::vector<char>& intensity,
    ...
    CHECK_EQ(expected_stride,
             cairo_format_stride_for_width(kCairoFormat, width));
