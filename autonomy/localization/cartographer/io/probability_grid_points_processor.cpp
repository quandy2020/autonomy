...
#include "autonomy/localization/cartographer/io/probability_grid_points_processor.hpp"
...
#include "autonomy/localization/cartographer/io/points_batch.hpp"
...
void DrawTrajectoriesIntoImage(const mapping::ProbabilityGrid& probability_grid, const Eigen::Array2i& offset,
                               const std::vector<mapping::proto::Trajectory>& trajectories,
                               cairo_surface_t* cairo_surface) {
  for (size_t i = 0; i < trajectories.size(); ++i) {
    DrawTrajectory(
        trajectories[i], GetColor(i),
        [&probability_grid, &offset](const transform::Rigid3d& pose) -> Eigen::Array2i {
          return probability_grid.limits().GetCellIndex(pose.cast<float>().translation().head<2>()) - offset;
        },
        cairo_surface);
  }
}
...
uint8 ProbabilityToColor(float probability_from_grid) {
  ...
  return ::cartographer::common::RoundToInt(
      255 * ((probability - mapping::kMinProbability) / (mapping::kMaxProbability - mapping::kMinProbability)));
}

std::string FileExtensionFromOutputType(const ProbabilityGridPointsProcessor::OutputType& output_type) {
...
ProbabilityGridPointsProcessor::OutputType OutputTypeFromString(const std::string& output_type) {
  if (output_type == "png") {
...
ProbabilityGridPointsProcessor::ProbabilityGridPointsProcessor(
    ...
    const DrawTrajectories& draw_trajectories, const OutputType& output_type, std::unique_ptr<FileWriter> file_writer,
    const std::vector<mapping::proto::Trajectory>& trajectories, PointsProcessor* const next)
    : draw_trajectories_(draw_trajectories),
      ...
      probability_grid_(CreateProbabilityGrid(resolution, &conversion_tables_)) {
  LOG_IF(WARNING, output_type == OutputType::kPb && draw_trajectories_ == DrawTrajectories::kYes)
      << "Drawing the trajectories is not supported when writing the "
         "probability grid as protobuf.";
}

std::unique_ptr<ProbabilityGridPointsProcessor> ProbabilityGridPointsProcessor::FromDictionary(
    const std::vector<mapping::proto::Trajectory>& trajectories, const FileWriterFactory& file_writer_factory,
  ...
  const auto output_type =
      dictionary->HasKey("output_type") ? OutputTypeFromString(dictionary->GetString("output_type")) : OutputType::kPng;
  return absl::make_unique<ProbabilityGridPointsProcessor>(
      dictionary->GetDouble("resolution"),
      mapping::CreateProbabilityGridRangeDataInserterOptions2D(dictionary->GetDictionary("range_data_inserter").get()),
      draw_trajectories, output_type,
      file_writer_factory(dictionary->GetString("filename") + FileExtensionFromOutputType(output_type)), trajectories,
      next);
}

void ProbabilityGridPointsProcessor::Process(std::unique_ptr<PointsBatch> batch) {
  range_data_inserter_.Insert({batch->origin, sensor::PointCloud(batch->points), {}}, &probability_grid_);
...
PointsProcessor::FlushResult ProbabilityGridPointsProcessor::Flush() {
  if (output_type_ == OutputType::kPng) {
    ...
    std::unique_ptr<Image> image = DrawProbabilityGrid(probability_grid_, &offset);
    if (image != nullptr) {
      if (draw_trajectories_ == ProbabilityGridPointsProcessor::DrawTrajectories::kYes) {
        DrawTrajectoriesIntoImage(probability_grid_, offset, trajectories_, image->GetCairoSurface().get());
      }
    ...
    file_writer_->Write(probability_grid_serialized.data(), probability_grid_serialized.size());
    CHECK(file_writer_->Close());
  } else {
    LOG(FATAL) << "Output Type " << FileExtensionFromOutputType(output_type_) << " is not supported.";
  }
...
std::unique_ptr<Image> DrawProbabilityGrid(const mapping::ProbabilityGrid& probability_grid, Eigen::Array2i* offset) {
  ...
  auto image = absl::make_unique<Image>(cell_limits.num_x_cells, cell_limits.num_y_cells);
  for (const Eigen::Array2i& xy_index : mapping::XYIndexRangeIterator(cell_limits)) {
    const Eigen::Array2i index = xy_index + *offset;
    constexpr uint8 kUnknownValue = 128;
    const uint8 value =
        probability_grid.IsKnown(index) ? ProbabilityToColor(probability_grid.GetProbability(index)) : kUnknownValue;
...
mapping::ProbabilityGrid CreateProbabilityGrid(const double resolution,
  ...
  return mapping::ProbabilityGrid(
      mapping::MapLimits(resolution, max,
                         mapping::CellLimits(kInitialProbabilityGridSize, kInitialProbabilityGridSize)),
      conversion_tables);
}
