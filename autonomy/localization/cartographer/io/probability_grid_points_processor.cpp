...
PointsProcessor::FlushResult ProbabilityGridPointsProcessor::Flush() {
    if (output_type_ == OutputType::kPng) {
        ...
        if (image != nullptr) {
            if (draw_trajectories_ ==
                ...
                DrawTrajectoriesIntoImage(probability_grid_, offset,
                                          trajectories_,
        ...
        LOG(FATAL) << "Output Type "
                   << FileExtensionFromOutputType(output_type_)
                   << " is not supported.";
    }
    ...
    switch (next_->Flush()) {
        case FlushResult::kRestartStream:
            LOG(FATAL)
                << "ProbabilityGrid generation must be configured to occur "
...
mapping::ProbabilityGrid CreateProbabilityGrid(
    ...
    Eigen::Vector2d max = 0.5 * kInitialProbabilityGridSize * resolution *
                          Eigen::Vector2d::Ones();
