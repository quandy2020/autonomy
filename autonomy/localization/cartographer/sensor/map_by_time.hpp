...
class MapByTime {
 public:
  ...
  void Trim(const mapping::MapById<mapping::NodeId, NodeType>& nodes, const mapping::NodeId& node_id) {
    ...
    // interpolation with the adjacent data outside the gap is still
    // possible.
  ...
  bool HasTrajectory(const int trajectory_id) const { return data_.count(trajectory_id) != 0; }
  ...
  class ConstIterator {
   public:
    ...
    explicit ConstIterator(typename std::map<common::Time, DataType>::const_iterator iterator) : iterator_(iterator) {}
    ...
    bool operator==(const ConstIterator& it) const { return iterator_ == it.iterator_; }
  ...
  ConstIterator EndOfTrajectory(const int trajectory_id) const { return ConstIterator(data_.at(trajectory_id).end()); }
