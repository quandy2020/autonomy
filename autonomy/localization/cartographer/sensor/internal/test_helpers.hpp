...
MATCHER_P(Near, point, std::string(negation ? "Doesn't" : "Does") + " match.") { return arg.isApprox(point, 0.001f); }
...
struct CollatorInput {
  ...
  void MoveToCollator(CollatorInterface* collator) { collator->AddSensorData(trajectory_id, std::move(data)); }
