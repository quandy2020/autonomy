...
TEST(PointCloudTest, TransformPointCloud) {
    ...
    const PointCloud transformed_point_cloud =
        TransformPointCloud(point_cloud, transform::Embed3D(transform::Rigid2f::Rotation(M_PI_2)));
...
TEST(PointCloudTest, TransformTimedPointCloud) {
    ...
    const TimedPointCloud transformed_point_cloud =
        TransformTimedPointCloud(point_cloud, transform::Embed3D(transform::Rigid2f::Rotation(M_PI_2)));
    EXPECT_NEAR(-0.5f, transformed_point_cloud[0].position.x(), 1e-6);
    EXPECT_NEAR(0.5f, transformed_point_cloud[0].position.y(), 1e-6);
    EXPECT_NEAR(-0.5f, transformed_point_cloud[1].position.x(), 1e-6);
    EXPECT_NEAR(3.5f, transformed_point_cloud[1].position.y(), 1e-6);
...
TEST(PointCloudTest, CopyIf) {
    std::vector<RangefinderPoint> points = {{{0.f, 0.f, 0.f}}, {{1.f, 1.f, 1.f}}, {{2.f, 2.f, 2.f}}};

    const PointCloud point_cloud(points);
    const PointCloud copied_point_cloud =
        point_cloud.copy_if([&](const RangefinderPoint& point) { return point.position.x() > 0.1f; });

    EXPECT_EQ(copied_point_cloud.size(), 2);
    EXPECT_THAT(copied_point_cloud.intensities(), IsEmpty());
    EXPECT_THAT(copied_point_cloud.points(), ElementsAre(RangefinderPoint{Eigen::Vector3f{1.f, 1.f, 1.f}},
...
TEST(PointCloudTest, CopyIfWithIntensities) {
    ...
    EXPECT_THAT(copied_point_cloud.points(), ElementsAre(RangefinderPoint{Eigen::Vector3f{1.f, 1.f, 1.f}},
                                                         RangefinderPoint{Eigen::Vector3f{2.f, 2.f, 2.f}}));
    EXPECT_THAT(copied_point_cloud.intensities(), ElementsAre(FloatNear(1.f, 1e-6), FloatNear(2.f, 1e-6)));
}
