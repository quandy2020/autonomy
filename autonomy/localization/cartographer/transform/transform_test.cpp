...
TEST(TransformTest, GetAngle) {
  ...
  for (int i = 0; i != 100; ++i) {
    ...
    EXPECT_NEAR(angle, GetAngle(Rigid3f::Rotation(AngleAxisVectorToRotationQuaternion(Eigen::Vector3f(angle * axis)))),
                1e-6f);
  }
}
...
TEST(TransformTest, Embed3D) {
  ...
  const Rigid3d rigid3d(Rigid3d::Translation(Eigen::Vector3d(1., 2., 3.)) *
                        Rigid3d::Rotation(Eigen::Quaterniond(1., 2., 3., 4.).normalized()));
  const Rigid3d expected = rigid3d * Rigid3d::Translation(Eigen::Vector3d(1., 2., 0.));
