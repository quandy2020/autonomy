...
TEST(BlockingQueueTest, testPushWithTimeout) {
    ...
    EXPECT_EQ(true, blocking_queue.PushWithTimeout(std::make_unique<int>(42), common::FromMilliseconds(150)));
    EXPECT_EQ(false, blocking_queue.PushWithTimeout(std::make_unique<int>(15), common::FromMilliseconds(150)));
    EXPECT_EQ(42, *blocking_queue.Pop());
...
TEST(BlockingQueueTest, testPushWithTimeoutInfinteQueue) {
    BlockingQueue<std::unique_ptr<int>> blocking_queue;
    EXPECT_EQ(true, blocking_queue.PushWithTimeout(std::make_unique<int>(42), common::FromMilliseconds(150)));
    EXPECT_EQ(true, blocking_queue.PushWithTimeout(std::make_unique<int>(45), common::FromMilliseconds(150)));
    EXPECT_EQ(42, *blocking_queue.Pop());
