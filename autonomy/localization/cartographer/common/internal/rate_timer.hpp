...
class RateTimer {
 public:
  ...
  double ComputeRate() const {
    ...
    return static_cast<double>(events_.size() - 1) / common::ToSeconds((events_.back().time - events_.front().time));
  }

  // Returns the ratio of the pulse rate (with supplied times) to the wall
  // time rate. For example, if a sensor produces pulses at 10 Hz, but we call
  // Pulse at 20 Hz wall time, this will return 2.
