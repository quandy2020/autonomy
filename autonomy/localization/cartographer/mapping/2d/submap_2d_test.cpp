...
TEST(Submap2DTest, TheRightNumberOfRangeDataAreInserted) {
    ...
    // Submaps should not be left without the right number of range data in
    // them.
...
TEST(Submap2DTest, ToFromProto) {
    ...
    Submap2D expected(Eigen::Vector2f(4.f, 5.f),
                      std::make_unique<ProbabilityGrid>(expected_map_limits, &conversion_tables), &conversion_tables);
