...
TEST(ProbabilityGridTest, ConstructorGridType) {
  ValueConversionTables conversion_tables;
  ProbabilityGrid probability_grid(MapLimits(1., Eigen::Vector2d(1., 1.), CellLimits(2, 2)), &conversion_tables);
...
TEST(ProbabilityGridTest, ToProto) {
  ...
  EXPECT_EQ(ToProto(probability_grid.limits()).DebugString(), proto.limits().DebugString());
...
TEST(ProbabilityGridTest, ApplyOdds) {
  ...
  probability_grid.ApplyLookupTable(Array2i(1, 0), ComputeLookupTableToApplyCorrespondenceCostOdds(Odds(0.9)));
  probability_grid.FinishUpdate();
  EXPECT_GT(probability_grid.GetProbability(Array2i(1, 0)), 0.5);

  probability_grid.SetProbability(Array2i(0, 1), 0.5);
  probability_grid.ApplyLookupTable(Array2i(0, 1), ComputeLookupTableToApplyCorrespondenceCostOdds(Odds(0.1)));
  probability_grid.FinishUpdate();
  EXPECT_LT(probability_grid.GetProbability(Array2i(0, 1)), 0.5);

  // Tests adding odds to an unknown cell.
  probability_grid.ApplyLookupTable(Array2i(1, 1), ComputeLookupTableToApplyCorrespondenceCostOdds(Odds(0.42)));
  ...
  probability_grid.ApplyLookupTable(Array2i(1, 1), ComputeLookupTableToApplyCorrespondenceCostOdds(Odds(0.9)));
  EXPECT_NEAR(probability_grid.GetProbability(Array2i(1, 1)), 0.42, 1e-4);
  probability_grid.FinishUpdate();
  probability_grid.ApplyLookupTable(Array2i(1, 1), ComputeLookupTableToApplyCorrespondenceCostOdds(Odds(0.9)));
...
TEST(ProbabilityGridTest, GetProbability) {
  ValueConversionTables conversion_tables;
  ProbabilityGrid probability_grid(MapLimits(1., Eigen::Vector2d(1., 2.), CellLimits(2, 2)), &conversion_tables);
  ...
  probability_grid.SetProbability(limits.GetCellIndex(Vector2f(-0.5f, 0.5f)), kMaxProbability);
  EXPECT_NEAR(probability_grid.GetProbability(limits.GetCellIndex(Vector2f(-0.5f, 0.5f))), kMaxProbability, 1e-6);
  for (const Array2i& xy_index : {limits.GetCellIndex(Vector2f(-0.5f, 1.5f)), limits.GetCellIndex(Vector2f(0.5f, 0.5f)),
...
TEST(ProbabilityGridTest, GetCellIndex) {
  ValueConversionTables conversion_tables;
  ProbabilityGrid probability_grid(MapLimits(2., Eigen::Vector2d(8., 14.), CellLimits(14, 8)), &conversion_tables);

  const MapLimits& limits = probability_grid.limits();
  const CellLimits& cell_limits = limits.cell_limits();
  ASSERT_EQ(14, cell_limits.num_x_cells);
  ASSERT_EQ(8, cell_limits.num_y_cells);
  EXPECT_TRUE((Array2i(0, 0) == limits.GetCellIndex(Vector2f(7.f, 13.f))).all());
  EXPECT_TRUE((Array2i(13, 0) == limits.GetCellIndex(Vector2f(7.f, -13.f))).all());
  EXPECT_TRUE((Array2i(0, 7) == limits.GetCellIndex(Vector2f(-7.f, 13.f))).all());
  EXPECT_TRUE((Array2i(13, 7) == limits.GetCellIndex(Vector2f(-7.f, -13.f))).all());

  // Check around the origin.
  EXPECT_TRUE((Array2i(6, 3) == limits.GetCellIndex(Vector2f(0.5f, 0.5f))).all());
  EXPECT_TRUE((Array2i(6, 3) == limits.GetCellIndex(Vector2f(1.5f, 1.5f))).all());
  EXPECT_TRUE((Array2i(7, 3) == limits.GetCellIndex(Vector2f(0.5f, -0.5f))).all());
  EXPECT_TRUE((Array2i(6, 4) == limits.GetCellIndex(Vector2f(-0.5f, 0.5f))).all());
  EXPECT_TRUE((Array2i(7, 4) == limits.GetCellIndex(Vector2f(-0.5f, -0.5f))).all());
}
...
TEST(ProbabilityGridTest, CorrectCropping) {
  ...
  ValueConversionTables conversion_tables;
  ProbabilityGrid probability_grid(MapLimits(0.05, Eigen::Vector2d(10., 10.), CellLimits(400, 400)),
                                   &conversion_tables);
  for (const Array2i& xy_index : XYIndexRangeIterator(Array2i(100, 100), Array2i(299, 299))) {
