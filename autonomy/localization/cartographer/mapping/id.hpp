 ...
 public:
  ...
  class ConstIterator {
   public:
    ...
    ConstIterator& operator--() {
      while (current_trajectory_ == end_trajectory_ || current_data_ == current_trajectory_->second.data_.begin()) {
        --current_trajectory_;
  ...
  class ConstTrajectoryIterator {
   public:
    ...
    bool operator==(const ConstTrajectoryIterator& it) const { return current_trajectory_ == it.current_trajectory_; }
  ...
  void Trim(const IdType& id) {
    ...
    if (std::next(it) == trajectory.data_.end()) {
      // We are removing the data with the highest index from this
      // trajectory. We assume that we will never append to it anymore. If
      // we did, we would have to make sure that gaps in indices are
      // properly chosen to maintain correct connectivity.
      trajectory.can_append_ = false;
    }
  ...
  DataType& at(const IdType& id) { return trajectories_.at(id.trajectory_id).data_.at(GetIndex(id)); }

  // Support querying by trajectory.
  ConstIterator BeginOfTrajectory(const int trajectory_id) const { return ConstIterator(*this, trajectory_id); }

  ConstIterator EndOfTrajectory(const int trajectory_id) const { return BeginOfTrajectory(trajectory_id + 1); }
  ...
  ConstIterator lower_bound(const int trajectory_id, const common::Time time) const {
    ...
    while (left != right) {
      ...
      // This could be 'right' in the presence of gaps, so we need to use
      // the previous element in this case.
