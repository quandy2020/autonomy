...
#include "autonomy/localization/cartographer/mapping/internal/2d/local_trajectory_builder_2d.hpp"
...
#include "autonomy/localization/cartographer/mapping/internal/collated_trajectory_builder.hpp"
#include "autonomy/localization/cartographer/mapping/internal/global_trajectory_builder.hpp"
#include "autonomy/localization/cartographer/mapping/internal/motion_filter.hpp"
#include "autonomy/localization/cartographer/sensor/internal/collator.hpp"
#include "autonomy/localization/cartographer/sensor/internal/trajectory_collator.hpp"
...
void MaybeAddPureLocalizationTrimmer(
    ...
    if (trajectory_options.has_pure_localization_trimmer()) {
        pose_graph->AddTrimmer(absl::make_unique<PureLocalizationTrimmer>(
            trajectory_id, trajectory_options.pure_localization_trimmer()
                               .max_submaps_to_keep()));
    }
}
...
int MapBuilder::AddTrajectoryBuilder(
    ...
    if (trajectory_options.has_pose_graph_odometry_motion_filter()) {
        LOG(INFO)
            << "Using a motion filter for adding odometry to the pose graph.";
        pose_graph_odometry_motion_filter.emplace(MotionFilter(
            trajectory_options.pose_graph_odometry_motion_filter()));
    }
    ...
    if (options_.use_trajectory_builder_3d()) {
        ...
        if (trajectory_options.has_trajectory_builder_3d_options()) {
            local_trajectory_builder =
                absl::make_unique<LocalTrajectoryBuilder3D>(
        ...
        if (trajectory_options.has_trajectory_builder_2d_options()) {
            local_trajectory_builder =
                absl::make_unique<LocalTrajectoryBuilder2D>(
...
std::map<int, int> MapBuilder::LoadState(
    ...
    for (const auto& landmark : pose_graph_proto.landmark_poses()) {
        pose_graph_->SetLandmarkPose(
            landmark.landmark_id(), transform::ToRigid3(landmark.global_pose()),
            true);
    }
    ...
    if (options_.use_trajectory_builder_3d()) {
        CHECK_NE(deserializer.header().format_version(),
            ...
            << "The pbstream file contains submaps without rotational "
               "histograms. "
               "This can be converted with the 'pbstream migrate' tool, see "
               "the "
    ...
    while (deserializer.ReadNextSerializedData(&proto)) {
        switch (proto.data_case()) {
            case SerializedData::kPoseGraph:
                LOG(ERROR)
                    << "Found multiple serialized `PoseGraph`. Serialized "
...
std::map<int, int> MapBuilder::LoadStateFromFile(
    ...
    if (state_filename.substr(std::max<int>(
        ...
        LOG(WARNING)
            << "The file containing the state should be a .pbstream file.";
    }
