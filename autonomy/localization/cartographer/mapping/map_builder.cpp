...
void MaybeAddPureLocalizationTrimmer(const int trajectory_id, const proto::TrajectoryBuilderOptions& trajectory_options,
                                     PoseGraph* pose_graph) {
    if (trajectory_options.pure_localization()) {
        LOG(WARNING) << "'TrajectoryBuilderOptions::pure_localization' field is "
                        "deprecated. "
                        "Use 'TrajectoryBuilderOptions::pure_localization_trimmer' "
                        "instead.";
        pose_graph->AddTrimmer(std::make_unique<PureLocalizationTrimmer>(trajectory_id, 3 /* max_submaps_to_keep */));
        return;
    }
...
MapBuilder::MapBuilder(const proto::MapBuilderOptions& options)
    ...
    if (options.use_trajectory_builder_2d()) {
        pose_graph_ = std::make_unique<PoseGraph2D>(options_.pose_graph_options(),
                                                     std::make_unique<optimization::OptimizationProblem2D>(
                                                         options_.pose_graph_options().optimization_problem_options()),
                                                     &thread_pool_);
    }
    if (options.use_trajectory_builder_3d()) {
        pose_graph_ = std::make_unique<PoseGraph3D>(options_.pose_graph_options(),
    ...
    if (options_.use_trajectory_builder_3d()) {
        ...
        trajectory_builders_.push_back(std::make_unique<CollatedTrajectoryBuilder>(
            ...
            CreateGlobalTrajectoryBuilder3D(std::move(local_trajectory_builder), trajectory_id,
                                            static_cast<PoseGraph3D*>(pose_graph_.get()), local_slam_result_callback,
