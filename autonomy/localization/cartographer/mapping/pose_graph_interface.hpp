...
{
public:
    // A "constraint" as in the paper by Konolige, Kurt, et al. "Efficient
    // sparse pose adjustment for 2d mapping." Intelligent Robots and Systems
    // (IROS), 2010 IEEE/RSJ International Conference on (pp. 22--29). IEEE,
    // 2010.
    struct Constraint {
        struct Pose {
    ...
    struct LandmarkNode {
        ...
        std::optional<transform::Rigid3d> global_landmark_pose;
        bool frozen = false;
    };
    ...
    virtual void SetLandmarkPose(const std::string& landmark_id, const transform::Rigid3d& global_pose,
    ...
    // Sets the callback function that is invoked whenever the global
