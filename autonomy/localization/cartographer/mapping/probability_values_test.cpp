...
TEST(ProbabilityValuesTest, ConversionLookUpTable) {
    EXPECT_NEAR(ValueToProbability(0), 1.f - ValueToCorrespondenceCost(0),
                1e-6);
    for (uint16 i = 1; i < 32768; ++i) {
...
TEST(ProbabilityValuesTest, CellUpdate) {
    ...
    for (int i_probability = 0; i_probability < num_evaluations;
        ...
        float p_value = (common::Clamp(p, kMinProbability, kMaxProbability) -
                         kMinProbability) *
...
TEST(ProbabilityValuesTest, EqualityLookupTableToApplyOdds) {
    ...
    for (int i = 0; i < 32768; ++i) {
        EXPECT_NEAR(
            probability_table[i],
            CorrespondenceCostValueToProbabilityValue(
                correspondence_table[ProbabilityValueToCorrespondenceCostValue(
                    i)]),
            1);
        EXPECT_NEAR(
            ProbabilityValueToCorrespondenceCostValue(
                probability_table[CorrespondenceCostValueToProbabilityValue(
                    i)]),
