...
class MotionFilterTest : public ::testing::Test {
 protected:
  ...
  common::Time SecondsSinceEpoch(int seconds) { return common::FromUniversal(seconds * 10000000); }
...
TEST_F(MotionFilterTest, LinearMotion) {
  ...
  EXPECT_TRUE(
      motion_filter.IsSimilar(SecondsSinceEpoch(42), transform::Rigid3d::Translation(Eigen::Vector3d(0.6, 0.15, 0.))));
}
...
TEST_F(MotionFilterTest, RotationalMotion) {
  ...
  EXPECT_TRUE(motion_filter.IsSimilar(SecondsSinceEpoch(42),
                                      transform::Rigid3d::Rotation(Eigen::AngleAxisd(1.9, Eigen::Vector3d::UnitY()))));
  EXPECT_FALSE(motion_filter.IsSimilar(SecondsSinceEpoch(42),
                                       transform::Rigid3d::Rotation(Eigen::AngleAxisd(2.1, Eigen::Vector3d::UnitY()))));
  EXPECT_TRUE(motion_filter.IsSimilar(SecondsSinceEpoch(42),
                                      transform::Rigid3d::Rotation(Eigen::AngleAxisd(4., Eigen::Vector3d::UnitY()))));
  EXPECT_FALSE(motion_filter.IsSimilar(SecondsSinceEpoch(42),
                                       transform::Rigid3d::Rotation(Eigen::AngleAxisd(5.9, Eigen::Vector3d::UnitY()))));
  EXPECT_TRUE(motion_filter.IsSimilar(SecondsSinceEpoch(42), transform::Rigid3d::Identity()));
}
