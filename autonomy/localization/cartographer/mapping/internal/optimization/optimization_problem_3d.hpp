...
#include "autonomy/localization/cartographer/mapping/internal/optimization/optimization_problem_interface.hpp"
#include "autonomy/localization/cartographer/mapping/pose_graph_interface.hpp"
#include "autonomy/localization/cartographer/mapping/proto/pose_graph/optimization_problem_options.pb.h"
...
#include "autonomy/localization/cartographer/sensor/map_by_time.hpp"
...
class OptimizationProblem3D : public OptimizationProblemInterface<NodeSpec3D, SubmapSpec3D, transform::Rigid3d> {
 public:
  ...
  const MapById<NodeId, NodeSpec3D>& node_data() const override { return node_data_; }
  const MapById<SubmapId, SubmapSpec3D>& submap_data() const override { return submap_data_; }
  const std::map<std::string, transform::Rigid3d>& landmark_data() const override { return landmark_data_; }
  const sensor::MapByTime<sensor::ImuData>& imu_data() const override { return imu_data_; }
