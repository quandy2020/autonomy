...
std::unique_ptr<transform::Rigid3d> Interpolate(
    ...
    if (prev_it->pose.has_value()) {
        return absl::make_unique<transform::Rigid3d>(
            Interpolate(
                transform::TimestampedTransform{prev_it->time,
                                                prev_it->pose.value()},
...
transform::Rigid3d GetInitialLandmarkPose(
    ...
    return transform::Rigid3d::FromArrays(
               std::get<0>(rotation_and_translation),
...
void AddLandmarkCostFunctions(
    ...
    for (const auto& landmark_node : landmark_nodes) {
        ...
        for (const auto& observation :
             landmark_node.second.landmark_observations) {
            ...
            // observation.
            auto next = node_data.lower_bound(observation.trajectory_id,
                                              observation.time);
...
void OptimizationProblem3D::Solve(
    ...
    if (!options_.fix_z_in_3d()) {
        for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
            ...
            const auto trajectory_end =
                node_data_.EndOfTrajectory(trajectory_id);
            ...
            TrajectoryData& trajectory_data =
                trajectory_data_.at(trajectory_id);

            problem.AddParameterBlock(trajectory_data.imu_calibration.data(),
                                      4);
            ...
            for (++node_it; node_it != trajectory_end; ++node_it) {
                ...
                const IntegrateImuResult<double> result =
                    IntegrateImu(imu_data, first_node_data.time,
                                 second_node_data.time, &imu_it);
                ...
                const common::Duration first_duration =
                    second_time - first_time;
                if (next_node_it != trajectory_end &&
                    next_node_it->id.node_index ==
                        second_node_id.node_index + 1) {
                ...
                problem.AddResidualBlock(
                    ...
                    nullptr /* loss function */,
                    C_nodes.at(first_node_id).rotation(),
                    C_nodes.at(second_node_id).rotation(),
                    trajectory_data.imu_calibration.data());
            }
            ...
            if (gravity_block_added) {
                ...
                problem.SetParameterLowerBound(
                    &trajectory_data.gravity_constant, 0, 0.0);
            }
        }
    }
    ...
    for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
        ...
        for (; node_it != trajectory_end; ++node_it) {
            ...
            const std::unique_ptr<transform::Rigid3d> fixed_frame_pose =
                Interpolate(fixed_frame_pose_data_, trajectory_id,
                            node_data.time);
            ...
            if (!fixed_frame_pose_initialized) {
                ...
                if (trajectory_data.fixed_frame_origin_in_map.has_value()) {
                    ...
                    fixed_frame_pose_in_map = node_data.global_pose *
                ...
                C_fixed_frames.emplace(
                    ...
                    std::forward_as_tuple(
                        transform::Rigid3d(
                            ...
                            Eigen::AngleAxisd(
                                transform::GetYaw(
                                    fixed_frame_pose_in_map.rotation()),
    ...
    if (options_.log_solver_summary()) {
        ...
        for (const auto& trajectory_id_and_data : trajectory_data_) {
            ...
            LOG(INFO) << "IMU correction was: "
                      << common::RadToDeg(
                             2. * std::acos(std::abs(imu_calibration[0])))
                      << " deg (" << imu_calibration[0] << ", "
                      << imu_calibration[1] << ", " << imu_calibration[2]
                      << ", " << imu_calibration[3] << ")";
        }
    }
