...
void AddLandmarkCostFunctions(const std::map<std::string, LandmarkNode>& landmark_nodes,
  ...
  for (const auto& landmark_node : landmark_nodes) {
    ...
    for (const auto& observation : landmark_node.second.landmark_observations) {
      ...
      problem->AddResidualBlock(LandmarkCostFunction3D::CreateAutoDiffCostFunction(observation, prev->data, next->data),
                                new ceres::HuberLoss(huber_scale), prev_node_pose->rotation(),
                                prev_node_pose->translation(), next_node_pose->rotation(),
                                next_node_pose->translation(), C_landmarks->at(landmark_id).rotation(),
...
void OptimizationProblem3D::TrimSubmap(const SubmapId& submap_id) { submap_data_.Trim(submap_id); }
...
void OptimizationProblem3D::Solve(const std::vector<Constraint>& constraints,
  ...
  for (const auto& submap_id_data : submap_data_) {
    ...
    if (first_submap) {
      ...
      C_submaps.Insert(submap_id_data.id, CeresPose(submap_id_data.data.global_pose, translation_parameterization(),
                                                    absl::make_unique<ceres::QuaternionManifold>(), &problem));
    }
    if (frozen) {
    ...
    for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
      ...
      for (++node_it; node_it != trajectory_end; ++node_it) {
        ...
        if (next_node_it != trajectory_end && next_node_it->id.node_index == second_node_id.node_index + 1) {
          ...
          problem.AddResidualBlock(
              AccelerationCostFunction3D::CreateAutoDiffCostFunction(
                  options_.acceleration_weight() / common::ToSeconds(first_duration + second_duration), delta_velocity,
        ...
        problem.AddResidualBlock(
            ...
            nullptr /* loss function */, C_nodes.at(first_node_id).rotation(), C_nodes.at(second_node_id).rotation(),
            trajectory_data.imu_calibration.data());
      }
        ...
        C_fixed_frames.emplace(
            ...
            std::forward_as_tuple(
                transform::Rigid3d(
                    ...
                    Eigen::AngleAxisd(transform::GetYaw(fixed_frame_pose_in_map.rotation()), Eigen::Vector3d::UnitZ())),
  ...
  if (options_.log_solver_summary()) {
    ...
    for (const auto& trajectory_id_and_data : trajectory_data_) {
      ...
      LOG(INFO) << "IMU correction was: " << common::RadToDeg(2. * std::acos(std::abs(imu_calibration[0]))) << " deg ("
