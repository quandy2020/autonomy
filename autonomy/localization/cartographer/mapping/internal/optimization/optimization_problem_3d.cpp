...
#include "autonomy/localization/cartographer/mapping/internal/optimization/optimization_problem_3d.hpp"
...
#include "Eigen/Core"
#include "autonomy/localization/cartographer/common/internal/ceres_solver_options.hpp"
#include "autonomy/localization/cartographer/common/math.hpp"
#include "autonomy/localization/cartographer/common/time.hpp"
#include "autonomy/localization/cartographer/mapping/internal/3d/imu_integration.hpp"
#include "autonomy/localization/cartographer/mapping/internal/3d/rotation_parameterization.hpp"
#include "autonomy/localization/cartographer/mapping/internal/optimization/ceres_pose.hpp"
#include "autonomy/localization/cartographer/mapping/internal/optimization/cost_functions/acceleration_cost_function_3d.hpp"
#include "autonomy/localization/cartographer/mapping/internal/optimization/cost_functions/landmark_cost_function_3d.hpp"
#include "autonomy/localization/cartographer/mapping/internal/optimization/cost_functions/rotation_cost_function_3d.hpp"
#include "autonomy/localization/cartographer/mapping/internal/optimization/cost_functions/spa_cost_function_3d.hpp"
...
// For odometry.
std::unique_ptr<transform::Rigid3d> Interpolate(const sensor::MapByTime<sensor::OdometryData>& map_by_time,
...
transform::Rigid3d GetInitialLandmarkPose(const LandmarkNode::LandmarkObservation& observation,
    ...
    const std::tuple<std::array<double, 4>, std::array<double, 3>> rotation_and_translation =
        InterpolateNodes3D(prev_node_pose.rotation(), prev_node_pose.translation(), next_node_pose.rotation(),
                           next_node_pose.translation(), interpolation_parameter);
...
void AddLandmarkCostFunctions(const std::map<std::string, LandmarkNode>& landmark_nodes,
                              const MapById<NodeId, NodeSpec3D>& node_data, MapById<NodeId, CeresPose>* C_nodes,
    ...
    for (const auto& landmark_node : landmark_nodes) {
        ...
        for (const auto& observation : landmark_node.second.landmark_observations) {
            ...
            // The landmark observation was made before the trajectory was
            // created.
            if (observation.time < begin_of_trajectory->data.time) {
                continue;
            }
            // Find the trajectory nodes before and after the landmark
            // observation.
            auto next = node_data.lower_bound(observation.trajectory_id, observation.time);
            // The landmark observation was made, but the next trajectory node
            ...
            // Add parameter blocks for the landmark ID if they were not added
            // before.
            ...
            if (!C_landmarks->count(landmark_id)) {
                ...
                C_landmarks->emplace(landmark_id, CeresPose(starting_point, nullptr /* translation_parametrization */,
                                                            std::make_unique<ceres::QuaternionManifold>(), problem));
            ...
            problem->AddResidualBlock(
                LandmarkCostFunction3D::CreateAutoDiffCostFunction(observation, prev->data, next->data),
                new ceres::HuberLoss(huber_scale), prev_node_pose->rotation(), prev_node_pose->translation(),
                next_node_pose->rotation(), next_node_pose->translation(), C_landmarks->at(landmark_id).rotation(),
...
OptimizationProblem3D::OptimizationProblem3D(const optimization::proto::OptimizationProblemOptions& options)
    : options_(options) {}

OptimizationProblem3D::~OptimizationProblem3D() {}

void OptimizationProblem3D::AddImuData(const int trajectory_id, const sensor::ImuData& imu_data) {
    imu_data_.Append(trajectory_id, imu_data);
}

void OptimizationProblem3D::AddOdometryData(const int trajectory_id, const sensor::OdometryData& odometry_data) {
    odometry_data_.Append(trajectory_id, odometry_data);
}

void OptimizationProblem3D::AddFixedFramePoseData(const int trajectory_id,
                                                  const sensor::FixedFramePoseData& fixed_frame_pose_data) {
    fixed_frame_pose_data_.Append(trajectory_id, fixed_frame_pose_data);
}

void OptimizationProblem3D::AddTrajectoryNode(const int trajectory_id, const NodeSpec3D& node_data) {
    node_data_.Append(trajectory_id, node_data);
    trajectory_data_[trajectory_id];
}

void OptimizationProblem3D::SetTrajectoryData(int trajectory_id, const TrajectoryData& trajectory_data) {
    trajectory_data_[trajectory_id] = trajectory_data;
}

void OptimizationProblem3D::InsertTrajectoryNode(const NodeId& node_id, const NodeSpec3D& node_data) {
...
void OptimizationProblem3D::AddSubmap(const int trajectory_id, const transform::Rigid3d& global_submap_pose) {
    submap_data_.Append(trajectory_id, SubmapSpec3D{global_submap_pose});
}

void OptimizationProblem3D::InsertSubmap(const SubmapId& submap_id, const transform::Rigid3d& global_submap_pose) {
...
void OptimizationProblem3D::Solve(const std::vector<Constraint>& constraints,
    ...
    const auto translation_parameterization = [this]() -> std::unique_ptr<ceres::Manifold> {
        return options_.fix_z_in_3d() ? std::make_unique<ceres::SubsetManifold>(3, std::vector<int>{2}) : nullptr;
    };
    ...
    for (const auto& submap_id_data : submap_data_) {
        ...
        if (first_submap) {
            ...
            C_submaps.Insert(
                submap_id_data.id,
                CeresPose(submap_id_data.data.global_pose, translation_parameterization(),
                          std::make_unique<ceres::AutoDiffManifold<ConstantYawQuaternionPlus, 4, 2>>(), &problem));
            problem.SetParameterBlockConstant(C_submaps.at(submap_id_data.id).translation());
        } else {
        ...
        if (frozen) {
            problem.SetParameterBlockConstant(C_submaps.at(submap_id_data.id).rotation());
            problem.SetParameterBlockConstant(C_submaps.at(submap_id_data.id).translation());
        }
    }
    for (const auto& node_id_data : node_data_) {
        const bool frozen = frozen_trajectories.count(node_id_data.id.trajectory_id) != 0;
        C_nodes.Insert(node_id_data.id, CeresPose(node_id_data.data.global_pose, translation_parameterization(),
                                                  std::make_unique<ceres::QuaternionManifold>(), &problem));
        if (frozen) {
            problem.SetParameterBlockConstant(C_nodes.at(node_id_data.id).rotation());
            problem.SetParameterBlockConstant(C_nodes.at(node_id_data.id).translation());
        }
    }
    ...
    for (const Constraint& constraint : constraints) {
        problem.AddResidualBlock(
            ...
            C_submaps.at(constraint.submap_id).rotation(), C_submaps.at(constraint.submap_id).translation(),
            C_nodes.at(constraint.node_id).rotation(), C_nodes.at(constraint.node_id).translation());
    }
    ...
    if (!options_.fix_z_in_3d()) {
        for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
            ...
            problem.SetManifold(trajectory_data.imu_calibration.data(), new ceres::QuaternionManifold());
            ...
            for (++node_it; node_it != trajectory_end; ++node_it) {
                ...
                while (std::next(imu_it) != imu_data.end() && std::next(imu_it)->time <= first_node_data.time) {
                    ++imu_it;
                }

                auto imu_it2 = imu_it;
                const IntegrateImuResult<double> result =
                    IntegrateImu(imu_data, first_node_data.time, second_node_data.time, &imu_it);
                ...
                if (next_node_it != trajectory_end && next_node_it->id.node_index == second_node_id.node_index + 1) {
                    ...
                    // 'delta_velocity' is the change in velocity from the point
                    // in time halfway between the first and second poses to
                    // halfway between second and third pose. It is computed
                    // from IMU data and still contains a delta due to gravity.
                    // The orientation of this vector is in the IMU frame at the
                    // second pose.
                    ...
                    problem.AddResidualBlock(
                        AccelerationCostFunction3D::CreateAutoDiffCostFunction(
                            options_.acceleration_weight() / common::ToSeconds(first_duration + second_duration),
                            delta_velocity, common::ToSeconds(first_duration), common::ToSeconds(second_duration)),
                        nullptr /* loss function */, C_nodes.at(second_node_id).rotation(),
                        C_nodes.at(first_node_id).translation(), C_nodes.at(second_node_id).translation(),
                        C_nodes.at(third_node_id).translation(), &trajectory_data.gravity_constant,
                        trajectory_data.imu_calibration.data());
                    gravity_block_added = true;
                }
                problem.AddResidualBlock(
                    RotationCostFunction3D::CreateAutoDiffCostFunction(
                        options_.rotation_weight() / common::ToSeconds(first_duration), result.delta_rotation),
                    nullptr /* loss function */, C_nodes.at(first_node_id).rotation(),
                    C_nodes.at(second_node_id).rotation(), trajectory_data.imu_calibration.data());
            }
    ...
    if (options_.fix_z_in_3d()) {
        // Add penalties for violating odometry (if available) and changes
        ...
        for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
            ...
            for (++node_it; node_it != trajectory_end; ++node_it) {
                ...
                // Add a relative pose constraint based on the odometry (if
                // available).
                ...
                if (relative_odometry != nullptr) {
                    problem.AddResidualBlock(SpaCostFunction3D::CreateAutoDiffCostFunction(Constraint::Pose{
                ...
                // Add a relative pose constraint based on consecutive local
                // SLAM poses.
                ...
                problem.AddResidualBlock(SpaCostFunction3D::CreateAutoDiffCostFunction(Constraint::Pose{
                                         ...
                                         nullptr /* loss function */, C_nodes.at(first_node_id).rotation(),
                                         C_nodes.at(first_node_id).translation(), C_nodes.at(second_node_id).rotation(),
                                         C_nodes.at(second_node_id).translation());
            }
        }
    }
        ...
        for (; node_it != trajectory_end; ++node_it) {
            ...
            if (!fixed_frame_pose_initialized) {
                ...
                if (trajectory_data.fixed_frame_origin_in_map.has_value()) {
                    ...
                    fixed_frame_pose_in_map = node_data.global_pose * constraint_pose.zbar_ij.inverse();
                }
                C_fixed_frames.emplace(
                    std::piecewise_construct, std::forward_as_tuple(trajectory_id),
                    std::forward_as_tuple(
                        transform::Rigid3d(fixed_frame_pose_in_map.translation(),
                                           Eigen::AngleAxisd(transform::GetYaw(fixed_frame_pose_in_map.rotation()),
                                                             Eigen::Vector3d::UnitZ())),
                        nullptr, std::make_unique<ceres::AutoDiffManifold<YawOnlyQuaternionPlus, 4, 1>>(), &problem));
                fixed_frame_pose_initialized = true;
            }

            problem.AddResidualBlock(SpaCostFunction3D::CreateAutoDiffCostFunction(constraint_pose),
    ...
    if (options_.log_solver_summary()) {
        ...
        for (const auto& trajectory_id_and_data : trajectory_data_) {
            ...
            LOG(INFO) << "IMU correction was: " << common::RadToDeg(2. * std::acos(std::abs(imu_calibration[0])))
                      << " deg (" << imu_calibration[0] << ", " << imu_calibration[1] << ", " << imu_calibration[2]
                      << ", " << imu_calibration[3] << ")";
        }
    }
    ...
    for (const auto& C_submap_id_data : C_submaps) {
        submap_data_.at(C_submap_id_data.id).global_pose = C_submap_id_data.data.ToRigid();
    }
    for (const auto& C_node_id_data : C_nodes) {
        node_data_.at(C_node_id_data.id).global_pose = C_node_id_data.data.ToRigid();
    }
    for (const auto& C_fixed_frame : C_fixed_frames) {
        trajectory_data_.at(C_fixed_frame.first).fixed_frame_origin_in_map = C_fixed_frame.second.ToRigid();
    }
...
std::unique_ptr<transform::Rigid3d> OptimizationProblem3D::CalculateOdometryBetweenNodes(
    const int trajectory_id, const NodeSpec3D& first_node_data, const NodeSpec3D& second_node_data) const {
    if (odometry_data_.HasTrajectory(trajectory_id)) {
        ...
        if (first_node_odometry != nullptr && second_node_odometry != nullptr) {
            const transform::Rigid3d relative_odometry = first_node_odometry->inverse() * (*second_node_odometry);
