...
#include "autonomy/localization/cartographer/mapping/internal/optimization/optimization_problem_2d.hpp"
...
#include "autonomy/localization/cartographer/mapping/internal/optimization/cost_functions/landmark_cost_function_2d.hpp"
#include "autonomy/localization/cartographer/mapping/internal/optimization/cost_functions/spa_cost_function_2d.hpp"
...
transform::Rigid3d GetInitialLandmarkPose(
    ...
    const std::tuple<std::array<double, 4>, std::array<double, 3>>
        rotation_and_translation = InterpolateNodes2D(
            prev_node_pose.data(), prev_node.gravity_alignment,
...
void OptimizationProblem2D::Solve(
    ...
    for (const auto& submap_id_data : submap_data_) {
        ...
        if (first_submap || frozen) {
            ...
            // trajectory.
            problem.SetParameterBlockConstant(
                C_submaps.at(submap_id_data.id).data());
        }
    }
    for (const auto& node_id_data : node_data_) {
        ...
        if (frozen) {
            problem.SetParameterBlockConstant(
                C_nodes.at(node_id_data.id).data());
        }
    }
    ...
    for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
        ...
        for (++node_it; node_it != trajectory_end; ++node_it) {
            ...
            // Add a relative pose constraint based on consecutive local SLAM
            // poses.
