...
TEST(LandmarkCostFunction3DTest, SmokeTest) {
    ...
    std::unique_ptr<ceres::CostFunction> cost_function(LandmarkCostFunction3D::CreateAutoDiffCostFunction(
        LandmarkObservation{
            0 /* trajectory ID */,
    ...
    const std::array<const double*, 6> parameter_blocks{{prev_node_rotation.data(), prev_node_translation.data(),
    ...
    EXPECT_THAT(residuals,
                ElementsAre(DoubleEq(1.), DoubleEq(0.), DoubleEq(0.), DoubleEq(0.), DoubleEq(0.), DoubleEq(0.)));
}
