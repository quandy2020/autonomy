...
std::unique_ptr<::cartographer::common::LuaParameterDictionary> ResolveLuaParameters(const std::string& lua_code) {
  auto file_resolver = absl::make_unique<::cartographer::common::ConfigurationFileResolver>(
      std::vector<std::string>{std::string(::cartographer::common::kSourceDirectory) + "/configuration_files"});
  return absl::make_unique<::cartographer::common::LuaParameterDictionary>(lua_code, std::move(file_resolver));
}

std::vector<cartographer::sensor::TimedPointCloudData> GenerateFakeRangeMeasurements(double travel_distance,
                                                                                     double duration,
                                                                                     double time_step) {
  const Eigen::Vector3f kDirection = Eigen::Vector3f(2., 1., 0.).normalized();
  return GenerateFakeRangeMeasurements(kDirection * travel_distance, duration, time_step,
                                       transform::Rigid3f::Identity());
}

std::vector<cartographer::sensor::TimedPointCloudData> GenerateFakeRangeMeasurements(
    const Eigen::Vector3f& translation, double duration, double time_step, const transform::Rigid3f& local_to_global) {
  ...
  for (double angle = 0.; angle < M_PI; angle += 0.01) {
    for (double height : {-0.4, -0.2, 0.0, 0.2, 0.4}) {
      ...
      point_cloud.push_back(
          {Eigen::Vector3d{kRadius * std::cos(angle), kRadius * std::sin(angle), height}.cast<float>(), 0.});
    }
  }
  ...
  for (double elapsed_time = 0.; elapsed_time < duration; elapsed_time += time_step) {
    cartographer::common::Time time =
        cartographer::common::FromUniversal(123) + cartographer::common::FromSeconds(elapsed_time);
    cartographer::transform::Rigid3f global_pose =
        local_to_global * cartographer::transform::Rigid3f::Translation(elapsed_time * kVelocity);
    cartographer::sensor::TimedPointCloud ranges =
        cartographer::sensor::TransformTimedPointCloud(point_cloud, global_pose.inverse());
    measurements.emplace_back(cartographer::sensor::TimedPointCloudData{time, Eigen::Vector3f::Zero(), ranges});
  }
  return measurements;
}

proto::Submap CreateFakeSubmap3D(int trajectory_id, int submap_index, bool finished) {
  proto::Submap proto;
  ...
  *proto.mutable_submap_3d()->mutable_local_pose() = transform::ToProto(transform::Rigid3d::Identity());
...
proto::Node CreateFakeNode(int trajectory_id, int node_index) {
  ...
  *proto.mutable_node_data()->mutable_local_pose() = transform::ToProto(transform::Rigid3d::Identity());
  return proto;
}
...
proto::PoseGraph::Constraint CreateFakeConstraint(const proto::Node& node, const proto::Submap& submap) {
  ...
  proto.mutable_submap_id()->set_submap_index(submap.submap_id().submap_index());
  proto.mutable_submap_id()->set_trajectory_id(submap.submap_id().trajectory_id());
  ...
  transform::Rigid3d pose(Eigen::Vector3d(2., 3., 4.), Eigen::AngleAxisd(M_PI / 8., Eigen::Vector3d::UnitX()));
...
proto::PoseGraph::LandmarkPose CreateFakeLandmark(const std::string& landmark_id,
...
void AddToProtoGraph(const proto::Node& node_data, proto::PoseGraph* pose_graph) {
  auto* trajectory = CreateTrajectoryIfNeeded(node_data.node_id().trajectory_id(), pose_graph);
