...
class FakeTrimmable : public Trimmable {
 public:
  ...
  int num_submaps(const int trajectory_id) const override { return submap_data_.size() - trimmed_submaps_.size(); }
  ...
  MapById<SubmapId, PoseGraphInterface::SubmapData>* mutable_submap_data() { return &submap_data_; }

  MapById<SubmapId, PoseGraphInterface::SubmapData> GetOptimizedSubmapData() const override { return submap_data_; }

  void set_trajectory_nodes(const MapById<NodeId, TrajectoryNode>& trajectory_nodes) {
    trajectory_nodes_ = trajectory_nodes;
  }

  MapById<NodeId, TrajectoryNode>* mutable_trajectory_nodes() { return &trajectory_nodes_; }

  const MapById<NodeId, TrajectoryNode>& GetTrajectoryNodes() const override { return trajectory_nodes_; }

  void set_constraints(const std::vector<PoseGraphInterface::Constraint>& constraints) { constraints_ = constraints; }

  std::vector<PoseGraphInterface::Constraint>* mutable_constraints() { return &constraints_; }

  const std::vector<PoseGraphInterface::Constraint>& GetConstraints() const override { return constraints_; }

  void TrimSubmap(const SubmapId& submap_id) override { trimmed_submaps_.push_back(submap_id); }
