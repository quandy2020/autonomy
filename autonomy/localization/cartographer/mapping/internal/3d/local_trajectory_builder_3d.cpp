...
LocalTrajectoryBuilder3D::AddRangeData(
    ...
    for (const auto& point_cloud_origin_data :
        ...
        for (const auto& hit : point_cloud_origin_data.ranges) {
            ...
            if (time_point < prev_time_point) {
                if (!warned) {
                    LOG(ERROR)
                        << "Timestamp of individual range data point jumps "
                           "backwards from "
    ...
    if (options_.use_intensities()) {
        accumulated_intensities.reserve(
            max_possible_number_of_accumulated_points);
    }
    ...
    for (const auto& point_cloud_origin_data :
        ...
        for (const auto& hit : point_cloud_origin_data.ranges) {
            ...
            const Eigen::Vector3f origin_in_local =
                *hits_poses_it *
                point_cloud_origin_data.origins.at(hit.origin_index);
