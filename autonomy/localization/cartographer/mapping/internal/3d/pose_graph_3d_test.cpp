...
public:
    ...
    void WaitForAllComputations() {
        PoseGraph3D::WaitForAllComputations();
    }
};
...
{
protected:
    PoseGraph3DTest() : thread_pool_(std::make_unique<common::ThreadPool>(1)) {}
    ...
    void BuildPoseGraph() {
        auto optimization_problem =
            std::make_unique<optimization::OptimizationProblem3D>(pose_graph_options_.optimization_problem_options());
        pose_graph_ = std::make_unique<PoseGraph3DForTesting>(pose_graph_options_, std::move(optimization_problem),
                                                               thread_pool_.get());
    }

    void BuildPoseGraphWithFakeOptimization() {
        auto optimization_problem = std::make_unique<MockOptimizationProblem3D>();
        pose_graph_ = std::make_unique<PoseGraph3DForTesting>(pose_graph_options_, std::move(optimization_problem),
                                                               thread_pool_.get());
    }
...
TEST_F(PoseGraph3DTest, PureLocalizationTrimmer) {
    ...
    for (int i = 0; i < num_submaps_to_create; ++i) {
        ...
        for (int j = 0; j < num_nodes_per_submap; ++j) {
            ...
            // TODO(gaschler): Also remove inter constraints when all references
            // are gone.
    ...
    pose_graph_->AddTrimmer(std::make_unique<PureLocalizationTrimmer>(trajectory_id, num_submaps_to_keep));
    pose_graph_->WaitForAllComputations();
...
TEST_F(PoseGraph3DTest, EvenSubmapTrimmer) {
    ...
    pose_graph_->AddTrimmer(std::make_unique<EvenSubmapTrimmer>(trajectory_id));
    pose_graph_->WaitForAllComputations();
    EXPECT_EQ(pose_graph_->GetAllSubmapPoses().SizeOfTrajectoryOrZero(trajectory_id), num_submaps_to_create);
    EXPECT_EQ(pose_graph_->GetAllSubmapData().SizeOfTrajectoryOrZero(trajectory_id), num_submaps_to_create);
    EXPECT_EQ(pose_graph_->GetTrajectoryNodes().SizeOfTrajectoryOrZero(trajectory_id),
