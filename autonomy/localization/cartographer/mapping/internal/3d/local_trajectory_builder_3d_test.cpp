...
#include "autonomy/localization/cartographer/mapping/internal/3d/local_trajectory_builder_options_3d.hpp"
...
class LocalTrajectoryBuilderTest : public ::testing::Test {
 protected:
  ...
  mapping::proto::LocalTrajectoryBuilderOptions3D CreateTrajectoryBuilderOptions3D() {
    ...
    return mapping::CreateLocalTrajectoryBuilderOptions3D(parameter_dictionary.get());
  }
  ...
  Eigen::Vector3f CollideWithBox(const Eigen::Vector3f& from, const Eigen::Vector3f& to) {
    float first = 100.f;
    if (to.x() > from.x()) {
  ...
  Eigen::Vector3f CollideWithBubbles(const Eigen::Vector3f& from, const Eigen::Vector3f& to) {
    float first = 1.f;
    ...
    for (const Eigen::Vector3f& center : bubbles_) {
      ...
      const float c = (from - center).squaredNorm() - kBubbleRadius * kBubbleRadius;
  ...
  sensor::TimedPointCloudData GeneratePointCloudData(const transform::Rigid3d& pose, const common::Time time) {
    ...
    for (int r = -8; r != 8; ++r) {
      for (int s = -250; s != 250; ++s) {
        const sensor::TimedRangefinderPoint first_point{
            Eigen::Vector3f{Eigen::AngleAxisf(M_PI * s / 250., Eigen::Vector3f::UnitZ()) *
        ...
        const sensor::TimedRangefinderPoint second_point{
            Eigen::Vector3f{Eigen::AngleAxisf(M_PI / 2., Eigen::Vector3f::UnitX()) *
    ...
    for (const auto& direction_in_world_frame :
         sensor::TransformTimedPointCloud(directions_in_rangefinder_frame, pose.cast<float>())) {
      const Eigen::Vector3f origin = pose.cast<float>() * Eigen::Vector3f::Zero();
      const sensor::TimedRangefinderPoint return_point{
          CollideWithBubbles(origin, CollideWithBox(origin, direction_in_world_frame.position)), 0.};
      returns_in_world_frame.push_back(return_point);
    }
    return {time, Eigen::Vector3f::Zero(),
            sensor::TransformTimedPointCloud(returns_in_world_frame, pose.inverse().cast<float>())};
  }

  void AddLinearOnlyImuObservation(const common::Time time, const transform::Rigid3d& expected_pose) {
    const Eigen::Vector3d gravity = expected_pose.rotation().inverse() * Eigen::Vector3d(0., 0., 9.81);
    local_trajectory_builder_->AddImuData(sensor::ImuData{time, gravity, Eigen::Vector3d::Zero()});
  }
  ...
  std::vector<TrajectoryNode> GenerateCorkscrewTrajectory() {
    ...
    for (int i = 0; i != 5; ++i) {
      ...
      trajectory.push_back(TrajectoryNode{current_time, transform::Rigid3d::Identity()});
    }
    ...
    for (double t = 0.; t <= 0.6; t += 0.05) {
      ...
      trajectory.push_back(TrajectoryNode{
          current_time,
          transform::Rigid3d::Translation(Eigen::Vector3d(std::sin(4. * t), 1. - std::cos(4. * t), 1. * t)) *
              transform::Rigid3d::Rotation(Eigen::AngleAxisd(0.3 * t, Eigen::Vector3d(1., -1., 2.).normalized()))});
    }
    return trajectory;
  }

  void VerifyAccuracy(const std::vector<TrajectoryNode>& expected_trajectory, double expected_accuracy) {
    int num_poses = 0;
    for (const TrajectoryNode& node : expected_trajectory) {
      ...
      const std::unique_ptr<LocalTrajectoryBuilder3D::MatchingResult> matching_result =
          local_trajectory_builder_->AddRangeData(kSensorId, point_cloud);
      if (matching_result != nullptr) {
        EXPECT_THAT(matching_result->local_pose, transform::IsNearly(node.pose, 1e-1));
        ++num_poses;
...
TEST_F(LocalTrajectoryBuilderTest, MoveInsideCubeUsingOnlyCeresScanMatcher) {
  local_trajectory_builder_.reset(new LocalTrajectoryBuilder3D(CreateTrajectoryBuilderOptions3D(), {kSensorId}));
