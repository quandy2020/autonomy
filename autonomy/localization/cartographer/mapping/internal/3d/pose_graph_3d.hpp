...
#include "autonomy/localization/cartographer/mapping/internal/pose_graph_data.hpp"
#include "autonomy/localization/cartographer/mapping/internal/trajectory_connectivity_state.hpp"
#include "autonomy/localization/cartographer/mapping/internal/work_queue.hpp"
#include "autonomy/localization/cartographer/mapping/pose_graph.hpp"
#include "autonomy/localization/cartographer/mapping/pose_graph_trimmer.hpp"
...
public:
    ...
    NodeId AddNode(std::shared_ptr<const TrajectoryNode::Data> constant_data, int trajectory_id,
                   const std::vector<std::shared_ptr<const Submap3D>>& insertion_submaps) LOCKS_EXCLUDED(mutex_);
...
private:
    ...
    std::vector<SubmapId> InitializeGlobalSubmapPoses(
        ...
        const std::vector<std::shared_ptr<const Submap3D>>& insertion_submaps) EXCLUSIVE_LOCKS_REQUIRED(mutex_);
    ...
    // queue.
    void HandleWorkQueue(const constraints::ConstraintBuilder3D::Result& result) LOCKS_EXCLUDED(mutex_)
        LOCKS_EXCLUDED(work_queue_mutex_);
    ...
    transform::Rigid3d ComputeLocalToGlobalTransform(
        const MapById<SubmapId, optimization::SubmapSpec3D>& global_submap_poses, int trajectory_id) const
        EXCLUSIVE_LOCKS_REQUIRED(mutex_);
    ...
    std::unordered_map<int, std::unique_ptr<common::FixedRatioSampler>> global_localization_samplers_
        GUARDED_BY(mutex_);
