...
class RealTimeCorrelativeScanMatcher3DTest : public ::testing::Test {
 protected:
  RealTimeCorrelativeScanMatcher3DTest()
      : hybrid_grid_(0.1f), expected_pose_(Eigen::Vector3d(-1., 0., 0.), Eigen::Quaterniond::Identity()) {
    ...
    real_time_correlative_scan_matcher_.reset(
        new RealTimeCorrelativeScanMatcher3D(CreateRealTimeCorrelativeScanMatcherOptions(parameter_dictionary.get())));
  }

  void TestFromInitialPose(const transform::Rigid3d& initial_pose) {
    transform::Rigid3d pose;

    const float score = real_time_correlative_scan_matcher_->Match(initial_pose, point_cloud_, hybrid_grid_, &pose);
...
TEST_F(RealTimeCorrelativeScanMatcher3DTest, RotationAroundX) {
  TestFromInitialPose(transform::Rigid3d(Eigen::Vector3d(-1., 0., 0.),
