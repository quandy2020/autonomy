...
#include "autonomy/localization/cartographer/mapping/3d/range_data_inserter_3d.hpp"
 ...
 protected:
  ...
  transform::Rigid3f GetRandomPose() {
    ...
    return transform::Rigid3f::Translation(Eigen::Vector3f(x, y, z)) *
           transform::Rigid3f::Rotation(Eigen::AngleAxisf(theta, Eigen::Vector3f::UnitZ()));
  }

  static proto::FastCorrelativeScanMatcherOptions3D CreateFastCorrelativeScanMatcher3DTestOptions3D(
    ...
    auto parameter_dictionary = common::MakeDictionary(
        ...
        // Unknown space has kMinProbability = 0.1, so we need to make sure
        // here to pick a larger number otherwise we always find matches.
        "min_low_resolution_score = 0.15, "
        "linear_xy_search_window = 0.8, "
        "linear_z_search_window = 0.8, "
        "angular_search_window = 0.3, "
        "}");
    return CreateFastCorrelativeScanMatcherOptions3D(parameter_dictionary.get());
  }

  static mapping::proto::RangeDataInserterOptions3D CreateRangeDataInserterTestOptions3D() {
    auto parameter_dictionary = common::MakeDictionary(
        "return { "
        "hit_probability = 0.7, "
  ...
  std::unique_ptr<FastCorrelativeScanMatcher3D> GetFastCorrelativeScanMatcher(
      const proto::FastCorrelativeScanMatcherOptions3D& options, const transform::Rigid3f& pose) {
    hybrid_grid_ = absl::make_unique<HybridGrid>(0.05f);
    range_data_inserter_.Insert(
        sensor::RangeData{pose.translation(), sensor::TransformPointCloud(point_cloud_, pose), {}}, hybrid_grid_.get(),
        /*intensity_hybrid_grid=*/nullptr);
    hybrid_grid_->FinishUpdate();

    return absl::make_unique<FastCorrelativeScanMatcher3D>(*hybrid_grid_, hybrid_grid_.get(),
                                                           &GetRotationalScanMatcherHistogram(), options);
  }

  TrajectoryNode::Data CreateConstantData(const sensor::PointCloud& low_resolution_point_cloud) {
    return TrajectoryNode::Data{common::FromUniversal(0),   Eigen::Quaterniond::Identity(),     {}, point_cloud_,
                                low_resolution_point_cloud, GetRotationalScanMatcherHistogram()};
  }
  ...
  std::uniform_real_distribution<float> distribution_ = std::uniform_real_distribution<float>(-1.f, 1.f);
  ...
  const Eigen::VectorXf rotational_scan_matcher_histogram_ = Eigen::VectorXf::Zero(10);
};
...
TEST_F(FastCorrelativeScanMatcher3DTest, CorrectPoseForMatch) {
  for (int i = 0; i != 20; ++i) {
    ...
    const std::unique_ptr<FastCorrelativeScanMatcher3D::Result> result = fast_correlative_scan_matcher->Match(
        transform::Rigid3d::Identity(), transform::Rigid3d::Identity(), CreateConstantData(point_cloud_), kMinScore);
    ...
    EXPECT_THAT(expected_pose, transform::IsNearly(result->pose_estimate.cast<float>(), 0.05f))
        << "Actual: " << transform::ToProto(result->pose_estimate).DebugString()
        << "\nExpected: " << transform::ToProto(expected_pose).DebugString();

    const std::unique_ptr<FastCorrelativeScanMatcher3D::Result> low_resolution_result =
        fast_correlative_scan_matcher->Match(
            transform::Rigid3d::Identity(), transform::Rigid3d::Identity(),
            CreateConstantData(sensor::PointCloud({{Eigen::Vector3f(42.f, 42.f, 42.f)}})), kMinScore);
    EXPECT_THAT(low_resolution_result, testing::IsNull()) << low_resolution_result->low_resolution_score;
  }
}
...
TEST_F(FastCorrelativeScanMatcher3DTest, CorrectPoseForMatchFullSubmap) {
  ...
  const std::unique_ptr<FastCorrelativeScanMatcher3D::Result> result = fast_correlative_scan_matcher->MatchFullSubmap(
      Eigen::Quaterniond::Identity(), Eigen::Quaterniond::Identity(), CreateConstantData(point_cloud_), kMinScore);
  EXPECT_THAT(result, testing::NotNull());
  EXPECT_LT(kMinScore, result->score);
  EXPECT_LT(0.09f, result->rotational_score);
  EXPECT_LT(0.14f, result->low_resolution_score);
