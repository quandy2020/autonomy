...
protected:
    ...
    std::unique_ptr<FastCorrelativeScanMatcher3D> GetFastCorrelativeScanMatcher(
        ...
        range_data_inserter_.Insert(
            sensor::RangeData{pose.translation(), sensor::TransformPointCloud(point_cloud_, pose), {}},
            hybrid_grid_.get(),
...
TEST_F(FastCorrelativeScanMatcher3DTest, CorrectPoseForMatch) {
    for (int i = 0; i != 20; ++i) {
        ...
        const std::unique_ptr<FastCorrelativeScanMatcher3D::Result> result =
            fast_correlative_scan_matcher->Match(transform::Rigid3d::Identity(), transform::Rigid3d::Identity(),
