...
public:
    ...
    static Eigen::VectorXf ComputeHistogram(
        const sensor::PointCloud& point_cloud, int histogram_size);

    explicit RotationalScanMatcher(const Eigen::VectorXf* histogram);

    // Scores how well 'histogram' rotated by 'initial_angle' can be understood
    // as further rotated by certain 'angles' relative to the 'nodes'. Each
    // angle results in a score between 0 (worst) and 1 (best).
