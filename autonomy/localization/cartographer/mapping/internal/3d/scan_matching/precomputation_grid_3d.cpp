...
// that property.
inline int DivideByTwoRoundingTowardsNegativeInfinity(const int value) { return value >> 1; }
...
Eigen::Array3i CellIndexAtHalfResolution(const Eigen::Array3i& cell_index) {
  return Eigen::Array3i(DivideByTwoRoundingTowardsNegativeInfinity(cell_index[0]),
...
PrecomputationGrid3D ConvertToPrecomputationGrid(const HybridGrid& hybrid_grid) {
  ...
  for (auto it = HybridGrid::Iterator(hybrid_grid); !it.Done(); it.Next()) {
    const int cell_value = common::RoundToInt((ValueToProbability(it.GetValue()) - kMinProbability) *
...
PrecomputationGrid3D PrecomputeGrid(const PrecomputationGrid3D& grid, const bool half_resolution,
  ...
  for (auto it = PrecomputationGrid3D::Iterator(grid); !it.Done(); it.Next()) {
    for (int i = 0; i != 8; ++i) {
      ...
      // If 'shift' is 2 ** (depth - 1), where depth 0 is the original
      // grid, this results in precomputation grids analogous to the 2D
      // case.
      const Eigen::Array3i cell_index = it.GetCellIndex() - shift * PrecomputationGrid3D::GetOctant(i);
      auto* const cell_value =
          result.mutable_value(half_resolution ? CellIndexAtHalfResolution(cell_index) : cell_index);
