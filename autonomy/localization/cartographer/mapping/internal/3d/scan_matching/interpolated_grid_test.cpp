...
#include "autonomy/localization/cartographer/mapping/internal/3d/scan_matching/interpolated_grid.hpp"
...
class InterpolatedGridTest : public ::testing::Test {
 protected:
  InterpolatedGridTest() : hybrid_grid_(0.1f), interpolated_grid_(hybrid_grid_) {
  ...
  for (double z = -1.; z < 3.; z += hybrid_grid_.resolution()) {
    for (double y = 1.; y < 5.; y += hybrid_grid_.resolution()) {
      for (double x = -8.; x < -2.; x += hybrid_grid_.resolution()) {
        EXPECT_NEAR(GetHybridGridProbability(x, y, z), interpolated_grid_.GetInterpolatedValue(x, y, z), 1e-6);
      }
    }
  }
}
  ...
  for (double z = -1.; z < 3.; z += hybrid_grid_.resolution()) {
    for (double y = 1.; y < 5.; y += hybrid_grid_.resolution()) {
      for (double x = -8.; x < -2.; x += hybrid_grid_.resolution()) {
        ...
        const float next_probability = GetHybridGridProbability(x + hybrid_grid_.resolution(), y, z);
        const float grid_difference = next_probability - start_probability;
        if (std::abs(grid_difference) < 1e-6f) {
          continue;
        }
        for (double sample = kSampleStep; sample < hybrid_grid_.resolution() - 2 * kSampleStep; sample += kSampleStep) {
          EXPECT_LT(0., grid_difference * (interpolated_grid_.GetInterpolatedValue(x + sample + kSampleStep, y, z) -
                                           interpolated_grid_.GetInterpolatedValue(x + sample, y, z)));
        }
      }
    }
  }
