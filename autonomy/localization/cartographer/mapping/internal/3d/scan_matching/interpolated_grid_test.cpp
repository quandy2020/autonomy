        ...
        for (double y = 1.; y < 5.; y += hybrid_grid_.resolution()) {
            for (double x = -8.; x < -2.; x += hybrid_grid_.resolution()) {
                EXPECT_NEAR(GetHybridGridProbability(x, y, z),
                            interpolated_grid_.GetInterpolatedValue(x, y, z),
                            1e-6);
            }
        }
    }
...
TEST_F(InterpolatedGridTest, MonotonicBehaviorBetweenGridPointsInX) {
    ...
    for (double z = -1.; z < 3.; z += hybrid_grid_.resolution()) {
        for (double y = 1.; y < 5.; y += hybrid_grid_.resolution()) {
            for (double x = -8.; x < -2.; x += hybrid_grid_.resolution()) {
                const float start_probability =
                    GetHybridGridProbability(x, y, z);
                const float next_probability = GetHybridGridProbability(
                    x + hybrid_grid_.resolution(), y, z);
                const float grid_difference =
                    next_probability - start_probability;
