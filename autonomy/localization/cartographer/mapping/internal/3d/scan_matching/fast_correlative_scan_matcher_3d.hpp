...
public:
    ...
    FastCorrelativeScanMatcher3D& operator=(
        const FastCorrelativeScanMatcher3D&) = delete;
...
private:
    ...
    Candidate3D BranchAndBound(
        const SearchParameters& search_parameters,
        const std::vector<DiscreteScan3D>& discrete_scans,
        const std::vector<Candidate3D>& candidates, int candidate_depth,
        float min_score) const;
