...
#include "autonomy/localization/cartographer/mapping/internal/3d/scan_matching/precomputation_grid_3d.hpp"
...
class PrecomputationGridStack3D {
 public:
  ...
  const PrecomputationGrid3D& Get(int depth) const { return precomputation_grids_.at(depth); }
...
class FastCorrelativeScanMatcher3D {
 public:
  ...
  FastCorrelativeScanMatcher3D(const HybridGrid& hybrid_grid, const HybridGrid* low_resolution_hybrid_grid,
  ...
  // 'Result' is only returned if a score above 'min_score' (excluding
  // equality) is possible.
  std::unique_ptr<Result> MatchFullSubmap(const Eigen::Quaterniond& global_node_rotation,
 ...
 private:
  ...
  std::unique_ptr<Result> MatchWithSearchParameters(const SearchParameters& search_parameters,
  ...
  DiscreteScan3D DiscretizeScan(const SearchParameters& search_parameters, const sensor::PointCloud& point_cloud,
                                const transform::Rigid3f& pose, float rotational_score) const;
  std::vector<DiscreteScan3D> GenerateDiscreteScans(const SearchParameters& search_parameters,
                                                    const sensor::PointCloud& point_cloud,
                                                    const Eigen::VectorXf& rotational_scan_matcher_histogram,
                                                    const Eigen::Quaterniond& gravity_alignment,
                                                    const transform::Rigid3f& global_node_pose,
                                                    const transform::Rigid3f& global_submap_pose) const;
  std::vector<Candidate3D> GenerateLowestResolutionCandidates(const SearchParameters& search_parameters,
                                                              int num_discrete_scans) const;
  void ScoreCandidates(int depth, const std::vector<DiscreteScan3D>& discrete_scans,
                       std::vector<Candidate3D>* const candidates) const;
  std::vector<Candidate3D> ComputeLowestResolutionCandidates(const SearchParameters& search_parameters,
  ...
  Candidate3D BranchAndBound(const SearchParameters& search_parameters,
                             ...
                             const std::vector<Candidate3D>& candidates, int candidate_depth, float min_score) const;
  transform::Rigid3f GetPoseFromCandidate(const std::vector<DiscreteScan3D>& discrete_scans,
