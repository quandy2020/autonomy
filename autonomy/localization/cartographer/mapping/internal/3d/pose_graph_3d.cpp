    ...
    if (insertion_submaps.size() == 1) {
        ...
        if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) {
            ...
            optimization_problem_->AddSubmap(
                trajectory_id,
                ComputeLocalToGlobalTransform(data_.global_submap_poses_3d,
                                              trajectory_id) *
...
NodeId PoseGraph3D::AppendNode(
    ...
    if (!CanAddWorkItemModifying(trajectory_id)) {
        LOG(WARNING)
            << "AddNode was called for finished or deleted trajectory.";
    }
    ...
    AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
        ...
        if (CanAddWorkItemModifying(trajectory_id)) {
            for (const auto& observation :
                 landmark_data.landmark_observations) {
...
WorkItem::Result PoseGraph3D::ComputeConstraintsForNode(
    ...
    std::set<NodeId> newly_finished_submap_node_ids;
    {
        ...
        optimization_problem_->AddTrajectoryNode(
            ...
            optimization::NodeSpec3D{constant_data->time, local_pose,
                                     global_pose});
...
void PoseGraph3D::HandleWorkQueue(
...

    {
        ...
        kConstraintsSameTrajectoryMetric->Set(
            inter_constraints_same_trajectory);
...
void PoseGraph3D::AddNodeFromProto(const transform::Rigid3d& global_pose,
...

    {
        ...
        data_.trajectory_nodes.Insert(
            node_id, TrajectoryNode{constant_data, global_pose});
    }
    ...
    AddWorkItem([this, node_id, global_pose]() LOCKS_EXCLUDED(mutex_) {
        ...
        optimization_problem_->InsertTrajectoryNode(
            node_id,
            optimization::NodeSpec3D{constant_data->time,
                                     constant_data->local_pose, global_pose});
        return WorkItem::Result::kDoNotRunOptimization;
    });
}
    ...
    for (const Constraint& constraint : data_.constraints) {
        if (constraint.tag == Constraint::Tag::INTRA_SUBMAP) {
            ...
            const cartographer::transform::Rigid3d optimized_submap_to_map =
                data_.global_submap_poses_3d.at(constraint.submap_id)
                    .global_pose;
            ...
            rotational_residual.Add(common::NormalizeAngleDifference(
                transform::GetAngle(residual)));
...
void PoseGraph3D::RunOptimization() {
    ...
    for (const int trajectory_id : node_data.trajectory_ids()) {
        for (const auto& node : node_data.trajectory(trajectory_id)) {
            data_.trajectory_nodes.at(node.id).global_pose =
...
MapById<NodeId, TrajectoryNodePose> PoseGraph3D::GetTrajectoryNodePoses()
    ...
    for (const auto& node_id_data : data_.trajectory_nodes) {
        ...
        node_poses.Insert(node_id_data.id,
                          TrajectoryNodePose{node_id_data.data.global_pose,
                                             constant_pose_data});
    }
    return node_poses;
}
    ...
    if (begin_it == end_it) {
        ...
        if (it != data_.initial_trajectory_poses.end()) {
            return GetInterpolatedGlobalTrajectoryPose(
                       it->second.to_trajectory_id, it->second.time) *
...
PoseGraph3D::TrimmingHandle::GetOptimizedSubmapData() const {
    ...
    for (const auto& submap_id_data : parent_->data_.submap_data) {
        if (submap_id_data.data.state != SubmapState::kFinished ||
            !parent_->data_.global_submap_poses_3d.Contains(
                submap_id_data.id)) {
            continue;
        }
        submaps.Insert(submap_id_data.id,
                       SubmapData{submap_id_data.data.submap,
                                  parent_->data_.global_submap_poses_3d
