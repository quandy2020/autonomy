...
#include "autonomy/localization/cartographer/mapping/internal/3d/pose_graph_3d.hpp"
...
#include "autonomy/localization/cartographer/mapping/proto/pose_graph/constraint_builder_options.pb.h"
#include "autonomy/localization/cartographer/sensor/compressed_point_cloud.hpp"
#include "autonomy/localization/cartographer/sensor/internal/voxel_filter.hpp"
...
std::vector<SubmapId> PoseGraph3D::InitializeGlobalSubmapPoses(
    ...
    if (insertion_submaps.size() == 1) {
        ...
        if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) {
            ...
            optimization_problem_->AddSubmap(
                trajectory_id, ComputeLocalToGlobalTransform(data_.global_submap_poses_3d, trajectory_id) *
    ...
    if (data_.submap_data.at(last_submap_id).submap == insertion_submaps.front()) {
        ...
        optimization_problem_->AddSubmap(
            trajectory_id,
            first_submap_pose * insertion_submaps[0]->local_pose().inverse() * insertion_submaps[1]->local_pose());
...
NodeId PoseGraph3D::AppendNode(std::shared_ptr<const TrajectoryNode::Data> constant_data, const int trajectory_id,
                               const std::vector<std::shared_ptr<const Submap3D>>& insertion_submaps,
    ...
    if (data_.submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0 ||
        ...
        // We grow 'data_.submap_data' as needed. This code assumes that the
        // first time we see a new submap is as 'insertion_submaps.back()'.
...
void PoseGraph3D::AddTrajectoryIfNeeded(const int trajectory_id) {
    ...
    if (!global_localization_samplers_[trajectory_id]) {
        global_localization_samplers_[trajectory_id] =
            std::make_unique<common::FixedRatioSampler>(options_.global_sampling_ratio());
    }
}

void PoseGraph3D::AddImuData(const int trajectory_id, const sensor::ImuData& imu_data) {
    AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
        std::lock_guard<std::mutex> locker(mutex_);
        if (CanAddWorkItemModifying(trajectory_id)) {
...
void PoseGraph3D::AddFixedFramePoseData(const int trajectory_id,
...
void PoseGraph3D::AddLandmarkData(int trajectory_id, const sensor::LandmarkData& landmark_data) {
    AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
        std::lock_guard<std::mutex> locker(mutex_);
        if (CanAddWorkItemModifying(trajectory_id)) {
...
void PoseGraph3D::ComputeConstraint(const NodeId& node_id, const SubmapId& submap_id) {
    const transform::Rigid3d global_node_pose = optimization_problem_->node_data().at(node_id).global_pose;

    const transform::Rigid3d global_submap_pose = optimization_problem_->submap_data().at(submap_id).global_pose;
    ...
    const Submap3D* submap;
    {
        ...
        if (node_id.trajectory_id == submap_id.trajectory_id ||
            node_time <
                last_connection_time + common::FromSeconds(options_.global_constraint_search_after_n_seconds())) {
            ...
            // In this situation, 'global_node_pose' and 'global_submap_pose'
            // have orientations agreeing on gravity. Their relationship
            // regarding yaw is arbitrary. Finding the correct yaw component
            // will be handled by the matching procedure in the
            // FastCorrelativeScanMatcher, and the given yaw is essentially
            // ignored.
    ...
    if (maybe_add_local_constraint) {
        constraint_builder_.MaybeAddConstraint(submap_id, submap, node_id, constant_data, global_node_pose,
                                               global_submap_pose);
...
WorkItem::Result PoseGraph3D::ComputeConstraintsForNode(const NodeId& node_id,
    ...
    std::set<NodeId> newly_finished_submap_node_ids;
    {
        ...
        const transform::Rigid3d global_pose = optimization_problem_->submap_data().at(matching_id).global_pose *
                                               insertion_submaps.front()->local_pose().inverse() * local_pose;
        optimization_problem_->AddTrajectoryNode(
            matching_id.trajectory_id, optimization::NodeSpec3D{constant_data->time, local_pose, global_pose});
        for (size_t i = 0; i < insertion_submaps.size(); ++i) {
            const SubmapId submap_id = submap_ids[i];
            // Even if this was the last node added to 'submap_id', the submap
            // will only be marked as finished in 'data_.submap_data' further
            // below.
            CHECK(data_.submap_data.at(submap_id).state == SubmapState::kNoConstraintSearch);
            data_.submap_data.at(submap_id).node_ids.emplace(node_id);
            const transform::Rigid3d constraint_transform = insertion_submaps[i]->local_pose().inverse() * local_pose;
            data_.constraints.push_back(Constraint{
                submap_id,
                node_id,
                {constraint_transform, options_.matcher_translation_weight(), options_.matcher_rotation_weight()},
...
common::Time PoseGraph3D::GetLatestNodeTime(const NodeId& node_id, const SubmapId& submap_id) const {
...
void PoseGraph3D::UpdateTrajectoryConnectivity(const Constraint& constraint) {
    ...
    data_.trajectory_connectivity_state.Connect(constraint.node_id.trajectory_id, constraint.submap_id.trajectory_id,
                                                time);
}
...
void PoseGraph3D::DeleteTrajectoriesIfNeeded() {
    ...
    for (auto& it : data_.trajectories_state) {
        if (it.second.deletion_state == InternalTrajectoryState::DeletionState::WAIT_FOR_DELETION) {
            // TODO(gaschler): Consider directly deleting from data_, which may
            // be more complete.
...
void PoseGraph3D::HandleWorkQueue(const constraints::ConstraintBuilder3D::Result& result) {
    {
        std::lock_guard<std::mutex> locker(mutex_);
...
void PoseGraph3D::DrainWorkQueue() {
    ...
    constraint_builder_.WhenDone(
        [this](const constraints::ConstraintBuilder3D::Result& result) { HandleWorkQueue(result); });
}
...
void PoseGraph3D::WaitForAllComputations() {
    ...
    // a WhenDone() callback.
    {
        ...
        while (work_queue_ != nullptr) {
            if (work_queue_cv_.wait_for(lock, common::FromSeconds(1), [this] { return work_queue_ == nullptr; })) {
                break;
            }
            lock.unlock();
            report_progress();
    ...
    constraint_builder_.WhenDone(
        [this, &notification](const constraints::ConstraintBuilder3D::Result& result) LOCKS_EXCLUDED(mutex_) {
            std::lock_guard<std::mutex> locker(mutex_);
            data_.constraints.insert(data_.constraints.end(), result.begin(), result.end());
            notification = true;
    ...
    while (!notification) {
        if (mutex_cv_.wait_for(lock, common::FromSeconds(1), [&notification] { return notification; })) {
            break;
        }
        lock.unlock();
        report_progress();
...
void PoseGraph3D::FreezeTrajectory(const int trajectory_id) {
    ...
    AddWorkItem([this, trajectory_id]() LOCKS_EXCLUDED(mutex_) {
        ...
        // This is required for localization against multiple frozen
        // trajectories because we lose inter-trajectory constraints when
        // freezing.
...
void PoseGraph3D::AddSubmapFromProto(const transform::Rigid3d& global_submap_pose, const proto::Submap& submap) {
    if (!submap.has_submap_3d()) {
        return;
    }

    const SubmapId submap_id = {submap.submap_id().trajectory_id(), submap.submap_id().submap_index()};
    std::shared_ptr<const Submap3D> submap_ptr = std::make_shared<const Submap3D>(submap.submap_3d());

    {
        ...
        data_.global_submap_poses_3d.Insert(submap_id, optimization::SubmapSpec3D{global_submap_pose});
    }

    // TODO(MichaelGrupp): MapBuilder does freezing before deserializing
    // submaps, so this should be fine.
...
void PoseGraph3D::AddNodeFromProto(const transform::Rigid3d& global_pose, const proto::Node& node) {
    ...
    AddWorkItem([this, node_id, global_pose]() LOCKS_EXCLUDED(mutex_) {
        ...
        optimization_problem_->InsertTrajectoryNode(
            node_id, optimization::NodeSpec3D{constant_data->time, constant_data->local_pose, global_pose});
        return WorkItem::Result::kDoNotRunOptimization;
    });
}

void PoseGraph3D::SetTrajectoryDataFromProto(const proto::TrajectoryData& data) {
    TrajectoryData trajectory_data;
    trajectory_data.gravity_constant = data.gravity_constant();
    trajectory_data.imu_calibration = {{data.imu_calibration().w(), data.imu_calibration().x(),
...
void PoseGraph3D::AddNodeToSubmap(const NodeId& node_id, const SubmapId& submap_id) {
...
void PoseGraph3D::AddSerializedConstraints(const std::vector<Constraint>& constraints) {
...
void PoseGraph3D::LogResidualHistograms() const {
    ...
    for (const Constraint& constraint : data_.constraints) {
        if (constraint.tag == Constraint::Tag::INTRA_SUBMAP) {
            ...
            const cartographer::transform::Rigid3d node_to_submap_constraint = constraint.pose.zbar_ij;
            ...
            rotational_residual.Add(common::NormalizeAngleDifference(transform::GetAngle(residual)));
...
void PoseGraph3D::RunOptimization() {
    ...
    // when executing the Solve. Solve is time consuming, so not taking the
    // mutex before Solve to avoid blocking foreground processing.
    ...
    for (const int trajectory_id : node_data.trajectory_ids()) {
        ...
        const auto local_to_old_global = ComputeLocalToGlobalTransform(data_.global_submap_poses_3d, trajectory_id);
...
std::map<int, PoseGraphInterface::TrajectoryState> PoseGraph3D::GetTrajectoryStates() const {
...
void PoseGraph3D::SetLandmarkPose(const std::string& landmark_id, const transform::Rigid3d& global_pose,
                                  const bool frozen) {
    AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
        std::lock_guard<std::mutex> locker(mutex_);
...
sensor::MapByTime<sensor::FixedFramePoseData> PoseGraph3D::GetFixedFramePoseData() const {
    std::lock_guard<std::mutex> locker(mutex_);
    return optimization_problem_->fixed_frame_pose_data();
}

std::map<std::string /* landmark ID */, PoseGraphInterface::LandmarkNode> PoseGraph3D::GetLandmarkNodes() const {
    std::lock_guard<std::mutex> locker(mutex_);
    return data_.landmark_nodes;
}

std::map<int, PoseGraphInterface::TrajectoryData> PoseGraph3D::GetTrajectoryData() const {
    std::lock_guard<std::mutex> locker(mutex_);
...
void PoseGraph3D::SetInitialTrajectoryPose(const int from_trajectory_id, const int to_trajectory_id,
...
transform::Rigid3d PoseGraph3D::GetInterpolatedGlobalTrajectoryPose(const int trajectory_id,
...
transform::Rigid3d PoseGraph3D::GetLocalToGlobalTransform(const int trajectory_id) const {
    std::lock_guard<std::mutex> locker(mutex_);
...
PoseGraphInterface::SubmapData PoseGraph3D::GetSubmapData(const SubmapId& submap_id) const {
    std::lock_guard<std::mutex> locker(mutex_);
    return GetSubmapDataUnderLock(submap_id);
}

MapById<SubmapId, PoseGraphInterface::SubmapData> PoseGraph3D::GetAllSubmapData() const {
    std::lock_guard<std::mutex> locker(mutex_);
    return GetSubmapDataUnderLock();
}

MapById<SubmapId, PoseGraphInterface::SubmapPose> PoseGraph3D::GetAllSubmapPoses() const {
    std::lock_guard<std::mutex> locker(mutex_);
    ...
    for (const auto& submap_id_data : data_.submap_data) {
        ...
        submap_poses.Insert(submap_id_data.id,
                            PoseGraphInterface::SubmapPose{submap_data.submap->num_range_data(), submap_data.pose});
    }
    return submap_poses;
}

transform::Rigid3d PoseGraph3D::ComputeLocalToGlobalTransform(
    const MapById<SubmapId, optimization::SubmapSpec3D>& global_submap_poses, const int trajectory_id) const {
...
PoseGraphInterface::SubmapData PoseGraph3D::GetSubmapDataUnderLock(const SubmapId& submap_id) const {
    ...
    return {submap, ComputeLocalToGlobalTransform(data_.global_submap_poses_3d, submap_id.trajectory_id) *
                        submap->local_pose()};
}

PoseGraph3D::TrimmingHandle::TrimmingHandle(PoseGraph3D* const parent) : parent_(parent) {}
...
std::vector<SubmapId> PoseGraph3D::TrimmingHandle::GetSubmapIds(int trajectory_id) const {
    std::vector<SubmapId> submap_ids;
    const auto& submap_data = parent_->optimization_problem_->submap_data();
...
MapById<SubmapId, PoseGraphInterface::SubmapData> PoseGraph3D::TrimmingHandle::GetOptimizedSubmapData() const {
    ...
    for (const auto& submap_id_data : parent_->data_.submap_data) {
        ...
        submaps.Insert(submap_id_data.id,
                       SubmapData{submap_id_data.data.submap,
                                  parent_->data_.global_submap_poses_3d.at(submap_id_data.id).global_pose});
    }
    return submaps;
}

const MapById<NodeId, TrajectoryNode>& PoseGraph3D::TrimmingHandle::GetTrajectoryNodes() const {
    return parent_->data_.trajectory_nodes;
}

const std::vector<PoseGraphInterface::Constraint>& PoseGraph3D::TrimmingHandle::GetConstraints() const {
...
void PoseGraph3D::TrimmingHandle::SetTrajectoryState(int trajectory_id, TrajectoryState state) {
...
void PoseGraph3D::TrimmingHandle::TrimSubmap(const SubmapId& submap_id) {
    ...
    // Compile all nodes that are still INTRA_SUBMAP constrained to other
    // submaps once the submap with 'submap_id' is gone. We need to use node_ids
    // instead of constraints here to be also compatible with frozen
    // trajectories that don't have intra-constraints.
    ...
    std::set_difference(parent_->data_.submap_data.at(submap_id).node_ids.begin(),
                        parent_->data_.submap_data.at(submap_id).node_ids.end(), nodes_to_retain.begin(),
                        nodes_to_retain.end(), std::inserter(nodes_to_remove, nodes_to_remove.begin()));
    ...
    // If the removal lets other submaps lose all their inter-submap
    // constraints, delete their corresponding constraint submap matchers to
    // save memory.
    {
        std::vector<Constraint> constraints;
        ...
        for (const Constraint& constraint : parent_->data_.constraints) {
            if (nodes_to_remove.count(constraint.node_id) == 0) {
                ...
                // A constraint to another submap will be removed, mark it as
                // affected.
...
MapById<SubmapId, PoseGraphInterface::SubmapData> PoseGraph3D::GetSubmapDataUnderLock() const {
    MapById<SubmapId, PoseGraphInterface::SubmapData> submaps;
    for (const auto& submap_id_data : data_.submap_data) {
...
void PoseGraph3D::SetGlobalSlamOptimizationCallback(PoseGraphInterface::GlobalSlamOptimizationCallback callback) {
    global_slam_optimization_callback_ = callback;
}

void PoseGraph3D::RegisterMetrics(metrics::FamilyFactory* family_factory) {
    auto* latency = family_factory->NewGaugeFamily("mapping_3d_pose_graph_work_queue_delay",
                                                   "Age of the oldest entry in the work queue in seconds");
    kWorkQueueDelayMetric = latency->Add({});
    auto* queue_size =
        family_factory->NewGaugeFamily("mapping_3d_pose_graph_work_queue_size", "Number of items in the work queue");
    kWorkQueueSizeMetric = queue_size->Add({});
    auto* constraints = family_factory->NewGaugeFamily("mapping_3d_pose_graph_constraints",
    ...
    auto* submaps =
        family_factory->NewGaugeFamily("mapping_3d_pose_graph_submaps", "Number of submaps in the pose graph.");
