...
public:
    ...
    void AddSensorData(const std::string& sensor_id,
        ...
        CHECK(local_trajectory_builder_) << "Cannot add TimedPointCloudData "
                                            "without a LocalTrajectoryBuilder.";
        ...
        if (matching_result->insertion_result != nullptr) {
            ...
            insertion_result = std::make_unique<InsertionResult>(
    ...
    void AddSensorData(const std::string& sensor_id, const sensor::OdometryData& odometry_data) override {
        ...
        // TODO(MichaelGrupp): Instead of having an optional filter on this
        // level, odometry could be marginalized between nodes in the pose
        // graph. Related issue: cartographer-project/cartographer/#1768
...
std::unique_ptr<TrajectoryBuilderInterface> CreateGlobalTrajectoryBuilder2D(
    ...
    const std::optional<MotionFilter>& pose_graph_odometry_motion_filter) {
    return std::make_unique<GlobalTrajectoryBuilder<LocalTrajectoryBuilder2D, mapping::PoseGraph2D>>(
...
std::unique_ptr<TrajectoryBuilderInterface> CreateGlobalTrajectoryBuilder3D(
    ...
    const TrajectoryBuilderInterface::LocalSlamResultCallback& local_slam_result_callback,
    const std::optional<MotionFilter>& pose_graph_odometry_motion_filter) {
    return std::make_unique<GlobalTrajectoryBuilder<LocalTrajectoryBuilder3D, mapping::PoseGraph3D>>(
        std::move(local_trajectory_builder), trajectory_id, pose_graph, local_slam_result_callback,
        pose_graph_odometry_motion_filter);
}
