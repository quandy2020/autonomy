    ...
    if (!initial_poses.empty()) {
        ...
        extrapolator->AddPose(imu_data.back().time,
                              transform::Rigid3d::Rotation(FromTwoVectors(
                                  imu_data.back().linear_acceleration,
...
void ImuBasedPoseExtrapolator::AddPose(const common::Time time,
    ...
    while (timed_pose_queue_.size() > 3 &&
        ...
        if (!previous_solution_.empty()) {
            CHECK_EQ(timed_pose_queue_.front().time,
                     previous_solution_.front().time);
...
ImuBasedPoseExtrapolator::ExtrapolatePosesWithGravity(
    ...
    for (size_t i = 0; i < timed_pose_queue_.size(); i++) {
        ...
        if (is_last) {
            nodes.emplace_back(
                gravity_from_node, nullptr,
                absl::make_unique<
                    ceres::AutoDiffManifold<ConstantYawQuaternionPlus, 4, 2>>(),
                &problem);
    ...
    problem.SetManifold(imu_calibration.data(),
    ...
    LOG_IF_EVERY_N(INFO, !fix_gravity, 20)
        << "Gravity was: " << gravity_constant;
...
std::vector<transform::Rigid3f> ImuBasedPoseExtrapolator::InterpolatePoses(
    ...
    for (auto it = times_begin; it != times_end; ++it) {
        const float factor =
            common::ToSeconds(*it - start.time) * duration_scale;
...
transform::Rigid3d ImuBasedPoseExtrapolator::InterpolateOdometry(
    ...
    std::deque<sensor::OdometryData>::const_iterator data =
        std::upper_bound(odometry_data_.begin(), odometry_data_.end(), time,
                         [](const common::Time& time,
                            const sensor::OdometryData& odometry_data) {
