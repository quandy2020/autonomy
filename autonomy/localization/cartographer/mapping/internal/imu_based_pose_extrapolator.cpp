...
std::unique_ptr<PoseExtrapolatorInterface> ImuBasedPoseExtrapolator::InitializeWithImu(
    ...
    auto extrapolator = std::make_unique<ImuBasedPoseExtrapolator>(options);
    ...
    if (!initial_poses.empty()) {
        ...
        extrapolator->AddPose(imu_data.back().time,
                              transform::Rigid3d::Rotation(
                                  FromTwoVectors(imu_data.back().linear_acceleration, Eigen::Vector3d::UnitZ())));
    }
    return extrapolator;
}
...
ImuBasedPoseExtrapolator::ExtrapolationResult ImuBasedPoseExtrapolator::ExtrapolatePosesWithGravity(
    ...
    for (size_t i = 0; i < timed_pose_queue_.size(); i++) {
        ...
        if (is_last) {
            ...
            nodes.emplace_back(gravity_from_node, nullptr, std::make_unique<ceres::QuaternionManifold>(), &problem);
        }
    }
    ...
    const transform::Rigid3d initial_estimate =
        ExtrapolatePoseWithImu<double>(prev_gravity_from_tracking.transform, prev_gravity_from_tracking.time,
            ...
            .pose;
    nodes.emplace_back(initial_estimate, nullptr, std::make_unique<ceres::QuaternionManifold>(), &problem);
