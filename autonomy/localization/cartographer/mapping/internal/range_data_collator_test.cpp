...
#include "autonomy/localization/cartographer/mapping/internal/range_data_collator.hpp"
...
sensor::TimedPointCloudData CreateFakeRangeData(int from, int to, bool fake_intensities) {
    double duration = common::ToSeconds(common::FromUniversal(to) - common::FromUniversal(from));
    sensor::TimedPointCloudData result{common::FromUniversal(to), Eigen::Vector3f(0., 1., 2.), {}, {}};
    ...
    for (int i = 0; i < kNumSamples; ++i) {
        ...
        result.ranges.push_back({Eigen::Vector3f{1., 2., static_cast<float>(fraction)}, relative_time});
        if (fake_intensities) {
...
TEST(RangeDataCollatorTest, SingleSensor) {
    ...
    auto output_0 = collator.AddRangeData(sensor_id, CreateFakeRangeData(200, 300, false));
    ...
    auto output_1 = collator.AddRangeData(sensor_id, CreateFakeRangeData(300, 500, false));
    ...
    EXPECT_NEAR(common::ToUniversal(output_1.time + common::FromSeconds(output_1.ranges[0].point_time.time)), 300, 2);
    auto output_2 = collator.AddRangeData(sensor_id, CreateFakeRangeData(-1000, 510, false));
...
TEST(RangeDataCollatorTest, TwoSensors) {
    ...
    auto output_0 = collator.AddRangeData(sensor_0, CreateFakeRangeData(200, 300, false));
    EXPECT_EQ(output_0.ranges.size(), 0);
    auto output_1 = collator.AddRangeData(sensor_1, CreateFakeRangeData(-1000, 310, false));
    ...
    EXPECT_NEAR(common::ToUniversal(output_1.time + common::FromSeconds(output_1.ranges[0].point_time.time)), -1000, 2);
    EXPECT_EQ(output_1.ranges.back().point_time.time, 0.f);
    EXPECT_TRUE(ArePointTimestampsSorted(output_1));
    auto output_2 = collator.AddRangeData(sensor_0, CreateFakeRangeData(300, 500, false));
    EXPECT_EQ(output_2.origins.size(), 2);
    EXPECT_EQ(common::ToUniversal(output_2.time), 310);
    ASSERT_EQ(output_2.ranges.size(), 2);
    EXPECT_NEAR(common::ToUniversal(output_2.time + common::FromSeconds(output_2.ranges[0].point_time.time)), 300, 2);
    EXPECT_EQ(output_2.ranges.back().point_time.time, 0.f);
    EXPECT_TRUE(ArePointTimestampsSorted(output_2));
    // Sending the same sensor will flush everything before.
    auto output_3 = collator.AddRangeData(sensor_0, CreateFakeRangeData(600, 700, false));
    EXPECT_EQ(common::ToUniversal(output_3.time), 500);
    EXPECT_EQ(output_1.ranges.size() + output_2.ranges.size() + output_3.ranges.size(), 3 * kNumSamples);
...
TEST(RangeDataCollatorTest, ThreeSensors) {
    ...
    auto output_0 = collator.AddRangeData(sensor_0, CreateFakeRangeData(100, 200, false));
    EXPECT_EQ(output_0.ranges.size(), 0);
    auto output_1 = collator.AddRangeData(sensor_1, CreateFakeRangeData(199, 250, false));
    EXPECT_EQ(output_1.ranges.size(), 0);
    auto output_2 = collator.AddRangeData(sensor_2, CreateFakeRangeData(210, 300, false));
    EXPECT_EQ(output_2.ranges.size(), kNumSamples + 1);
    EXPECT_TRUE(ArePointTimestampsSorted(output_2));
    auto output_3 = collator.AddRangeData(sensor_2, CreateFakeRangeData(400, 500, false));
...
TEST(RangeDataCollatorTest, ThreeSensorsWithIntensities) {
    ...
    const std::string sensor_2 = "sensor_2";
    RangeDataCollator collator({sensor_0, sensor_1, sensor_2});
    auto output_0 = collator.AddRangeData(sensor_0, CreateFakeRangeData(100, 200, true));
    EXPECT_EQ(output_0.ranges.size(), 0);
    auto output_1 = collator.AddRangeData(sensor_1, CreateFakeRangeData(199, 250, true));
    EXPECT_EQ(output_1.ranges.size(), 0);
    auto output_2 = collator.AddRangeData(sensor_2, CreateFakeRangeData(210, 300, true));
    EXPECT_EQ(output_2.ranges.size(), kNumSamples + 1);
    EXPECT_TRUE(ArePointTimestampsSorted(output_2));
    IntensitiesAreConsistent(output_2);
    auto output_3 = collator.AddRangeData(sensor_2, CreateFakeRangeData(400, 500, true));
    EXPECT_EQ(output_2.ranges.size() + output_3.ranges.size(), 3 * kNumSamples);
    EXPECT_TRUE(ArePointTimestampsSorted(output_3));
