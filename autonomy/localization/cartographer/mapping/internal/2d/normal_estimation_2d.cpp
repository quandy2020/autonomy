...
float EstimateNormal(const sensor::PointCloud& returns,
    ...
    for (size_t sample_point_index = sample_window_begin;
        ...
        if (sample_point_index == estimation_point_index)
            continue;
        const Eigen::Vector3f& sample_point =
            returns[sample_point_index].position;
...
std::vector<float> EstimateNormals(
    ...
    const size_t max_num_samples =
        normal_estimation_options.num_normal_samples();
    ...
    for (size_t current_point = 0; current_point < range_data.returns.size();
        ...
        for (; sample_window_begin > 0 &&
               ...
               (hit - range_data.returns[sample_window_begin - 1].position)
                       .norm() < sample_radius;
             --sample_window_begin) {
        }
        size_t sample_window_end = current_point;
        for (; sample_window_end < range_data.returns.size() &&
               sample_window_end - current_point <
                   ceil(max_num_samples / 2.0) + 1 &&
