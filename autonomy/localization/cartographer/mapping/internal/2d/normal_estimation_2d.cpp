...
float NormalTo2DAngle(const Eigen::Vector3f& v) { return std::atan2(v[1], v[0]); }
...
// sample_window.
float EstimateNormal(const sensor::PointCloud& returns, const size_t estimation_point_index,
                     const size_t sample_window_begin, const size_t sample_window_end,
  ...
  const Eigen::Vector3f& estimation_point_to_observation = sensor_origin - estimation_point;
  for (size_t sample_point_index = sample_window_begin; sample_point_index < sample_window_end; ++sample_point_index) {
...
proto::NormalEstimationOptions2D CreateNormalEstimationOptions2D(common::LuaParameterDictionary* parameter_dictionary) {
  ...
  options.set_num_normal_samples(parameter_dictionary->GetInt("num_normal_samples"));
...
std::vector<float> EstimateNormals(const sensor::RangeData& range_data,
  ...
  for (size_t current_point = 0; current_point < range_data.returns.size(); ++current_point) {
    ...
    for (; sample_window_begin > 0 && current_point - sample_window_begin < max_num_samples / 2 &&
           (hit - range_data.returns[sample_window_begin - 1].position).norm() < sample_radius;
         --sample_window_begin) {
    }
    size_t sample_window_end = current_point;
    for (; sample_window_end < range_data.returns.size() &&
    ...
    const float normal_estimate =
        EstimateNormal(range_data.returns, current_point, sample_window_begin, sample_window_end, range_data.origin);
