...
class RangeDataInserterTest2DTSDF : public ::testing::Test {
 protected:
  RangeDataInserterTest2DTSDF()
      : tsdf_(MapLimits(1., Eigen::Vector2d(1., 7.), CellLimits(8, 1)), 2.0, 10.0, &conversion_tables_) {
...
class MockCellProperties {
 public:
  MockCellProperties(const Eigen::Array2i& cell_index, const TSDF2D& tsdf)
      : is_known_(tsdf.IsKnown(cell_index)), tsd_(tsdf.GetTSD(cell_index)), weight_(tsdf.GetWeight(cell_index)){};
...
::std::ostream& operator<<(::std::ostream& os, const MockCellProperties& bar) {
  return os << std::to_string(bar.is_known_) + "\t" + std::to_string(bar.tsd_) + "\t" + std::to_string(bar.weight_);
}

MATCHER_P3(EqualCellProperties, expected_is_known, expected_tsd, expected_weight,
           std::string("Expected ") + std::to_string(expected_is_known) + "\t" + std::to_string(expected_tsd) + "\t" +
...
TEST_F(RangeDataInserterTest2DTSDF, InsertPoint) {
  ...
  for (float y = 1.5; y < 6.; ++y) {
    ...
    float x = -0.5f;
    Eigen::Array2i cell_index = tsdf_.limits().GetCellIndex(Eigen::Vector2f(x, y));
    float expected_tsdf = std::max(std::min(3.5f - y, truncation_distance), -truncation_distance);
    float expected_weight = 1.f;
    EXPECT_THAT(MockCellProperties(cell_index, tsdf_), EqualCellProperties(true, expected_tsdf, expected_weight));
    // Cells don't intersect with ray.
    x = 0.5f;
    cell_index = tsdf_.limits().GetCellIndex(Eigen::Vector2f(x, y));
    expected_tsdf = -truncation_distance;
    expected_weight = 0.;
    EXPECT_THAT(MockCellProperties(cell_index, tsdf_), EqualCellProperties(false, expected_tsdf, expected_weight));
    x = 1.5f;
    cell_index = tsdf_.limits().GetCellIndex(Eigen::Vector2f(x, y));
    EXPECT_THAT(MockCellProperties(cell_index, tsdf_), EqualCellProperties(false, expected_tsdf, expected_weight));
  }
  ...
  // Cells don't intersect with ray.
  Eigen::Array2i cell_index = tsdf_.limits().GetCellIndex(Eigen::Vector2f(0.5, 6.5));
  EXPECT_THAT(MockCellProperties(cell_index, tsdf_), EqualCellProperties(false, -truncation_distance, 0.));
  ...
  for (float y = 1.5; y < 6.; ++y) {
    ...
    Eigen::Array2i cell_index = tsdf_.limits().GetCellIndex(Eigen::Vector2f(x, y));
    float expected_tsdf = std::max(std::min(3.5f - y, truncation_distance), -truncation_distance);
    EXPECT_THAT(MockCellProperties(cell_index, tsdf_), EqualCellProperties(true, expected_tsdf, maximum_weight));
  }
}
...
TEST_F(RangeDataInserterTest2DTSDF, InsertSmallAnglePointWithoutNormalProjection) {
  auto range_data = sensor::RangeData();
  range_data.returns.push_back({Eigen::Vector3f{-0.5f, 3.5f, 0.f}});
  ...
  float y = 2.5f;
  Eigen::Array2i cell_index = tsdf_.limits().GetCellIndex(Eigen::Vector2f(x, y));
  Eigen::Vector2f ray = Eigen::Vector2f(-0.5f, -0.5f) - Eigen::Vector2f(5.5f, 3.5f);
  float ray_length = ray.norm();
  Eigen::Vector2f origin_to_cell = Eigen::Vector2f(x, y) - Eigen::Vector2f(-0.5f, -0.5f);
...
TEST_F(RangeDataInserterTest2DTSDF, InsertPointsWithAngleScanNormalToRayWeight) {
  float bandwidth = 10.f;
  options_.set_update_weight_angle_scan_normal_to_ray_kernel_bandwidth(bandwidth);
  range_data_inserter_ = absl::make_unique<TSDFRangeDataInserter2D>(options_);
  auto range_data = sensor::RangeData();
  range_data.returns.push_back({Eigen::Vector3f{-0.5f, 3.5f, 0.f}});
  range_data.returns.push_back({Eigen::Vector3f{5.5f, 3.5f, 0.f}});
  ...
  expected_weight = 1.f / (std::sqrt(2 * M_PI) * bandwidth) * std::exp(angle * angle / (2 * std::pow(bandwidth, 2)));
  EXPECT_NEAR(expected_weight, tsdf_.GetWeight(cell_index), 1e-3);
  x = 6.5f;
  y = 4.5f;
  cell_index = tsdf_.limits().GetCellIndex(Eigen::Vector2f(x, y));
...
TEST_F(RangeDataInserterTest2DTSDF, InsertPointsWithDistanceCellToHit) {
  ...
  for (float y = 1.5; y < 6.; ++y) {
    ...
    float expected_weight =
        1.f / (std::sqrt(2 * M_PI) * bandwidth) * std::exp(std::pow(expected_tsdf, 2) / (2 * std::pow(bandwidth, 2)));
    EXPECT_THAT(MockCellProperties(cell_index, tsdf_), EqualCellProperties(true, expected_tsdf, expected_weight));
  }
}
