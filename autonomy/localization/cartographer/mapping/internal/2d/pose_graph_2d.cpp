...
#include "autonomy/localization/cartographer/mapping/internal/2d/pose_graph_2d.hpp"
...
#include "autonomy/localization/cartographer/mapping/internal/2d/overlapping_submaps_trimmer_2d.hpp"
...
PoseGraph2D::PoseGraph2D(const proto::PoseGraphOptions& options,
    ...
    if (options.has_overlapping_submaps_trimmer_2d()) {
        ...
        AddTrimmer(std::make_unique<OverlappingSubmapsTrimmer2D>(trimmer_options.fresh_submaps_count(),
...
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
    ...
    if (data_.submap_data.at(last_submap_id).submap == insertion_submaps.front()) {
        ...
        optimization_problem_->AddSubmap(
            trajectory_id, first_submap_pose * constraints::ComputeSubmapPose(*insertion_submaps[0]).inverse() *
...
NodeId PoseGraph2D::AddNode(std::shared_ptr<const TrajectoryNode::Data> constant_data, const int trajectory_id,
    ...
    AddWorkItem([=]() { return ComputeConstraintsForNode(node_id, insertion_submaps, newly_finished_submap); });
    return node_id;
}
...
WorkItem::Result PoseGraph2D::ComputeConstraintsForNode(const NodeId& node_id,
    ...
    std::set<NodeId> newly_finished_submap_node_ids;
    {
        ...
        optimization_problem_->AddTrajectoryNode(
            matching_id.trajectory_id, optimization::NodeSpec2D{constant_data->time, local_pose_2d, global_pose_2d,
...
void PoseGraph2D::AddSubmapFromProto(const transform::Rigid3d& global_submap_pose, const proto::Submap& submap) {
    ...
    AddWorkItem([this, submap_id, global_submap_pose_2d]() {
        std::lock_guard<std::mutex> locker(mutex_);
...
bool PoseGraph2D::CanAddWorkItemModifying(int trajectory_id) {
    ...
    if (it->second.state == TrajectoryState::FINISHED) {
        ...
        LOG(FATAL) << "trajectory_id " << trajectory_id << " has finished but modification is requested, skipping.";
        return false;
    }
    if (it->second.deletion_state != InternalTrajectoryState::DeletionState::NORMAL) {
        LOG(FATAL) << "trajectory_id " << trajectory_id
                   << " has been scheduled for deletion but modification is "
                      "requested, skipping.";
        return false;
    }
    if (it->second.state == TrajectoryState::DELETED) {
        LOG(FATAL) << "trajectory_id " << trajectory_id << " has been deleted but modification is requested, skipping.";
        return false;
    }
    return true;
}
...
void PoseGraph2D::SetLandmarkPose(const std::string& landmark_id, const transform::Rigid3d& global_pose,
                                  const bool frozen) {
    AddWorkItem([=]() {
        std::lock_guard<std::mutex> locker(mutex_);
...
PoseGraphInterface::SubmapData PoseGraph2D::GetSubmapDataUnderLock(const SubmapId& submap_id) const {
    ...
    return {submap, ComputeLocalToGlobalTransform(data_.global_submap_poses_2d, submap_id.trajectory_id) *
                        submap->local_pose()};
}
