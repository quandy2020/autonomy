...
#include "autonomy/localization/cartographer/mapping/internal/2d/tsd_value_converter.hpp"
 ...
 protected:
  TSDValueConverterTest()
      : truncation_distance_(0.1f),
        max_weight_(10.0f),
        tsd_value_converter_(truncation_distance_, max_weight_, &conversion_tables) {}
...
TEST_F(TSDValueConverterTest, ValueToTSDConversions) {
  for (uint16 i = 1; i < 32768; ++i) {
    EXPECT_EQ(tsd_value_converter_.TSDToValue(tsd_value_converter_.ValueToTSD(i)), i);
  }
}
...
TEST_F(TSDValueConverterTest, ValueToTSDConversionsWithUpdateMarker) {
  for (uint16 i = 1; i < 32768; ++i) {
    EXPECT_EQ(
        tsd_value_converter_.TSDToValue(tsd_value_converter_.ValueToTSD(i + tsd_value_converter_.getUpdateMarker())),
        i);
  }
}
...
TEST_F(TSDValueConverterTest, ValueToWeightConversions) {
  for (uint16 i = 1; i < 32768; ++i) {
    EXPECT_EQ(tsd_value_converter_.WeightToValue(tsd_value_converter_.ValueToWeight(i)), i);
  }
}
...
TEST_F(TSDValueConverterTest, ValueToWeightConversionsWithUpdateMarker) {
  for (uint16 i = 1; i < 32768; ++i) {
    EXPECT_EQ(tsd_value_converter_.WeightToValue(
                  tsd_value_converter_.ValueToWeight(i + tsd_value_converter_.getUpdateMarker())),
              i);
  }
}
...
TEST_F(TSDValueConverterTest, TSDToValueConversions) {
  ...
  for (uint16 i = 0; i < num_samples; ++i) {
    float sdf_sample = -truncation_distance_ + i * 2.f * truncation_distance_ / num_samples;
    EXPECT_NEAR(tsd_value_converter_.ValueToTSD(tsd_value_converter_.TSDToValue(sdf_sample)), sdf_sample, tolerance);
  }
}
...
TEST_F(TSDValueConverterTest, WeightToValueConversions) {
  ...
  for (uint16 i = 0; i < num_samples; ++i) {
    ...
    EXPECT_NEAR(tsd_value_converter_.ValueToWeight(tsd_value_converter_.WeightToValue(weight_sample)), weight_sample,
                tolerance);
  }
}
...
TEST_F(TSDValueConverterTest, WeightToValueOutOfRangeConversions) {
  float tolerance = max_weight_ / 32767.f;
  EXPECT_NEAR(tsd_value_converter_.ValueToWeight(tsd_value_converter_.WeightToValue(2.f * max_weight_)), max_weight_,
              tolerance);
  EXPECT_NEAR(tsd_value_converter_.ValueToWeight(tsd_value_converter_.WeightToValue(-max_weight_)), 0.f, tolerance);
}

TEST_F(TSDValueConverterTest, TSDToValueOutOfRangeConversions) {
  float tolerance = truncation_distance_ * 2.f / 32767.f;
  EXPECT_NEAR(tsd_value_converter_.ValueToTSD(tsd_value_converter_.TSDToValue(2.f * truncation_distance_)),
