...
TEST(TSDF2DTest, ProtoConstructor) {
  ...
  *proto.mutable_limits() = ToProto(limits);
  ...
  for (int i = 6; i < 12; ++i) {
    ...
    proto.mutable_tsdf_2d()->mutable_weight_cells()->Add(static_cast<uint16>(i));
  }
...
TEST(TSDF2DTest, ConstructorGridType) {
  ValueConversionTables conversion_tables;
  TSDF2D tsdf(MapLimits(1., Eigen::Vector2d(1., 1.), CellLimits(2, 2)), 1.0f, 10.0f, &conversion_tables);
...
TEST(TSDF2DTest, GetCellIndex) {
  ValueConversionTables conversion_tables;
  TSDF2D tsdf(MapLimits(2., Eigen::Vector2d(8., 14.), CellLimits(14, 8)), 1.f, 10.f, &conversion_tables);
...
TEST(TSDF2DTest, WriteRead) {
  ...
  ValueConversionTables conversion_tables;
  TSDF2D tsdf(MapLimits(1., Eigen::Vector2d(1., 2.), CellLimits(2, 2)), truncation_distance, max_weight,
              &conversion_tables);
  ...
  std::mt19937 rng(42);
  std::uniform_real_distribution<float> tsd_distribution(-truncation_distance, truncation_distance);
  ...
  for (size_t i = 0; i < 1; ++i) {
    ...
    EXPECT_NEAR(tsdf.GetTSDAndWeight(limits.GetCellIndex(Vector2f(-0.5f, 0.5f))).first, tsd,
                2.f * truncation_distance / 32768.f);
    EXPECT_NEAR(tsdf.GetTSDAndWeight(limits.GetCellIndex(Vector2f(-0.5f, 0.5f))).second, weight, max_weight / 32768.f);
    EXPECT_NEAR(tsdf.GetTSD(limits.GetCellIndex(Vector2f(-0.5f, 0.5f))), tsd, 2.f * truncation_distance / 32768.f);
    EXPECT_NEAR(tsdf.GetWeight(limits.GetCellIndex(Vector2f(-0.5f, 0.5f))), weight, max_weight / 32768.f);
  }
...
TEST(TSDF2DTest, CorrectCropping) {
  ...
  const float max_weight = 10.f;
  std::mt19937 rng(42);
  std::uniform_real_distribution<float> tsdf_distribution(-truncation_distance, truncation_distance);
  std::uniform_real_distribution<float> weight_distribution(0.f, max_weight);
  ValueConversionTables conversion_tables;
  TSDF2D tsdf(MapLimits(0.05, Eigen::Vector2d(10., 10.), CellLimits(400, 400)), truncation_distance, max_weight,
              &conversion_tables);
