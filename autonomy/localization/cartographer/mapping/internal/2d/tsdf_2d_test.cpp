...
TEST(TSDF2DTest, ProtoConstructor) {
    ...
    *proto.mutable_limits() = ToProto(limits);
    ...
    EXPECT_EQ(proto.limits().DebugString(),
...
TEST(TSDF2DTest, ToProto) {
    ...
    EXPECT_EQ(ToProto(tsdf.limits()).DebugString(),
...
TEST(TSDF2DTest, WriteRead) {
    ...
    for (size_t i = 0; i < 1; ++i) {
        ...
        EXPECT_NEAR(
            tsdf.GetTSDAndWeight(limits.GetCellIndex(Vector2f(-0.5f, 0.5f)))
                .first,
        ...
        EXPECT_NEAR(tsdf.GetTSD(limits.GetCellIndex(Vector2f(-0.5f, 0.5f))),
                    tsd, 2.f * truncation_distance / 32768.f);
...
TEST(TSDF2DTest, CorrectCropping) {
    ...
    const float max_weight = 10.f;
    std::mt19937 rng(42);
    std::uniform_real_distribution<float> tsdf_distribution(
    ...
    for (const Array2i& xy_index :
        ...
        tsdf.SetCell(xy_index, tsdf_distribution(rng),
                     weight_distribution(rng));
    }
    Array2i offset;
    CellLimits limits;
