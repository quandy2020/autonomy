...
#include "autonomy/localization/cartographer/mapping/internal/2d/scan_matching/tsdf_match_cost_function_2d.hpp"
...
protected:
    TSDFSpaceCostFunction2DTest()
        : tsdf_(MapLimits(0.1, Eigen::Vector2d(2.05, 2.05), CellLimits(40, 40)), 0.3, 1.0, &conversion_tables_) {
...
TEST_F(TSDFSpaceCostFunction2DTest, MatchEmptyTSDF) {
    ...
    std::unique_ptr<ceres::CostFunction> cost_function(CreateTSDFMatchCostFunction2D(1.f, matching_cloud, tsdf_));
    ...
    bool valid_result = cost_function->Evaluate(parameter_blocks.data(), residuals.data(), jacobians_ptrs.data());
...
TEST_F(TSDFSpaceCostFunction2DTest, ExactInitialPose) {
    ...
    for (int i = 0; i < 1; ++i)
        jacobians_ptrs[i] = jacobians[i].data();
    const bool valid_result = cost_function->Evaluate(parameter_blocks.data(), residuals.data(), jacobians_ptrs.data());
    EXPECT_TRUE(valid_result);
    EXPECT_THAT(residuals, ElementsAre(DoubleNear(0., 1e-3)));
    EXPECT_THAT(jacobians[0], ElementsAre(DoubleNear(0., 1e-3), DoubleNear(-1., 1e-3), DoubleNear(0., 1e-3)));
}
...
TEST_F(TSDFSpaceCostFunction2DTest, PertubatedInitialPose) {
    ...
    valid_result = cost_function->Evaluate(parameter_blocks.data(), residuals.data(), jacobians_ptrs.data());
    EXPECT_TRUE(valid_result);
    EXPECT_THAT(residuals, ElementsAre(DoubleNear(0.1, 1e-3)));
    EXPECT_THAT(jacobians[0], ElementsAre(DoubleNear(0., 1e-3), DoubleNear(-1., 1e-3), DoubleNear(0., 1e-3)));
}
