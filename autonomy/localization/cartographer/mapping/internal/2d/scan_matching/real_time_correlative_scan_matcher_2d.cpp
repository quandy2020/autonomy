...
float ComputeCandidateScore(const TSDF2D& tsdf, const DiscreteScan2D& discrete_scan, int x_index_offset,
                            int y_index_offset) {
...
std::vector<Candidate2D> RealTimeCorrelativeScanMatcher2D::GenerateExhaustiveSearchCandidates(
    ...
    for (int scan_index = 0; scan_index != search_parameters.num_scans; ++scan_index) {
        for (int x_index_offset = search_parameters.linear_bounds[scan_index].min_x;
             x_index_offset <= search_parameters.linear_bounds[scan_index].max_x; ++x_index_offset) {
...
void RealTimeCorrelativeScanMatcher2D::ScoreCandidates(const Grid2D& grid,
    ...
    for (Candidate2D& candidate : *candidates) {
        switch (grid.GetGridType()) {
            ...
            case GridType::TSDF:
                candidate.score =
                    ComputeCandidateScore(static_cast<const TSDF2D&>(grid), discrete_scans[candidate.scan_index],
