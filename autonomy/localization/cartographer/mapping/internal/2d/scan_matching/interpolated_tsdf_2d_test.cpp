...
TEST_F(InterpolatedTSDF2DTest, InterpolatesGridPoints) {
    ...
    for (const Eigen::Vector2f& point : inner_points) {
        EXPECT_NEAR(
            ...
            interpolated_tsdf_.GetCorrespondenceCost(
...
TEST_F(InterpolatedTSDF2DTest, InterpolatesWithinCell) {
    ...
    tsdf_.SetCell(tsdf_.limits().GetCellIndex(Eigen::Vector2f(0.f, 0.f)),
                  tsd_00, w_00);
    tsdf_.SetCell(tsdf_.limits().GetCellIndex(Eigen::Vector2f(0.f, 1.f)),
                  tsd_01, w_01);
    tsdf_.SetCell(tsdf_.limits().GetCellIndex(Eigen::Vector2f(1.f, 0.f)),
                  tsd_10, w_10);
    tsdf_.SetCell(tsdf_.limits().GetCellIndex(Eigen::Vector2f(1.f, 1.f)),
                  tsd_11, w_11);

    const double kSampleStep = tsdf_.limits().resolution() / 100.;
    for (double x = 0. + kSampleStep; x < 1.;
         x += tsdf_.limits().resolution()) {
        for (double y = 0. + kSampleStep; y < 1.;
             y += tsdf_.limits().resolution()) {
            const float tsd_expected =
                (x * tsd_10 + (1.f - x) * tsd_00) * (1.f - y) +
                (x * tsd_11 + (1.f - x) * tsd_01) * y;
            EXPECT_NEAR(interpolated_tsdf_.GetCorrespondenceCost(x, y),
                        tsd_expected, 1e-3);
