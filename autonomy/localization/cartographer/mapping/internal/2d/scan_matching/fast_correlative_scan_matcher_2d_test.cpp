            ...
            for (int y = 0; y != width; ++y) {
                for (int x = 0; x != width; ++x) {
                    max_score = std::max<float>(
                        max_score, probability_grid.GetProbability(
                                       xy_index + Eigen::Array2i(x, y)));
                }
            }
            EXPECT_NEAR(max_score,
                        precomputation_grid.ToScore(
...
CreateFastCorrelativeScanMatcherTestOptions2D(
    ...
    return CreateFastCorrelativeScanMatcherOptions2D(
        ...
        range_data_inserter.Insert(
            sensor::RangeData{
                ...
                sensor::TransformPointCloud(
                    point_cloud,
                    transform::Embed3D(expected_pose.cast<float>())),
                {}},
            &probability_grid);
