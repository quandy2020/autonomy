...
class TSDFMatchCostFunction2D {
 public:
  TSDFMatchCostFunction2D(const double residual_scaling_factor, const sensor::PointCloud& point_cloud,
                          const TSDF2D& grid)
      : residual_scaling_factor_(residual_scaling_factor), point_cloud_(point_cloud), interpolated_grid_(grid) {}
  ...
  bool operator()(const T* const pose, T* residual) const {
    ...
    for (size_t i = 0; i < point_cloud_.size(); ++i) {
      ...
      residual[i] = T(point_cloud_.size()) * residual_scaling_factor_ *
                    interpolated_grid_.GetCorrespondenceCost(world[0], world[1]) * point_weight;
    }
...
ceres::CostFunction* CreateTSDFMatchCostFunction2D(const double scaling_factor, const sensor::PointCloud& point_cloud,
                                                   const TSDF2D& tsdf) {
  return new ceres::AutoDiffCostFunction<TSDFMatchCostFunction2D, ceres::DYNAMIC /* residuals */,
                                         3 /* pose variables */>(
      new TSDFMatchCostFunction2D(scaling_factor, point_cloud, tsdf), point_cloud.size());
}
