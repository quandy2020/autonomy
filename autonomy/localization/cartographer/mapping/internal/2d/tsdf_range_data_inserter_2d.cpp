...
proto::TSDFRangeDataInserterOptions2D CreateTSDFRangeDataInserterOptions2D(
    ...
    options.set_maximum_weight(
        parameter_dictionary->GetDouble("maximum_weight"));
...
void TSDFRangeDataInserter2D::InsertHit(
    ...
    if (options_.update_weight_angle_scan_normal_to_ray_kernel_bandwidth() !=
        ...
        float angle_ray_normal = common::NormalizeAngleDifference(
            normal - common::atan2(negative_ray));
    ...
    for (const Eigen::Array2i& cell_index : ray_mask) {
        ...
        update_tsd = common::Clamp(update_tsd, -truncation_distance,
                                   truncation_distance);
        float update_weight =
            weight_factor_range * weight_factor_angle_ray_normal;
        if (options_.update_weight_distance_cell_to_hit_kernel_bandwidth() !=
            0.f) {
