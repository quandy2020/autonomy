    ...
    PoseGraph2DTest() : thread_pool_(1) {
...

        {
          ...
          return {
            ...
            active_submaps_ =
                std::make_unique<ActiveSubmaps2D>(mapping::CreateSubmapsOptions2D(parameter_dictionary.get()));
        }

        {
          ...
          return {
            ...
            pose_graph_ = std::make_unique<PoseGraph2D>(
                options, std::make_unique<optimization::OptimizationProblem2D>(options.optimization_problem_options()),
                &thread_pool_);
        }
