...
class TSDValueConverter {
 public:
  ...
  inline float ValueToTSD(const uint16 value) const { return (*value_to_tsd_)[value]; }

  // Converts a uint16 (which may or may not have the update marker set) to a
  // value in the range [min_weight_, max_weight_].
  inline float ValueToWeight(const uint16 value) const { return (*value_to_weight_)[value]; }
 ...
 private:
  ...
  inline float ClampTSD(const float tsd) const { return common::Clamp(tsd, min_tsd_, max_tsd_); }
  // Clamps weight to be in the range [min_tsd_, max_tsd_].
  inline float ClampWeight(const float weight) const { return common::Clamp(weight, min_weight_, max_weight_); }
