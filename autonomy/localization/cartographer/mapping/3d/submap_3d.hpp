...
#include "autonomy/localization/cartographer/mapping/proto/submap_visualization.pb.h"
#include "autonomy/localization/cartographer/mapping/proto/submaps_options_3d.pb.h"
#include "autonomy/localization/cartographer/mapping/submaps.hpp"
...
class Submap3D : public Submap {
 public:
  ...
  const HybridGrid& high_resolution_hybrid_grid() const { return *high_resolution_hybrid_grid_; }
  const HybridGrid& low_resolution_hybrid_grid() const { return *low_resolution_hybrid_grid_; }
  ...
  void ForgetIntensityHybridGrid() { high_resolution_intensity_hybrid_grid_.reset(); }

  const Eigen::VectorXf& rotational_scan_matcher_histogram() const { return rotational_scan_matcher_histogram_; }

  // Insert 'range_data' into this submap using 'range_data_inserter'. The
  // submap must not be finished yet.
  void InsertData(const sensor::RangeData& range_data, const RangeDataInserter3D& range_data_inserter,
                  float high_resolution_max_range, const Eigen::Quaterniond& local_from_gravity_aligned,
...
class ActiveSubmaps3D {
 public:
  ...
  // 'local_from_gravity_aligned' is used for the orientation of new submaps
  // so that the z axis approximately aligns with gravity.
  ...
  std::vector<std::shared_ptr<const Submap3D>> InsertData(
      const sensor::RangeData& range_data_in_local, const Eigen::Quaterniond& local_from_gravity_aligned,
