...
TEST(SubmapsTest, ToFromProto) {
  ...
  const Submap3D expected(
      0.05, 0.25, transform::Rigid3d(Eigen::Vector3d(1., 2., 0.), Eigen::Quaterniond(0., 0., 0., 1.)), histogram);
  ...
  EXPECT_TRUE(expected.rotational_scan_matcher_histogram().isApprox(actual.rotational_scan_matcher_histogram(), 1e-6));
}
