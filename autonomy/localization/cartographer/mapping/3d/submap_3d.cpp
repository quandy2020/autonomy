...
std::vector<PixelData> AccumulatePixelData(
    ...
    for (const Eigen::Array4i& voxel_index_and_probability :
        ...
        const Eigen::Array2i pixel_index =
            voxel_index_and_probability.head<2>();
        if ((pixel_index < min_index).any() ||
            (pixel_index > max_index).any()) {
...
std::string ComputePixelValues(
    ...
    for (const PixelData& pixel : accumulated_pixel_data) {
        ...
        const float z_difference =
            pixel.count > 0 ? pixel.max_z - pixel.min_z : 0;
        ...
        const float average_probability =
            ClampProbability((pixel.probability_sum +
                              free_space_probability * free_space_weight) /
                             total_weight);
        const int delta =
            128 - ProbabilityToLogOddsInteger(average_probability);
...
proto::SubmapsOptions3D CreateSubmapsOptions3D(
    ...
    options.set_low_resolution(
        parameter_dictionary->GetDouble("low_resolution"));
...
void Submap3D::UpdateFromProto(const proto::Submap3D& submap_3d) {
    ...
    rotational_scan_matcher_histogram_ = Eigen::VectorXf::Zero(
...
void ActiveSubmaps3D::AddSubmap(
    ...
    submaps_.emplace_back(new Submap3D(
        options_.high_resolution(), options_.low_resolution(),
        local_submap_pose, initial_rotational_scan_matcher_histogram));
}
