...
#include "autonomy/localization/cartographer/mapping/3d/submap_3d.hpp"
...
#include "autonomy/localization/cartographer/sensor/range_data.hpp"
...
sensor::RangeData FilterRangeDataByMaxRange(const sensor::RangeData& range_data, const float max_range) {
  sensor::RangeData result{range_data.origin, {}, {}};
  result.returns = range_data.returns.copy_if(
      [&](const sensor::RangefinderPoint& point) { return (point.position - range_data.origin).norm() <= max_range; });
  return result;
}

std::vector<PixelData> AccumulatePixelData(const int width, const int height, const Eigen::Array2i& min_index,
  ...
  for (const Eigen::Array4i& voxel_index_and_probability : voxel_indices_and_probabilities) {
    ...
    if ((pixel_index < min_index).any() || (pixel_index > max_index).any()) {
      // Out of bounds. This could happen because of floating point
      // inaccuracy.
      continue;
    }
    ...
    const float probability = ValueToProbability(voxel_index_and_probability[3]);
...
std::vector<Eigen::Array4i> ExtractVoxelData(const HybridGrid& hybrid_grid, const transform::Rigid3f& transform,
  ...
  for (auto it = HybridGrid::Iterator(hybrid_grid); !it.Done(); it.Next()) {
    ...
    const Eigen::Vector3f cell_center_submap = hybrid_grid.GetCenterOfCell(it.GetCellIndex());
    const Eigen::Vector3f cell_center_global = transform * cell_center_submap;
    const Eigen::Array4i voxel_index_and_probability(common::RoundToInt(cell_center_global.x() * resolution_inverse),
...
std::string ComputePixelValues(const std::vector<PixelData>& accumulated_pixel_data) {
  ...
  for (const PixelData& pixel : accumulated_pixel_data) {
    ...
    // TODO(whess): Document the approach and make it more independent from
    ...
    const float average_probability =
        ClampProbability((pixel.probability_sum + free_space_probability * free_space_weight) / total_weight);
...
void AddToTextureProto(const HybridGrid& hybrid_grid, const transform::Rigid3d& global_submap_pose,
  ...
  const std::vector<Eigen::Array4i> voxel_indices_and_probabilities =
      ExtractVoxelData(hybrid_grid, global_submap_pose.cast<float>(), &min_index, &max_index);
  ...
  const std::vector<PixelData> accumulated_pixel_data =
      AccumulatePixelData(width, height, min_index, max_index, voxel_indices_and_probabilities);
  ...
  *texture->mutable_slice_pose() =
      transform::ToProto(global_submap_pose.inverse() * transform::Rigid3d::Translation(Eigen::Vector3d(
...
proto::SubmapsOptions3D CreateSubmapsOptions3D(common::LuaParameterDictionary* parameter_dictionary) {
  ...
  options.set_high_resolution(parameter_dictionary->GetDouble("high_resolution"));
  options.set_high_resolution_max_range(parameter_dictionary->GetDouble("high_resolution_max_range"));
  ...
  *options.mutable_range_data_inserter_options() =
      CreateRangeDataInserterOptions3D(parameter_dictionary->GetDictionary("range_data_inserter").get());
  CHECK_GT(options.num_range_data(), 0);
  return options;
}

Submap3D::Submap3D(const float high_resolution, const float low_resolution, const transform::Rigid3d& local_submap_pose,
                   const Eigen::VectorXf& rotational_scan_matcher_histogram)
    : Submap(local_submap_pose),
      high_resolution_hybrid_grid_(absl::make_unique<HybridGrid>(high_resolution)),
      low_resolution_hybrid_grid_(absl::make_unique<HybridGrid>(low_resolution)),
      high_resolution_intensity_hybrid_grid_(absl::make_unique<IntensityHybridGrid>(high_resolution)),
...
proto::Submap Submap3D::ToProto(const bool include_probability_grid_data) const {
  proto::Submap proto;
  ...
  *submap_3d->mutable_local_pose() = transform::ToProto(local_pose());
  ...
  if (include_probability_grid_data) {
    *submap_3d->mutable_high_resolution_hybrid_grid() = high_resolution_hybrid_grid().ToProto();
    *submap_3d->mutable_low_resolution_hybrid_grid() = low_resolution_hybrid_grid().ToProto();
  }
  for (Eigen::VectorXf::Index i = 0; i != rotational_scan_matcher_histogram_.size(); ++i) {
    submap_3d->add_rotational_scan_matcher_histogram(rotational_scan_matcher_histogram_(i));
  }
  return proto;
}
...
void Submap3D::UpdateFromProto(const proto::Submap3D& submap_3d) {
  ...
  if (submap_3d.has_high_resolution_hybrid_grid()) {
    high_resolution_hybrid_grid_ = absl::make_unique<HybridGrid>(submap_3d.high_resolution_hybrid_grid());
  }
  if (submap_3d.has_low_resolution_hybrid_grid()) {
    low_resolution_hybrid_grid_ = absl::make_unique<HybridGrid>(submap_3d.low_resolution_hybrid_grid());
  }
  rotational_scan_matcher_histogram_ = Eigen::VectorXf::Zero(submap_3d.rotational_scan_matcher_histogram_size());
  for (Eigen::VectorXf::Index i = 0; i != submap_3d.rotational_scan_matcher_histogram_size(); ++i) {
    rotational_scan_matcher_histogram_(i) = submap_3d.rotational_scan_matcher_histogram(i);
  }
}
...
void Submap3D::ToResponseProto(const transform::Rigid3d& global_submap_pose,
  ...
  AddToTextureProto(*high_resolution_hybrid_grid_, global_submap_pose, response->add_textures());
  AddToTextureProto(*low_resolution_hybrid_grid_, global_submap_pose, response->add_textures());
}

void Submap3D::InsertData(const sensor::RangeData& range_data_in_local, const RangeDataInserter3D& range_data_inserter,
                          const float high_resolution_max_range, const Eigen::Quaterniond& local_from_gravity_aligned,
  ...
  const sensor::RangeData transformed_range_data =
      sensor::TransformRangeData(range_data_in_local, local_pose().inverse().cast<float>());
  range_data_inserter.Insert(FilterRangeDataByMaxRange(transformed_range_data, high_resolution_max_range),
                             high_resolution_hybrid_grid_.get(), high_resolution_intensity_hybrid_grid_.get());
  range_data_inserter.Insert(transformed_range_data, low_resolution_hybrid_grid_.get(),
                             /*intensity_hybrid_grid=*/nullptr);
  set_num_range_data(num_range_data() + 1);
  const float yaw_in_submap_from_gravity =
      transform::GetYaw(local_pose().inverse().rotation() * local_from_gravity_aligned);
  rotational_scan_matcher_histogram_ +=
      scan_matching::RotationalScanMatcher::RotateHistogram(scan_histogram_in_gravity, yaw_in_submap_from_gravity);
}
...
ActiveSubmaps3D::ActiveSubmaps3D(const proto::SubmapsOptions3D& options)
    : options_(options), range_data_inserter_(options.range_data_inserter_options()) {}

std::vector<std::shared_ptr<const Submap3D>> ActiveSubmaps3D::submaps() const {
  return std::vector<std::shared_ptr<const Submap3D>>(submaps_.begin(), submaps_.end());
}

std::vector<std::shared_ptr<const Submap3D>> ActiveSubmaps3D::InsertData(
    const sensor::RangeData& range_data, const Eigen::Quaterniond& local_from_gravity_aligned,
    const Eigen::VectorXf& rotational_scan_matcher_histogram_in_gravity) {
  if (submaps_.empty() || submaps_.back()->num_range_data() == options_.num_range_data()) {
    AddSubmap(transform::Rigid3d(range_data.origin.cast<double>(), local_from_gravity_aligned),
              rotational_scan_matcher_histogram_in_gravity.size());
  }
  for (auto& submap : submaps_) {
    submap->InsertData(range_data, range_data_inserter_, options_.high_resolution_max_range(),
                       local_from_gravity_aligned, rotational_scan_matcher_histogram_in_gravity);
  }
...
void ActiveSubmaps3D::AddSubmap(const transform::Rigid3d& local_submap_pose,
  ...
  if (submaps_.size() >= 2) {
    ...
    // We use `ForgetIntensityHybridGrid` to reduce memory usage. Since we
    // use active submaps and their associated intensity hybrid grids for
    // scan matching, we call `ForgetIntensityHybridGrid` once we remove the
    // submap from active submaps and no longer need the intensity hybrid
    // grid.
  ...
  submaps_.emplace_back(new Submap3D(options_.high_resolution(), options_.low_resolution(), local_submap_pose,
