...
struct Span {
  ...
  bool operator<(const Span& other) const {
    return std::forward_as_tuple(start_index, end_index) < std::forward_as_tuple(other.start_index, other.end_index);
  }
};
...
std::vector<Floor> FindFloors(const proto::Trajectory& trajectory, const std::vector<Span>& spans,
  ...
  for (size_t i = 0; i < spans.size(); ++i) {
    ...
    // Otherwise, add this short piece to the level before and after it. It
    // is likely some intermediate level on stairs.
    size_t index = i - 1;
    if (index < spans.size()) {
  ...
  for (auto& level : level_spans) {
    ...
    for (const auto& span : level.second) {
      if (!IsShort(trajectory, span)) {
        // To figure out the median height of this floor, we only care
