...
TEST(PoseExtrapolatorTest, EstimateGravityOrientationWithIMU) {
    ...
    auto extrapolator = PoseExtrapolator::InitializeWithImu(
        common::FromSeconds(kPoseQueueDuration), kGravityTimeConstant,
        imu_data);
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        extrapolator->AddImuData(sensor::ImuData{
            current_time, gravity_acceleration, angular_velocity});
    }
...
TEST(PoseExtrapolatorTest, ExtrapolateWithPoses) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        transform::Rigid3d expected_pose =
            ...
            transform::Rigid3d(
                ...
                Eigen::AngleAxisd(
                    kExtrapolateDuration * angular_velocity.norm(),
                    angular_velocity.normalized()));
        EXPECT_THAT(
            extrapolator.ExtrapolatePose(
                current_time + common::FromSeconds(kExtrapolateDuration)),
            transform::IsNearly(expected_pose, kExtrapolatePrecision));
        EXPECT_EQ(
            common::ToUniversal(extrapolator.GetLastExtrapolatedTime()),
            common::ToUniversal(current_time +
    ...
    EXPECT_NEAR(
        0.f,
        (gravity_axis.axis().normalized() - Eigen::Vector3d::UnitZ()).norm(),
...
TEST(PoseExtrapolatorTest, ExtrapolateWithIMU) {
    ...
    for (int i = 0; i < kPoseQueueDuration + 2; ++i) {
        current_time += common::FromSeconds(1);
        extrapolator->AddImuData(sensor::ImuData{current_time,
                                                 initial_gravity_acceleration,
                                                 initial_angular_velocity});
        extrapolator->AddPose(current_time, current_pose);
    }
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 10; ++j) {
            current_time += common::FromSeconds(kGravityTimeConstant);
            extrapolator->AddImuData(
                sensor::ImuData{current_time, initial_gravity_acceleration,
                                angular_velocity_yaw});
            Eigen::Quaterniond expected_rotation(Eigen::AngleAxisd(
                kGravityTimeConstant * angular_velocity_yaw.norm(),
