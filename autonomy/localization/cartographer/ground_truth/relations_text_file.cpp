...
common::Time UnixToCommonTime(double unix_time) {
  ...
  return common::FromUniversal(common::kUtsEpochOffsetFromUnixEpochInSeconds * kUtsTicksPerSecond) +
...
proto::GroundTruth ReadRelationsTextFile(const std::string& relations_filename) {
  ...
  while (relations_stream >> unix_time_1 >> unix_time_2 >> x >> y >> z >> roll >> pitch >> yaw) {
    ...
    const transform::Rigid3d expected =
        transform::Rigid3d(transform::Rigid3d::Vector(x, y, z), transform::RollPitchYaw(roll, pitch, yaw));
