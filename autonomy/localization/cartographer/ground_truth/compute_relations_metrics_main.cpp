...
#include "autonomy/localization/cartographer/ground_truth/relations_text_file.hpp"
#include "autonomy/localization/cartographer/io/proto_stream.h"
#include "autonomy/localization/cartographer/io/proto_stream_deserializer.h"
#include "autonomy/localization/cartographer/mapping/proto/pose_graph.pb.h"
...
Error ComputeError(const transform::Rigid3d& pose1, const transform::Rigid3d& pose2,
                   const transform::Rigid3d& expected) {
  const transform::Rigid3d error = (pose1.inverse() * pose2) * expected.inverse();
  return Error{error.translation().squaredNorm(), common::Pow2(transform::GetAngle(error))};
}
...
std::string MeanAndStdDevString(const std::vector<double>& values) {
  ...
  out << std::fixed << std::setprecision(5) << mean << " +/- " << standard_deviation;
...
std::string StatisticsString(const std::vector<Error>& errors) {
  ...
  for (const Error& error : errors) {
    ...
    rotational_errors_degrees.push_back(common::RadToDeg(std::sqrt(error.rotational_squared)));
    squared_rotational_errors_degrees.push_back(common::Pow2(rotational_errors_degrees.back()));
  }
  return "Abs translational error " + MeanAndStdDevString(translational_errors) +
         " m\n"
...
void WriteRelationMetricsToFile(const std::vector<Error>& errors, const proto::GroundTruth& ground_truth,
  ...
  relation_errors_file << "translational_error,squared_translational_error,rotational_"
  ...
  for (int relation_index = 0; relation_index < ground_truth.relation_size(); ++relation_index) {
    ...
    double rotational_errors_degree = common::RadToDeg(std::sqrt(error.rotational_squared));
    double squared_rotational_errors_degree = common::Pow2(rotational_errors_degree);
    relation_errors_file << translational_error << "," << squared_translational_error << "," << rotational_errors_degree
                         << "," << squared_rotational_errors_degree << "," << relation.expected().translation().x()
                         << "," << relation.expected().translation().y() << "," << relation.expected().translation().z()
                         << "," << relation.expected().rotation().w() << "," << relation.expected().rotation().x()
                         << "," << relation.expected().rotation().y() << "," << relation.expected().rotation().z()
                         << "," << relation.covered_distance() << "\n";
  }
  relation_errors_file.close();
}

transform::Rigid3d LookupTransform(const transform::TransformInterpolationBuffer& transform_interpolation_buffer,
                                   const common::Time time) {
  const common::Time earliest_time = transform_interpolation_buffer.earliest_time();
  ...
  return transform_interpolation_buffer.Lookup(transform_interpolation_buffer.latest_time());
}

void Run(const std::string& pose_graph_filename, const std::string& relations_filename,
         const bool read_text_file_with_unix_timestamps, const bool write_relation_metrics) {
  ...
  const transform::TransformInterpolationBuffer transform_interpolation_buffer(pose_graph.trajectory(0));
  ...
  if (read_text_file_with_unix_timestamps) {
    ...
    std::ifstream ground_truth_stream(relations_filename.c_str(), std::ios::binary);
    CHECK(ground_truth.ParseFromIstream(&ground_truth_stream));
  }

  std::vector<Error> errors;
  for (const auto& relation : ground_truth.relation()) {
    const auto pose1 = LookupTransform(transform_interpolation_buffer, common::FromUniversal(relation.timestamp1()));
    const auto pose2 = LookupTransform(transform_interpolation_buffer, common::FromUniversal(relation.timestamp2()));
    const transform::Rigid3d expected = transform::ToRigid3(relation.expected());
    errors.push_back(ComputeError(pose1, pose2, expected));
  }

  const std::string relation_metrics_filename = pose_graph_filename + ".relation_metrics.csv";
