...
void RegisterMapBuilderServerMetrics(metrics::FamilyFactory* factory) { MapBuilderServer::RegisterMetrics(factory); }
...
std::unique_ptr<MapBuilderServerInterface> CreateMapBuilderServer(
  ...
  return absl::make_unique<MapBuilderServer>(map_builder_server_options, std::move(map_builder));
}
