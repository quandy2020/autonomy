...
#include "autonomy/localization/cartographer/cloud/client/map_builder_stub.hpp"
#include "autonomy/localization/cartographer/cloud/internal/map_builder_server.hpp"
#include "autonomy/localization/cartographer/cloud/map_builder_server_options.hpp"
#include "autonomy/localization/cartographer/io/internal/in_memory_proto_stream.hpp"
#include "autonomy/localization/cartographer/io/internal/testing/test_helpers.hpp"
...
#include "autonomy/localization/cartographer/mapping/internal/testing/mock_pose_graph.hpp"
#include "autonomy/localization/cartographer/mapping/internal/testing/mock_trajectory_builder.hpp"
...
class ClientServerTestBase : public T {
 protected:
  void SetUp() override {
    // TODO(cschuet): Due to the hard-coded addresses these tests will
    // become flaky when run in parallel.
...
TEST_P(ClientServerTestByGridType, LocalSlam2D) {
  ...
  EXPECT_EQ(stub_->pose_graph()->GetTrajectoryStates().at(trajectory_id), PoseGraphInterface::TrajectoryState::ACTIVE);
  stub_->FinishTrajectory(trajectory_id);
...
TEST_P(ClientServerTestByGridType, LocalSlamAndDelete2D) {
  ...
  stub_->pose_graph()->RunFinalOptimization();
  EXPECT_EQ(stub_->pose_graph()->GetTrajectoryStates().at(trajectory_id), PoseGraphInterface::TrajectoryState::DELETED);
...
TEST_F(ClientServerTest, GlobalSlam3D) {
  ...
  // First range data will not call back while initializing PoseExtrapolator,
  // so expect one less.
