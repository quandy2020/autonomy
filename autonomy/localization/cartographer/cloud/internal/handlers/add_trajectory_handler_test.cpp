...
#include "autonomy/localization/cartographer/cloud/internal/testing/handler_test.hpp"
#include "autonomy/localization/cartographer/cloud/internal/testing/test_helpers.hpp"
#include "autonomy/localization/cartographer/mapping/internal/testing/mock_map_builder.hpp"
...
class AddTrajectoryHandlerTest : public testing::HandlerTest<AddTrajectorySignature, AddTrajectoryHandler> {
 public:
  void SetUp() override {
    ...
    EXPECT_CALL(*mock_map_builder_context_, GetLocalSlamResultCallbackForSubscriptions()).WillOnce(Return(nullptr));
    EXPECT_CALL(*mock_map_builder_context_, map_builder()).WillOnce(ReturnRef(*mock_map_builder_));
  }

 protected:
  std::set<mapping::TrajectoryBuilderInterface::SensorId> ParseSensorIds(const proto::AddTrajectoryRequest& request) {
...
TEST_F(AddTrajectoryHandlerTest, NoLocalSlamUploader) {
  ...
  EXPECT_CALL(*mock_map_builder_,
              AddTrajectoryBuilder(ContainerEq(ParseSensorIds(request)),
                                   Truly(testing::BuildProtoPredicateEquals(&request.trajectory_builder_options())), _))
      .WillOnce(Return(13));
  EXPECT_CALL(*mock_map_builder_context_, RegisterClientIdForTrajectory(Eq("CLIENT_ID"), Eq(13)));
...
TEST_F(AddTrajectoryHandlerTest, WithLocalSlamUploader) {
  ...
  EXPECT_CALL(*mock_map_builder_,
      ...
      .WillOnce(Return(13));
  EXPECT_CALL(*mock_map_builder_context_, RegisterClientIdForTrajectory(Eq("CLIENT_ID"), Eq(13)));
  auto upstream_trajectory_builder_options = request.trajectory_builder_options();
  ...
  EXPECT_CALL(*mock_local_trajectory_uploader_,
              AddTrajectory(Eq("CLIENT_ID"), Eq(13), ParseSensorIds(request),
                            Truly(testing::BuildProtoPredicateEquals(&upstream_trajectory_builder_options))))
