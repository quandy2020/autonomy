...
metrics::Family<metrics::Counter>* AddSensorDataBatchHandler::counter_metrics_family_ =
    metrics::Family<metrics::Counter>::Null();

void AddSensorDataBatchHandler::OnRequest(const proto::AddSensorDataBatchRequest& request) {
    for (const proto::SensorData& sensor_data : request.sensor_data()) {
        if (!GetContext<MapBuilderContextInterface>()->CheckClientIdForTrajectory(
                sensor_data.sensor_metadata().client_id(), sensor_data.sensor_metadata().trajectory_id())) {
            LOG(ERROR) << "Unknown trajectory with ID " << sensor_data.sensor_metadata().trajectory_id()
                       << " and client_id " << sensor_data.sensor_metadata().client_id();
            Finish(::grpc::Status(::grpc::NOT_FOUND, "Unknown trajectory"));
            return;
        }
        ClientMetrics* const metrics = GetOrCreateClientMetrics(sensor_data.sensor_metadata().client_id(),
        ...
        switch (sensor_data.sensor_data_case()) {
            case proto::SensorData::kOdometryData:
                this->template GetUnsynchronizedContext<MapBuilderContextInterface>()->EnqueueSensorData(
                    sensor_data.sensor_metadata().trajectory_id(),
                    sensor::MakeDispatchable(sensor_data.sensor_metadata().sensor_id(),
            ...
            case proto::SensorData::kLocalSlamResultData:
                GetContext<MapBuilderContextInterface>()->EnqueueLocalSlamResultData(
                    sensor_data.sensor_metadata().trajectory_id(), sensor_data.sensor_metadata().sensor_id(),
                    sensor_data.local_slam_result_data());
                metrics->local_slam_result_counter->Increment();
                break;
            default:
                LOG(FATAL) << "Unknown sensor data type: " << sensor_data.sensor_data_case();
        }
    }
    Send(absl::make_unique<google::protobuf::Empty>());
}

void AddSensorDataBatchHandler::RegisterMetrics(metrics::FamilyFactory* family_factory) {
    counter_metrics_family_ =
        family_factory->NewCounterFamily("cartographer_sensor_data_total", "Sensor data received");
}

AddSensorDataBatchHandler::ClientMetrics* AddSensorDataBatchHandler::GetOrCreateClientMetrics(
    ...
    new_metrics->odometry_sensor_counter = counter_metrics_family_->Add(
        {{"client_id", client_id}, {"trajectory_id", std::to_string(trajectory_id)}, {"sensor", "odometry"}});
    new_metrics->imu_sensor_counter = counter_metrics_family_->Add(
        {{"client_id", client_id}, {"trajectory_id", std::to_string(trajectory_id)}, {"sensor", "imu"}});
    new_metrics->fixed_frame_pose_counter = counter_metrics_family_->Add(
        {{"client_id", client_id}, {"trajectory_id", std::to_string(trajectory_id)}, {"sensor", "fixed_frame_pose"}});
    new_metrics->landmark_counter = counter_metrics_family_->Add(
        {{"client_id", client_id}, {"trajectory_id", std::to_string(trajectory_id)}, {"sensor", "landmark"}});
    new_metrics->local_slam_result_counter = counter_metrics_family_->Add(
        {{"client_id", client_id}, {"trajectory_id", std::to_string(trajectory_id)}, {"sensor", "local_slam_result"}});
    new_metrics->timed_point_cloud_counter = counter_metrics_family_->Add(
        {{"client_id", client_id}, {"trajectory_id", std::to_string(trajectory_id)}, {"sensor", "timed_point_cloud"}});
