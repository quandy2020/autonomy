...
#include "autonomy/localization/cartographer/sensor/data.hpp"
...
class MapBuilderContextInterface : public autonomy::common::async_grpc::ExecutionContext {
 public:
  ...
  // Calling with 'nullptr' signals subscribers that the subscription has
  // ended, e.g. this happens when the corresponding trajectory was finished
  // and hence no more local SLAM updates will occur. The callback can return
  // 'false' to indicate that the client is not interested in more local SLAM
  // updates and 'MapBuilderServer' will end the subscription.
  ...
  // subscription.
  using GlobalSlamOptimizationCallback = std::function<bool(const std::map<int /* trajectory_id */, mapping::SubmapId>&,
  ...
  virtual LocalSlamSubscriptionId SubscribeLocalSlamResults(int trajectory_id,
                                                            LocalSlamSubscriptionCallback callback) = 0;
  ...
  virtual void EnqueueLocalSlamResultData(int trajectory_id, const std::string& sensor_id,
