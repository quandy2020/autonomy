...
bool IsNewSubmap(const mapping::proto::Submap& submap) {
    return (submap.has_submap_2d() &&
            submap.submap_2d().num_range_data() == 1) ||
...
public:
    ...
    grpc::Status AddTrajectory(const std::string& client_id,
    ...
    void EnqueueSensorData(
        std::unique_ptr<proto::SensorData> sensor_data) final;
    void TryRecovery();

    SensorId GetLocalSlamResultSensorId(int local_trajectory_id) const final {
        return SensorId{
            SensorId::SensorType::LOCAL_SLAM_RESULT,
...
LocalTrajectoryUploader::LocalTrajectoryUploader(
    ...
    client_channel_ =
        ::grpc::CreateChannel(uplink_server_address, channel_creds);
...
void LocalTrajectoryUploader::TryRecovery() {
    if (client_channel_->GetState(false /* try_to_connect */) !=
        ...
        if (!client_channel_->WaitForConnected(deadline)) {
            LOG(ERROR)
                << "Failed to re-connect to uplink prior to recovery attempt.";
            return;
        }
    }
        ...
        if (sensor_data) {
            ...
            if (sensor_data->sensor_data_case() ==
                ...
                IsNewSubmap(sensor_data->local_slam_result_data().submaps(
                    sensor_data->local_slam_result_data().submaps_size() -
                    1))) {
                break;
            } else {
                send_queue_.Pop();
    ...
    for (const auto& entry : local_trajectory_id_to_trajectory_info_) {
        ...
        if (!status.ok()) {
            LOG(ERROR)
                << "Failed to create trajectory. Aborting recovery attempt. "
...
void LocalTrajectoryUploader::ProcessSendQueue() {
    ...
    while (!shutting_down_) {
        ...
        if (sensor_data) {
            if (!TranslateTrajectoryId(
                    sensor_data->mutable_sensor_metadata())) {
                batch_request.clear_sensor_data();
                TryRecovery();
                continue;
            }
            proto::SensorData* added_sensor_data =
            ...
            *added_sensor_data = *sensor_data;
            ...
            if (batch_request.sensor_data_size() == batch_size_) {
                autonomy::common::async_grpc::Client<
                    handlers::AddSensorDataBatchSignature>
                    client(client_channel_,
                           common::FromSeconds(kConnectionTimeoutInSeconds),
                           autonomy::common::async_grpc::
                               CreateUnlimitedConstantDelayStrategy(
...
grpc::Status LocalTrajectoryUploader::RegisterTrajectory(
    ...
    *request.add_expected_sensor_ids() =
    ...
    LOG(INFO) << "Created trajectory for client_id: "
              << trajectory_info.client_id
...
std::unique_ptr<LocalTrajectoryUploaderInterface> CreateLocalTrajectoryUploader(
    const std::string& uplink_server_address, int batch_size,
    bool enable_ssl_encryption, bool enable_google_auth) {
    return make_unique<LocalTrajectoryUploader>(
        uplink_server_address, batch_size, enable_ssl_encryption,
        enable_google_auth);
}
