...
#include "autonomy/localization/cartographer/cloud/internal/local_trajectory_uploader.hpp"
...
#include "autonomy/localization/cartographer/common/internal/blocking_queue.hpp"
...
class LocalTrajectoryUploader : public LocalTrajectoryUploaderInterface {
 ...
 public:
  LocalTrajectoryUploader(const std::string& uplink_server_address, int batch_size, bool enable_ssl_encryption,
  ...
  grpc::Status AddTrajectory(const std::string& client_id, int local_trajectory_id,
                             const std::set<SensorId>& expected_sensor_ids,
                             const mapping::proto::TrajectoryBuilderOptions& trajectory_options) final;
  grpc::Status FinishTrajectory(const std::string& client_id, int local_trajectory_id) final;
...
LocalTrajectoryUploader::LocalTrajectoryUploader(const std::string& uplink_server_address, int batch_size,
                                                 bool enable_ssl_encryption, bool enable_google_auth)
    : batch_size_(batch_size) {
  auto channel_creds = enable_google_auth
                           ? grpc::GoogleDefaultCredentials()
                           : (enable_ssl_encryption ? ::grpc::SslCredentials(::grpc::SslCredentialsOptions())
                                                    : ::grpc::InsecureChannelCredentials());

  client_channel_ = ::grpc::CreateChannel(uplink_server_address, channel_creds);
  std::chrono::system_clock::time_point deadline =
      std::chrono::system_clock::now() + std::chrono::seconds(kConnectionTimeoutInSeconds);
...
void LocalTrajectoryUploader::Start() {
  ...
  upload_thread_ = make_unique<std::thread>([this]() { this->ProcessSendQueue(); });
}
...
void LocalTrajectoryUploader::TryRecovery() {
  if (client_channel_->GetState(false /* try_to_connect */) != grpc_connectivity_state::GRPC_CHANNEL_READY) {
    LOG(INFO) << "Trying to re-connect to uplink...";
    std::chrono::system_clock::time_point deadline =
        std::chrono::system_clock::now() + std::chrono::seconds(kConnectionRecoveryTimeoutInSeconds);
    if (!client_channel_->WaitForConnected(deadline)) {
  ...
  while (true) {
    ...
    proto::SensorData* sensor_data = send_queue_.PeekWithTimeout<proto::SensorData>(kPopTimeout);
    if (sensor_data) {
      CHECK_GE(sensor_data->local_slam_result_data().submaps_size(), 0);
      if (sensor_data->sensor_data_case() == proto::SensorData::kLocalSlamResultData &&
  ...
  for (const auto& entry : local_trajectory_id_to_trajectory_info_) {
    ...
    if (!status.ok()) {
      LOG(ERROR) << "Failed to create trajectory. Aborting recovery attempt. " << status.error_message();
      return;
    }
  }
  ...
  while (!shutting_down_) {
    ...
    if (sensor_data) {
      ...
      *added_sensor_data = *sensor_data;
      ...
      // A submap also holds a trajectory id that must be translated to
      // uplink's trajectory id.
      if (added_sensor_data->has_local_slam_result_data()) {
        for (mapping::proto::Submap& mutable_submap :
          ...
          mutable_submap.mutable_submap_id()->set_trajectory_id(added_sensor_data->sensor_metadata().trajectory_id());
        }
      }

      if (batch_request.sensor_data_size() == batch_size_) {
        autonomy::common::async_grpc::Client<handlers::AddSensorDataBatchSignature> client(
            client_channel_, common::FromSeconds(kConnectionTimeoutInSeconds),
            autonomy::common::async_grpc::CreateUnlimitedConstantDelayStrategy(common::FromSeconds(1),
                                                                               kUnrecoverableStatusCodes));
...
bool LocalTrajectoryUploader::TranslateTrajectoryId(proto::SensorMetadata* sensor_metadata) {
  auto it = local_trajectory_id_to_trajectory_info_.find(sensor_metadata->trajectory_id());
...
grpc::Status LocalTrajectoryUploader::AddTrajectory(
    const std::string& client_id, int local_trajectory_id, const std::set<SensorId>& expected_sensor_ids,
    const mapping::proto::TrajectoryBuilderOptions& trajectory_options) {
  CHECK_EQ(local_trajectory_id_to_trajectory_info_.count(local_trajectory_id), 0);
  local_trajectory_id_to_trajectory_info_.emplace(
      local_trajectory_id, TrajectoryInfo{{}, expected_sensor_ids, trajectory_options, client_id});
  return RegisterTrajectory(local_trajectory_id);
}

grpc::Status LocalTrajectoryUploader::RegisterTrajectory(int local_trajectory_id) {
  ...
  *request.mutable_trajectory_builder_options() = trajectory_info.trajectory_options;
  ...
  *request.add_expected_sensor_ids() = cloud::ToProto(GetLocalSlamResultSensorId(local_trajectory_id));
  autonomy::common::async_grpc::Client<handlers::AddTrajectorySignature> client(
      client_channel_, common::FromSeconds(kConnectionTimeoutInSeconds));
  ::grpc::Status status;
  if (!client.Write(request, &status)) {
    LOG(ERROR) << "Failed to register local_trajectory_id " << local_trajectory_id << " with uplink server. "
...
grpc::Status LocalTrajectoryUploader::FinishTrajectory(const std::string& client_id, int local_trajectory_id) {
  ...
  if (!cloud_trajectory_id.has_value()) {
    return grpc::Status(grpc::StatusCode::UNAVAILABLE, "trajectory_id has not been created in uplink, ignoring.");
  }
  proto::FinishTrajectoryRequest request;
  request.set_client_id(client_id);
  request.set_trajectory_id(cloud_trajectory_id.value());
  autonomy::common::async_grpc::Client<handlers::FinishTrajectorySignature> client(
      client_channel_, common::FromSeconds(kConnectionTimeoutInSeconds));
  grpc::Status status;
...
void LocalTrajectoryUploader::EnqueueSensorData(std::unique_ptr<proto::SensorData> sensor_data) {
...
std::unique_ptr<LocalTrajectoryUploaderInterface> CreateLocalTrajectoryUploader(
    const std::string& uplink_server_address, int batch_size, bool enable_ssl_encryption, bool enable_google_auth) {
  return make_unique<LocalTrajectoryUploader>(uplink_server_address, batch_size, enable_ssl_encryption,
                                              enable_google_auth);
}
