    ...
    return
        [this](int trajectory_id, common::Time time,
            ...
            if (it == client_ids_.end()) {
                LOG(ERROR) << "Unknown trajectory_id " << trajectory_id
                           << ". Ignoring.";
                return;
            }
            map_builder_server_->OnLocalSlamResult(
                trajectory_id, it->second, time, local_pose,
                std::move(range_data), std::move(insertion_result));
        };
}
...
void MapBuilderContext<SubmapType>::RegisterClientIdForTrajectory(
    ...
    LOG(INFO) << "Registering trajectory_id " << trajectory_id
