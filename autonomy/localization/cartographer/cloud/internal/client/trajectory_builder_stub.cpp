...
void TrajectoryBuilderStub::AddSensorData(
    ...
    if (!add_rangefinder_client_) {
        add_rangefinder_client_ =
            absl::make_unique<autonomy::common::async_grpc::Client<
                handlers::AddRangefinderDataSignature>>(client_channel_);
    }
...
void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id,
    ...
    if (!add_imu_client_) {
        add_imu_client_ =
            absl::make_unique<autonomy::common::async_grpc::Client<
                handlers::AddImuDataSignature>>(client_channel_);
    }
...
void TrajectoryBuilderStub::AddSensorData(
    ...
    if (!add_odometry_client_) {
        add_odometry_client_ =
            absl::make_unique<autonomy::common::async_grpc::Client<
                handlers::AddOdometryDataSignature>>(client_channel_);
    }
...
void TrajectoryBuilderStub::AddSensorData(
    ...
    if (!add_fixed_frame_pose_client_) {
        add_fixed_frame_pose_client_ =
            absl::make_unique<autonomy::common::async_grpc::Client<
                handlers::AddFixedFramePoseDataSignature>>(client_channel_);
    }
...
void TrajectoryBuilderStub::AddSensorData(
    ...
    if (!add_landmark_client_) {
        add_landmark_client_ =
            absl::make_unique<autonomy::common::async_grpc::Client<
                handlers::AddLandmarkDataSignature>>(client_channel_);
    }
...
void TrajectoryBuilderStub::RunLocalSlamResultsReader(
    autonomy::common::async_grpc::Client<
        handlers::ReceiveLocalSlamResultsSignature>* client,
    LocalSlamResultCallback local_slam_result_callback) {
    ...
    while (client->StreamRead(&response)) {
        ...
        transform::Rigid3d local_pose =
            transform::ToRigid3(response.local_pose());
