...
#include "autonomy/localization/cartographer/cloud/internal/client/trajectory_builder_stub.hpp"
...
#include "autonomy/localization/cartographer/mapping/internal/local_slam_result_data.hpp"
...
TrajectoryBuilderStub::TrajectoryBuilderStub(std::shared_ptr<::grpc::Channel> client_channel, const int trajectory_id,
  ...
  if (local_slam_result_callback) {
    ...
    auto* receive_local_slam_results_client_ptr = &receive_local_slam_results_client_;
    receive_local_slam_results_thread_ =
        absl::make_unique<std::thread>([receive_local_slam_results_client_ptr, local_slam_result_callback]() {
          RunLocalSlamResultsReader(receive_local_slam_results_client_ptr, local_slam_result_callback);
        });
  }
}
...
void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id,
  ...
  if (!add_rangefinder_client_) {
    add_rangefinder_client_ =
        absl::make_unique<autonomy::common::async_grpc::Client<handlers::AddRangefinderDataSignature>>(client_channel_);
  }
  proto::AddRangefinderDataRequest request;
  CreateAddRangeFinderDataRequest(sensor_id, trajectory_id_, client_id_, sensor::ToProto(timed_point_cloud_data),
                                  &request);
  add_rangefinder_client_->Write(request);
}

void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id, const sensor::ImuData& imu_data) {
  if (!add_imu_client_) {
    add_imu_client_ =
        absl::make_unique<autonomy::common::async_grpc::Client<handlers::AddImuDataSignature>>(client_channel_);
  }
  proto::AddImuDataRequest request;
  CreateAddImuDataRequest(sensor_id, trajectory_id_, client_id_, sensor::ToProto(imu_data), &request);
  add_imu_client_->Write(request);
}

void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id, const sensor::OdometryData& odometry_data) {
  if (!add_odometry_client_) {
    add_odometry_client_ =
        absl::make_unique<autonomy::common::async_grpc::Client<handlers::AddOdometryDataSignature>>(client_channel_);
  }
  proto::AddOdometryDataRequest request;
  CreateAddOdometryDataRequest(sensor_id, trajectory_id_, client_id_, sensor::ToProto(odometry_data), &request);
...
void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id,
  ...
  if (!add_fixed_frame_pose_client_) {
    add_fixed_frame_pose_client_ =
        absl::make_unique<autonomy::common::async_grpc::Client<handlers::AddFixedFramePoseDataSignature>>(
            client_channel_);
  }
  proto::AddFixedFramePoseDataRequest request;
  CreateAddFixedFramePoseDataRequest(sensor_id, trajectory_id_, client_id_, sensor::ToProto(fixed_frame_pose),
                                     &request);
  add_fixed_frame_pose_client_->Write(request);
}

void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id, const sensor::LandmarkData& landmark_data) {
  if (!add_landmark_client_) {
    add_landmark_client_ =
        absl::make_unique<autonomy::common::async_grpc::Client<handlers::AddLandmarkDataSignature>>(client_channel_);
  }
  proto::AddLandmarkDataRequest request;
  CreateAddLandmarkDataRequest(sensor_id, trajectory_id_, client_id_, sensor::ToProto(landmark_data), &request);
...
void TrajectoryBuilderStub::RunLocalSlamResultsReader(
    autonomy::common::async_grpc::Client<handlers::ReceiveLocalSlamResultsSignature>* client,
    LocalSlamResultCallback local_slam_result_callback) {
  ...
  while (client->StreamRead(&response)) {
    ...
    auto insertion_result = response.has_insertion_result()
                                ? absl::make_unique<InsertionResult>(InsertionResult{
                                      mapping::NodeId{response.insertion_result().node_id().trajectory_id(),
                                                      response.insertion_result().node_id().node_index()}})
                                : nullptr;
    local_slam_result_callback(trajectory_id, time, local_pose, range_data, std::move(insertion_result));
  }
  client->StreamFinish();
}
