...
void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id,
    ...
    if (!add_rangefinder_client_) {
        add_rangefinder_client_ =
            absl::make_unique<autonomy::common::async_grpc::Client<handlers::AddRangefinderDataSignature>>(
                client_channel_);
    }
...
void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id, const sensor::OdometryData& odometry_data) {
    if (!add_odometry_client_) {
        add_odometry_client_ =
            absl::make_unique<autonomy::common::async_grpc::Client<handlers::AddOdometryDataSignature>>(
                client_channel_);
    }
...
void TrajectoryBuilderStub::AddSensorData(const std::string& sensor_id, const sensor::LandmarkData& landmark_data) {
    if (!add_landmark_client_) {
        add_landmark_client_ =
            absl::make_unique<autonomy::common::async_grpc::Client<handlers::AddLandmarkDataSignature>>(
                client_channel_);
    }
