...
void DenseMapBuilder::Clear() {
    ...
    needs_refresh_.store(false, std::memory_order_relaxed);
}

void DenseMapBuilder::ScheduleOn(::autonomy::common::ThreadPool* pool) {
    schedule_pool_ = pool;
}

void DenseMapBuilder::ScheduleIntegrate() {
    // Task-friendly marker: flag refresh; optional no-op Schedule on pool.
    RequestRebuild();
    if (schedule_pool_) {
        auto task = std::make_unique<::autonomy::common::Task>();
        task->SetWorkItem([]() {
            // Marker only — Integrate consumes needs_refresh_ on the feed path.
        });
        schedule_pool_->Schedule(std::move(task));
    }
}
