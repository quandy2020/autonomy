...
void trajectory_io::save_frame_trajectory(const std::string& path, const std::string& format) const {
    ...
    if (num_valid_frms == 0) {
        AWARN << "there are no valid frames, cannot dump frame trajectory";
        return;
    }
    ...
    AINFO << "dump frame trajectory in "" << format << "" format from frame " << reference_keyframes.begin()->first << " to frame " << reference_keyframes.rbegin()->first << " (" << num_valid_frms << " frames)";
    ...
    for (unsigned int i = 0; i < num_valid_frms; ++i, ++rk_itr, ++rc_itr) {
        ...
        if (is_lost_frms.at(frm_id)) {
            AWARN << "frame " << frm_id << " was lost";
            continue;
        }

        // check if the frame was skipped or not
        if (frm_id != i + offset) {
            AWARN << "frame(s) from " << prev_frm_id + 1 << " to " << frm_id - 1 << " was/were skipped";
            offset = frm_id - i;
        }
    ...
    if (rk_itr != rk_itr_end || rc_itr != rc_itr_end) {
        AERROR << "the sizes of frame statistics are not matched";
    }
...
void trajectory_io::save_keyframe_trajectory(const std::string& path, const std::string& format) const {
    ...
    if (keyfrms.empty()) {
        AWARN << "there are no valid keyframes, cannot dump keyframe trajectory";
        return;
    }
    ...
    AINFO << "dump keyframe trajectory in "" << format << "" format from keyframe " << (*keyfrms.begin())->id_ << " to keyframe " << (*keyfrms.rbegin())->id_ << " (" << keyfrms.size() << " keyframes)";
