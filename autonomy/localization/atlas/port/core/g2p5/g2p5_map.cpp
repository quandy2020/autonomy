...
#include "autonomy/localization/atlas/port/core/g2p5/g2p5_map.hpp"

#include <malloc.h>
#include <cstdlib>
#include <execution>

namespace atlas_lio::g2p5 {
        ...
        for (int byi = 0; byi < grid_size_y_; ++byi) {
            ...
            for (int sxi = 0; sxi < sub_grid_width_; ++sxi) {
                for (int syi = 0; syi < sub_grid_width_; ++syi) {
                    ...
                    const float occ =
                        (visit_cnt > 3) ? static_cast<float>(hit_cnt) / static_cast<float>(visit_cnt)
                                        : -1.f;
                    if (occ < 0.f) {
                        continue;
                    }
                    if (occ > options_.occupancy_ratio_) {
                        (*occu_map.mutable_data())[MapIdx(image_width, x, y)] = 100;
                    } else {
                        const int index = MapIdx(image_width, x, y);
                        const int index_min = std::max(0, index - 1);
                        const int index_max = std::min(grid_map_size_1, index + 1);
                        for (int extend = index_min; extend <= index_max; ++extend) {
