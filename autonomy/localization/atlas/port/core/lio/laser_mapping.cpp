...
#include "autonomy/localization/atlas/port/common/options.hpp"
...
bool LaserMapping::LoadParamsFromYAML(const std::string &yaml_file) {
    ...
    try {
        ...
        if (yaml["fasterlio"]["global_map_voxel"]) {
            global_map_voxel_ = yaml["fasterlio"]["global_map_voxel"].as<float>();
        } else if (filter_size_map_min_ > 0.0) {
            global_map_voxel_ = static_cast<float>(filter_size_map_min_);
        }
...
void LaserMapping::ProcessIMU(const atlas_lio::IMUPtr &imu) {
    publish_count_++;
...
bool LaserMapping::SyncPackages() {
    ...
    if (!lidar_pushed_) {
        ...
        const double scan_span_s =
            measures_.scan_->empty() ? 0.0
                                     : measures_.scan_->points.back().time / double(1000);
        // Autosim Habitat clouds are instantaneous snapshots (no per-point time).
        // Do not invent a 0.1 s scan duration: that predicts the ESKF to t+0.1
        // while points stay at t, which rotates the map vs the live scan.
        if (measures_.scan_->points.size() <= 1) {
            LOG(WARNING) << "Too few input point cloud!";
            lidar_end_time_ = measures_.lidar_begin_time_ + lidar_mean_scantime_;
        } else if (scan_span_s < 1e-3) {
            lidar_end_time_ = measures_.lidar_begin_time_;
        } else if (scan_span_s < 0.5 * lidar_mean_scantime_) {
            lidar_end_time_ = measures_.lidar_begin_time_ + lidar_mean_scantime_;
        } else {
            scan_num_++;
    ...
    while (!imu_buffer_.empty()) {
        const double imu_time = imu_buffer_.front()->timestamp;
...
CloudPtr LaserMapping::GetGlobalMap(bool use_lio_pose, bool use_voxel, float res) {
    ...
    if (res <= 0.f) {
        res = global_map_voxel_ > 0.f ? global_map_voxel_ : 0.1f;
    }
...
void LaserMapping::ApplyPosePrior(const SE3& T_prior, double weight) {
    if (!(weight > 0.0) || !std::isfinite(weight)) {
        return;
    }
    const double alpha = std::min(0.3, weight / (weight + 10.0));
    auto Blend = [&](NavState& state) {
        const SE3 current = state.GetPose();
        const SE3 delta = current.inverse() * T_prior;
        state.SetPose(current * SE3::exp(alpha * delta.log()));
    };
    Blend(state_point_);
    NavState scan_state = kf_.GetX();
    Blend(scan_state);
    kf_.ChangeX(scan_state);
    NavState imu_state = kf_imu_.GetX();
    Blend(imu_state);
    kf_imu_.ChangeX(imu_state);
}
