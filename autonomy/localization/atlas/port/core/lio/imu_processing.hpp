...
#include "autonomy/localization/atlas/port/common/measure_group.hpp"
#include "autonomy/localization/atlas/port/common/point_def.hpp"
#include "autonomy/localization/atlas/port/core/lio/eskf.hpp"
#include "autonomy/localization/atlas/port/core/lio/imu_filter.hpp"
#include "autonomy/localization/atlas/port/core/lio/pose6d.hpp"
#include "autonomy/localization/atlas/port/utils/timer.hpp"
...
class ImuProcess {
   ...
   private:
    ...
    atlas_lio::IMUPtr last_imu_ = nullptr;
    std::deque<atlas_lio::IMUPtr> imu_queue_;
...
inline ImuProcess::ImuProcess() : b_first_frame_(true), imu_need_init_(true) {
    ...
    last_imu_.reset(new atlas_lio::IMU());
}
...
inline void ImuProcess::UndistortPcl(const MeasureGroup &meas, ESKF &kf_state, CloudPtr &pcl_out) {
    ...
    for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
        ...
        // Autosim lidar hitch often yields IMU Δt ≈ 0.1 s (lidar-rate). The old
        // 0.1 s abort skipped the rest of the scan's IMU predict, freezing pose
        // (yaw stuck, map vs live scan rotate apart). Integrate moderate gaps.
        if (dt <= 0.0) {
            continue;
        }
        if (dt > 0.5) {
