...
#include "autonomy/localization/atlas/port/core/ivox3d/ivox3d.hpp"
#include "autonomy/localization/atlas/port/core/lio/eskf.hpp"
#include "autonomy/localization/atlas/port/core/lio/imu_processing.hpp"
...
class LaserMapping {
   public:
    ...
    void ProcessIMU(const atlas_lio::IMUPtr &msg_in);
     ...
     * @return
     */
    CloudPtr GetGlobalMap(bool use_lio_pose, bool use_voxel = true, float res = -1.f);

    /// Pull the IMU pose a fraction of the way toward an external prior.
    /// Velocity and biases stay on the ESKF. `weight` is the same loose
    /// visual weight used by the Ceres pose prior.
    void ApplyPosePrior(const SE3& T_prior, double weight);
   ...
   private:
    ...
    std::deque<atlas_lio::IMUPtr> imu_buffer_;
