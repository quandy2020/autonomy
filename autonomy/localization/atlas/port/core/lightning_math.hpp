...
#include "autonomy/localization/atlas/port/common/pose_rpy.hpp"

namespace atlas_lio::math {
...
/// 从pose中取出yaw pitch roll（固定轴 XYZ / roll-pitch-yaw）
inline PoseRPYD SE3ToRollPitchYaw(const SE3& pose) {
    ...
    output.pitch = std::atan2(-rot(2, 0),
                              std::sqrt(rot(2, 1) * rot(2, 1) + rot(2, 2) * rot(2, 2)));
    output.roll = std::atan2(rot(2, 1), rot(2, 2));
    output.yaw = std::atan2(rot(1, 0), rot(0, 0));
    return output;
}
