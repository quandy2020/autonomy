...
/**
 * Autolink Image → system::feed_*_frame (sensor-layer ROS IO for vision).
 *
 ...
 *
 * Prefer sensor::ImuBridge for IMU topic ownership. If imu_topic is set here,
 * OnImu only fills ImuSensor (system::feed_imu stays with ImuBridge).
 */
class CameraBridge {
public:
    struct Options {
        ...
        /**
         * Legacy fallback IMU topic. Prefer empty — use ImuBridge instead.
         * When set, samples are delegated to ImuSensor only (not system::feed_imu).
         */
    ...
    CameraBridge(system* slam, Options options);
    ~CameraBridge();
    ...
    void SetCameraSensor(sensor::CameraSensor* camera) { camera_sensor_ = camera; }
...
private:
    ...
    sensor::CameraSensor* camera_sensor_ = nullptr;
...
//! Compat alias — prefer CameraBridge in new code.
using ImageBridge = CameraBridge;
