...
#include "autonomy/localization/atlas/frontend/initialize/bearing_vector.hpp"
#include "autonomy/localization/atlas/frontend/initialize/perspective.hpp"
...
#include "autonomy/localization/atlas/frontend/initializer.hpp"
...
initializer::initializer(data::map_database* map_db,
    ...
    ADEBUG << "CONSTRUCT: module::initializer";
}

initializer::~initializer() {
    ADEBUG << "DESTRUCT: module::initializer";
}
...
bool initializer::try_initialize_for_monocular(data::frame& curr_frm) {
    ...
    assert(initializer_);
    ADEBUG << "try to initialize with the initial frame and the current frame: frame " << init_frm_.id_ << " - frame " << curr_frm.id_;
...
bool initializer::create_map_for_monocular(data::bow_vocabulary* bow_vocab, data::frame& curr_frm) {
    ...
    if (indefinite_scale) {
        ...
        if (curr_keyfrm->get_num_tracked_landmarks(1) < min_num_triangulated_pts_ && median_scale < 0) {
            AINFO << "seems to be wrong initialization, resetting";
    ...
    AINFO << "new map created with " << map_db_->get_num_landmarks() << " points: frame " << init_frm_.id_ << " - frame " << curr_frm.id_;
