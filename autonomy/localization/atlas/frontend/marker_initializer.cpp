...
void marker_initializer::check_marker_initialization(data::marker& mkr, size_t needed_observations_for_initialization) {
    ...
    if (mkr.observations_.size() < needed_observations_for_initialization) {
        ADEBUG << "Not using marker " << id << " yet, not enough keyframes (" << mkr.observations_.size() << ", need " << needed_observations_for_initialization << ")";
        return;
    }
    ...
    for (const auto& id_keyfrm : mkr.observations_) {
        ...
        if (keyfrm->markers_2d_.find(id) == keyfrm->markers_2d_.end()) {
            AWARN << "Couldn't find 2D marker " << id << " in keyframe that should have it";
            continue;
        }
    ...
    if (valid_keyframes.size() < needed_observations_for_initialization) {
        ADEBUG << "Not using marker " << id << " yet, not enough valid keyframes (" << valid_keyframes.size() << ", need " << needed_observations_for_initialization << ")";
        return;
    }
    ...
    for (const auto& keyfrm : valid_keyframes) {
        ...
        if (!m2d.marker_model_) {
            AERROR << "Need marker model to be set to initialize 3D marker pos from 2D corners";
            return;
        }
    ...
    ADEBUG << "Initialized corner positions for marker " << id << " from " << valid_keyframes.size() << " keyframes";
}
