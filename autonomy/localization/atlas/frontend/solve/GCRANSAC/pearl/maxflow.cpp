...
#include <stdio.h>
#include "autonomy/localization/atlas/frontend/solve/GCRANSAC/pearl/graph.h"
...
void Graph<captype, tcaptype, flowtype>::Copy(Graph<captype, tcaptype, flowtype> *g0)
{
	...
	{
		if (i->first)
			i->first = (arc *)((char *)arcs + (((char *)i->first) - ((char *)g0->arcs)));
		if (i->parent && i->parent != TERMINAL && i->parent != ORPHAN)
			i->parent = (arc *)((char *)arcs + (((char *)i->parent) - ((char *)g0->arcs)));
		if (i->next)
			i->next = (node *)((char *)nodes + (((char *)i->next) - ((char *)g0->nodes)));
	}
	...
	for (a = arcs; a < arc_last; a++)
	{
		...
		if (a->next)
			a->next = (arc *)((char *)arcs + (((char *)a->next) - ((char *)g0->arcs)));
