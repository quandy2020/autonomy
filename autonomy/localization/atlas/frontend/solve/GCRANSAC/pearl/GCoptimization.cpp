...
#include "autonomy/localization/atlas/frontend/solve/GCRANSAC/pearl/LinkedBlockList.h"
#include <stdio.h>
#include <stdlib.h>
#include <vector>
#include <iostream>
...
#else
inline static bool utIsInterruptPending()
{
	return false;
}
static void flushnow() {}
...
{
	if (nLabels <= 1)
		handleError("Number of labels must be >= 2");
	if (nSites <= 0)
		handleError("Number of sites must be >= 1");
...
bool GCoptimization::alpha_expansion(LabelID alpha_label)
{
	...
	try
	{
		...
		if (m_setupDataCostsExpansion)
			(this->*m_setupDataCostsExpansion)(size, alpha_label, &e, activeSites);
		if (m_setupSmoothCostsExpansion)
			(this->*m_setupSmoothCostsExpansion)(size, alpha_label, &e, activeSites);
...
void GCoptimization::alpha_beta_swap(LabelID alpha_label, LabelID beta_label)
{
	...
	try
	{
		...
		e.add_variable(size);
		if (m_setupDataCostsSwap)
			(this->*m_setupDataCostsSwap)(size, alpha_label, beta_label, &e, activeSites);
		if (m_setupSmoothCostsSwap)
			(this->*m_setupSmoothCostsSwap)(size, alpha_label, beta_label, &e, activeSites);
		checkInterrupt();
		e.minimize();
		checkInterrupt();
...
void GCoptimizationGridGraph::giveNeighborInfo(SiteID site, SiteID *numSites, SiteID **neighbors, EnergyTermType **weights)
{
	...
	*neighbors = &m_neighbors[site * 4];
	...
	if (m_weightedGraph)
		*weights = &m_neighborsWeights[site * 4];
	else
...
void GCoptimizationGeneralGraph::finalizeNeighbors()
{
	...
	if (!m_numNeighbors || !m_neighborsIndexes || !m_neighborsWeights)
		handleError("Not enough memory.");
	...
	for (site = 0; site < m_num_sites; site++)
	{
		if (m_neighbors && !m_neighbors[site].isEmpty())
		{
			...
			if (!m_neighborsIndexes[site] || !m_neighborsWeights[site])
				handleError("Not enough memory.");
												 ...
												 EnergyTermType **neighborsWeights)
{
	...
	for (size_t site = 0; site < m_num_sites; site++)
		m_numNeighborsTotal += m_numNeighbors[site];
