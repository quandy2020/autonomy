...
camera_database::camera_database() {
    ADEBUG << "CONSTRUCT: data::camera_database";
}
...
camera_database::~camera_database() {
    ...
    ADEBUG << "DESTRUCT: data::camera_database";
}
...
void camera_database::from_json(const nlohmann::json& json_cameras) {
    ...
    AINFO << "decoding " << json_cameras.size() << " camera(s) to load";
    for (const auto& json_id_camera : json_cameras.items()) {
        ...
        if (cameras_.count(camera_name)) {
            AINFO << "A camera with the same name ("" << camera_name << "") already existed in database.";
            continue;
        }

        AINFO << "load a camera "" << camera_name << "" from JSON";
...
nlohmann::json camera_database::to_json() const {
    ...
    AINFO << "encoding " << cameras_.size() << " camera(s) to store";
...
bool camera_database::from_db(sqlite3* db) {
    ...
    if (ret != SQLITE_OK) {
        AERROR << "SQLite error: " << sqlite3_errmsg(db);
        return false;
    }
    ...
    while ((ret = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        AINFO << "load a camera "" << camera_name << "" from database";
        camera::base* camera = nullptr;
...
bool camera_database::to_db(sqlite3* db) const {
    ...
    if (ret != SQLITE_OK) {
        AERROR << "SQLite error (prepare): " << sqlite3_errmsg(db);
        return false;
    }
    ...
    for (const auto& name_camera : cameras_) {
        ...
        if (ret != SQLITE_DONE) {
            AERROR << "SQLite step is not done: " << sqlite3_errmsg(db);
            return false;
        }
        else {
            sqlite3_reset(stmt);
    ...
    if (ret != SQLITE_OK) {
        AERROR << "SQLite error (commit): " << sqlite3_errmsg(db);
        return false;
    }
    else {
        return true;
