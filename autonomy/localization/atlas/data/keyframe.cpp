...
namespace {
using namespace autonomy::localization::atlas;
using namespace autonomy::localization::atlas::data;
...
std::shared_ptr<keyframe> keyframe::make_keyframe(unsigned int id, const frame& frm) {
    ...
    ptr->graph_node_ = autonomy::localization::atlas::make_unique<graph_node>(ptr);
    return ptr;
}
...
std::shared_ptr<keyframe> keyframe::from_stmt(sqlite3_stmt* stmt,
    ...
    if (camera->setup_type_ != autonomy::localization::atlas::camera::setup_type_t::Monocular) {
...
void keyframe::prepare_for_erasing(map_database* map_db, bow_database* bow_db) {
    if (graph_node_->is_spanning_root()) {
        AWARN << "cannot erase the root node: " << id_;
        return;
    }
    ...
    ADEBUG << "keyframe::prepare_for_erasing " << id_;
