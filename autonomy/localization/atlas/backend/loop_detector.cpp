...
#include "autonomy/localization/atlas/frontend/solve/pnp_solver.hpp"
...
loop_detector::loop_detector(data::bow_database* bow_db, data::bow_vocabulary* bow_vocab, const YAML::Node& yaml_node, const bool fix_scale_in_Sim3_estimation)
    ...
    ADEBUG << "CONSTRUCT: loop_detector";
}
...
bool loop_detector::validate_candidates_impl() {
    ...
    ADEBUG << "detect loop candidate via Sim3 estimation: keyframe " << selected_candidate_->id_ << " - keyframe " << cur_keyfrm_->id_;
    ...
    ADEBUG << "acquired " << num_final_matches << " matches after projection-match";

    if (num_final_matches_thr_ <= num_final_matches) {
        return true;
    }
    else {
        ADEBUG << "destruct loop candidate because enough matches not acquired (< " << num_final_matches_thr_ << ")";
        return false;
    }
}
...
bool loop_detector::select_loop_candidate_via_Sim3(const std::unordered_set<std::shared_ptr<data::keyframe>>& loop_candidates,
    ...
    for (const auto& candidate : loop_candidates) {
        ...
        ADEBUG << "Checking if the loop candidate is appropriate: keyframe " << candidate->id_ << " - keyframe " << cur_keyfrm_->id_ << " (num_matches: " << num_matches << ")";
        ...
        if (num_matches_thr_brute_force_ > 0) {
            ...
            ADEBUG << "num_matches_brute_force: " << num_matches_brute_force;
        ...
        if (num_valid_obs < min_num_matches_after_pose_optimize) {
            ADEBUG << "1. Number of inliers (" << num_valid_obs << ") < threshold (" << min_num_matches_after_pose_optimize << ")";
            continue;
        }
        ...
        if (already_found_landmarks.size() + num_found < min_num_valid_obs1) {
            ADEBUG << "2. Number of matches (" << already_found_landmarks.size() + num_found << ") < threshold (" << min_num_valid_obs1 << ")";
            continue;
        }
        ...
        if (num_valid_obs1 < min_num_valid_obs1) {
            ADEBUG << "2. Number of inliers (" << num_valid_obs1 << ") < threshold (" << min_num_valid_obs1 << ")";
            continue;
        }
        ...
        if (num_valid_obs1 + num_additional < min_num_valid_obs2) {
            ADEBUG << "3. Number of matches (" << num_valid_obs1 + num_additional << ") < threshold (" << min_num_valid_obs2 << ")";
            return false;
        }
        ...
        if (num_valid_obs2 < min_num_valid_obs2) {
            ADEBUG << "3. Number of inliers (" << num_valid_obs2 << ") < threshold (" << min_num_valid_obs2 << ")";
            return false;
        }
        ...
        if (scales.size() < 1) {
            ADEBUG << "not enough scale references " << scales.size();
            continue;
        }
        ...
        ADEBUG << "found loop candidate via nonlinear Sim3 optimization: keyframe " << candidate->id_ << " - keyframe " << cur_keyfrm_->id_ << " (num_optimized_inliers: " << num_optimized_inliers << ")";
