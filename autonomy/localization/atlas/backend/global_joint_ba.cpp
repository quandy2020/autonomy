...
GlobalJointBA::GlobalJointBA(data::map_database* map_db,
...
void GlobalJointBA::set_residual_mask(estimate::ResidualMask mask) {
    mask_ = mask;
}

void GlobalJointBA::set_lidar_residual_source(
    estimate::ILidarResidualSource* src) {
    lidar_src_ = src;
}

void GlobalJointBA::set_odom_residual_source(
    estimate::IOdomResidualSource* src) {
    odom_src_ = src;
}

void GlobalJointBA::abort() {
    std::lock_guard<std::mutex> lock(mtx_thread_);
    abort_loop_BA_ = true;
}

bool GlobalJointBA::is_running() const {
    std::lock_guard<std::mutex> lock(mtx_thread_);
    return loop_BA_is_running_;
}

void GlobalJointBA::optimize(const std::shared_ptr<data::keyframe>& curr_keyfrm) {
    ...
    bool ok = false;
    {
        ...
        if (mask_.vision) {
            const auto global_BA = optimize::global_bundle_adjuster(
                map_db_, num_iter_, use_huber_kernel_, verbose_);
            ...
            // Vision GBA skipped — still allow inertial / lidar / odom refine.
            ok = !abort_loop_BA_;
            for (const auto& kf : keyfrms) {
                if (!kf) {
                    continue;
                }
                optimized_keyfrm_ids.insert(kf->id_);
                keyfrm_to_pose_cw_after_global_BA[kf->id_] = kf->get_pose_cw();
            }
        }
        ...
        if (!ok) {
            ...
            auto keyfrms_update = curr_keyfrm->graph_node_->get_keyframes_from_root();
            std::unordered_set<unsigned int> already_found_landmark_ids;
            std::vector<std::shared_ptr<data::landmark>> lms;
            for (const auto& keyfrm : keyfrms_update) {
            ...
            if (mask_.imu && tracker_ && tracker_->imu_is_enabled()
                && tracker_->inertial_is_initialized() && !abort_loop_BA_) {
            ...
            // Lidar point-plane refine on loop keyframe (same residual source as Local).
            if (mask_.lidar && lidar_src_ && curr_keyfrm && !abort_loop_BA_) {
                const double t = curr_keyfrm->timestamp_;
                const auto batch = lidar_src_->Pull(t - 2.0, t + 0.1);
                if (!batch.empty()) {
                    const int n = estimate::ApplyLidarPointPlaneRefine(
                        curr_keyfrm.get(), batch, /*iterations=*/5);
                    AINFO << "loop bundle adjustment: lidar global refine edges="
                          << n;
                }
            }

            if (mask_.odom && odom_src_ && curr_keyfrm && !abort_loop_BA_) {
                const double t = curr_keyfrm->timestamp_;
                const auto batch = odom_src_->Pull(t - 2.0, t + 0.1);
                if (!batch.empty()) {
                    estimate::ApplyOdomDeltaRefine(curr_keyfrm.get(), batch);
                    AINFO << "loop bundle adjustment: odom global refine deltas="
                          << batch.deltas.size();
                }
            }
        }
