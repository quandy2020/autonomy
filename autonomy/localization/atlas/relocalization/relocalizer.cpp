...
#include "autonomy/localization/atlas/frontend/local_map_updater.hpp"
#include "autonomy/localization/atlas/data/landmark_line.hpp"
#include "autonomy/localization/atlas/frontend/match/base.hpp"
#include "autonomy/localization/atlas/relocalization/relocalizer.hpp"
...
Relocalizer::Relocalizer(const std::shared_ptr<optimize::pose_optimizer>& pose_optimizer,
    ...
    ADEBUG << "CONSTRUCT: module::relocalizer";
}
...
bool Relocalizer::reloc_by_candidates(data::frame& curr_frm,
    ...
    ADEBUG << "Start relocalization. Number of candidate keyframes is " << num_candidates;
    ...
    for (unsigned int i = 0; i < num_candidates; ++i) {
        ...
        if (candidate_keyfrm->will_be_erased()) {
            ADEBUG << "keyframe will be erased. candidate keyframe id is " << candidate_keyfrm->id_;
            continue;
        }

        bool ok = reloc_by_candidate(curr_frm, candidate_keyfrm, use_robust_matcher);
        if (ok) {
            AINFO << "relocalization succeeded (frame=" << curr_frm.id_ << ", keyframe=" << candidate_keyfrm->id_ << ")";
...
bool Relocalizer::relocalize_by_pnp_solver(data::frame& curr_frm,
    ...
    if (num_matches < min_num_bow_matches_) {
        ADEBUG << "Number of 2D-3D matches (" << num_matches << ") < threshold (" << min_num_bow_matches_ << "). candidate keyframe id is " << candidate_keyfrm->id_;
        return false;
    }
    ...
    if (search_neighbor_) {
        ...
        for (const auto& ngh_keyfrm : ngh_keyfrms) {
            ...
            ADEBUG << "Number of additional 2D-3D matches (detected=" << num_additional_matches << ", associated=" << num_associated << "). neighbor keyframe=" << ngh_keyfrm->id_;
        }
    }
    ...
    if (!pnp_solver->solution_is_valid()) {
        ADEBUG << "solution is not valid. candidate keyframe id is " << candidate_keyfrm->id_;
        return false;
    }
...
bool Relocalizer::optimize_pose(data::frame& curr_frm,
    ...
    if (num_valid_obs < min_num_bow_matches_ / 2) {
        ADEBUG << "Number of inliers (" << num_valid_obs << ") < threshold (" << min_num_bow_matches_ / 2 << "). candidate keyframe id is " << candidate_keyfrm->id_;
        return false;
    }
...
bool Relocalizer::refine_pose(data::frame& curr_frm,
    ...
    if (num_valid_obs2 < min_num_valid_obs_) {
        ADEBUG << "Number of observatoins (" << num_valid_obs2 << ") < threshold (" << min_num_valid_obs_ << "). candidate keyframe id is " << candidate_keyfrm->id_;
        return false;
    }
...
bool Relocalizer::refine_pose_by_local_map(data::frame& curr_frm,
    ...
    ADEBUG << "refine_pose_by_local_map: keyfrms=" << local_keyfrms.size() << ", lms=" << local_landmarks.size() << " nearest_covisibility id=" << nearest_covisibility->id_;
    ...
    for (size_t i = 0; i < margins.size(); ++i) {
        ...
        ADEBUG << "refine_pose_by_local_map: iter=" << i << ", margin=" << margin
               << ", num_additional_matches=" << num_additional_matches
               << ", num_valid_obs=" << num_valid_obs;

        if (i == margins.size() - 1) {
            const auto num_tracked_lms = candidate_keyfrm->get_num_tracked_landmarks(0);
            const double ratio = 0.2;
            ADEBUG << "refine_pose_by_local_map: num_valid_obs=" << num_valid_obs
                   << ", num_tracked_lms=" << num_tracked_lms;
