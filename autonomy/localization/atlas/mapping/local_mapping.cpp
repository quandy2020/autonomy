...
#include "autonomy/localization/atlas/frontend/two_view_triangulator.hpp"
...
#include "autonomy/localization/atlas/frontend/two_view_triangulator_line.hpp"
...
LocalMapping::LocalMapping(const YAML::Node& yaml_node, data::map_database* map_db, data::bow_database* bow_db,
    ...
    ADEBUG << "load monocular mappping parameters";
    if (yaml_node["baseline_dist_thr"]) {
        ...
        ADEBUG << "Use baseline_dist_thr: " << baseline_dist_thr_;
    }
    else {
...
std::shared_future<void> LocalMapping::async_add_keyframe(const std::shared_ptr<data::keyframe>& keyfrm) {
    std::shared_future<void> future;
    {
        ...
        future = promise_add_keyfrm_queue_.back().get_future().share();
    }
    if (use_pool_scheduling_) {
        MaybeScheduleDrain();
    }
...
void LocalMapping::MaybeScheduleDrain() {
    ...
    common_sched::Schedule(thread_pool_, [this]() {
        ...
        if (keyframe_is_queued() && !terminate_is_requested()) {
            MaybeScheduleDrain();
...
void LocalMapping::DrainKeyframesOnce() {
    while (!terminate_is_requested() && keyframe_is_queued()) {
        ...
        if (global_optimizer_ && cur_keyfrm_
            && cur_keyfrm_->graph_node_
            && !cur_keyfrm_->graph_node_->is_spanning_root()) {
            global_optimizer_->queue_keyframe(cur_keyfrm_);
        }
    }
}
...
void LocalMapping::update_new_keyframe() {
    ...
    for (const auto& lm : cur_landmarks) {
        ...
        if (!lm->has_representative_descriptor()) {
            AWARN << "has not representative descriptor " << lm->id_;
            lm->compute_descriptor();
        }
        if (!lm->has_valid_prediction_parameters()) {
            AWARN << "has not valid prediction parameters";
            lm->update_mean_normal_and_obs_scale_variance();
        }
    }
