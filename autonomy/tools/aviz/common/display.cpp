...
void Display::setStatus(int level, const QString& name, const QString& text) {
    QMetaObject::invokeMethod(this, "setStatusInternal", Qt::QueuedConnection,
                              Q_ARG(int, level), Q_ARG(QString, name),
                              Q_ARG(QString, text));
}

void Display::setStatusStd(int level, const std::string& name,
                           const std::string& text) {
    setStatus(level, QString::fromStdString(name),
...
void Display::setMissingTransformToFixedFrame(
    ...
    std::string error_string =
        "Could not transform " +
        (additional_info.empty() ? "from [" : additional_info + " from [") +
...
void Display::setStatusInternal(int level, const QString& name,
                                const QString& text) {
    StatusEntry entry;
    entry.level = level;
    entry.name = name;
...
void Display::deleteStatus(const QString& name) {
    QMetaObject::invokeMethod(this, "deleteStatusInternal",
                              Qt::QueuedConnection, Q_ARG(QString, name));
}

void Display::deleteStatusInternal(const QString& name) {
    status_map_.remove(name);
}

void Display::clearStatuses() {
    QMetaObject::invokeMethod(this, "clearStatusesInternal",
                              Qt::QueuedConnection);
}
...
SceneViewer* Display::getSceneViewer() const {
