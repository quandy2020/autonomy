...
public:
    ...
    /**
     * This potentially changes the return values of error() and
     * statusMessage().
     */
     ...
     * statusMessage().
     */
    QString writeString(const Config& config,
     ...
     * statusMessage().
     */
    void writeStream(const Config& config, std::ostream& out,
                     const QString& filename = "data stream");

    /// Return true if the latest write operation had an error.
    bool error();

    /// Return an error message if the latest write call had an error, else the
    /// empty string.
