...
class FrameTransformerException : public std::runtime_error {
 public:
  explicit FrameTransformerException(const char* error_message) : std::runtime_error(error_message) {}
...
/**
 * FrameTransformer provides an interface for transforming poses between different
 * coordinate frames. This is adapted for aviz system (not using ROS tf2).
 */
class FrameTransformer {
 public:
  ...
  /**
   * The initialization of a FrameTransformer object is delegated to this method.
   */
   ...
   * \param time The time at which to get the transform (nanoseconds since epoch)
   * \returns The transformed pose (x, y, z, qx, qy, qz, qw)
   */
  virtual bool transform(const std::string& source_frame, const std::string& target_frame, double x, double y, double z,
                         double qx, double qy, double qz, double qw, double& out_x, double& out_y, double& out_z,
                         double& out_qx, double& out_qy, double& out_qz, double& out_qw, uint64_t time = 0) = 0;

  /// Check if a transform is available.
  /**
   * \param source_frame The source frame name
   * \param target_frame The target frame name
   * \param time The time at which to check (nanoseconds since epoch, 0 for latest)
   * \returns True if transform is available
   */
  virtual bool canTransform(const std::string& source_frame, const std::string& target_frame, uint64_t time = 0) = 0;
   ...
   */
  virtual bool frameHasProblems(const std::string& frame, std::string& error) const = 0;
