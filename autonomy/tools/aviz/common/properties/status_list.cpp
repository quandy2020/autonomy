...
#include "autonomy/tools/aviz/common/properties/status_list.hpp"
...
void StatusList::setStatus(Level level, const QString& name,
                           const QString& text) {
    QHash<QString, StatusProperty*>::iterator child_iter =
...
void StatusList::updateLevel() {
    ...
    for (iter = status_children_.begin(); iter != status_children_.end();
         iter++) {
