...
/// A single element of a property tree, with a name, value, description, and
/// possibly children.
/**
...
public:
     ...
     * also, then the changed() signal is connected via QObject::connect() to
     * the slot described by `changed_slot` on the parent or the receiver. If
     * both parent and receiver are specified, receiver is the one which gets
     * connected.
    ...
    /// Destructor which also removes this property from its parent's list of
    /// children.
    virtual ~Property();
    ...
    /// Return the first child Property with the given name if found, else the
    /// FailureProperty.
    ...
    /// Return the child Property at the given index, without checking the
    /// bounds.
    virtual Property* childAtUnchecked(int index) const;

    /// Return true if the list of children includes possible_child, false if
    /// not.
    ...
    /// Hook to provide custom painting of the value data (right-hand column) in
    /// a subclass.
    ...
    /// Set the model managing this Property and all its child properties,
    /// recursively.
    ...
    /// Return the row number of this property within its parent, or -1 if it
    /// has no parent.
    ...
    /// Load the value of this property and/or its children from the given
    /// Config reference.
    virtual void load(const Config& config);

    /// Write the value of this property and/or its children into the given
    /// Config reference.
    ...
    /// If save is true and getReadOnly() is false, shouldBeSaved will return
    /// true, otherwise false.
    void setShouldBeSaved(bool save);

    /// If true, the children of this property should set their ItemIsEnabled
    /// flag to false.
...
protected:
    ...
    /// True if row_number_within_parent_ of all children is valid, false if
    /// not.
