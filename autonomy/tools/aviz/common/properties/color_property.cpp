...
ColorProperty::ColorProperty(const QString& name, const QColor& default_value, const QString& description,
...
void ColorProperty::updateString() { value_ = printColor(color_); }
...
QWidget* ColorProperty::createEditor(QWidget* parent, const QStyleOptionViewItem& option) {
  ...
  QObject::connect(dialog, &QColorDialog::colorSelected, this, &ColorProperty::setColor);
  return dialog;
}

QColor ColorProperty::getColor() const { return color_; }

Ogre::ColourValue ColorProperty::getOgreColor() const { return qtToOgre(color_); }
