...
void PropertyTreeModel::beginInsert(Property* parent, int index) {
    QModelIndex parent_index =
        parent == root_property_
            ? QModelIndex()
            : createIndex(parent->rowNumberInParent(), 0, parent);
...
void PropertyTreeModel::beginRemove(Property* parent, int start_index,
                                    int count) {
    QModelIndex parent_index =
        parent == root_property_
            ? QModelIndex()
...
QModelIndex PropertyTreeModel::parent(const QModelIndex& index) const {
    ...
    return createIndex(parent_property->rowNumberInParent(), 0,
                       parent_property);
}
...
bool PropertyTreeModel::setData(const QModelIndex& index, const QVariant& value,
                                int role) {
