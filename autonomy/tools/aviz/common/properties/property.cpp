...
Property::Property(const QString& name, const QVariant default_value,
...
void Property::removeChildren(int start_index, int count) {
    ...
    for (int i = start_index; i < start_index + count; i++) {
        ...
        child->setParent(nullptr);  // prevent child destructor from calling
                                    // getParent()->takeChild().
        delete child;
    }
    children_.erase(children_.begin() + start_index,
                    children_.begin() + start_index + count);
    child_indexes_valid_ = false;
    if (model_) {
        model_->endRemove();
    }
...
void Property::setIcon(const QIcon& icon) {
    icon_ = icon;
}

QIcon Property::getIcon() const {
    return icon_;
}
...
int Property::numChildren() const {
...
QVariant Property::getViewData(int column, int role) const {
    if (role == Qt::ForegroundRole && parent_ &&
        parent_->getDisableChildren()) {
        return QApplication::palette().brush(QPalette::Disabled,
                                             QPalette::Text);
    }
    ...
    switch (column) {
        ...
        case 1:  // right column: values
            switch (role) {
                case Qt::DisplayRole:
                case Qt::EditRole:
                    return value_.type() == QVariant::Bool ? QVariant()
                                                           : getValue();
...
Qt::ItemFlags Property::getViewFlags(int column) const {
    ...
    Qt::ItemFlags enabled_flag = (parent_ && parent_->getDisableChildren())
                                     ? Qt::NoItemFlags
                                     : Qt::ItemIsEnabled;
    ...
    if (value_.isValid()) {
        if (value_.type() == QVariant::Bool) {
            return Qt::ItemIsUserCheckable | enabled_flag |
                   Qt::ItemIsSelectable;
        }
...
PropertyTreeModel* Property::getModel() const {
    return model_;
}
...
void Property::load(const Config& config) {
    if (config.getType() == Config::Value) {
        ...
        // A special map entry named "Value" means the value of this property,
        // not a child. (If child "Value"does not exist, loadValue() will do
        // nothing.)
        ...
        for (int i = 0; i < num_property_children; i++) {
            Property* child = children_.at(i);
            // Load the child Property with the config under the child
            // property's name.
...
void Property::loadValue(const Config& config) {
    if (config.getType() == Config::Value) {
        switch (static_cast<int>(value_.type())) {
            ...
            default:
                printf(
                    "Property::loadValue() TODO: error handling - unexpected "
                    "QVariant type %d.\n",
...
void Property::save(Config config) const {
    ...
    } else {  // Else there are no child properties, so just save the value
              // itself.
        if (value_.isValid()) {
...
bool Property::shouldBeSaved() const {
    return !is_read_only_ && save_;
}

void Property::setShouldBeSaved(bool save) {
    save_ = save;
}
...
QWidget* Property::createEditor(QWidget* parent,
    ...
    switch (static_cast<int>(value_.type())) {
        ...
        case QMetaType::Float:
        case QVariant::Double: {
            // For now, use QLineEdit for float (can be extended with FloatEdit
            // later)
...
bool Property::getHidden() const {
    return hidden_;
}

void Property::setReadOnly(bool read_only) {
