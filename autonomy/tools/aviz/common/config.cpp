...
Config::Node::Node() : type_(Empty) { data_.map = nullptr; }

Config::Node::~Node() { deleteData(); }
...
Config::Config(QVariant value) : node_(new Config::Node()) { setValue(value); }
...
Config Config::invalidConfig() { return Config(NodePtr()); }

Config::Type Config::getType() const { return isValid() ? node_->type_ : Invalid; }
...
void Config::mapSetValue(const QString& key, QVariant value) { mapMakeChild(key).setValue(value); }
...
bool Config::mapGetFloat(const QString& key, float* value_out) const {
  QVariant v;
  if (mapGetValue(key, &v) && (static_cast<int>(v.type()) == static_cast<int>(QMetaType::Float) ||
                               v.type() == QVariant::Double || v.type() == QVariant::String)) {
    bool ok;
...
bool Config::isValid() const { return node_ != nullptr; }
...
QVariant Config::getValue() const { return (isValid() && node_->type_ == Value) ? *node_->data_.value : QVariant(); }

int Config::listLength() const { return (isValid() && node_->type_ == List) ? node_->data_.list->size() : 0; }
