...
void Config::copy(
    const Config& source)  // NOLINT linter wants #include <algorithm>
{
    if (!source.isValid()) {
        node_ = NodePtr();
        return;
...
bool Config::mapGetInt(const QString& key, int* value_out) const {
    QVariant v;
    if (mapGetValue(key, &v) &&
        (v.type() == QVariant::Int || v.type() == QVariant::String)) {
        bool ok;
        int i = v.toInt(&ok);
        if (ok) {
            *value_out = i;
...
bool Config::mapGetBool(const QString& key, bool* value_out) const {
    QVariant v;
    if (mapGetValue(key, &v) &&
        (v.type() == QVariant::Bool || v.type() == QVariant::String)) {
        *value_out = v.toBool();
        return true;
    }
    return false;
...
QVariant Config::getValue() const {
    return (isValid() && node_->type_ == Value) ? *node_->data_.value
                                                : QVariant();
}
...
Config Config::listChildAt(int i) const {
    if (isValid() && node_->type_ == List && i >= 0 &&
        i < node_->data_.list->size()) {
