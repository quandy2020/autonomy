...
public:
    ...
    bool getTransform(const std::string& frame, uint64_t time_ns,
                      QVector3D& position, QQuaternion& orientation) override;
    bool transform(const std::string& frame, uint64_t time_ns, double x,
                   double y, double z, double qx, double qy, double qz,
                   double qw, double& out_x, double& out_y, double& out_z,
                   double& out_qx, double& out_qy, double& out_qz,
                   double& out_qw) override;
    void update() override;
    bool frameHasProblems(const std::string& frame,
                          std::string& error) override;
    bool transformHasProblems(const std::string& frame,
                              std::string& error) override;
    bool transformHasProblems(const std::string& frame, uint64_t time_ns,
                              std::string& error) override;
    const std::string& getFixedFrame() override;
    transformation::TransformationLibraryConnector::WeakPtr getConnector()
        override;
    std::shared_ptr<transformation::FrameTransformer> getTransformer() override;

public Q_SLOTS:
    void setTransformerPlugin(
        std::shared_ptr<transformation::FrameTransformer> transformer) override;
    ...
    bool getTransform(const std::string& frame, int32_t sec, uint32_t nanosec,
                      Ogre::Vector3& position, Ogre::Quaternion& orientation);

    // Helper methods
    void setTransform(const std::string& frame, const std::string& parent_frame,
