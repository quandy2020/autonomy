...
bool FrameManager::getTransform(const std::string& frame, uint64_t time_ns,
    ...
    if (it != transforms_.end()) {
        position = QVector3D(it->second.position.x, it->second.position.y,
                             it->second.position.z);
        orientation =
            QQuaternion(it->second.orientation.w, it->second.orientation.x,
...
bool FrameManager::transform(const std::string& frame, uint64_t time_ns,
                             double x, double y, double z, double qx, double qy,
                             double qz, double qw, double& out_x, double& out_y,
                             double& out_z, double& out_qx, double& out_qy,
                             double& out_qz, double& out_qw) {
    QVector3D position;
    QQuaternion orientation;
    ...
    // Apply transform (simplified - would do proper quaternion multiplication
...
bool FrameManager::transformHasProblems(const std::string& frame,
                                        uint64_t time_ns, std::string& error) {
    (void)time_ns;
...
bool FrameManager::getTransform(const std::string& frame,
    ...
    orientation = Ogre::Quaternion(qorient.scalar(), qorient.x(), qorient.y(),
                                   qorient.z());
    return true;
}

bool FrameManager::getTransform(const std::string& frame, int32_t sec,
                                uint32_t nanosec, Ogre::Vector3& position,
                                Ogre::Quaternion& orientation) {
    (void)sec;
    (void)nanosec;
...
void FrameManager::setTransform(const std::string& frame,
