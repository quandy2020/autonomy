...
/**
 * Each Display represents a visualization element (robot model, point cloud,
 * etc.) that can be enabled/disabled and updated over time.
 *
...
public:
    ...
    /// Main initialization, called after constructor, before load() or
    /// setEnabled().
    ...
    /// Load the settings for this display from the given Config node, which
    /// must be a map.
    /**
    ...
    /**
     * \param wall_dt Wall-clock time, in seconds, since the last time the
     * update list was run through. \param ros_dt ROS time, in seconds, since
     * the last time the update list was run through.
     */
     ...
     * This is thread-safe.
     */
    void setStatusStd(int level, const std::string& name,
     ...
     */
    void setMissingTransformToFixedFrame(
    ...
    /// Return the panel containing the associated widget, or nullptr if there
    /// is none.
    /**
     * \see setAssociatedWidget()
     */
    ...
    /// Return the Ogre::SceneNode holding all 3D scene elements shown by this
    /// Display.
...
protected:
    ...
    /// Derived classes override this to do the actual work of enabling
    /// themselves.
    virtual void onEnable();

    /// Derived classes override this to do the actual work of disabling
    /// themselves.
    ...
    /// This DisplayContext pointer is the main connection a Display has into
    /// the rest of aviz.
    /**
