...
#ifndef AVIZ_COMMON__CONFIG_HPP_
#define AVIZ_COMMON__CONFIG_HPP_
 ...
 * writer classes.
 * Currently there is just YAML support, as that is all that AViz supports
 * right now.
...
public:
    ...
    /// The copy constructor copies only the reference to the data, not the data
    /// itself.
    Config(const Config& source);

    /// The converting constructor, makes a Value type Config object with the
    /// given value.
    ...
    /// Return the Type of the referenced Node, or Invalid if no Node is
    /// referenced.
    Type getType() const;
     ...
     * nothing is changed.
     *
     * If this Config is currently invalid and new_type is not Invalid, this
     * will create a new Node and reference it.
     */
    ...
    /// Return a reference to the child if the Node is a Map which contains the
    /// given key.
    /**
     ...
     *
     * If the Config is Invalid or the Node is not a Map, this returns false.
     */
     ...
     *
     * If the Config is invalid or the Node is not a Map, returns false.
     */
    ...
    /// Ensure this is a valid Config object, set the type to Value, then set
    /// the value. Set the value and the type to Value, if the Config object is
    /// valid.
    ...
    /// Return the length of the List in this Node, or 0 if this Node does not
    /// have type List.
    ...
    /// Append a new empty Node to the list and return a reference, unless the
    /// Node is not a List.
     ...
     * iter.advance()) { QString key = iter.currentKey(); Config child =
     * iter.currentChild(); printf( "key %s has value %s.\n", qPrintable(key),
     * qPrintable(child.getValue().toString()));
     *   }
     *
    ...
    public:
        ...
        /// Return true if the iterator currently points to a valid entry, false
        /// if not.
        /**
    ...
    private:
        // *INDENT-ON*
        /// Private constructor enforces that MapIterators are only made by the
        /// Config class.
        MapIterator();
...
#endif  // AVIZ_COMMON__CONFIG_HPP_
