...
/**
 * Light-weight container wrapper for M_ObjectHandleToSelectionHandler that
 * allows iterating directly over handlers.
 */
class HandlerRange
{
public:
 ...
 *
 * The HandlerManager must be locked (using one of the lock methods) if
 * exclusive access to the handlers is necessary.
 */
...
public:
    ...
    /**
     * Registers a new handle-handler pair. Locks internally to guarantee
     * exclusive access. \param handle \param handler
     */
    virtual void addHandler(CollObjectHandle handle,
                            SelectionHandlerWeakPtr handler) = 0;

    /**
     * Removes a handle-handler pair identified by its handle. Locks internally
     * to guarantee exclusive access \param handle
     */
     ...
     */
    virtual std::unique_lock<std::recursive_mutex> lock(
        std::defer_lock_t defer_lock) = 0;
    ...
    /**
     * Gives access to all managed handlers in form of a container that can be
     * accessed via a forward iterator. Its usage needs to be protected by
     * explicitly calling lock \return container of all handlers
     */
