...
class AutolinkTopicDisplay : public aviz::common::Display {
 public:
  explicit AutolinkTopicDisplay(const QString& class_id)
      : aviz::common::Display(), topic_(""), node_(nullptr), reader_(nullptr) {
    setClassId(class_id);
  }

  ~AutolinkTopicDisplay() override { unsubscribe(); }
 ...
 protected:
   ...
   */
  void onDisable() override { unsubscribe(); }
  ...
  virtual void subscribe() {
    ...
    if (!node_) {
      ...
      node_ = autolink::CreateNode("aviz_display_" + display_name.toStdString());
      if (!node_) {
        AVIZ_ERROR("Failed to create autolink node for display: " + display_name.toStdString());
        return;
      }
    }

    // Use std::bind to avoid lambda type issues
    using CallbackType = std::function<void(const std::shared_ptr<MessageType>&)>;
    CallbackType callback = std::bind(&AutolinkTopicDisplay<MessageType>::processMessage, this, std::placeholders::_1);
