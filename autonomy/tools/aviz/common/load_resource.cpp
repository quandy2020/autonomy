...
std::string findResourceFile(const std::string& resource_path) {
    ...
    if (!autolink_path.empty()) {
        search_paths.push_back(autolink::common::GetAbsolutePath(
            autolink_path, "share/aviz/" + resource_path));
    }
...
std::string loadResourceFile(const std::string& resource_path) {
    ...
    if (full_path.empty()) {
        std::cerr << "aviz_common: Resource not found: " << resource_path
                  << std::endl;
        return "";
    }
    ...
    if (!file.is_open()) {
        std::cerr << "aviz_common: Failed to open resource: " << full_path
                  << std::endl;
        return "";
    }
