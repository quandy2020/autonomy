...
void PluginFactory::registerPlugin(const PluginDescription& description,
                                   DisplayCreator creator) {
    // Check if already registered
    if (class_id_to_index_.find(description.class_id) !=
        class_id_to_index_.end()) {
        std::cerr << "PluginFactory: Plugin " << description.class_id
...
std::unique_ptr<aviz::common::Display> PluginFactory::createDisplay(
    const std::string& class_id, const std::string& name) {
    auto it = creators_.find(class_id);
    if (it == creators_.end()) {
        std::cerr << "PluginFactory: Unknown plugin class_id: " << class_id
                  << std::endl;
        return nullptr;
    }
...
const PluginDescription* PluginFactory::getPluginDescription(
