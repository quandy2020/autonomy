...
MarkerDisplay::MarkerDisplay(const QString& name)
    : AutolinkTopicDisplay<autonomy::commsgs::visualization_msgs::Marker>(QString("aviz/Marker")) {
  setName(name);
}

MarkerDisplay::~MarkerDisplay() = default;

void MarkerDisplay::onInitialize() { AutolinkTopicDisplay::onInitialize(); }
...
void MarkerDisplay::processMarkerMessage(const std::shared_ptr<autonomy::commsgs::visualization_msgs::Marker>& msg) {
  processMessage(msg);
}

void MarkerDisplay::processMessage(const std::shared_ptr<autonomy::commsgs::visualization_msgs::Marker>& msg) {
...
void MarkerDisplay::createMarker(const autonomy::commsgs::visualization_msgs::Marker& marker) {
  ...
  Ogre::Quaternion orientation = aviz::common::quaternionMsgToOgre(marker.pose.orientation);
  Ogre::Vector3 scale_vec(static_cast<float>(marker.scale.x), static_cast<float>(marker.scale.y),
                          static_cast<float>(marker.scale.z));

  Ogre::ColourValue color(static_cast<float>(marker.color.r), static_cast<float>(marker.color.g),
                          static_cast<float>(marker.color.b), static_cast<float>(marker.color.a));

  // Create marker based on type
  switch (marker.type) {
    case 0: {  // ARROW
      auto arrow = std::make_unique<aviz::rendering::Arrow>(scene_manager_, scene_node_);
      arrow->setPosition(position);
      arrow->setOrientation(orientation);
      arrow->set(scale_vec.x, scale_vec.y * 0.1f, scale_vec.z, scale_vec.z);
      arrow->setColor(color.r, color.g, color.b, color.a);
      arrow_markers_[key] = std::move(arrow);
      break;
    }
    case 1: {  // CUBE
      auto shape = std::make_unique<aviz::rendering::Shape>(aviz::rendering::Shape::Cube, scene_manager_, scene_node_);
    ...
    case 2: {  // SPHERE
      auto shape =
          std::make_unique<aviz::rendering::Shape>(aviz::rendering::Shape::Sphere, scene_manager_, scene_node_);
      shape->setPosition(position);
      shape->setOrientation(orientation);
      shape->setScale(scale_vec);
      shape->setColor(color.r, color.g, color.b, color.a);
    ...
    case 3: {  // CYLINDER
      auto shape =
          std::make_unique<aviz::rendering::Shape>(aviz::rendering::Shape::Cylinder, scene_manager_, scene_node_);
      shape->setPosition(position);
      shape->setOrientation(orientation);
      shape->setScale(scale_vec);
      shape->setColor(color.r, color.g, color.b, color.a);
    ...
    // TODO: Implement other marker types (LINE_STRIP, LINE_LIST, POINTS, etc.)
    default:
