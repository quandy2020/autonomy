 ...
 */
class MarkerDisplay
    : public AutolinkTopicDisplay<autonomy::commsgs::visualization_msgs::Marker>
{
    Q_OBJECT
...
public:
    ...
    void processMarkerMessage(
        const std::shared_ptr<autonomy::commsgs::visualization_msgs::Marker>&
            msg);
...
private:
    ...
    std::map<std::string, std::unique_ptr<aviz::rendering::Shape>>
        shape_markers_;
    std::map<std::string, std::unique_ptr<aviz::rendering::Arrow>>
        arrow_markers_;
    std::map<std::string, std::unique_ptr<aviz::rendering::Axes>> axes_markers_;
    std::map<std::string, std::unique_ptr<aviz::rendering::BillboardLine>>
        line_markers_;
};
