...
RobotModelDisplay::RobotModelDisplay(const QString& name)
    : aviz::common::Display(), robot_loaded_(false) {
    setName(name);
    setClassId("aviz/RobotModel");

    robot_description_property_ = new aviz::common::properties::StringProperty(
        QString("Robot Description"), QString(""),
        QString("URDF robot description (XML string or file path)."), nullptr,
        SLOT(updateRobotDescription()), this);

    visual_enabled_property_ = new aviz::common::properties::BoolProperty(
        QString("Visual Enabled"), true,
        QString("Whether to show the visual representation of the robot."),
        nullptr, SLOT(updateVisualVisible()), this);

    collision_enabled_property_ = new aviz::common::properties::BoolProperty(
        QString("Collision Enabled"), false,
        QString("Whether to show the collision representation of the robot."),
        nullptr, SLOT(updateCollisionVisible()), this);

    alpha_property_ = new aviz::common::properties::FloatProperty(
        QString("Alpha"), 1.0f,
        QString("Amount of transparency to apply to the robot model."), nullptr,
