...
class OdometryDisplay
    : public AutolinkTopicDisplay<autonomy::commsgs::planning_msgs::Odometry>
{
    Q_OBJECT
...
protected:
    /** @brief Overridden from AutolinkTopicDisplay to get Arrow/Axes visibility
     * correct. */
...
private:
    ...
    bool messageIsSimilarToPrevious(
        const std::shared_ptr<autonomy::commsgs::planning_msgs::Odometry>&
            message);

    std::unique_ptr<aviz::rendering::Arrow> createAndSetArrow(
        const Ogre::Vector3& position, const Ogre::Quaternion& orientation,
        bool use_arrow);

    std::unique_ptr<aviz::rendering::Axes> createAndSetAxes(
        const Ogre::Vector3& position, const Ogre::Quaternion& orientation,
        bool use_axes);
    ...
    std::shared_ptr<autonomy::commsgs::planning_msgs::Odometry>
        last_used_message_;
