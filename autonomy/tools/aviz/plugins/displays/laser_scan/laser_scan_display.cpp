...
LaserScanDisplay::LaserScanDisplay(const QString& name)
    : AutolinkTopicDisplay<autonomy::commsgs::sensor_msgs::LaserScan>(QString("aviz/LaserScan")) {
  setName(name);
  color_property_ = new aviz::common::properties::ColorProperty(QString("Color"), QColor(255, 255, 0),
                                                                QString("Color to draw the laser scan points."),
                                                                nullptr, SLOT(updateColorAndAlpha()), this);

  alpha_property_ = new aviz::common::properties::FloatProperty(
      QString("Alpha"), 1.0f, QString("Amount of transparency to apply to the laser scan."), nullptr,
      SLOT(updateColorAndAlpha()), this);
  alpha_property_->setMin(0.0f);
  alpha_property_->setMax(1.0f);

  buffer_size_property_ = new aviz::common::properties::IntProperty(
      QString("Buffer Size"), 1, QString("Number of scans to display."), nullptr, SLOT(updateColorAndAlpha()), this);
  buffer_size_property_->setMin(1);
}

LaserScanDisplay::~LaserScanDisplay() = default;

void LaserScanDisplay::onInitialize() { AutolinkTopicDisplay::onInitialize(); }
...
void LaserScanDisplay::processMessage(const std::shared_ptr<autonomy::commsgs::sensor_msgs::LaserScan>& msg) {
  if (!msg || !scene_manager_ || !scene_node_) {
    return;
  }

  // Validate message
  if (!aviz::common::validateFloat(msg->angle_min) || !aviz::common::validateFloat(msg->angle_max) ||
      !aviz::common::validateFloat(msg->angle_increment) || !aviz::common::validateFloat(msg->range_min) ||
...
void LaserScanDisplay::convertLaserScanToPointCloud(const autonomy::commsgs::sensor_msgs::LaserScan& scan) {
  point_cloud_points_.clear();
  point_cloud_colors_.clear();
  ...
  // TODO: Render point cloud using Ogre (would use PointCloudCommon in full implementation)
}

void LaserScanDisplay::updateColorAndAlpha() { queueRender(); }
