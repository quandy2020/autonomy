...
GridDisplay::GridDisplay() {
    cell_count_property_ = new IntProperty(QString("Plane Cell Count"), 10,
                                           QString("The number of cells to draw in the plane of the grid."), nullptr,
                                           SLOT(updateCellCount()), this);
    cell_count_property_->setMin(1);

    height_property_ = new IntProperty(QString("Normal Cell Count"), 0,
                                       QString("The number of cells to draw along the normal vector of the grid. "
                                               " Setting to anything but 0 makes the grid 3D."),
                                       nullptr, SLOT(updateHeight()), this);
    height_property_->setMin(0);

    cell_size_property_ =
        new FloatProperty(QString("Cell Size"), 1.0f, QString("The length, in meters, of the side of each cell."),
                          nullptr, SLOT(updateCellSize()), this);
    cell_size_property_->setMin(0.0001f);

    style_property_ = new EnumProperty(QString("Line Style"), QString("Lines"),
                                       QString("The rendering operation to use to draw the grid lines."), nullptr,
    ...
    line_width_property_ =
        new FloatProperty(QString("Line Width"), 0.03f, QString("The width, in meters, of each grid line."),
                          style_property_, SLOT(updateLineWidth()), this);
    line_width_property_->setMin(0.001f);
    line_width_property_->hide();

    color_property_ = new ColorProperty(QString("Color"), Qt::gray, QString("The color of the grid lines."), nullptr,
                                        SLOT(updateColor()), this);
    alpha_property_ =
        new FloatProperty(QString("Alpha"), 0.5f, QString("The amount of transparency to apply to the grid lines."),
                          nullptr, SLOT(updateColor()), this);
    alpha_property_->setMin(0.0f);
    alpha_property_->setMax(1.0f);

    plane_property_ = new EnumProperty(QString("Plane"), QString("XY"), QString("The plane to draw the grid along."),
                                       nullptr, SLOT(updatePlane()), this);
    plane_property_->addOption("XY", XY);
    plane_property_->addOption("XZ", XZ);
    plane_property_->addOption("YZ", YZ);

    offset_property_ =
        new VectorProperty(QString("Offset"), Ogre::Vector3::ZERO,
                           QString("Allows you to offset the grid from the origin of the reference frame.  In meters."),
...
void GridDisplay::onInitialize() {
    ...
    if (scene_manager_ && scene_node_) {
        grid_ = std::make_unique<Grid>(scene_manager_, scene_node_, (Grid::Style)style_property_->getOptionInt(),
