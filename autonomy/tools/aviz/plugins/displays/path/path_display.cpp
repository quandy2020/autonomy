...
PathDisplay::PathDisplay() : AutolinkTopicDisplay<autonomy::commsgs::planning_msgs::Path>("aviz/Path") {
    ...
    pose_arrow_color_property_ = new aviz::common::properties::ColorProperty(
        QString("Pose Color"), QColor(255, 85, 255), QString("Color to draw the poses."), nullptr,
        SLOT(updatePoseArrowColor()), this);
    pose_arrow_shaft_length_property_ = new aviz::common::properties::FloatProperty(
        QString("Shaft Length"), 0.1f, QString("Length of the arrow shaft."), nullptr, SLOT(updatePoseArrowGeometry()),
        this);
    ...
    pose_arrow_head_diameter_property_ = new aviz::common::properties::FloatProperty(
        QString("Head Diameter"), 0.3f, QString("Diameter of the arrow head."), nullptr,
        SLOT(updatePoseArrowGeometry()), this);
...
void PathDisplay::updateArrowMarkers(std::vector<Arrow*>& arrow_vect, const autonomy::commsgs::planning_msgs::Path& msg,
    ...
    for (size_t i = 0; i < num_points; ++i) {
        ...
        arrow_vect[i]->set(
            pose_arrow_shaft_length_property_->getFloat(), pose_arrow_shaft_diameter_property_->getFloat(),
            pose_arrow_head_length_property_->getFloat(), pose_arrow_head_diameter_property_->getFloat());
        const auto& pos = msg.poses[i].pose.position;
