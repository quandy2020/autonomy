...
PathDisplay::PathDisplay()
    : AutolinkTopicDisplay<autonomy::commsgs::planning_msgs::Path>(
          "aviz/Path") {
    ...
    line_width_property_ = new aviz::common::properties::FloatProperty(
        ...
        QString("The width, in meters, of each path line. Only works with the "
                "'Billboards' style."),
    ...
    color_property_ = new aviz::common::properties::ColorProperty(
        QString("Color"), QColor(25, 255, 0),
        QString("Color to draw the path."), nullptr, nullptr, this);

    alpha_property_ = new aviz::common::properties::FloatProperty(
        QString("Alpha"), 1.0f,
        QString("Amount of transparency to apply to the path."), nullptr,
        nullptr, this);

    buffer_length_property_ = new aviz::common::properties::IntProperty(
        QString("Buffer Length"), 1, QString("Number of paths to display."),
        nullptr, SLOT(updateBufferLength()), this);
    buffer_length_property_->setMin(1);

    offset_property_ = new aviz::common::properties::VectorProperty(
        QString("Offset"), Ogre::Vector3::ZERO,
        QString("Allows you to offset the path from the origin of the "
                "reference frame.  In meters."),
        nullptr, SLOT(updateOffset()), this);

    pose_style_property_ = new aviz::common::properties::EnumProperty(
        QString("Pose Style"), QString("None"),
    ...
    pose_axes_length_property_ = new aviz::common::properties::FloatProperty(
        QString("Length"), 0.3f, QString("Length of the axes."), nullptr,
        SLOT(updatePoseAxisGeometry()), this);
    pose_axes_radius_property_ = new aviz::common::properties::FloatProperty(
        QString("Radius"), 0.03f, QString("Radius of the axes."), nullptr,
        SLOT(updatePoseAxisGeometry()), this);
    pose_axes_length_property_->hide();
    pose_axes_radius_property_->hide();

    pose_arrow_color_property_ = new aviz::common::properties::ColorProperty(
        QString("Pose Color"), QColor(255, 85, 255),
        QString("Color to draw the poses."), nullptr,
        SLOT(updatePoseArrowColor()), this);
    pose_arrow_shaft_length_property_ =
        new aviz::common::properties::FloatProperty(
            QString("Shaft Length"), 0.1f,
            QString("Length of the arrow shaft."), nullptr,
            SLOT(updatePoseArrowGeometry()), this);
    pose_arrow_head_length_property_ =
        new aviz::common::properties::FloatProperty(
            QString("Head Length"), 0.2f, QString("Length of the arrow head."),
            nullptr, SLOT(updatePoseArrowGeometry()), this);
    pose_arrow_shaft_diameter_property_ =
        new aviz::common::properties::FloatProperty(
            QString("Shaft Diameter"), 0.1f,
            QString("Diameter of the arrow shaft."), nullptr,
            SLOT(updatePoseArrowGeometry()), this);
    pose_arrow_head_diameter_property_ =
        new aviz::common::properties::FloatProperty(
            QString("Head Diameter"), 0.3f,
            QString("Diameter of the arrow head."), nullptr,
            SLOT(updatePoseArrowGeometry()), this);
    ...
    lines_material_ =
        aviz::rendering::MaterialManager::createMaterialWithNoLighting(
            material_name);
}
...
void PathDisplay::allocateAxesVector(std::vector<Axes*>& axes_vect,
                                     size_t num) {
    ...
    if (num > vector_size) {
        ...
        for (auto i = vector_size; i < num; ++i) {
            axes_vect.push_back(
                new Axes(scene_manager_, scene_node_,
...
void PathDisplay::allocateArrowVector(std::vector<Arrow*>& arrow_vect,
                                      size_t num) {
...
void PathDisplay::updatePoseStyle() {
    auto pose_style =
        static_cast<PoseStyle>(pose_style_property_->getOptionInt());
    switch (pose_style) {
        case AXES:
...
void PathDisplay::updateBufferLength() {
    ...
    switch (style) {
        ...
        case BILLBOARDS:  // billboards with configurable width
            ...
            for (size_t i = 0; i < buffer_length; i++) {
                auto billboard_line =
                    new BillboardLine(scene_manager_, scene_node_);
...
void PathDisplay::processMessage(
    const std::shared_ptr<autonomy::commsgs::planning_msgs::Path>& msg) {
...
void PathDisplay::updateManualObject(
    ...
    manual_object->begin(lines_material_->getName(),
                         Ogre::RenderOperation::OT_LINE_STRIP,
                         "aviz_rendering");

    for (const auto& pose_stamped : msg.poses) {
        Ogre::Vector3 pos =
            transform * pointMsgToOgre(pose_stamped.pose.position);
        manual_object->position(pos);
        aviz::rendering::MaterialManager::enableAlphaBlending(lines_material_,
                                                              color.a);
...
void PathDisplay::updatePoseMarkers(
    size_t buffer_index, const autonomy::commsgs::planning_msgs::Path& msg,
    const Ogre::Matrix4& transform) {
    auto pose_style =
        static_cast<PoseStyle>(pose_style_property_->getOptionInt());
    if (buffer_index >= axes_chain_.size() ||
        buffer_index >= arrow_chain_.size()) {
        return;
    }
...
void PathDisplay::updateAxesMarkers(
    ...
    for (size_t i = 0; i < num_points; ++i) {
        ...
        Ogre::Quaternion orientation(
            quaternionMsgToOgre(msg.poses[i].pose.orientation));

        // Extract the rotation part of the transformation matrix as a
        // quaternion
        Ogre::Matrix3 rot3x3;
...
void PathDisplay::updateArrowMarkers(
    ...
    for (size_t i = 0; i < num_points; ++i) {
        QColor color = pose_arrow_color_property_->getColor();
        arrow_vect[i]->setColor(color.redF(), color.greenF(), color.blueF(),
                                1.0f);
