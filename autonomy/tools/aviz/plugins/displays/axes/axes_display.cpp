...
AxesDisplay::AxesDisplay() : axes_(nullptr) {
    length_property_ = new FloatProperty(
        QString("Length"), 1.0f, QString("Length of each axis, in meters."),
        nullptr, SLOT(updateShape()), this);
    length_property_->setMin(0.0001f);

    radius_property_ = new FloatProperty(
        QString("Radius"), 0.1f, QString("Radius of each axis, in meters."),
        nullptr, SLOT(updateShape()), this);
...
void AxesDisplay::onInitialize() {
    if (scene_manager_ && scene_node_) {
        axes_ = std::make_shared<Axes>(scene_manager_, scene_node_,
                                       length_property_->getFloat(),
