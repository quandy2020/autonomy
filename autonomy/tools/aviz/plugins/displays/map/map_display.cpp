...
MapDisplay::MapDisplay(const QString& name)
    : AutolinkTopicDisplay<autonomy::commsgs::map_msgs::OccupancyGrid>(
          QString("aviz/Map")) {
    setName(name);
    alpha_property_ = new aviz::common::properties::FloatProperty(
        QString("Alpha"), 0.7f,
        QString("Amount of transparency to apply to the map."), nullptr,
        SLOT(updateAlpha()), this);
    alpha_property_->setMin(0.0f);
    alpha_property_->setMax(1.0f);

    color_scheme_property_ = new aviz::common::properties::ColorProperty(
        QString("Color Scheme"), QColor(0, 0, 0),
        QString("Color scheme for the map."), nullptr, SLOT(updateAlpha()),
        this);
}
