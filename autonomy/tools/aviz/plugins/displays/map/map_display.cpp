...
MapDisplay::MapDisplay(const QString& name)
  ...
  setName(name);
  alpha_property_ = new aviz::common::properties::FloatProperty(QString("Alpha"), 0.7f,
                                                                QString("Amount of transparency to apply to the map."),
                                                                nullptr, SLOT(updateAlpha()), this);
...
void MapDisplay::onInitialize() { AutolinkTopicDisplay::onInitialize(); }
...
void MapDisplay::updateAlpha() { queueRender(); }
