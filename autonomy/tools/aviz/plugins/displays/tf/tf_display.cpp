...
TfDisplay::TfDisplay(const QString& name)
    : aviz::common::Display(), last_update_time_(0.0f) {
    setName(name);
    setClassId("aviz/TF");

    show_names_property_ = new aviz::common::properties::BoolProperty(
        QString("Show Names"), true, QString("Whether to show frame names."),
        nullptr, SLOT(updateShowNames()), this);

    show_axes_property_ = new aviz::common::properties::BoolProperty(
        QString("Show Axes"), true,
        QString("Whether to show coordinate axes for each frame."), nullptr,
        SLOT(updateShowAxes()), this);

    show_arrows_property_ = new aviz::common::properties::BoolProperty(
        QString("Show Arrows"), false,
        QString("Whether to show arrows connecting parent and child frames."),
    ...
    frame_timeout_property_ = new aviz::common::properties::FloatProperty(
        ...
        QString("How long to keep showing a frame that has stopped being "
                "published [s]."),
        nullptr, SLOT(updateFrameEnabled()), this);
    ...
    update_interval_property_ = new aviz::common::properties::FloatProperty(
        ...
        QString("The interval, in seconds, at which to update the frame list. "
    ...
    axes_radius_property_ = new aviz::common::properties::FloatProperty(
        QString("Axes Radius"), 0.01f,
        QString("Radius of each axis, in meters."), nullptr,
        SLOT(updateShowAxes()), this);
    axes_radius_property_->setMin(0.001f);

    arrow_length_property_ = new aviz::common::properties::FloatProperty(
        QString("Arrow Length"), 0.3f,
        QString("Length of each arrow, in meters."), nullptr,
        SLOT(updateShowArrows()), this);
    arrow_length_property_->setMin(0.01f);

    arrow_radius_property_ = new aviz::common::properties::FloatProperty(
        QString("Arrow Radius"), 0.02f,
        QString("Radius of each arrow, in meters."), nullptr,
        SLOT(updateShowArrows()), this);
...
void TfDisplay::updateTransforms() {
    ...
    // context_->getFrameManager()->getAllFrameNames(); for (const auto&
    // frame_id : frames) {
    //     std::string parent_frame;
    //     Ogre::Vector3 position;
    //     Ogre::Quaternion orientation;
    //     if (context_->getFrameManager()->getTransform(frame_id, parent_frame,
...
void TfDisplay::createFrameVisual(const std::string& frame_id,
    ...
    if (show_arrows_property_->getBool() && !parent_frame_id.empty()) {
        ...
        if (frame_arrows_.find(arrow_key) == frame_arrows_.end()) {
            frame_arrows_[arrow_key] = std::make_unique<aviz::rendering::Arrow>(
                scene_manager_, scene_node_, arrow_length_property_->getFloat(),
                arrow_radius_property_->getFloat(),
                arrow_length_property_->getFloat() * 0.3f,
                arrow_radius_property_->getFloat() * 1.5f);
        }
