...
MarkerArrayDisplay::MarkerArrayDisplay(const QString& name)
    : AutolinkTopicDisplay<autonomy::commsgs::visualization_msgs::MarkerArray>(
          QString("aviz/MarkerArray")),
...
void MarkerArrayDisplay::processMessage(
    const std::shared_ptr<autonomy::commsgs::visualization_msgs::MarkerArray>&
        msg) {
    ...
    for (const auto& marker : msg->markers) {
        auto marker_ptr =
            std::make_shared<autonomy::commsgs::visualization_msgs::Marker>(
                marker);
