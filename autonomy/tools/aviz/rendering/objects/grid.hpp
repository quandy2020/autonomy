 ...
 *
 * Displays a grid of cells, drawn with lines.  A grid with an identity
 * orientation is drawn along the XZ plane.
 */
...
public:
     ...
     * @param line_width The line width of the cells if it is rendered in
     * Billboards style
     * @param color The color of the lines
     */
    Grid(Ogre::SceneManager* manager, Ogre::SceneNode* parent_node, Style style,
         uint32_t cell_count, float cell_length, float line_width,
...
private:
    typedef std::function<void(const Ogre::Vector3& p1,
    ...
    void createGridPlane(float extent, uint32_t height,
                         AddLineFunction addLine) const;
    void createVerticalLinesBetweenPlanes(float extent,
                                          AddLineFunction addLine) const;
    void addBillboardLine(const Ogre::Vector3& p1,
    ...
    Ogre::SceneNode*
        scene_node_;  ///< The scene node that this grid is attached to
    Ogre::ManualObject*
        manual_object_;  ///< The manual object used to draw the grid
