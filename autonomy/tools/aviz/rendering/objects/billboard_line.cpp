...
BillboardLine::BillboardLine(Ogre::SceneManager* scene_manager,
    ...
    std::string material_name =
        "BillboardLineMaterial" + std::to_string(count++);
...
void BillboardLine::setupChainContainers() {
    ...
    for (uint32_t i = static_cast<uint32_t>(chain_containers_.size());
         i < num_chains; ++i) {
        createChain();
    }

    chains_per_container_ =
        max_points_per_line_ > 0 ? MAX_ELEMENTS / max_points_per_line_ : 1;
...
void BillboardLine::setupChainsInChainContainers() const {
    ...
    for (; it != end; ++it) {
        ...
        // shorten the number of chains in the last bbchain, to avoid memory
        // wasteage
        if (it + 1 == end) {
            uint32_t lines_left = num_lines_ % chains_per_container_;

            // Handle the case where num_lines_ is a multiple of lines_per_chain
            (*it)->setNumberOfChains((lines_left == 0) ? chains_per_container_
                                                       : lines_left);
        } else {
...
void BillboardLine::addPoint(const Ogre::Vector3& point,
    ...
    assert(current_line_ < num_lines_);
    assert(chain_containers_[current_chain_container_]->getNumChainElements(
               current_line_ % chains_per_container_) <= max_points_per_line_);
