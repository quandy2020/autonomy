...
void Grid::createManualGrid() const {
  AddLineFunction addLineFunction = std::bind(&Grid::addManualLine, this, std::placeholders::_1, std::placeholders::_2);
...
uint32_t Grid::numberOfVerticalLines() const { return (cell_count_ + 1) * (cell_count_ + 1) * height_count_; }
...
void Grid::setUserData(const Ogre::Any& data) { manual_object_->getUserObjectBindings().setUserAny(data); }
