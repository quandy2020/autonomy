...
Grid::Grid(Ogre::SceneManager* scene_manager, Ogre::SceneNode* parent_node,
           Style style, uint32_t cell_count, float cell_length,
    ...
    billboard_line_ =
        std::make_shared<BillboardLine>(scene_manager_, scene_node_);

    std::string grid_material_name = grid_name + "Material";
    material_ =
        MaterialManager::createMaterialWithNoLighting(grid_material_name);
...
void Grid::createManualGrid() const {
    AddLineFunction addLineFunction =
        std::bind(&Grid::addManualLine, this, std::placeholders::_1,
                  std::placeholders::_2);

    const uint32_t number_of_vertices_in_plane =
        cell_count_ * 4 * (height_count_ + 1);
    manual_object_->estimateVertexCount(number_of_vertices_in_plane +
...
void Grid::createBillboardGrid() const {
    AddLineFunction addLineFunction =
        std::bind(&Grid::addBillboardLine, this, std::placeholders::_1,
                  std::placeholders::_2);
    ...
    const uint32_t number_of_lines_in_plane =
        (cell_count_ + 1) * 2 * (height_count_ + 1);
    billboard_line_->setNumLines(number_of_lines_in_plane +
                                 numberOfVerticalLines());
...
void Grid::createLines(AddLineFunction addLine) const {
    ...
    for (uint32_t current_height = 0; current_height <= height_count_;
         ++current_height) {
...
void Grid::createGridPlane(float extent, uint32_t height,
                           AddLineFunction addLine) const {
    float real_height =
        (height_count_ / 2.0f - static_cast<float>(height)) * cell_length_;
...
void Grid::createVerticalLinesBetweenPlanes(float extent,
                                            AddLineFunction addLine) const {
...
void Grid::addManualLine(const Ogre::Vector3& p1,
                         const Ogre::Vector3& p2) const {
...
void Grid::addBillboardLine(const Ogre::Vector3& p1,
                            const Ogre::Vector3& p2) const {
