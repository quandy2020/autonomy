...
/**
 * @file detector.cpp
 * @brief Fixed-profile YOLO26 preprocessing, parsing, and engine adapter.
 */
...
bool ValidateModelContract(const common::network::Engine& engine,
    ...
    const int64_t width = static_cast<int64_t>(options.detector_width());
    const int64_t height = static_cast<int64_t>(options.detector_height());
    const int64_t max_detections =
        static_cast<int64_t>(options.max_detections());
    return ValidateTensor(*images, "model input 'images'",
                          {1, 3, height, width}, error) &&
           ValidateTensor(*output0, "model output 'output0'",
                          {1, max_detections, 6}, error);
}
...
YoloDetector::YoloDetector(proto::ShadowOptions options,
...
std::unique_ptr<YoloDetector> YoloDetector::Create(
    ...
    inference_options.backend_id = options.detector_backend();
    inference_options.model_path = options.detector_model_path();
    ...
    return std::unique_ptr<YoloDetector>(new YoloDetector(
        options, std::make_unique<EngineDetectorRunner>(std::move(engine))));
}

std::unique_ptr<YoloDetector> YoloDetector::Create(
    const proto::ShadowOptions& options, std::unique_ptr<DetectorRunner> runner,
    std::string* error) {
    if (error != nullptr) {
        error->clear();
    }
    ...
    return std::unique_ptr<YoloDetector>(
        new YoloDetector(options, std::move(runner)));
}
...
bool YoloDetector::Detect(
    ...
    if (detections == nullptr) {
        SetError(error, "detection output is null.");
        return false;
    }

    const uint32_t configured_width = options_.detector_width();
    const uint32_t configured_height = options_.detector_height();
    if (configured_width >
            static_cast<uint32_t>(std::numeric_limits<int>::max()) ||
        configured_height >
            static_cast<uint32_t>(std::numeric_limits<int>::max())) {
    ...
    int padding_left = 0;
    int padding_top = 0;
    if (!PrepareInput(image, static_cast<int>(configured_width),
                      static_cast<int>(configured_height), &inputs, &scale,
                      &padding_left, &padding_top, error)) {
        return false;
    }
    ...
    const size_t expected_count =
        static_cast<size_t>(options_.max_detections()) * 6;
    ...
    *parsed.mutable_header() = image.header();
    ...
    const float person_class_id =
        static_cast<float>(options_.person_class_id());
    for (uint32_t index = 0; index < options_.max_detections(); ++index) {
        ...
        const float class_id = values[offset + 5];
        if (!std::isfinite(confidence) || !std::isfinite(class_id) ||
            confidence < options_.confidence_threshold() ||
