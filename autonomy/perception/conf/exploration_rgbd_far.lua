-- RGB-D FAR / long-range exploration.
-- Location: autonomy/perception/conf/exploration_rgbd_far.lua
-- Referenced by PerceptionOptions.exploration_config (still Lua for explorer backends).
-- Not loaded via LoadModuleConf; read by the exploration component.


return {
  map_frame = "map",
  default_area_half_extent_m = 20.0,
  completion_coverage_ratio = 0.9,

  explorer_backend = "far",

  sensor_range_m = 8.0,
  camera_height_m = 0.6,
  depth_min_m = 0.1,
  depth_max_m = 8.0,
  depth_stride = 4,
  ground_height_tol_m = 0.15,

  costmap_resolution_m = 0.1,
  costmap_size_cells = 400,
  collision_radius_m = 0.35,

  lookahead_distance_m = 4.5,
  use_line_of_sight_lookahead = true,
  min_frontier_cluster_size = 5,

  far_connect_dist_m = 6.0,
  far_contour_stride_cells = 3,
  far_attempt_unknown = true,
  use_prior_map = true,
  prior_map_topic = "/map",
  prior_map_lethal_threshold = 65,
  far_is_static_env = false,
  far2d_graph_pool_size = 256,
  far2d_dumper_thred = 8,
  far_goal_distance_weight = 1.0,
  far_goal_unknown_weight = 0.1,
  publish_vg_markers = true,
  use_planner_costmap = true,
  planner_costmap_topic = "/global_costmap",
  planner_costmap_lethal_threshold = 50,
  far_use_trajectory_edges = true,
  far_trajectory_node_count = 24,

  viewpoint_num_z = 3,
  viewpoint_resolution_z_m = 0.5,
  use_momentum = true,
  local_path_optimization_iters = 2,
  point_cloud_voxel_m = 0.2,
}
