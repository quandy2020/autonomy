...
TEST(DepthLifterTest, LiftsConstantDepthPlane) {
    DepthLifter lifter(Options());
...
TEST(DepthLifterTest, OmitsBoxesWithTooFewSamples) {
...
TEST(DepthLifterTest, Scales16BitDepth) {
    DepthLifter lifter(Options());
    Detection2DArray detections_2d;
    *detections_2d.add_detections() = Box(3.0, 3.0, 4.0, 4.0);
    const auto depth =
...
TEST(DepthLifterTest, UsesBaseFrameWhenTransformValid) {
    DepthLifter lifter(Options());
    Detection2DArray detections_2d;
    *detections_2d.add_detections() = Box(3.0, 3.0, 4.0, 4.0);
    const auto depth =
...
TEST(DepthLifterTest, FallsBackToCameraWithoutTransform) {
    DepthLifter lifter(Options());
    Detection2DArray detections_2d;
    *detections_2d.add_detections() = Box(3.0, 3.0, 4.0, 4.0);
    const auto depth =
