...
/**
 * @file open_worker_test.cpp
 * @brief Tests for non-blocking open detection worker.
 */
...
class BlockingRunner final : public DetectorRunner
{
public:
...
TEST(OpenAsyncWorkerTest, RejectsSubmitWhileBusy) {
    ...
    OpenAsyncWorker worker(std::move(detector), 1);

    ASSERT_TRUE(worker.TrySubmit(TinyRgb()));
    runner_ptr->WaitUntilStarted();
    EXPECT_FALSE(worker.TrySubmit(TinyRgb()));

    runner_ptr->Release();
    automsgs::msgs::vision_msgs::Detection2DArray latest;
    for (int i = 0; i < 50; ++i) {
        if (worker.TryGetLatest(&latest)) {
            break;
        }
