...
void RegulatedPurePursuitController::Configure(
    ...
    if (!costmap_wrapper_) {
        throw autonomy::control::common::ControllerException(
            "RegulatedPurePursuitController: costmap_wrapper is null");
    }
    costmap_ = costmap_wrapper_->getCostmap();
    if (!costmap_) {
        throw autonomy::control::common::ControllerException(
            "RegulatedPurePursuitController: costmap is null");
    }
    ...
    param_handler_ = std::make_unique<ParameterHandler>(
        options.pure_pursuit_controller_options(),
        costmap_->getSizeInMetersX());
    pp_options_ = &param_handler_->GetOptions();

    // Handles global path transformations
    if (!tf_buffer_) {
        AWARN << "RegulatedPurePursuitController configured without TF buffer; "
                 "controller will be inactive until TF is "
                 "provided.";
        path_handler_.reset();
    } else {
        path_handler_ = std::make_unique<PathHandler>(
            pp_options_->transform_tolerance(), tf_buffer_, costmap_wrapper_);
    }

    // Checks for imminent collisions
    collision_checker_ = std::make_unique<CollisionChecker>(
        node_, costmap_wrapper_, pp_options_);
    ...
    // global_path_pub_ =
    // node->CreateWriter<commsgs::planning_msgs::Path>("received_global_plan");
    // carrot_pub_ =
    // node->CreateWriter<commsgs::geometry_msgs::PointStamped>("lookahead_point");
    // curvature_carrot_pub_ =
    // node->CreateWriter<commsgs::geometry_msgs::PointStamped>("curvature_lookahead_point");
...
std::unique_ptr<commsgs::geometry_msgs::PointStamped>
...
double RegulatedPurePursuitController::getLookAheadDistance(
    ...
    if (pp_options_->use_velocity_scaled_lookahead_dist()) {
        const double lookahead_dist =
            fabs(speed.twist.linear.x) * pp_options_->lookahead_time();
        return std::clamp(lookahead_dist, pp_options_->min_lookahead_dist(),
                          pp_options_->max_lookahead_dist());
    }
...
uint32 RegulatedPurePursuitController::ComputeVelocityCommands(
    ...
    if (!path_handler_) {
        message =
            "RegulatedPurePursuitController not initialized (missing TF buffer "
            "/ path handler)";
        return proto::CONTROLLER_RESULT_NOT_INITIALIZED;
    }
    if (!pp_options_ || !param_handler_) {
        message =
            "RegulatedPurePursuitController not initialized (missing "
            "parameters)";
        return proto::CONTROLLER_RESULT_NOT_INITIALIZED;
    }
    ...
    const double max_robot_pose_search_dist =
        pp_options_->max_robot_pose_search_dist();
    const bool interpolate_curvature_after_goal =
        pp_options_->interpolate_curvature_after_goal();
    auto transformed_plan = path_handler_->transformGlobalPlan(
        pose, max_robot_pose_search_dist, interpolate_curvature_after_goal);
    if (global_path_pub_) {
    ...
    last_dist_to_goal_ =
        std::hypot(transformed_plan.poses.back().pose.position.x,
                   transformed_plan.poses.back().pose.position.y);
    last_angle_to_goal_ =
        transform::tf2::getYaw(transformed_plan.poses.back().pose.orientation);
    ...
    if (allow_reversing) {
        ...
        // if the lookahead distance is further than the cusp, use the cusp
        // distance instead
    ...
    const bool use_fixed_curvature_lookahead =
        pp_options_->use_fixed_curvature_lookahead();
    if (use_fixed_curvature_lookahead) {
        auto curvature_lookahead_pose =
            getLookAheadPoint(curv_lookahead_dist, transformed_plan,
        ...
        if (curvature_carrot_pub_) {
            curvature_carrot_pub_->Write(
                *createCarrotMsg(curvature_lookahead_pose));
        }
    }
    ...
    // For shouldRotateToPath, using x_vel_sign in order to support
    // allow_reversing and rotate_to_path_carrot_pose for the direction carrot
    // pose:
    //        - equal to "normal" carrot_pose when curvature_lookahead_pose =
    //        false
    //        - otherwise equal to curvature_lookahead_pose (which can be
    //        interpolated after goal)
    ...
    if (shouldRotateToGoalHeading(carrot_pose)) {
        ...
        if (collision_checker_) {
            pose_cost = collision_checker_->costAtPose(pose.pose.position.x,
                                                       pose.pose.position.y);
        }
        applyConstraints(regulation_curvature, speed, pose_cost,
                         transformed_plan, linear_vel, x_vel_sign);

        if (cancelling_ && pp_options_) {
            const double& dt = control_duration_;
            const double cancel_deceleration =
                pp_options_->cancel_deceleration();
            linear_vel =
                speed.twist.linear.x - x_vel_sign * dt * cancel_deceleration;
        ...
        // Apply curvature to angular velocity after constraining linear
        // velocity
    ...
    if (use_collision_detection && collision_checker_) {
        if (collision_checker_->isCollisionImminent(pose, linear_vel,
                                                    angular_vel, carrot_dist)) {
            throw autonomy::control::common::NoValidControl(
...
bool RegulatedPurePursuitController::shouldRotateToPath(
    ...
    if (x_vel_sign < 0.0) {
        ...
        angle_to_path =
            std::fmod(angle_to_path + M_PI + M_PI, 2.0 * M_PI) - M_PI;
    }
    const bool use_rotate_to_heading =
        pp_options_ ? pp_options_->use_rotate_to_heading() : false;
    const double rotate_to_heading_min_angle =
        pp_options_ ? pp_options_->rotate_to_heading_min_angle() : 0.1;
    return use_rotate_to_heading &&
           fabs(angle_to_path) > rotate_to_heading_min_angle;
}
...
void RegulatedPurePursuitController::rotateToHeading(
    ...
    const double rotate_to_heading_angular_vel =
        pp_options_ ? pp_options_->rotate_to_heading_angular_vel() : 1.0;
    angular_vel = sign * rotate_to_heading_angular_vel;

    const double& dt = control_duration_;
    const double max_angular_accel =
        pp_options_ ? pp_options_->max_angular_accel() : 1.0;
    const double min_feasible_angular_speed =
        curr_speed.twist.angular.z - max_angular_accel * dt;
...
RegulatedPurePursuitController::circleSegmentIntersection(
    ...
    // This can be derived from solving the system of equations of a line and a
...
RegulatedPurePursuitController::getLookAheadPoint(
    ...
    auto goal_pose_it =
        std::find_if(transformed_plan.poses.begin(),
                     transformed_plan.poses.end(), [&](const auto& ps) {
    ...
    if (goal_pose_it == transformed_plan.poses.end()) {
        if (interpolate_after_goal) {
            ...
            // Use the circle intersection to find the position at the correct
            // look ahead distance
            const auto interpolated_position =
                circleSegmentIntersection(last_pose_it->pose.position,
                                          projected_position, lookahead_dist);
        ...
        // origin) This can be found with a closed form for the intersection of
        // a segment and a circle Because of the way we did the std::find_if,
        // prev_pose is guaranteed to be inside the circle, and goal_pose is
        // guaranteed to be outside the circle.
        auto prev_pose_it = std::prev(goal_pose_it);
        auto point = circleSegmentIntersection(prev_pose_it->pose.position,
...
void RegulatedPurePursuitController::applyConstraints(
    ...
    if (pp_options_->use_regulated_linear_velocity_scaling()) {
        curvature_vel = heuristics::curvatureConstraint(
            linear_vel, curvature,
            pp_options_->regulated_linear_scaling_min_radius());
    }

    // limit the linear velocity by proximity to obstacles
    if (pp_options_->use_cost_regulated_linear_velocity_scaling()) {
        cost_vel = heuristics::costConstraint(linear_vel, pose_cost,
                                              costmap_wrapper_, *pp_options_);
    }
    ...
    linear_vel =
        std::max(linear_vel, pp_options_->regulated_linear_scaling_min_speed());

    // Apply constraint to reduce speed on approach to the final goal pose
    linear_vel = heuristics::approachVelocityConstraint(
        linear_vel, path, pp_options_->min_approach_linear_velocity(),
        pp_options_->approach_velocity_scaling_dist());

    // Limit linear velocities to be valid
    linear_vel =
        std::clamp(fabs(linear_vel), 0.0, pp_options_->desired_linear_vel());
    linear_vel = sign * linear_vel;
}

bool RegulatedPurePursuitController::IsGoalReached(double dist_tolerance,
...
void RegulatedPurePursuitController::SetSpeedLimit(const double& speed_limit,
    ...
    if (speed_limit == map::costmap_2d::NO_SPEED_LIMIT) {
        ...
        if (percentage) {
            ...
            param_handler_->SetDesiredLinearVel(
                param_handler_->GetBaseDesiredLinearVel() * speed_limit /
                100.0);
        } else {
...
double RegulatedPurePursuitController::findVelocitySignChange(
    const commsgs::planning_msgs::Path& transformed_plan) {
    // Iterating through the transformed global path to determine the position
    // of the cusp
    for (unsigned int pose_id = 1; pose_id < transformed_plan.poses.size() - 1;
         ++pose_id) {
        ...
        product and determine it's distance from the robot. If there is no cusp
        in the path, then just determine the distance to the goal location. */
        const double dot_prod = (oa_x * ab_x) + (oa_y * ab_y);
        if (dot_prod < 0.0) {
            // returning the distance if there is a cusp
            // The transformed path is in the robots frame, so robot is at the
            // origin
        ...
        double yaw1 = transform::tf2::getYaw(
            transformed_plan.poses[pose_id - 1].pose.orientation);
        double yaw2 = transform::tf2::getYaw(
            transformed_plan.poses[pose_id].pose.orientation);
        double yaw3 = transform::tf2::getYaw(
            transformed_plan.poses[pose_id + 1].pose.orientation);
