...
void RegulatedPurePursuitController::Configure(
    ...
    //     tf2::durationFromSec(params_->transform_tolerance), tf_,
    //     costmap_ros_);

    // // Checks for imminent collisions
    // collision_checker_ = std::make_unique<CollisionChecker>(node,
    // costmap_ros_, params_);
    ...
    // global_path_pub_ =
    // node->create_publisher<commsgs::planning_msgs::Path>("received_global_plan",
    // 1); carrot_pub_ =
    // node->create_publisher<commsgs::geometry_msgs::PointStamped>("lookahead_point",
    // 1); curvature_carrot_pub_ =
    // node->create_publisher<commsgs::geometry_msgs::PointStamped>(
...
double RegulatedPurePursuitController::getLookAheadDistance(
    ...
    if (params_->use_velocity_scaled_lookahead_dist) {
        ...
        lookahead_dist = std::clamp(lookahead_dist, params_->min_lookahead_dist,
...
uint32 RegulatedPurePursuitController::ComputeVelocityCommands(
    ...
    if (shouldRotateToGoalHeading(carrot_pose)) {
        ...
        double angle_to_goal =
            transform::tf2::getYaw(transform::tf2::Quaternion{
        ...
        applyConstraints(regulation_curvature, velocity.twist,
                         collision_checker_->costAtPose(pose.pose.position.x,
                                                        pose.pose.position.y),
                         transformed_plan, linear_vel, x_vel_sign);

        // Cancel functionality removed - cancelling_ and finished_cancelling_
        // are no longer used Apply curvature to angular velocity after
    ...
    if (params_->use_collision_detection &&
        collision_checker_->isCollisionImminent(pose, linear_vel, angular_vel,
                                                carrot_dist)) {
...
bool RegulatedPurePursuitController::shouldRotateToPath(
    ...
    if (x_vel_sign < 0.0) {
        angle_to_path = autonomy::common::math::NormalizeAngleDifference(
            angle_to_path + M_PI);
    }
...
void RegulatedPurePursuitController::applyConstraints(
    ...
    if (params_->use_regulated_linear_velocity_scaling) {
        curvature_vel =
            curvatureConstraint(linear_vel, curvature,
    ...
    if (params_->use_cost_regulated_linear_velocity_scaling) {
        cost_vel =
            costConstraint(linear_vel, pose_cost, costmap_wrapper_, params_);
    }
...
double RegulatedPurePursuitController::findVelocitySignChange(
    ...
    auto quat_equal = [](const commsgs::geometry_msgs::Quaternion& q1,
        ...
        return (std::abs(q1.x - q2.x) < tol && std::abs(q1.y - q2.y) < tol &&
                std::abs(q1.z - q2.z) < tol && std::abs(q1.w - q2.w) < tol);
    };
    ...
    for (unsigned int pose_id = 1; pose_id < transformed_plan.poses.size() - 1;
        ...
        /* Checking for the existance of cusp, in the path, using the dot
