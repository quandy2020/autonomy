...
double PathHandler::getCostmapMaxExtent() const {
  ...
  const double max_costmap_dim_meters = std::max(costmap->getSizeInMetersX(), costmap->getSizeInMetersY());
...
commsgs::planning_msgs::Path PathHandler::transformGlobalPlan(const commsgs::geometry_msgs::PoseStamped& pose,
  ...
  if (reject_unit_path && global_plan_.poses.size() == 1) {
    throw autonomy::control::common::InvalidPath("Received plan with length of one");
  }
  ...
  auto transformGlobalPoseToLocal = [&](const auto& global_plan_pose) {
    ...
    try {
      // Use Buffer's transform method directly
      transformed_pose = tf_buffer_->transform(stamped_pose, costmap_wrapper_->getBaseFrameID(),
...
bool PathHandler::transformPose(const std::string frame, const commsgs::geometry_msgs::PoseStamped& in_pose,
  ...
  try {
    out_pose = tf_buffer_->transform(in_pose, frame, static_cast<float>(transform_tolerance_));
