...
PathHandler::PathHandler(
    transform::tf2::Duration transform_tolerance, std::shared_ptr<TfBuffer> tf,
...
double PathHandler::getCostmapMaxExtent() const {
    const double max_costmap_dim_meters =
        std::max(costmap_wrapper_->getCostmap()->getSizeInMetersX(),
...
commsgs::planning_msgs::Path PathHandler::transformGlobalPlan(
    ...
    auto transformGlobalPoseToLocal = [&](const auto& global_plan_pose) {
        ...
        if (!transformPose(costmap_wrapper_->getBaseFrameID(), stamped_pose,
                           transformed_pose)) {
            throw common::ControllerTFError(
...
bool PathHandler::transformPose(
    const std::string frame, const commsgs::geometry_msgs::PoseStamped& in_pose,
