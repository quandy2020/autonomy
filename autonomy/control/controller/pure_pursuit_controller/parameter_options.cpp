...
proto::PurePursuitControllerOptions LoadOptions(
  ...
  // This is important because proto3 scalar fields don't have presence by default.
  ...
  options.set_max_robot_pose_search_dist(0.0);  // filled by ParameterHandler based on costmap size
  ...
  if (parameter_dictionary->HasKey("desired_linear_vel")) {
    options.set_desired_linear_vel(parameter_dictionary->GetDouble("desired_linear_vel"));
  }
  if (parameter_dictionary->HasKey("lookahead_dist")) {
    options.set_lookahead_dist(parameter_dictionary->GetDouble("lookahead_dist"));
  }
  if (parameter_dictionary->HasKey("min_lookahead_dist")) {
    options.set_min_lookahead_dist(parameter_dictionary->GetDouble("min_lookahead_dist"));
  }
  if (parameter_dictionary->HasKey("max_lookahead_dist")) {
    options.set_max_lookahead_dist(parameter_dictionary->GetDouble("max_lookahead_dist"));
  }
  if (parameter_dictionary->HasKey("lookahead_time")) {
    options.set_lookahead_time(parameter_dictionary->GetDouble("lookahead_time"));
  }
  if (parameter_dictionary->HasKey("use_velocity_scaled_lookahead_dist")) {
    options.set_use_velocity_scaled_lookahead_dist(parameter_dictionary->GetBool("use_velocity_scaled_lookahead_dist"));
  }

  if (parameter_dictionary->HasKey("rotate_to_heading_angular_vel")) {
    options.set_rotate_to_heading_angular_vel(parameter_dictionary->GetDouble("rotate_to_heading_angular_vel"));
  }
  if (parameter_dictionary->HasKey("rotate_to_heading_min_angle")) {
    options.set_rotate_to_heading_min_angle(parameter_dictionary->GetDouble("rotate_to_heading_min_angle"));
  }
  if (parameter_dictionary->HasKey("max_angular_accel")) {
    options.set_max_angular_accel(parameter_dictionary->GetDouble("max_angular_accel"));
  }
  if (parameter_dictionary->HasKey("use_rotate_to_heading")) {
    options.set_use_rotate_to_heading(parameter_dictionary->GetBool("use_rotate_to_heading"));
  }

  if (parameter_dictionary->HasKey("use_cancel_deceleration")) {
    options.set_use_cancel_deceleration(parameter_dictionary->GetBool("use_cancel_deceleration"));
  }
  if (parameter_dictionary->HasKey("cancel_deceleration")) {
    options.set_cancel_deceleration(parameter_dictionary->GetDouble("cancel_deceleration"));
  }

  if (parameter_dictionary->HasKey("allow_reversing")) {
    options.set_allow_reversing(parameter_dictionary->GetBool("allow_reversing"));
  }
  ...
  if (parameter_dictionary->HasKey("use_fixed_curvature_lookahead")) {
    options.set_use_fixed_curvature_lookahead(parameter_dictionary->GetBool("use_fixed_curvature_lookahead"));
  }
  if (parameter_dictionary->HasKey("curvature_lookahead_dist")) {
    options.set_curvature_lookahead_dist(parameter_dictionary->GetDouble("curvature_lookahead_dist"));
  }
  if (parameter_dictionary->HasKey("interpolate_curvature_after_goal")) {
    options.set_interpolate_curvature_after_goal(parameter_dictionary->GetBool("interpolate_curvature_after_goal"));
  }

  if (parameter_dictionary->HasKey("use_regulated_linear_velocity_scaling")) {
    options.set_use_regulated_linear_velocity_scaling(
        parameter_dictionary->GetBool("use_regulated_linear_velocity_scaling"));
  }
  if (parameter_dictionary->HasKey("regulated_linear_scaling_min_radius")) {
    options.set_regulated_linear_scaling_min_radius(
        parameter_dictionary->GetDouble("regulated_linear_scaling_min_radius"));
  }
  if (parameter_dictionary->HasKey("regulated_linear_scaling_min_speed")) {
    options.set_regulated_linear_scaling_min_speed(
        parameter_dictionary->GetDouble("regulated_linear_scaling_min_speed"));
  }

  if (parameter_dictionary->HasKey("use_cost_regulated_linear_velocity_scaling")) {
    options.set_use_cost_regulated_linear_velocity_scaling(
        parameter_dictionary->GetBool("use_cost_regulated_linear_velocity_scaling"));
  }
  if (parameter_dictionary->HasKey("cost_scaling_dist")) {
    options.set_cost_scaling_dist(parameter_dictionary->GetDouble("cost_scaling_dist"));
  }
  if (parameter_dictionary->HasKey("cost_scaling_gain")) {
    options.set_cost_scaling_gain(parameter_dictionary->GetDouble("cost_scaling_gain"));
  }
  if (parameter_dictionary->HasKey("inflation_cost_scaling_factor")) {
    options.set_inflation_cost_scaling_factor(parameter_dictionary->GetDouble("inflation_cost_scaling_factor"));
  }

  if (parameter_dictionary->HasKey("min_approach_linear_velocity")) {
    options.set_min_approach_linear_velocity(parameter_dictionary->GetDouble("min_approach_linear_velocity"));
  }
  if (parameter_dictionary->HasKey("approach_velocity_scaling_dist")) {
    options.set_approach_velocity_scaling_dist(parameter_dictionary->GetDouble("approach_velocity_scaling_dist"));
  }
  ...
  if (parameter_dictionary->HasKey("max_allowed_time_to_collision_up_to_carrot")) {
    options.set_max_allowed_time_to_collision_up_to_carrot(
        parameter_dictionary->GetDouble("max_allowed_time_to_collision_up_to_carrot"));
  }
