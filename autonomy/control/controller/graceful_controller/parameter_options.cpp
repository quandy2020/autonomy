...
proto::GracefulControllerOptions LoadOptions(
    ...
    if (parameter_dictionary->HasKey("max_lookahead")) {
        options.set_max_lookahead(
            parameter_dictionary->GetDouble("max_lookahead"));
    }
    if (parameter_dictionary->HasKey("min_lookahead")) {
        options.set_min_lookahead(
            parameter_dictionary->GetDouble("min_lookahead"));
    }
    ...
    if (parameter_dictionary->HasKey("v_linear_min")) {
        options.set_v_linear_min(
            parameter_dictionary->GetDouble("v_linear_min"));
    }
    if (parameter_dictionary->HasKey("v_linear_max")) {
        options.set_v_linear_max(
            parameter_dictionary->GetDouble("v_linear_max"));
    }
    if (parameter_dictionary->HasKey("v_angular_max")) {
        options.set_v_angular_max(
            parameter_dictionary->GetDouble("v_angular_max"));
    }
    if (parameter_dictionary->HasKey("v_angular_min_in_place")) {
        options.set_v_angular_min_in_place(
            parameter_dictionary->GetDouble("v_angular_min_in_place"));
    }
    ...
    if (parameter_dictionary->HasKey("initial_rotation")) {
        options.set_initial_rotation(
            parameter_dictionary->GetBool("initial_rotation"));
    }
    if (parameter_dictionary->HasKey("initial_rotation_tolerance")) {
        options.set_initial_rotation_tolerance(
            parameter_dictionary->GetDouble("initial_rotation_tolerance"));
    }
    if (parameter_dictionary->HasKey("prefer_final_rotation")) {
        options.set_prefer_final_rotation(
            parameter_dictionary->GetBool("prefer_final_rotation"));
    }
    if (parameter_dictionary->HasKey("rotation_scaling_factor")) {
        options.set_rotation_scaling_factor(
            parameter_dictionary->GetDouble("rotation_scaling_factor"));
    }
