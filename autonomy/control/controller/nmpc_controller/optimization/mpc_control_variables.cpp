...
MpcControlVariables::MpcControlVariables(
...
void MpcControlVariables::SetVariables(const ifopt::Component::VectorXd& x) {
...
ifopt::Component::VectorXd MpcControlVariables::GetValues() const {
...
ifopt::Component::VecBound MpcControlVariables::GetBounds() const {
    double u0_lo = 0.0;
    double u0_hi = 0.0;
    double u1_lo = 0.0;
    double u1_hi = 0.0;
...
void MpcControlVariables::SetControls(
    const std::vector<models::Control2D>& controls) {
    controls_ = controls;
}

void MpcControlVariables::ShiftWarmStart() {
    if (controls_.size() < 2) {
        return;
    }
