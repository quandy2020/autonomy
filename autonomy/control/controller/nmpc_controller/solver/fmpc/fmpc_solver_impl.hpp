...
/**
 * @file fmpc_solver_impl.hpp
 * @brief Template implementation of nmpc_controller::fmpc::FmpcSolver
 */
...
#include "autonomy/control/controller/nmpc_controller/solver/fmpc/math_utils.hpp"
...
namespace fmpc_solver_impl_detail
{
// Elapsed time between two time points [ms].
    ...
    const Variable & initial_variable)
{
  ...
  computation_duration_.setup = fmpc_solver_impl_detail::calcDuration(start_time, setup_time);
  ...
  computation_duration_.opt = fmpc_solver_impl_detail::calcDuration(setup_time, end_time);
  computation_duration_.solve = fmpc_solver_impl_detail::calcDuration(start_time, end_time);
...
typename FmpcSolver<StateDim, InputDim, IneqDim>::Status FmpcSolver<StateDim, InputDim, IneqDim>::procOnce(int iter)
{
  ...
  // Step 1: calculate coefficients of linearized KKT condition
  {
    ...
    double duration = fmpc_solver_impl_detail::calcDuration(start_time, std::chrono::system_clock::now());
...
bool FmpcSolver<StateDim, InputDim, IneqDim>::backwardPass()
{
  ...
  for(int i = config_.horizon_steps - 1; i >= 0; i--)
  {
    ...
    // Pre-process for gain calculation
    {
      ...
      computation_duration_.gain_pre += fmpc_solver_impl_detail::calcDuration(start_time_gain_pre, std::chrono::system_clock::now());
    }
    ...
    // Solve linear equation for gain calculation
    {
      ...
      computation_duration_.gain_solve += fmpc_solver_impl_detail::calcDuration(start_time_gain_solve, std::chrono::system_clock::now());
    }
    ...
    // Post-process for gain calculation
    {
      ...
      computation_duration_.gain_post += fmpc_solver_impl_detail::calcDuration(start_time_gain_post, std::chrono::system_clock::now());
    }
...
bool FmpcSolver<StateDim, InputDim, IneqDim>::updateVariables()
{
  ...
  double alpha_nu_max = 1.0;
  {
    ...
    computation_duration_.fraction += fmpc_solver_impl_detail::calcDuration(start_time_fraction, std::chrono::system_clock::now());
  }
