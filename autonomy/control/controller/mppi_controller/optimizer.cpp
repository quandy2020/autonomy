...
void Optimizer::getParams() {
    ...
    // 如果 proto 中未设置（为 0），则回退到一组与 nav2_mppi_controller
    // 类似的默认值， 以保证示例应用即使未加载 Lua 配置也能安全运行。
    ...
    s.iteration_count =
        options_->iteration_count() > 0 ? options_->iteration_count() : 1;
    s.temperature =
        options_->temperature() > 0.0 ? options_->temperature() : 0.3;
    s.gamma = options_->gamma() > 0.0 ? options_->gamma() : 0.015;
    s.base_constraints.vx_max =
        options_->vx_max() != 0.0 ? options_->vx_max() : 0.5f;
    s.base_constraints.vx_min =
        options_->vx_min() != 0.0 ? options_->vx_min() : -0.35f;
    s.base_constraints.vy =
        options_->vy_max() != 0.0 ? options_->vy_max() : 0.5f;
    s.base_constraints.wz =
        options_->wz_max() != 0.0 ? options_->wz_max() : 1.9f;
...
void Optimizer::applyControlSequenceConstraints() {
    ...
    float vx_last = tools::clamp(s.constraints.vx_min, s.constraints.vx_max,
                                 control_sequence_.vx(0));
    float wz_last = tools::clamp(-s.constraints.wz, s.constraints.wz,
                                 control_sequence_.wz(0));
    control_sequence_.vx(0) = vx_last;
    control_sequence_.wz(0) = wz_last;
    float vy_last = 0;
    if (isHolonomic()) {
        vy_last = tools::clamp(-s.constraints.vy, s.constraints.vy,
    ...
    for (unsigned int i = 1; i != control_sequence_.vx.size(); i++) {
        ...
        vx_curr =
            tools::clamp(s.constraints.vx_min, s.constraints.vx_max, vx_curr);
        if (vx_last > 0) {
            vx_curr = tools::clamp(vx_last + min_delta_vx,
                                   vx_last + max_delta_vx, vx_curr);
        } else {
            vx_curr = tools::clamp(vx_last - max_delta_vx,
                                   vx_last - min_delta_vx, vx_curr);
        }
        vx_last = vx_curr;

        float& wz_curr = control_sequence_.wz(i);
        wz_curr = tools::clamp(-s.constraints.wz, s.constraints.wz, wz_curr);
        wz_curr = tools::clamp(wz_last - max_delta_wz, wz_last + max_delta_wz,
                               wz_curr);
        wz_last = wz_curr;
        ...
        if (isHolonomic()) {
            ...
            vy_curr =
                tools::clamp(-s.constraints.vy, s.constraints.vy, vy_curr);
            if (vy_last > 0) {
                vy_curr = tools::clamp(vy_last + min_delta_vy,
                                       vy_last + max_delta_vy, vy_curr);
            } else {
                vy_curr = tools::clamp(vy_last - max_delta_vy,
                                       vy_last - min_delta_vy, vy_curr);
            }
            vy_last = vy_curr;
        }
    }
...
void Optimizer::integrateStateVelocities(
    ...
    float initial_yaw = static_cast<float>(
        autonomy::transform::tf2::getYaw(state_.pose.pose.orientation));
...
void Optimizer::integrateStateVelocities(models::Trajectories& trajectories,
    ...
    auto initial_yaw = static_cast<float>(
        autonomy::transform::tf2::getYaw(state.pose.pose.orientation));
...
void Optimizer::updateControlSequence() {
    ...
    if (s.sampling_std.wz > 0.0f) {
        ...
        const float gamma_wz =
            s.gamma / (s.sampling_std.wz * s.sampling_std.wz);
        costs_ +=
            (gamma_wz * (bounded_noises_wz.rowwise() * wz_T).rowwise().sum())
                .eval();
    }
    ...
    if (is_holo) {
        ...
        const float gamma_vy =
            s.gamma / (s.sampling_std.vy * s.sampling_std.vy);
        costs_ +=
            (gamma_vy * (bounded_noises_vy.rowwise() * vy_T).rowwise().sum())
                .eval();
    }
...
commsgs::geometry_msgs::TwistStamped Optimizer::getControlFromSequenceAsTwist(
    ...
    if (isHolonomic()) {
        ...
        return tools::toTwistStamped(vx, vy, wz, stamp,
