...
TrajectoryVisualizer::TrajectoryVisualizer(std::shared_ptr<autolink::Node> parent, const std::string& name,
                                           const std::string& frame_id, const proto::MPPIControllerOptions* options) {
  frame_id_ = frame_id;
  trajectories_publisher_ =
      parent->CreateWriter<commsgs::visualization_msgs::MarkerArray>(name + "/candidate_trajectories");
  transformed_path_pub_ = parent->CreateWriter<commsgs::planning_msgs::Path>(name + "/transformed_global_plan");
  optimal_path_pub_ = parent->CreateWriter<commsgs::planning_msgs::Path>(name + "/optimal_path");
  options_ = options;
...
void TrajectoryVisualizer::add(const Eigen::ArrayXXf& trajectory, const std::string& marker_namespace,
  ...
  auto add_marker = [&](auto i) {
    ...
    auto scale = i != size - 1 ? tools::createScale(0.03, 0.03, 0.07) : tools::createScale(0.07, 0.07, 0.09);
    auto color = tools::createColor(0, component, component, 1);
    auto marker = tools::createMarker(marker_id_++, pose, scale, color, frame_id_, marker_namespace);
...
void TrajectoryVisualizer::add(const models::Trajectories& trajectories, const std::string& marker_namespace) {
  ...
  for (size_t i = 0; i < n_rows; i += trajectory_step_) {
    for (size_t j = 0; j < n_cols; j += time_step_) {
      ...
      auto pose = tools::createPose(trajectories.x(i, j), trajectories.y(i, j), 0.03);
      auto scale = tools::createScale(0.03, 0.03, 0.03);
      auto color = tools::createColor(0, green_component, blue_component, 1);
      auto marker = tools::createMarker(marker_id_++, pose, scale, color, frame_id_, marker_namespace);
...
void TrajectoryVisualizer::visualize(const commsgs::planning_msgs::Path& plan) {
  if (trajectories_publisher_ && trajectories_publisher_->HasReader()) {
    auto msg = std::make_shared<commsgs::visualization_msgs::MarkerArray>(*points_);
    trajectories_publisher_->Write(msg);
  }

  if (optimal_path_pub_ && optimal_path_pub_->HasReader()) {
    auto msg = std::make_shared<commsgs::planning_msgs::Path>(*optimal_path_);
