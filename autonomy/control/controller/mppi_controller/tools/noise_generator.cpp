...
void NoiseGenerator::initialize(models::OptimizerSettings& settings,
                                bool is_holonomic) {
    settings_ = settings;
    ...
    ndistribution_vx_ =
        std::normal_distribution<float>(0.0f, settings_.sampling_std.vx);
    ndistribution_vy_ =
        std::normal_distribution<float>(0.0f, settings_.sampling_std.vy);
    ndistribution_wz_ =
        std::normal_distribution<float>(0.0f, settings_.sampling_std.wz);
...
void NoiseGenerator::generateNextNoises() {
    ...
    {
        std::lock_guard<std::mutex> guard(noise_lock_);
        ready_ = true;
    }
    noise_cond_.notify_all();
}
