...
// Forward declaration of utils method, since utils.hpp can't be included here
// due to recursive inclusion.
namespace utils {
 ...
 public:
  ...
  virtual void predict(models::State& state) {
    ...
    for (unsigned int i = 1; i < n_cols; i++) {
      auto lower_bound_vx =
          (state.vx.col(i - 1) > 0).select(state.vx.col(i - 1) + min_delta_vx, state.vx.col(i - 1) - max_delta_vx);
      auto upper_bound_vx =
          (state.vx.col(i - 1) > 0).select(state.vx.col(i - 1) + max_delta_vx, state.vx.col(i - 1) - min_delta_vx);

      state.cvx.col(i - 1) = state.cvx.col(i - 1).cwiseMax(lower_bound_vx).cwiseMin(upper_bound_vx);
      ...
      if (is_holo) {
        auto lower_bound_vy =
            (state.vy.col(i - 1) > 0).select(state.vy.col(i - 1) + min_delta_vy, state.vy.col(i - 1) - max_delta_vy);
        auto upper_bound_vy =
            (state.vy.col(i - 1) > 0).select(state.vy.col(i - 1) + max_delta_vy, state.vy.col(i - 1) - min_delta_vy);
        state.cvy.col(i - 1) = state.cvy.col(i - 1).cwiseMax(lower_bound_vy).cwiseMin(upper_bound_vy);
...
class AckermannMotionModel : public MotionModel {
 public:
   ...
   */
  explicit AckermannMotionModel(const proto::MPPIControllerOptions* options, const std::string& name) {
    if (options && options->has_ackermann_constraints()) {
      min_turning_r_ = options->ackermann_constraints().min_turning_r();
    } else {
      min_turning_r_ = 0.2;  // Default
    }
  }
  ...
  void applyConstraints(models::ControlSequence& control_sequence) override {
    ...
    for (int i = 0; i < steps; i++) {
      ...
      wz_curr = std::clamp(wz_curr, -1.0f * wz_constrained, wz_constrained);
    }
  }
