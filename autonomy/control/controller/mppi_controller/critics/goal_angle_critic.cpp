...
void GoalAngleCritic::initialize() {
    ...
    AINFO << "GoalAngleCritic instantiated with " << power_ << " power, "
          << weight_ << " weight, and " << threshold_to_consider_
          << " angular threshold.";
}
...
void GoalAngleCritic::score(CriticData& data) {
    ...
    if (power_ > 1u) {
        data.costs += (((tools::shortest_angular_distance(
                             data.trajectories.yaws, goal_yaw)
                             .abs())
                            .rowwise()
                            .mean()) *
                       weight_)
...
CLASS_LOADER_REGISTER_CLASS(
    autonomy::control::controller::mppi_controller::critics::GoalAngleCritic,
