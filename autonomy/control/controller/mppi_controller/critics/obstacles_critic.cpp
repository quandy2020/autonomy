...
void ObstaclesCritic::initialize() {
    ...
    if (options_->has_obstacles_critic()) {
        ...
        collision_margin_distance_ =
            static_cast<float>(critic.collision_margin_distance());
    ...
    AINFO << "ObstaclesCritic instantiated with " << power_ << " power and "
          << critical_weight_ << " / " << repulsion_weight_ << " weights. "
...
float ObstaclesCritic::distanceToObstacle(const CollisionCost& cost) {
    ...
    float dist_to_obj =
        (scale_factor * min_radius - log(cost.cost) + log(253.0f)) /
        scale_factor;
...
void ObstaclesCritic::score(CriticData& data) {
    ...
    auto repulsive_cost_normalized =
        (repulsive_cost - repulsive_cost.minCoeff()) / traj_len;
...
CLASS_LOADER_REGISTER_CLASS(
    autonomy::control::controller::mppi_controller::critics::ObstaclesCritic,
