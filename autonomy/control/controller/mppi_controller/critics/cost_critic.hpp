...
protected:
    ...
    inline bool inCollision(float cost, float x, float y, float theta) {
        ...
        if (consider_footprint_ && (cost >= possible_collision_cost_ ||
                                    possible_collision_cost_ < 1.0f)) {
            score_cost =
                static_cast<float>(collision_checker_.footprintCostAtPose(
     ...
     * be in collision
     */
    inline float findCircumscribedCost(
     ...
     */
    inline bool worldToMapFloat(float wx, float wy, unsigned int& mx,
