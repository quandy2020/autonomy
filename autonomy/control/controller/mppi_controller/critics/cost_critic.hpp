 ...
 * @brief Critic objective function for avoiding obstacles using costmap's
 * inflated cost
 */
 ...
 protected:
  ...
  inline bool inCollision(float cost, float x, float y, float theta) {
    ...
    if (consider_footprint_ && (cost >= possible_collision_cost_ || possible_collision_cost_ < 1.0f)) {
      score_cost = static_cast<float>(
          collision_checker_.footprintCostAtPose(static_cast<double>(x), static_cast<double>(y),
                                                 static_cast<double>(theta), costmap_ros_->getRobotFootprint()));
    }
   ...
   * be in collision
   */
  inline float findCircumscribedCost(std::shared_ptr<map::costmap_2d::Costmap2DWrapper> costmap);
