...
void PathFollowCritic::initialize() {
    ...
    AINFO << "PathFollowCritic instantiated with " << power_ << " power and "
          << weight_ << " weight";
}
...
void PathFollowCritic::score(CriticData& data) {
    ...
    if (data.path.x.size() < 2 ||
        tools::withinPositionGoalTolerance(threshold_to_consider_,
...
CLASS_LOADER_REGISTER_CLASS(
    autonomy::control::controller::mppi_controller::critics::PathFollowCritic,
