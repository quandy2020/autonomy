...
void GoalCritic::initialize() {
    ...
    if (options_->has_goal_critic()) {
        const auto& critic = options_->goal_critic();
    ...
    AINFO << "GoalCritic instantiated with " << power_ << " power and " << weight_ << " weight.";
}
...
// Plugins
CLASS_LOADER_REGISTER_CLASS(autonomy::control::controller::mppi_controller::critics::GoalCritic,
