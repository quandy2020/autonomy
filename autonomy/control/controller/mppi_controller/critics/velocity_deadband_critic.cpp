...
void VelocityDeadbandCritic::initialize() {
    ...
    if (options_->has_velocity_deadband_critic()) {
        ...
        for (int i = 0; i < critic.deadband_velocities_size() && i < 3; ++i) {
            deadband_velocities_.push_back(static_cast<float>(critic.deadband_velocities(i)));
        }
    ...
    AINFO << "VelocityDeadbandCritic instantiated with " << power_ << " power, " << weight_
          << " weight, deadband_velocity [" << deadband_velocities_.at(0) << "," << deadband_velocities_.at(1) << ","
...
// Plugins
CLASS_LOADER_REGISTER_CLASS(autonomy::control::controller::mppi_controller::critics::VelocityDeadbandCritic,
