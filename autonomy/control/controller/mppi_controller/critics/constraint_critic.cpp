...
void ConstraintCritic::initialize() {
  ...
  AINFO << "ConstraintCritic instantiated with " << power_ << " power and " << weight_ << " weight.";
...
void ConstraintCritic::score(CriticData& data) {
  ...
  if (diff != nullptr) {
    if (power_ > 1u) {
      data.costs += (((((data.state.vx - max_vel_).max(0.0f) + (min_vel_ - data.state.vx).max(0.0f)) * data.model_dt)
                          .rowwise()
                          .sum()
                          .eval()) *
                     weight_)
  ...
  if (omni != nullptr) {
    ...
    if (power_ > 1u) {
      data.costs += ((((vel_total - max_vel_).max(0.0f) + (min_vel_ - vel_total).max(0.0f)) * data.model_dt)
                         .rowwise()
                         .sum()
                         .eval() *
                     weight_)
  ...
  if (acker != nullptr) {
    ...
    if (power_ > 1u) {
      data.costs +=
          ((((vx - max_vel_).max(0.0f) + (min_vel_ - vx).max(0.0f) + out_of_turning_rad_motion) * data.model_dt)
               .rowwise()
               .sum()
               .eval() *
           weight_)
...
// Plugins
CLASS_LOADER_REGISTER_CLASS(autonomy::control::controller::mppi_controller::critics::ConstraintCritic,
