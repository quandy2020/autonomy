...
void CostCritic::initialize() {
  ...
  if (options_->has_cost_critic()) {
    ...
    weight_ = static_cast<float>(critic.cost_weight()) / 254.0f;  // Normalized
  ...
  AINFO << "CostCritic instantiated with " << power_ << " power and " << critical_cost_ << " / " << weight_
        << " weights. "
        << "Critic will collision check based on " << (consider_footprint_ ? "footprint" : "circular") << " cost.";
}

float CostCritic::findCircumscribedCost(std::shared_ptr<map::costmap_2d::Costmap2DWrapper> costmap) {
  double result = -1.0;
  ...
  if (inflation_layer != nullptr) {
    ...
    if (inflation_radius < circum_radius) {
      AERROR << "The inflation radius (" << inflation_radius << ") is smaller than the circumscribed radius ("
...
void CostCritic::score(CriticData& data) {
  ...
  if (power_ > 1u) {
    data.costs += (repulsive_cost * (weight_ / static_cast<float>(strided_traj_cols))).pow(power_);
  } else {
...
// Plugins
CLASS_LOADER_REGISTER_CLASS(autonomy::control::controller::mppi_controller::critics::CostCritic,
