...
void TwirlingCritic::initialize() {
    ...
    AINFO << "TwirlingCritic instantiated with " << power_ << " power and "
          << weight_ << " weight.";
}
...
void TwirlingCritic::score(CriticData& data) {
    ...
    if (power_ > 1u) {
        data.costs += ((data.state.wz.abs().rowwise().mean()) * weight_)
                          .pow(power_)
                          .eval();
    } else {
...
CLASS_LOADER_REGISTER_CLASS(
    autonomy::control::controller::mppi_controller::critics::TwirlingCritic,
