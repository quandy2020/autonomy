...
void PathAlignCritic::initialize() {
  ...
  if (options_->has_path_align_critic()) {
    ...
    max_path_occupancy_ratio_ = static_cast<float>(critic.max_path_occupancy_ratio());
  ...
  AINFO << "PathAlignCritic instantiated with " << power_ << " power and " << weight_ << " weight";
}
...
void PathAlignCritic::score(CriticData& data) {
  ...
  for (size_t t = 0; t < batch_size; ++t) {
    ...
    for (int p = 1; p < traj_sampled_size; p++) {
      ...
      path_pt = tools::findClosestPathPt(path_integrated_distances, traj_integrated_distance, path_pt);
      ...
      if (path_pts_valid[path_pt]) {
        ...
        if (use_path_orientations_) {
          dyaw = tools::shortest_angular_distance(pose.theta, T_yaw(t, p));
...
// Plugins
CLASS_LOADER_REGISTER_CLASS(autonomy::control::controller::mppi_controller::critics::PathAlignCritic,
