...
void PathAngleCritic::initialize() {
  ...
  if (options_->has_path_angle_critic()) {
    ...
    max_angle_to_furthest_ = static_cast<float>(critic.max_angle_to_furthest());
  ...
  AINFO << "PathAngleCritic instantiated with " << power_ << " power and " << weight_
        << " weight. Mode set to: " << modeToStr(mode_);
}
...
void PathAngleCritic::score(CriticData& data) {
  ...
  auto offsetted_idx =
      std::min(*data.furthest_reached_path_point + offset_from_furthest_, static_cast<size_t>(data.path.x.size()) - 1);
  ...
  switch (mode_) {
    case PathAngleMode::FORWARD_PREFERENCE:
      if (tools::posePointAngle(pose, goal_x, goal_y, true) < max_angle_to_furthest_) {
        return;
      }
      break;
    case PathAngleMode::NO_DIRECTIONAL_PREFERENCE:
      if (tools::posePointAngle(pose, goal_x, goal_y, false) < max_angle_to_furthest_) {
        return;
      }
      break;
    case PathAngleMode::CONSIDER_FEASIBLE_PATH_ORIENTATIONS:
      if (tools::posePointAngle(pose, goal_x, goal_y, goal_yaw) < max_angle_to_furthest_) {
        return;
      }
      break;
    default:
  ...
  auto yaws_between_points =
      diff_y.binaryExpr(diff_x, [&](const float& y, const float& x) { return atan2f(y, x); }).eval();

  switch (mode_) {
    case PathAngleMode::FORWARD_PREFERENCE: {
      auto last_yaws = data.trajectories.yaws.col(last_idx);
      auto yaws = tools::shortest_angular_distance(last_yaws, yaws_between_points).abs();
      if (power_ > 1u) {
    ...
    case PathAngleMode::NO_DIRECTIONAL_PREFERENCE: {
      auto last_yaws = data.trajectories.yaws.col(last_idx);
      auto yaws_between_points_corrected = tools::normalize_yaws_between_points(last_yaws, yaws_between_points);
      auto corrected_yaws = tools::shortest_angular_distance(last_yaws, yaws_between_points_corrected).abs();
      if (power_ > 1u) {
    ...
    case PathAngleMode::CONSIDER_FEASIBLE_PATH_ORIENTATIONS: {
      auto last_yaws = data.trajectories.yaws.col(last_idx);
      auto yaws_between_points_corrected = tools::normalize_yaws_between_points(goal_yaw, yaws_between_points);
      auto corrected_yaws = tools::shortest_angular_distance(last_yaws, yaws_between_points_corrected).abs();
      if (power_ > 1u) {
        data.costs += (corrected_yaws * weight_).pow(power_);
      } else {
...
// Plugins
CLASS_LOADER_REGISTER_CLASS(autonomy::control::controller::mppi_controller::critics::PathAngleCritic,
