...
void MPPIController::Configure(const proto::ControllerOptions& options, std::string name,
  ...
  // 在独立应用（如 examples/apps/mppi_controller_app.cpp）中，没有 ROS2 节点，
  // 这里使用空 Node 指针初始化 Optimizer 与 PathHandler，使其仍能依赖 proto 参数和 costmap 运行。
  auto parent_node = std::shared_ptr<autolink::Node>();  // nullptr

  optimizer_.initialize(parent_node, name_, costmap_wrapper_, &options_);
  path_handler_.initialize(parent_node, name_, costmap_wrapper_, tf_buffer_, &options_);
...
void MPPIController::Activate() { AINFO << "Activated MPPI Controller: ", name_.c_str(); }

void MPPIController::Deactivate() { AINFO << "Deactivated MPPI Controller: ", name_.c_str(); }

void MPPIController::Reset() { optimizer_.reset(false /*Don't reset zone-based speed limits between requests*/); }

uint32 MPPIController::ComputeVelocityCommands(const commsgs::geometry_msgs::PoseStamped& pose,
  ...
  commsgs::geometry_msgs::Pose goal = path_handler_.getTransformedGoal(pose.header.stamp).pose;

  commsgs::planning_msgs::Path transformed_plan = path_handler_.transformPath(pose);
  ...
  cmd_vel = optimizer_.evalControl(pose, robot_speed, transformed_plan, goal, goal_checker);
  ...
  if (visualize_ && trajectory_visualizer_.get()) {
    Eigen::ArrayXXf optimal_trajectory;
    Visualize(std::move(transformed_plan), cmd_vel.header.stamp, optimal_trajectory);
  }
...
void MPPIController::Visualize(commsgs::planning_msgs::Path transformed_plan,
...
void MPPIController::SetPlan(const commsgs::planning_msgs::Path& path) { path_handler_.setPath(path); }
...
// Plugins
CLASS_LOADER_REGISTER_CLASS(autonomy::control::controller::mppi_controller::MPPIController,
