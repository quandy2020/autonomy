...
void CostmapObstacleConverter::UpdateAsPoints(
    ...
    for (unsigned int my = 0; my < size_y;
         my += static_cast<unsigned int>(stride)) {
...
void CostmapObstacleConverter::UpdateAsClusters(
    ...
    for (unsigned int seed_y = 0; seed_y < size_y; ++seed_y) {
        for (unsigned int seed_x = 0; seed_x < size_x; ++seed_x) {
            if (visited[index(seed_x, seed_y)] ||
                !is_obstacle(seed_x, seed_y)) {
                continue;
            }
            ...
            const double ratio = (lambda2 > 1e-9) ? (lambda1 / lambda2) : 1e9;
            ...
            if (ratio > 4.0) {
                ...
                for (size_t i = 0; i < pts.size(); ++i) {
                    const double proj =
                        (pts[i] - Eigen::Vector2d(cx, cy)).dot(axis);
                ...
                const Eigen::Vector2d p0 =
                    Eigen::Vector2d(cx, cy) + min_proj * axis;
                const Eigen::Vector2d p1 =
                    Eigen::Vector2d(cx, cy) + max_proj * axis;
