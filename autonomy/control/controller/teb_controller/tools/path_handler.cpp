...
PathHandler::transformGlobalPlan(
    ...
    const double max_dist = options_->max_global_plan_lookahead_dist() > 0.0
    ...
    for (size_t i = closest_idx; i + 1 < global_plan_.poses.size(); ++i) {
        accumulated += euclidean_distance(global_plan_.poses[i],
                                          global_plan_.poses[i + 1]);
    ...
    if (local_frame != global_frame) {
        for (size_t i = 0; i < transformed.size(); ++i) {
            ...
            try {
                ...
                AWARN << "TEB PathHandler: local transform failed: "
                      << ex.what();
                break;
            }
        }
