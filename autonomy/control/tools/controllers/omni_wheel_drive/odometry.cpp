...
#include "autonomy/control/tools/controllers/omni_wheel_drive/odometry.hpp"
...
bool Odometry::updateFromPos(const std::vector<double> & wheels_pos, double time_sec)
{
  ...
  return updateFromVel(wheels_vel, time_sec);
}

bool Odometry::updateFromVel(const std::vector<double> & wheels_vel, double time_sec)
{
  const double dt = time_sec - timestamp_;
...
{
  Eigen::MatrixXd A(static_cast<int>(wheels_vel.size()), 3);
  Eigen::VectorXd omega(static_cast<int>(wheels_vel.size()));

  const double angle_bw_wheels = (2.0 * M_PI) / static_cast<double>(wheels_vel.size());
  ...
  return A.bdcSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(omega * wheel_radius_);
}
  ...
  double robot_radius, double wheel_radius, double wheel_offset, size_t wheel_count)
{
  ...
  wheels_old_pos_.assign(wheel_count, 0.0);
}

void Odometry::integrate(double dx, double dy, double dheading)
{
