...
ControllerServer::ControllerServer(const proto::ControllerOptions& options) : options_{options} {
  progress_checker_loader_ = std::make_shared<::autolink::class_loader::ClassLoader>("libautonomy_control_plugins.so");
...
void ControllerServer::Start() {
  ...
  for (size_t i = 0; i != progress_checker_ids_.size(); i++) {
    try {
      ...
      AINFO << "Created progress_checker: " << progress_checker_ids_[i] << " of type " << progress_checker_types_[i];
  ...
  if (goal_checkers_.find(c_name) == goal_checkers_.end()) {
    if (goal_checkers_.size() == 1 && c_name.empty()) {
      AWARN << "No goal checker was specified in parameter "
            ...
            << " Server will use only plugin loaded " << goal_checker_ids_concat_ << ". This warning will appear once.";
...
void ControllerServer::ComputeControl() {
  ...
  try {
    ...
    while (::autolink::OK()) {
      ...
      if (cycle_seconds > 0) {
        ...
        if (cycle_seconds > 1.0 / controller_frequency_) {
          AWARN << "Control loop missed its desired rate of " << controller_frequency_ << " Hz. Current loop rate is "
                << 1.0 / cycle_seconds << " Hz.";
        }
      }
    }
    ...
    OnGoalExit();
    std::shared_ptr<Action::Result> result = std::make_shared<Action::Result>();
    result->set_error_code(autonomy::tasks::behavior_tree::proto::FollowPathErrorCode::FOLLOW_PATH_ERROR_INVALID_PATH);
    if (action_server_) {
      action_server_->TerminateCurrent(result);
    }
    return;
...
void ControllerServer::ComputeAndPublishVelocity() {
  ...
  try {
    controllers_[current_controller_]->ComputeVelocityCommands(pose, twist_stamped, cmd_vel_2d,
  ...
  feedback->set_distance_to_goal(map::costmap_2d::utils::calculate_path_length(current_path_, find_closest_pose_idx()));
