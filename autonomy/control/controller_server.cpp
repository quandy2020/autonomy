...
bool ControllerServer::FindGoalCheckerId(const std::string& c_name,
    ...
    if (goal_checkers_.find(c_name) == goal_checkers_.end()) {
        if (goal_checkers_.size() == 1 && c_name.empty()) {
            LOG(WARNING) << "No goal checker was specified in parameter "
            ...
            LOG(ERROR)
                << "FollowPath called with goal_checker name " << c_name
...
bool ControllerServer::FindProgressCheckerId(
    ...
    if (progress_checkers_.find(c_name) == progress_checkers_.end()) {
        if (progress_checkers_.size() == 1 && c_name.empty()) {
            LOG(WARNING) << "No progress checker was specified in parameter "
...
void ControllerServer::ComputeControl() {
    ...
    //     if (!goal) {
    //         return;  //  goal would be nullptr if action_server_ is
    //         inactivate.
    //     }
    ...
    //     } else {
    //         throw common::ControllerException("Failed to find goal checker
    //         name: " + gc_name);
    //     }
    ...
    //     } else {
    //         throw common::ControllerException("Failed to find progress
    //         checker name: " + pc_name);
    //     }
    ...
    //             LOG(INFO) << "Cancellation was successful. Stopping the
    //             robot."; action_server_->terminate_all(); OnGoalExit();
    //             return;
    //         } else {
    //             RCLCPP_INFO_THROTTLE(
    //                 get_logger(), *get_clock(), 1000, "Waiting for the
    //                 controller to finish cancellation");
    //         }
    //     }
    //         // Don't compute a trajectory until costmap is valid (after clear
    //         costmap) rclcpp::Rate r(100); auto waiting_start = now(); while
    //         (!costmap_wrapper_->isCurrent()) {
    //             if (now() - waiting_start > costmap_update_timeout_) {
    //                 throw common::ControllerTimedOut("Costmap timed out
    //                 waiting for update");
    //             }
    //             r.sleep();
    //         }
    ...
    //             RCLCPP_WARN(
    //             get_logger(),
    //             "Control loop missed its desired rate of %.4f Hz. Current
    //             loop rate is %.4f Hz.", controller_frequency_, 1 /
    ...
    //     OnGoalExit();
    //     std::shared_ptr<Action::Result> result =
    //     std::make_shared<Action::Result>(); result->error_code =
    //     Action::Result::TF_ERROR; action_server_->terminate_current(result);
    //     return;
...
void ControllerServer::SetPlannerPath(
    ...
    DLOG(INFO) << "Path end point is (" << end_pose_.pose.position.x << ", "
...
void ControllerServer::ComputeAndPublishVelocity() {
    ...
    // common::geometry_msgs::Twist2D twist =
    ...
    //         {
    //             throw common::PatienceExceeded("Controller patience
    //             exceeded");
    //         }
    //     } else {
    ...
    // std::make_shared<Action::Feedback>(); feedback->speed =
    ...
    //     ++curr_idx) {
    //         double curr_dist =
    //         map::costmap_2d::utils::euclidean_distance(pose,
    //         current_path.poses[curr_idx]); if (curr_dist < curr_min_dist) {
    ...
    // find_closest_pose_idx()); action_server_->publish_feedback(feedback);

    // RCLCPP_DEBUG(get_logger(), "Publishing velocity at time %.2f",
    // now().seconds()); PublishVelocity(cmd_vel_2d);
}
...
void ControllerServer::UpdateGlobalPath() {
    ...
    //     } else {
    //         RCLCPP_INFO(
    //             get_logger(), "Terminating action, invalid controller %s
    //             requested.", goal->controller_id.c_str());
    ...
    //     } else {
    //         RCLCPP_INFO(
    //             get_logger(), "Terminating action, invalid goal checker %s
    //             requested.", goal->goal_checker_id.c_str());
    //         action_server_->terminate_current();
    //         return;
    //     }
    //     std::string current_progress_checker;
    //     if (FindProgressCheckerId(goal->progress_checker_id,
    //     current_progress_checker)) { if (current_progress_checker_ !=
    //     current_progress_checker) {
    //         LOG(INGO) << "Change of progress checker " << progress_checker_id
    //         << " requested, resetting it"; current_progress_checker_ =
    //         current_progress_checker;
    //         progress_checkers_[current_progress_checker_]->reset();
    //     }
    //     } else {
    //         LOG(INFO) << "Terminating action, invalid progress checker " <<
...
void ControllerServer::PublishVelocity(
    ...
    auto cmd_vel =
        std::make_unique<commsgs::geometry_msgs::TwistStamped>(velocity);
...
bool ControllerServer::IsGoalReached() {
    ...
    // commsgs::geometry_msgs::Twist2D twist =
    // GetThresholdedTwist(odom_sub_->getTwist()); commsgs::geometry_msgs::Twist
    // velocity = nav_2d_utils::twist2Dto3D(twist);

    commsgs::geometry_msgs::PoseStamped transformed_end_pose;
    // commsgs::builtin_interfaces::Duration
    // tolerance(commsgs::builtin_interfaces::Duration::from_seconds(costmap_wrapper_->getTransformTolerance()));
    // nav_2d_utils::transformPose(
    //     costmap_wrapper_->getTfBuffer(),
    //     costmap_wrapper_->getGlobalFrameID(), end_pose_,
    //     transformed_end_pose, tolerance);
