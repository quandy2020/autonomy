...
ControllerServer::ControllerServer(const proto::ControllerOptions& options)
    ...
    progress_checker_loader_ =
        std::make_shared<::autolink::class_loader::ClassLoader>(
            "libautonomy_control_plugins.so");
...
void ControllerServer::Start() {
    ...
    costmap_update_timeout_ =
        commsgs::builtin_interfaces::Duration::FromSeconds(0.3);  // 300ms
    std::string speed_limit_topic = "speed_limit";

    // 2. 初始化成本地图
    if (!costmap_wrapper_) {
        costmap_wrapper_ = std::make_shared<map::costmap_2d::Costmap2DWrapper>(
            options_.costmap_2d_options(), "local_costmap", node_.get());
    }
    ...
    for (size_t i = 0; i != progress_checker_ids_.size(); i++) {
        try {
            ...
            common::ProgressChecker::SharedPtr progress_checker =
                progress_checker_loader_
                    ->CreateClassObj<common::ProgressChecker>(
                        progress_checker_types_[i]);
            if (!progress_checker) {
                throw std::runtime_error("Failed to create progress checker: " +
                                         progress_checker_types_[i]);
            }
            AINFO << "Created progress_checker: " << progress_checker_ids_[i]
                  << " of type " << progress_checker_types_[i];
            progress_checker->Initialize(progress_checker_ids_[i]);
            progress_checkers_.insert(
                {progress_checker_ids_[i], progress_checker});
        } catch (const std::exception& ex) {
            AFATAL << "Failed to create progress_checker. Exception: "
                   << ex.what();
            throw;
        }
    }
    ...
    for (size_t i = 0; i != progress_checker_ids_.size(); i++) {
        progress_checker_ids_concat_ +=
            progress_checker_ids_[i] + std::string(" ");
    }
    AINFO << "Controller Server has " << progress_checker_ids_concat_
    ...
    for (size_t i = 0; i != goal_checker_ids_.size(); i++) {
        try {
            ...
            common::GoalChecker::SharedPtr goal_checker =
                goal_checker_loader_->CreateClassObj<common::GoalChecker>(
                    goal_checker_types_[i]);
            if (!goal_checker) {
                throw std::runtime_error("Failed to create goal checker: " +
                                         goal_checker_types_[i]);
            }
            AINFO << "Created goal checker: " << goal_checker_ids_[i]
                  << " of type " << goal_checker_types_[i];
    ...
    AINFO << "Controller Server has " << goal_checker_ids_concat_
          << "goal checkers available.";
    ...
    for (size_t i = 0; i != controller_ids_.size(); i++) {
        try {
            ...
            common::ControllerInterface::SharedPtr controller =
                controller_loader_->CreateClassObj<common::ControllerInterface>(
                    controller_types_[i]);
            if (!controller) {
                throw std::runtime_error("Failed to create controller: " +
                                         controller_types_[i]);
            }
            AINFO << "Created controller: " << controller_ids_[i] << " of type "
                  << controller_types_[i];
            // 需要 TF buffer，暂时传 nullptr，后续需要从 costmap_wrapper_ 获取
            controller->Configure(options_, controller_ids_[i], nullptr,
                                  costmap_wrapper_);
    ...
    AINFO << "Controller Server has " << controller_ids_concat_
    ...
    // 4.2 速度发布者
    vel_publisher_ =
        node_->CreateWriter<commsgs::geometry_msgs::TwistStamped>("cmd_vel");

    // 4.3 速度限制订阅者
    speed_limit_subscriber_ =
        node_->CreateReader<commsgs::planning_msgs::SpeedLimit>(
            speed_limit_topic, std::bind(&ControllerServer::SpeedLimitCallback,
    ...
    try {
        action_server_ = std::make_shared<ActionServer>(
            ...
            std::chrono::milliseconds(500), use_realtime_priority_);
    ...
    if (controllers_.find(c_name) == controllers_.end()) {
        if (controllers_.size() == 1 && c_name.empty()) {
            LOG_FIRST_N(WARNING, 1)
                ...
                << controller_ids_concat_ << ". This warning will appear once.";
        } else {
...
bool ControllerServer::FindProgressCheckerId(
    ...
    if (progress_checkers_.find(c_name) == progress_checkers_.end()) {
        if (progress_checkers_.size() == 1 && c_name.empty()) {
            ...
            AERROR
                << "FollowPath called with progress_checker name " << c_name
...
void ControllerServer::ComputeControl() {
    ...
    try {
        ...
        if (FindControllerId(c_name, current_controller)) {
            ...
            throw common::InvalidController("Failed to find controller name: " +
                                            c_name);
        }
        ...
        if (FindGoalCheckerId(gc_name, current_goal_checker)) {
            ...
            throw common::ControllerException(
                "Failed to find goal checker name: " + gc_name);
        }
        ...
        if (FindProgressCheckerId(pc_name, current_progress_checker)) {
            ...
            throw common::ControllerException(
                "Failed to find progress checker name: " + pc_name);
        }
        ...
        while (::autolink::OK()) {
            ...
            if (action_server_ == nullptr ||
                !action_server_->GetCurrentGoal()) {
                AINFO
                    << "Action server unavailable or no active goal. Stopping.";
                return;
            }
            ...
            if (action_server_->IsCancelRequested()) {
                ...
                if (controllers_[current_controller_]) {
                    // 如果控制器支持取消，等待其完成
                    // 注意：ControllerInterface 没有 Cancel()
                    // 方法，这里先直接终止
                    ...
                    result->set_error_code(
                        autonomy::tasks::behavior_tree::proto::
                            FollowPathErrorCode::FOLLOW_PATH_ERROR_NONE);
            ...
            while (!costmap_wrapper_->isCurrent()) {
                ...
                if (elapsed.Seconds() > costmap_update_timeout_.Seconds()) {
                    throw common::ControllerTimedOut(
            ...
            if (IsGoalReached()) {
                ...
                std::shared_ptr<Action::Result> result =
                    std::make_shared<Action::Result>();
                result->set_error_code(
                    autonomy::tasks::behavior_tree::proto::FollowPathErrorCode::
                        FOLLOW_PATH_ERROR_NONE);
            ...
            if (cycle_seconds > 0) {
                ...
                if (cycle_seconds > 1.0 / controller_frequency_) {
                    AWARN << "Control loop missed its desired rate of "
                          << controller_frequency_
                          << " Hz. Current loop rate is " << 1.0 / cycle_seconds
                          << " Hz.";
                }
            }
        }
        ...
        result->set_error_code(
            autonomy::tasks::behavior_tree::proto::FollowPathErrorCode::
                FOLLOW_PATH_ERROR_INVALID_CONTROLLER);
        if (action_server_) {
            action_server_->TerminateCurrent(result);
        }
        return;
        ...
        result->set_error_code(
            autonomy::tasks::behavior_tree::proto::FollowPathErrorCode::
                FOLLOW_PATH_ERROR_NO_VALID_CONTROL);
        if (action_server_) {
            action_server_->TerminateCurrent(result);
        }
        return;
        ...
        result->set_error_code(
            autonomy::tasks::behavior_tree::proto::FollowPathErrorCode::
                FOLLOW_PATH_ERROR_FAILED_TO_MAKE_PROGRESS);
        if (action_server_) {
            action_server_->TerminateCurrent(result);
        }
        return;
        ...
        result->set_error_code(
            autonomy::tasks::behavior_tree::proto::FollowPathErrorCode::
                FOLLOW_PATH_ERROR_PATIENCE_EXCEEDED);
        if (action_server_) {
            action_server_->TerminateCurrent(result);
        }
        return;
        ...
        result->set_error_code(
            autonomy::tasks::behavior_tree::proto::FollowPathErrorCode::
                FOLLOW_PATH_ERROR_INVALID_PATH);
        if (action_server_) {
            action_server_->TerminateCurrent(result);
        }
        return;
...
void ControllerServer::SetPlannerPath(
    ...
    ADEBUG << "Path end point is (" << end_pose_.pose.position.x << ", "
...
void ControllerServer::ComputeAndPublishVelocity() {
    ...
    try {
        controllers_[current_controller_]->ComputeVelocityCommands(
            pose, twist_stamped, cmd_vel_2d,
            goal_checkers_[current_goal_checker_].get(), message);
        last_valid_cmd_time_ = Time::Now();
        cmd_vel_2d.header.frame_id = costmap_wrapper_->getBaseFrameID();
        cmd_vel_2d.header.stamp = last_valid_cmd_time_;
        // Only no valid control exception types are valid to attempt to have
        // control patience, as other types will not be resolved with more
        // attempts
    } catch (common::NoValidControl& e) {
        if (failure_tolerance_ > 0 || failure_tolerance_ == -1.0) {
            ...
            if ((Time::Now() - last_valid_cmd_time_).Seconds() >
                    failure_tolerance_ &&
    ...
    auto find_closest_pose_idx = [&pose, &current_path]() {
        ...
        for (size_t curr_idx = 0; curr_idx < current_path.poses.size();
             ++curr_idx) {
            double curr_dist = map::costmap_2d::utils::euclidean_distance(
    ...
    feedback->set_distance_to_goal(
        map::costmap_2d::utils::calculate_path_length(current_path_,
                                                      find_closest_pose_idx()));
...
void ControllerServer::UpdateGlobalPath() {
    ...
    if (action_server_ && action_server_->IsPreemptRequested()) {
        ...
        if (FindControllerId(goal->controller_id(), current_controller)) {
            ...
            AINFO << "Terminating action, invalid controller "
        ...
        if (FindGoalCheckerId(goal->goal_checker_id(), current_goal_checker)) {
            ...
            AINFO << "Terminating action, invalid goal checker "
        ...
        if (FindProgressCheckerId(goal->progress_checker_id(),
            ...
            if (current_progress_checker_ != current_progress_checker) {
                AINFO << "Change of progress checker "
                      << current_progress_checker << " requested, resetting it";
                current_progress_checker_ = current_progress_checker;
                progress_checkers_[current_progress_checker_]->Reset();
            }
        } else {
            AINFO << "Terminating action, invalid progress checker "
...
bool ControllerServer::IsGoalReached() {
    ...
    commsgs::geometry_msgs::Twist2D twist_geo =
        GetThresholdedTwist(twist_geo_raw);
    ...
    commsgs::builtin_interfaces::Duration tolerance(
        commsgs::builtin_interfaces::Duration::FromSeconds(
            costmap_wrapper_->getTransformTolerance()));
    // Use costmap wrapper's transform method instead
    if (!costmap_wrapper_->transformPoseToGlobalFrame(end_pose_,
