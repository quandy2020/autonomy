  ...
  Parameters * params)
{
    ...
    footprint_collision_checker_ = std::make_unique<map::costmap_2d::
        FootprintCollisionChecker<map::costmap_2d::Costmap2D *>>(costmap_);
    footprint_collision_checker_->setCostmap(costmap_);

//   carrot_arc_pub_ = node->create_publisher<commsgs::planning_msgs::Path>("lookahead_collision_arc", 1);
