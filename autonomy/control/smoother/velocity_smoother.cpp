...
VelocitySmoother::VelocitySmoother(
    ...
    max_velocities_ = options.max_velocity_size() == 3
                          ? std::vector<double>(options.max_velocity().begin(),
                                                options.max_velocity().end())
                          : std::vector<double>{0.50, 0.0, 2.5};
    min_velocities_ = options.min_velocity_size() == 3
                          ? std::vector<double>(options.min_velocity().begin(),
                                                options.min_velocity().end())
                          : std::vector<double>{-0.50, 0.0, -2.5};
    max_accels_ = options.max_accel_size() == 3
                      ? std::vector<double>(options.max_accel().begin(),
                                            options.max_accel().end())
                      : std::vector<double>{2.5, 0.0, 3.2};
    max_decels_ = options.max_decel_size() == 3
                      ? std::vector<double>(options.max_decel().begin(),
                                            options.max_decel().end())
                      : std::vector<double>{-2.5, 0.0, -3.2};

    // Validate kinematics parameters
    for (unsigned int i = 0; i != 3; i++) {
        if (max_decels_[i] > 0.0) {
            AERROR << "Positive values set of deceleration! These should be "
                      "negative to slow down!";
            return;
        }
        if (max_accels_[i] < 0.0) {
            AERROR << "Negative values set of acceleration! These should be "
                      "positive to speed up!";
            return;
        }
        if (min_velocities_[i] > 0.0) {
            AERROR << "Positive values set of min_velocities! These should be "
                      "negative!";
            return;
        }
        if (max_velocities_[i] < 0.0) {
            AERROR << "Negative values set of max_velocities! These should be "
                      "positive!";
            return;
        }
    ...
    odom_duration_ =
        options.odom_duration() > 0.0 ? options.odom_duration() : 0.1;
    deadband_velocities_ =
        options.deadband_velocity_size() == 3
            ? std::vector<double>(options.deadband_velocity().begin(),
                                  options.deadband_velocity().end())
    ...
    // Get control type
    std::string feedback =
        options.feedback().empty() ? "OPEN_LOOP" : options.feedback();
...
double VelocitySmoother::findEtaConstraint(const double v_curr,
                                           const double v_cmd,
                                           const double accel,
                                           const double decel) {
...
double VelocitySmoother::applyConstraints(const double v_curr,
                                          const double v_cmd,
                                          const double accel,
                                          const double decel,
                                          const double eta) {
...
void VelocitySmoother::smootherTimer() {
    ...
    // dvw>, eta, to reduce all axes proportionally to follow the same
    // direction, within change of velocity bounds. In case eta reduces another
    // axis out of its own limit, apply accel constraint to guarantee output is
    // within limits, even if it deviates from requested command slightly.
    double eta = 1.0;
