...
public:
    ...
    /**
     * @brief Check whether navigate action servers are ready.
     */
    bool CheckServersReady() const;
    /**
     * @brief Wait until navigate action servers are ready.
     * @param[in] timeout Maximum wait duration.
     * @return true if servers became ready before timeout.
     */
    bool WaitForServers(
        std::chrono::milliseconds timeout = std::chrono::seconds(10));
    /**
     * @brief Check whether a navigation goal is currently active.
     */
    bool CheckNavigating() const;
...
private:
    ...
    bool StartThroughPoses(const proto::NavigationCommandRequest& request,
                           StreamCallback stream_callback);
    bool CancelActiveSession(const proto::NavigationCommandRequest& request,
                      StreamCallback stream_callback);
