...
Block::Block(const std::string& name) : name_(name), depth_(0), begin_time_(), end_time_() {}
...
std::uint64_t Block::begin_time_since_epoch() const {
  return std::chrono::duration_cast<std::chrono::nanoseconds>(begin_time_.time_since_epoch()).count();
}

std::uint64_t Block::end_time_since_epoch() const {
  return std::chrono::duration_cast<std::chrono::nanoseconds>(end_time_.time_since_epoch()).count();
}
