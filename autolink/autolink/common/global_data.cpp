...
bool GlobalData::IsChannelEnableArenaShm(uint64_t channel_id) const {
    auto channel_name = GlobalData::GetChannelById(channel_id);
...
bool GlobalData::InitConfig() {
    auto config_path = GetAbsolutePath(WorkRoot(), "conf/autolink.pb.conf");
    if (!GetProtoFromFile(config_path, &config_)) {
        AERROR << "read autolink default conf failed!";
        return false;
    }
