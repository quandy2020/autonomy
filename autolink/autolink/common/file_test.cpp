...
TEST(FileTest, proto_set_get_test) {
    ...
    autolink::proto::UnitTest read_message;
...
TEST(FileTest, Glob) {
    ...
    // Match one.
    EXPECT_THAT(Glob("/autolink"),
                testing::ElementsAre(std::string("/autolink")));
    EXPECT_THAT(Glob("/apol?o"),
                testing::ElementsAre(std::string("/autolink")));
    // Match multiple.
    EXPECT_THAT(
        Glob("/apol?o/modules/p*"),
        testing::AllOf(testing::Contains(std::string("modules/perception")),
