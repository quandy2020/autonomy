...
std::string heapprofile_filename =
    "autolink_benchmark_writer_mem.prof";  // NOLINT
...
void DisplayUsage() {
    AINFO
        ...
        << "    -O, --heapprofile_filename=filename: the filename to dump the "
           "profile to, default value is autolink_benchmark_writer_mem.prof. "
           "Only work "  // NOLINT
           "with -H option\n"
        << "Example:\n"
        << "    " << BINARY_NAME << " -h\n"
    ...
    do {
        ...
        switch (opt) {
            case 's':
                ...
                switch (arg[arg.length() - 1]) {
                    ...
                    case 'M':
                        ...
                        break;
                    default:
                        AERROR << "Invalid identifier. It should be 'K' or 'M' "
                                  "or 'B'";
                        exit(-1);
                }
                message_size =
                    std::stoi(arg.substr(0, arg.length() - 1)) * base_size;
            ...
            case 't':
                ...
                if (transport_freq < 0) {
                    AERROR << "Invalid transport frequency. It should greater "
                              "than 0";
                    exit(-1);
                }
                break;
...
int main(int argc, char** argv) {
    ...
    auto node = autolink::CreateNode(BINARY_NAME);

    autolink::proto::RoleAttributes attrs;
    attrs.set_channel_name("/autolink/benchmark");
    ...
    autolink::Rate rate_ctl(static_cast<float>(transport_freq));
    rate_init.Sleep();
