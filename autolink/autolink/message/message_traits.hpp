...
// #include "autolink/message/protobuf_traits.hpp"  // File not found, protobuf
// traits are defined inline
...
bool ParseFromArenaMessageWrapper(ArenaMessageWrapper* wrapper, T* message,
                                  T** message_ptr) {
    if (wrapper == nullptr || message == nullptr) {
        if (message_ptr != nullptr) {
            *message_ptr = nullptr;
        }
        return false;
    }
    // For protobuf messages, parse from the data in the wrapper
    // Note: The wrapper size is typically 1024 bytes as used in shm_dispatcher
    // We'll parse using GetData() with the message's byte size
    void* data = wrapper->GetData();
    if (data != nullptr && message->ParseFromArray(data, 1024)) {
        if (message_ptr != nullptr) {
            *message_ptr = message;
        }
        return true;
...
// Fallback for protobuf messages: place message into arena wrapper via
// ArenaManager
template <typename T, typename std::enable_if<
                          std::is_base_of<google::protobuf::Message, T>::value,
                          bool>::type = 0>
bool SerializeToArenaMessageWrapper(const T& message,
                                    ...
                                    T** message_ptr) {
    auto ptr = wrapper->SetMessage<T>(message);
    if (message_ptr) {
        *message_ptr = ptr;
    }
    return ptr != nullptr;
...
// Fallback for protobuf messages: leave desc empty or implement if needed.
template <typename T, typename std::enable_if<
                          std::is_base_of<google::protobuf::Message, T>::value,
                          int>::type = 0>
void GetDescriptorString(const T& /*message*/, std::string* desc_str) {
    if (desc_str) {
        desc_str->clear();
    }
}
