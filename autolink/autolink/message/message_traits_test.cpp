...
TEST(MessageTraitsTest, message_type) {
    proto::UnitTest ut;
    std::string msg_type = MessageType(ut);
    EXPECT_EQ(msg_type, "autolink.proto.UnitTest");
...
TEST(MessageTraitsTest, descriptor) {
    const std::string pb_desc =
        "\n\x81\x2\n\x1E"
        "autolink/proto/unit_test.proto\x12\xE"
        "autolink.proto\"1\n\bUnitTest\x12\x12\n\nclass_name\x18\x1 "
        "\x1(\t\x12\x11\n\tcase_name\x18\x2 "
        "\x1(\t\"S\n\aChatter\x12\x11\n\ttimestamp\x18\x1 "
        ...
        "\x1(\x4\x12\xF\n\acontent\x18\x3 \x1(\tb\x6proto3";
    std::string desc;
    proto::UnitTest ut;
    GetDescriptorString(ut, &desc);
    // GetDescriptorString for protobuf messages clears the desc string
    EXPECT_EQ("", desc);

    desc = "";
    GetDescriptorString<RawMessage>("autolink.proto.UnitTest", &desc);
    EXPECT_EQ(pb_desc, desc);

    desc = "";
    GetDescriptorString<Data>("autolink", &desc);
    EXPECT_EQ("", desc);

    desc = "";
    GetDescriptorString<Message>("autolink", &desc);
