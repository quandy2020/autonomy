...
class SendGoalRequest {
 public:
  ...
  static proto::GoalUUID GoalUUIDToProto(const GoalUUID& uuid) {
    ...
    proto_uuid.set_uuid(std::string(reinterpret_cast<const char*>(uuid.data()), uuid.size()));
    return proto_uuid;
  }
...
class SendGoalResponse {
 public:
  ...
  proto::SendGoalResponse ToProto() const {
    ...
    proto_msg.set_stamp_nanosec(static_cast<uint32_t>(stamp % 1000000000LL));
    return proto_msg;
  }

  void FromProto(const proto::SendGoalResponse& proto_msg) {
    accepted = proto_msg.accepted();
    stamp = static_cast<int64_t>(proto_msg.stamp_sec()) * 1000000000LL + proto_msg.stamp_nanosec();
  }
...
class CancelGoalRequest {
 public:
  ...
  proto::CancelGoalRequest ToProto() const {
    ...
    uuid_msg->set_uuid(std::string(reinterpret_cast<const char*>(goal_id.data()), goal_id.size()));
    return proto_msg;
  }

  void FromProto(const proto::CancelGoalRequest& proto_msg) {
    if (proto_msg.has_goal_id() && proto_msg.goal_id().uuid().size() == 16) {
...
class CancelGoalResponse {
 public:
  ...
  proto::CancelGoalResponse ToProto() const {
    ...
    proto_msg.set_return_code(goals_canceling > 0 ? 0 : 1);  // 0 = ERROR_NONE
  ...
  void FromProto(const proto::CancelGoalResponse& proto_msg) {
    goals_canceling = proto_msg.return_code() == 0 ? proto_msg.goals_canceling_size() : 0;
  }
...
class FeedbackMessage {
 public:
  ...
  static std::string TypeName() { return proto::FeedbackMessage::descriptor()->full_name(); }
  ...
  static void GetDescriptorString(const std::string& type, std::string* desc_str) {
    ...
    message::ProtobufFactory::Instance()->GetDescriptorString(proto::FeedbackMessage::descriptor()->full_name(),
                                                              desc_str);
  }
};
