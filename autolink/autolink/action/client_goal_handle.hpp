...
class ClientGoalHandle {
 public:
  ...
  using FeedbackCallback =
      std::function<void(std::shared_ptr<ClientGoalHandle<ActionT>>, std::shared_ptr<const Feedback>)>;
   ...
   */
  const GoalUUID& GetGoalId() const { return goal_info_.goal_id; }
   ...
   */
  int64_t GetGoalStamp() const { return goal_info_.stamp; }
   ...
   */
  ClientGoalHandle(const GoalInfo& goal_info, FeedbackCallback feedback_callback, ResultCallback result_callback);
 ...
 private:
  ...
  void CallFeedbackCallback(std::shared_ptr<ClientGoalHandle<ActionT>> shared_this,
...
template <typename ActionT>
ClientGoalHandle<ActionT>::ClientGoalHandle(const GoalInfo& goal_info, FeedbackCallback feedback_callback,
...
template <typename ActionT>
std::shared_future<typename ClientGoalHandle<ActionT>::WrappedResult> ClientGoalHandle<ActionT>::AsyncGetResult() {
  std::lock_guard<std::recursive_mutex> lock(handle_mutex_);
...
template <typename ActionT>
void ClientGoalHandle<ActionT>::CallFeedbackCallback(std::shared_ptr<ClientGoalHandle<ActionT>> shared_this,
                                                     std::shared_ptr<const Feedback> feedback_message) {
