...
class Client {
 public:
   ...
   */
  explicit Client(std::shared_ptr<Node> node, const std::string& action_name) : node_(node), action_name_(action_name) {
    Init();
  }
   ...
   */
  std::shared_future<std::shared_ptr<GoalHandle>> AsyncSendGoal(const Goal& goal,
   ...
   */
  std::shared_future<WrappedResult> AsyncGetResult(std::shared_ptr<GoalHandle> goal_handle,
   ...
   * @throws UnknownGoalHandleError If the goal is unknown.
   */
  std::shared_future<bool> AsyncCancelGoal(std::shared_ptr<GoalHandle> goal_handle);
 ...
 private:
  ...
  void HandleFeedback(const std::shared_ptr<internal::FeedbackMessage<ActionT>>& feedback_msg);
  void HandleStatus(const std::shared_ptr<internal::StatusMessage>& status_msg);

  std::shared_ptr<Node> node_;
  std::string action_name_;

  // Services
  std::shared_ptr<autolink::Client<internal::SendGoalRequest<ActionT>, internal::SendGoalResponse>> send_goal_client_;
  std::shared_ptr<autolink::Client<internal::CancelGoalRequest, internal::CancelGoalResponse>> cancel_goal_client_;
  std::shared_ptr<autolink::Client<internal::GetResultRequest, internal::GetResultResponse<ActionT>>>
      get_result_client_;

  // Subscribers
  std::shared_ptr<Reader<internal::FeedbackMessage<ActionT>>> feedback_reader_;
...
void Client<ActionT>::Init() {
  ...
  send_goal_client_ =
      node_->CreateClient<internal::SendGoalRequest<ActionT>, internal::SendGoalResponse>(action_name_ + "/send_goal");

  // Create CancelGoal client
  cancel_goal_client_ =
      node_->CreateClient<internal::CancelGoalRequest, internal::CancelGoalResponse>(action_name_ + "/cancel_goal");

  // Create GetResult client
  get_result_client_ = node_->CreateClient<internal::GetResultRequest, internal::GetResultResponse<ActionT>>(
      action_name_ + "/get_result");

  // Create feedback reader
  // Use std::bind to avoid lambda type issues in C++11
  auto feedback_callback = std::bind(&Client<ActionT>::HandleFeedback, this, std::placeholders::_1);
  std::string feedback_channel = action_name_ + "/feedback";
  feedback_reader_ = node_->CreateReader<internal::FeedbackMessage<ActionT>>(feedback_channel, feedback_callback);
  if (!feedback_reader_) {
    AERROR << "Client::Init: Failed to create feedback reader";
  }

  // Create status reader
  auto status_callback = std::bind(&Client<ActionT>::HandleStatus, this, std::placeholders::_1);
  status_reader_ = node_->CreateReader<internal::StatusMessage>(action_name_ + "/status", status_callback);
}

template <typename ActionT>
std::shared_future<std::shared_ptr<typename Client<ActionT>::GoalHandle>> Client<ActionT>::AsyncSendGoal(
    const Goal& goal, const SendGoalOptions& options) {
  auto promise = std::make_shared<std::promise<std::shared_ptr<GoalHandle>>>();
  std::shared_future<std::shared_ptr<GoalHandle>> future(promise->get_future());
  ...
  // Send request
  AINFO << "AsyncSendGoal: sending goal request for goal ID: " << ToString(goal_id);
  ...
  std::thread([this, promise, goal_id, options, request, response_future]() {
    try {
      AINFO << "AsyncSendGoal: waiting for response for goal ID: " << ToString(goal_id);
      ...
      AINFO << "AsyncSendGoal: received response for goal ID: " << ToString(goal_id);

      if (!response) {
        AERROR << "AsyncSendGoal: received null response for goal ID: " << ToString(goal_id);
        promise->set_value(nullptr);
        if (options.goal_response_callback) {
          options.goal_response_callback(nullptr);
        }
      ...
      if (!response->accepted) {
        AWARN << "AsyncSendGoal: goal rejected for goal ID: " << ToString(goal_id);
        promise->set_value(nullptr);
        if (options.goal_response_callback) {
          options.goal_response_callback(nullptr);
        }
      ...
      ADEBUG << "AsyncSendGoal: goal accepted for goal ID: " << ToString(goal_id);
      ...
      auto goal_handle = std::make_shared<GoalHandle>(goal_info, options.feedback_callback, options.result_callback);
      ...
      ADEBUG << "AsyncSendGoal: created goal handle for goal ID: " << ToString(goal_id);
      ...
      if (options.result_callback) {
        // Make result aware
        ADEBUG << "AsyncSendGoal: calling AsyncGetResult for goal ID: " << ToString(goal_id);
        this->AsyncGetResult(goal_handle, options.result_callback);
      }
    } catch (const std::exception& e) {
      AERROR << "Error in AsyncSendGoal response for goal ID: " << ToString(goal_id) << ": " << e.what();
      promise->set_value(nullptr);
      if (options.goal_response_callback) {
        options.goal_response_callback(nullptr);
      }
...
template <typename ActionT>
std::shared_future<typename Client<ActionT>::WrappedResult> Client<ActionT>::AsyncGetResult(
    std::shared_ptr<GoalHandle> goal_handle, typename GoalHandle::ResultCallback result_callback) {
  ...
  std::thread([this, goal_handle, response_future]() {
    try {
      auto response = response_future.get();
      if (!response) {
        AWARN << "AsyncGetResult: received null response for goal ID: " << ToString(goal_handle->GetGoalId());
        return;
      }

      GoalStatus response_status = static_cast<GoalStatus>(response->status);
      ADEBUG << "AsyncGetResult: received response for goal ID: " << ToString(goal_handle->GetGoalId())
      ...
      if (response_status == GoalStatus::SUCCEEDED || response_status == GoalStatus::CANCELED ||
        ...
        wrapped_result.result = std::make_shared<Result>(response->result);

        ADEBUG << "AsyncGetResult: setting result for goal ID: " << ToString(goal_handle->GetGoalId())
               << " with code: " << static_cast<int>(wrapped_result.code);
...
template <typename ActionT>
std::shared_future<bool> Client<ActionT>::AsyncCancelGoal(std::shared_ptr<GoalHandle> goal_handle) {
  std::lock_guard<std::recursive_mutex> lock(goal_handles_mutex_);
  if (goal_handles_.count(goal_handle->GetGoalId()) == 0) {
    throw UnknownGoalHandleError();
  }
...
bool Client<ActionT>::ActionServerIsReady() const {
  bool ready = send_goal_client_ && send_goal_client_->ServiceIsReady() && cancel_goal_client_ &&
               cancel_goal_client_->ServiceIsReady() && get_result_client_ && get_result_client_->ServiceIsReady();
  ADEBUG << "ActionServerIsReady: send_goal=" << (send_goal_client_ && send_goal_client_->ServiceIsReady())
         << ", cancel_goal=" << (cancel_goal_client_ && cancel_goal_client_->ServiceIsReady())
         << ", get_result=" << (get_result_client_ && get_result_client_->ServiceIsReady()) << ", overall=" << ready;
  return ready;
}

template <typename ActionT>
void Client<ActionT>::HandleFeedback(const std::shared_ptr<internal::FeedbackMessage<ActionT>>& feedback_msg) {
  if (!feedback_msg) {
  ...
  ADEBUG << "HandleFeedback: received feedback message for goal ID: " << ToString(goal_id);
  if (goal_handles_.count(goal_id) == 0) {
    AWARN << "HandleFeedback: received feedback for unknown goal ID: " << ToString(goal_id) << ". Ignoring...";
    return;
  }

  auto goal_handle = goal_handles_[goal_id].lock();
  if (!goal_handle) {
    ADEBUG << "HandleFeedback: goal handle expired for goal ID: " << ToString(goal_id) << ". Removing from map.";
    goal_handles_.erase(goal_id);
    return;
  }

  auto feedback = std::make_shared<Feedback>(feedback_msg->feedback);
  ADEBUG << "HandleFeedback: calling feedback callback for goal ID: " << ToString(goal_id) << ", progress: N/A";
  goal_handle->CallFeedbackCallback(goal_handle, feedback);
}

template <typename ActionT>
void Client<ActionT>::HandleStatus(const std::shared_ptr<internal::StatusMessage>& status_msg) {
  if (!status_msg) {
    return;
  }
