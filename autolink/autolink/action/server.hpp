...
class Server {
 public:
  ...
  /// goal.
  using CancelCallback = std::function<CancelResponse(std::shared_ptr<GoalHandle>)>;
   ...
   */
  Server(std::shared_ptr<Node> node, const std::string& action_name, GoalCallback handle_goal,
 ...
 private:
  ...
  // Service callbacks
  void HandleSendGoal(const std::shared_ptr<internal::SendGoalRequest<ActionT>>& request,
                      std::shared_ptr<internal::SendGoalResponse>& response);

  void HandleCancelGoal(const std::shared_ptr<internal::CancelGoalRequest>& request,
                        std::shared_ptr<internal::CancelGoalResponse>& response);

  void HandleGetResult(const std::shared_ptr<internal::GetResultRequest>& request,
  ...
  void PublishFeedback(const GoalUUID& uuid, std::shared_ptr<Feedback> feedback);
  ...
  // Services
  std::shared_ptr<Service<internal::SendGoalRequest<ActionT>, internal::SendGoalResponse>> send_goal_service_;
  std::shared_ptr<Service<internal::CancelGoalRequest, internal::CancelGoalResponse>> cancel_goal_service_;
  std::shared_ptr<Service<internal::GetResultRequest, internal::GetResultResponse<ActionT>>> get_result_service_;

  // Publishers
  std::shared_ptr<Writer<internal::FeedbackMessage<ActionT>>> feedback_writer_;
...
void Server<ActionT>::Init() {
  ...
  send_goal_service_ = node_->CreateService<internal::SendGoalRequest<ActionT>, internal::SendGoalResponse>(
      action_name_ + "/send_goal",
      [this](const std::shared_ptr<internal::SendGoalRequest<ActionT>>& req,
             std::shared_ptr<internal::SendGoalResponse>& res) { this->HandleSendGoal(req, res); });
  ...
  cancel_goal_service_ = node_->CreateService<internal::CancelGoalRequest, internal::CancelGoalResponse>(
      action_name_ + "/cancel_goal",
      [this](const std::shared_ptr<internal::CancelGoalRequest>& req,
             std::shared_ptr<internal::CancelGoalResponse>& res) { this->HandleCancelGoal(req, res); });
  ...
  get_result_service_ = node_->CreateService<internal::GetResultRequest, internal::GetResultResponse<ActionT>>(
      action_name_ + "/get_result",
      [this](const std::shared_ptr<internal::GetResultRequest>& req,
             std::shared_ptr<internal::GetResultResponse<ActionT>>& res) { this->HandleGetResult(req, res); });
  ...
  AINFO << "Server::Init: Creating feedback writer for channel: " << feedback_channel;
  feedback_writer_ = node_->CreateWriter<internal::FeedbackMessage<ActionT>>(feedback_channel);
  if (feedback_writer_) {
  ...
  status_writer_ = node_->CreateWriter<internal::StatusMessage>(action_name_ + "/status");
  AINFO << "Server::Init: status writer created";
  AINFO << "Server::Init: all services and writers initialized successfully";
}

template <typename ActionT>
void Server<ActionT>::HandleSendGoal(const std::shared_ptr<internal::SendGoalRequest<ActionT>>& request,
                                     std::shared_ptr<internal::SendGoalResponse>& response) {
  ...
  AINFO << "Server::HandleSendGoal: received goal request for goal ID: " << ToString(goal_id);
  ...
  ADEBUG << "HandleSendGoal: goal callback returned: " << static_cast<int>(goal_response);

  response->accepted =
      (goal_response == GoalResponse::ACCEPT_AND_EXECUTE || goal_response == GoalResponse::ACCEPT_AND_DEFER);
  response->stamp = Time::Now().ToNanosecond();

  ADEBUG << "HandleSendGoal: response accepted=" << response->accepted << " for goal ID: " << ToString(goal_id);

  if (response->accepted) {
    // Create goal handle
    // Use std::bind to avoid lambda type issues in C++11
    auto on_terminal = std::bind(&Server<ActionT>::OnTerminalState, this, std::placeholders::_1, std::placeholders::_2);
    auto on_exec = std::bind(&Server<ActionT>::OnExecuting, this, std::placeholders::_1);
    auto pub_feedback = std::bind(&Server<ActionT>::PublishFeedback, this, goal_id, std::placeholders::_1);

    auto goal_handle = std::make_shared<GoalHandle>(goal_id, goal, on_terminal, on_exec, pub_feedback);
...
template <typename ActionT>
void Server<ActionT>::HandleCancelGoal(const std::shared_ptr<internal::CancelGoalRequest>& request,
                                       std::shared_ptr<internal::CancelGoalResponse>& response) {
  if (!request || !response) {
...
template <typename ActionT>
void Server<ActionT>::HandleGetResult(const std::shared_ptr<internal::GetResultRequest>& request,
                                      std::shared_ptr<internal::GetResultResponse<ActionT>>& response) {
  if (!request || !response) {
  ...
  // First check if result is already stored (goal completed)
  {
    ...
    if (results_.count(goal_id) > 0) {
      ...
      ADEBUG << "HandleGetResult: found result in storage for goal ID: " << ToString(goal_id)
  ...
  if (goal_handle) {
    ...
    if (status == GoalStatus::SUCCEEDED || status == GoalStatus::CANCELED || status == GoalStatus::ABORTED) {
    ...
    AWARN << "HandleGetResult: goal handle not found for goal ID: " << ToString(goal_id);
    response->status = static_cast<int8_t>(GoalStatus::UNKNOWN);
  }
}

template <typename ActionT>
void Server<ActionT>::OnTerminalState(const GoalUUID& uuid, std::shared_ptr<Result> result) {
  ...
  // Store result with status
  {
    ...
    ADEBUG << "OnTerminalState: stored result for goal ID: " << ToString(uuid)
...
template <typename ActionT>
void Server<ActionT>::PublishFeedback(const GoalUUID& uuid, std::shared_ptr<Feedback> feedback) {
  if (!feedback_writer_ || !feedback) {
    AWARN << "PublishFeedback: feedback_writer_ is null or feedback is null";
    return;
  }
  ...
  ADEBUG << "PublishFeedback: attempting to publish feedback for goal ID: " << ToString(uuid)
         << ", HasReader()=" << has_reader;
  ...
  if (!success) {
    AWARN << "PublishFeedback: failed to write feedback message for goal ID: " << ToString(uuid)
          << ", HasReader()=" << has_reader;
  } else {
    ADEBUG << "PublishFeedback: successfully published feedback for goal ID: " << ToString(uuid) << ", progress: N/A";
  }
}
...
template <typename ActionT>
std::shared_ptr<typename Server<ActionT>::GoalHandle> Server<ActionT>::GetGoalHandle(const GoalUUID& uuid) {
  std::lock_guard<std::mutex> lock(goal_handles_mutex_);
  auto it = goal_handles_.find(uuid);
  if (it != goal_handles_.end()) {
