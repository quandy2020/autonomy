...
class ServerGoalHandle {
 public:
   ...
   */
  const std::shared_ptr<const Goal> GetGoal() const { return goal_; }
   ...
   */
  const GoalUUID& GetGoalId() const { return uuid_; }
   ...
   */
  ServerGoalHandle(const GoalUUID& uuid, std::shared_ptr<const Goal> goal,
                   std::function<void(const GoalUUID&, std::shared_ptr<Result>)> on_terminal_state,
 ...
 private:
  ...
  std::function<void(const GoalUUID&, std::shared_ptr<Result>)> on_terminal_state_;
...
ServerGoalHandle<ActionT>::ServerGoalHandle(
    ...
    std::function<void(const GoalUUID&, std::shared_ptr<typename ActionT::Result>)> on_terminal_state,
    std::function<void(const GoalUUID&)> on_executing,
...
ServerGoalHandle<ActionT>::~ServerGoalHandle() {
  ...
  if (status_ != GoalStatus::SUCCEEDED && status_ != GoalStatus::CANCELED && status_ != GoalStatus::ABORTED) {
...
bool ServerGoalHandle<ActionT>::IsActive() const {
  std::lock_guard<std::mutex> lock(status_mutex_);
  return status_ == GoalStatus::ACCEPTED || status_ == GoalStatus::EXECUTING || status_ == GoalStatus::CANCELING;
}
...
template <typename ActionT>
void ServerGoalHandle<ActionT>::PublishFeedback(std::shared_ptr<typename ActionT::Feedback> feedback_msg) {
  if (!feedback_msg) {
  ...
  if (!IsExecuting()) {
    ...
    AWARN << "PublishFeedback: goal is not executing. Current status: " << static_cast<int>(current_status)
  ...
  bool should_call_callback = false;
  {
    ...
    if (status_ == GoalStatus::ACCEPTED) {
      ...
      AWARN << "ServerGoalHandle::Execute: status is not ACCEPTED, "
               "current status="
            << static_cast<int>(status_) << " for goal ID: " << ToString(uuid_);
    }
  }
...
template <typename ActionT>
void ServerGoalHandle<ActionT>::Abort(std::shared_ptr<typename ActionT::Result> result_msg) {
  {
    std::lock_guard<std::mutex> lock(status_mutex_);
    if (status_ != GoalStatus::EXECUTING && status_ != GoalStatus::CANCELING) {
      return;
    }
...
template <typename ActionT>
void ServerGoalHandle<ActionT>::Succeed(std::shared_ptr<typename ActionT::Result> result_msg) {
  {
    std::lock_guard<std::mutex> lock(status_mutex_);
...
template <typename ActionT>
void ServerGoalHandle<ActionT>::Canceled(std::shared_ptr<typename ActionT::Result> result_msg) {
  {
    std::lock_guard<std::mutex> lock(status_mutex_);
