...
const std::size_t MessageInfo::kSize =
    2 * ID_SIZE + sizeof(uint64_t) + sizeof(uint64_t) + sizeof(int32_t) + sizeof(uint64_t);
...
bool MessageInfo::operator==(const MessageInfo& another) const {
  return sender_id_ == another.sender_id_ && channel_id_ == another.channel_id_ && seq_num_ == another.seq_num_ &&
         spare_id_ == another.spare_id_;
}
...
bool MessageInfo::SerializeTo(std::string* dst) const {
  ...
  dst->append(reinterpret_cast<const char*>(&channel_id_), sizeof(channel_id_));
  dst->append(reinterpret_cast<const char*>(&seq_num_), sizeof(seq_num_));
  dst->append(spare_id_.data(), ID_SIZE);
  dst->append(reinterpret_cast<const char*>(&msg_seq_num_), sizeof(msg_seq_num_));
...
bool MessageInfo::SerializeTo(char* dst, std::size_t len) const {
  ...
  ptr += ID_SIZE;
  std::memcpy(ptr, reinterpret_cast<const char*>(&channel_id_), sizeof(channel_id_));
  ...
  ptr += ID_SIZE;
  std::memcpy(ptr, reinterpret_cast<const char*>(&msg_seq_num_), sizeof(msg_seq_num_));
  ptr += sizeof(msg_seq_num_);
  std::memcpy(ptr, reinterpret_cast<const char*>(&send_time_), sizeof(send_time_));
  return true;
}

bool MessageInfo::DeserializeFrom(const std::string& src) { return DeserializeFrom(src.data(), src.size()); }
...
bool MessageInfo::DeserializeFrom(const char* src, std::size_t len) {
  ...
  ptr += ID_SIZE;
  std::memcpy(reinterpret_cast<char*>(&channel_id_), ptr, sizeof(channel_id_));
  ptr += sizeof(channel_id_);
  std::memcpy(reinterpret_cast<char*>(&seq_num_), ptr, sizeof(seq_num_));
  ptr += sizeof(seq_num_);
  spare_id_.set_data(ptr);
  ptr += ID_SIZE;
  std::memcpy(reinterpret_cast<char*>(&msg_seq_num_), ptr, sizeof(msg_seq_num_));
  ptr += sizeof(msg_seq_num_);
