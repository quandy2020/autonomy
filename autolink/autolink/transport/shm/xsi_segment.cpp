...
bool XsiSegment::OpenOrCreate() {
    ...
    while (retry < 2) {
        shmid =
            shmget(key_, conf_.managed_shm_size(), 0644 | IPC_CREAT | IPC_EXCL);
        if (shmid != -1) {
            break;
        }
    ...
    arena_blocks_ =
        new (static_cast<char*>(managed_shm_) + sizeof(State) +
             conf_.block_num() * sizeof(Block)) Block[ShmConf::ARENA_BLOCK_NUM];
    ...
    for (; ai < ShmConf::ARENA_BLOCK_NUM; ++ai) {
        uint8_t* addr = new (static_cast<char*>(managed_shm_) + sizeof(State) +
            ...
            uint8_t[ShmConf::ARENA_MESSAGE_SIZE];
...
bool XsiSegment::OpenOnly() {
    ...
    arena_blocks_ = reinterpret_cast<Block*>(static_cast<char*>(managed_shm_) +
                                             sizeof(State) +
