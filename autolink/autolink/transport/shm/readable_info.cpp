...
const size_t ReadableInfo::kSize =
    sizeof(uint64_t) * 2 + sizeof(int32_t) + sizeof(int32_t);

ReadableInfo::ReadableInfo()
    : host_id_(0), block_index_(0), arena_block_index_(-1), channel_id_(0) {}
...
bool ReadableInfo::SerializeTo(std::string* dst) const {
    ...
    dst->append(reinterpret_cast<char*>(const_cast<uint64_t*>(&channel_id_)),
                sizeof(channel_id_));
    dst->append(
        reinterpret_cast<char*>(const_cast<int32_t*>(&arena_block_index_)),
        sizeof(arena_block_index_));
    return true;
}
...
bool ReadableInfo::DeserializeFrom(const char* src, std::size_t len) {
    ...
    memcpy(reinterpret_cast<char*>(&arena_block_index_), ptr,
           sizeof(arena_block_index_));
    return true;
}
