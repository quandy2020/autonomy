...
bool ArenaSegment::Init(uint64_t message_size, uint64_t block_num) {
    ...
    for (uint32_t retry = 0;
         retry < 2 && !OpenOrCreate(message_size, block_num); ++retry) {
    }
...
bool ArenaSegment::OpenOrCreate(uint64_t message_size, uint64_t block_num) {
    auto arena_conf =
        autolink::common::GlobalData::Instance()->GetChannelArenaConf(
            channel_id_);
    ...
    if (shared_buffer_size == 0) {
        ...
        options.initial_block = reinterpret_cast<char*>(
            reinterpret_cast<uint64_t>(shm_address_) +
            sizeof(ArenaSegmentState) + block_num * sizeof(ArenaSegmentBlock) +
            block_num * message_size);
...
void* ArenaSegment::GetShmAddress() {
    return shm_address_;
}
...
uint64_t ArenaSegment::GetNextWritableBlockIndex() {
    ...
    while (1) {
        uint64_t next_idx =
            state_->struct_.message_seq_.fetch_add(1) % block_num;
...
bool ProtobufArenaManager::EnableSegment(uint64_t channel_id) {
    ...
    auto autolink_config = autolink::common::GlobalData::Instance()->Config();
    if (!autolink_config.has_transport_conf()) {
        return false;
    }
    if (!autolink_config.transport_conf().has_shm_conf()) {
        return false;
    }
    if (!autolink_config.transport_conf().shm_conf().has_arena_shm_conf()) {
        return false;
    }
