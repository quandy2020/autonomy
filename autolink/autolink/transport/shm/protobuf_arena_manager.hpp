...
public:
    ...
    uint64_t GetBaseAddress(
        const message::ArenaMessageWrapper* wrapper) override;
    ...
    void* GetAvailableBuffer(uint64_t channel_id) {
        ...
        if (!segment) {
            if (non_arena_buffers_.find(channel_id) ==
                non_arena_buffers_.end()) {
                return nullptr;
            }
    ...
    void AcquireArenaMessage(uint64_t channel_id, std::shared_ptr<M>& ret_msg) {
        ...
        ret_msg = std::shared_ptr<M>(
            ...
            [segment, wb](M* ptr) {
                int32_t lock_num =
                    segment->blocks_[wb.block_index_].lock_num_.load();
