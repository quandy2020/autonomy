...
ArenaAddressAllocator::ArenaAddressAllocator()
    ...
    // TODO(all): configurable and address management for shared memory of
    // autolink
    ...
    Init(1024, reinterpret_cast<void*>(0x710000000000),
         2ULL * 1024 * 1024 * 1024);
}
...
bool ArenaAddressAllocator::OpenNodeShm(uint64_t capacity, void* base_address,
    ...
    reclaim_stack_ =
        reinterpret_cast<uint64_t*>(reinterpret_cast<uint64_t>(nodes_) +
                                    sizeof(ArenaAddressNode) * capacity);
    return true;
}
...
ArenaAddressNode* ArenaAddressAllocator::FindOrInsertNode(
    ...
    if (key < node->key_) {
        ...
        auto target =
            FindOrInsertNode(node->right_, &(node->right_), node, key);
...
void* ArenaAddressAllocator::Allocate(uint64_t key) {
    ...
    while (!meta_->struct_.occupied_.compare_exchange_strong(idle_status,
                                                             occupied_status)) {
