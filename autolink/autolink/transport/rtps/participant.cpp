...
bool Participant::CreateParticipant(
    ...
    AINFO << "part name: " << participant_qos.name()
          << ", port: " << send_port_;
...
bool Participant::CheckIPVaild(std::string ip_env) {
    ...
    for (std::string ip_interface : ip_vec) {
        if (ip_interface == ip_env) {
            AINFO << "Find same the ip interface in host as autolink ip: "
                  << ip_env;
            return true;
        }
    }
    AERROR << "The same ip interface in host as autolink ip was not found: "
           << ip_env;
    return false;
}
