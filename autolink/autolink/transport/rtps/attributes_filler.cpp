...
bool AttributesFiller::FillInPubAttr(
  ...
  // set default heartbeat period (Fast-DDS 2.x: Time_t uses seconds()/fraction() getters; assign from constructor)
  pub_attr->times.heartbeatPeriod = eprosima::fastrtps::Time_t(1, 0);
  if (qos.mps() != 0) {
    ...
    uint32_t frac = static_cast<uint32_t>(fractions & 0xffffffff);
    int32_t sec = static_cast<int32_t>(fractions >> 32);
