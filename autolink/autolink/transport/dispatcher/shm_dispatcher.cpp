...
ShmDispatcher::ShmDispatcher() : host_id_(0) {
    Init();
}
    ...
    while (!is_shutdown_.load()) {
...

        {
            ...
            if (arena_block_index != -1) {
                ...
                uint32_t& arena_previous_index =
                    arena_previous_indexes_[channel_id];
                if (arena_block_index != 0 &&
                    arena_previous_index != UINT32_MAX) {
