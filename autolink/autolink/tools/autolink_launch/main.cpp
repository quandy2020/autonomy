...
void LogError(const std::string& msg) {
  std::cerr << "[autolink_launch_" << getpid() << "] ERROR " << msg << std::endl;
}
...
int StartProcess(ProcessEntry& pe) {
  ...
  if (pe.process_type == "binary") {
    ...
    while (pos < name.size()) {
      ...
      if (end > pos) args.push_back(name.substr(pos, end - pos));
      pos = end;
    ...
    for (const auto& a : pe.extra_args_list) args.push_back(a);
  ...
  for (auto& s : pe.args) argv_ptrs.push_back(&s[0]);
  ...
  pe.started = true;
  pe.time_of_death = 0;
  LogInfo("Start process [" + pe.name + "] successfully. pid: " + std::to_string(pid));
  return pid;
}

bool IsAlive(ProcessEntry& pe) {
  if (!pe.started || pe.pid <= 0) return false;
  int status = 0;
  ...
  for (auto& pe : g_procs) {
    if (pe.started && pe.pid > 0) {
      for (int i = 0; i < force_stop_timeout_secs * 10; ++i) {
        if (waitpid(pe.pid, nullptr, WNOHANG) == pe.pid) break;
        usleep(100000);
      }
      if (waitpid(pe.pid, nullptr, WNOHANG) != pe.pid) {
        LogWarn("Begin force kill process [" + pe.name + "][" + std::to_string(pe.pid) + "].");
        kill(pe.pid, SIGKILL);
...
void CheckCleanup(int* dead_count, bool* all_died) {
  ...
  *all_died = false;
  for (auto& pe : g_procs) {
    if (g_shutdown) break;
    if (IsAlive(pe)) continue;
    if (!pe.started) continue;

    if (pe.exception_handler == "respawn" && pe.respawn_cnt < pe.respawn_limit) {
      LogWarn("child process [" + pe.name + "][" + std::to_string(pe.pid) + "] exit, respawn!");
      pe.started = false;
      pe.pid = -1;
      ...
      LogWarn("child process [" + pe.name + "][" + std::to_string(pe.pid) + "] exit, stop all");
      g_shutdown = 1;
      return;
    } else {
      (*dead_count)++;
...
bool RunMonitor() {
  ...
  for (auto& pe : g_procs) GetExitState(pe);
...
int Start(const std::string& launch_file) {
  ...
  if (path.empty()) path = "autolink.launch";
  std::string launch_path = GetLaunchPath();
  if (path[0] != '/') {
    std::string full = launch_path + "/" + path;
    if (access(full.c_str(), R_OK) == 0) path = full;
  }
  ...
  for (const tinyxml2::XMLElement* module = root->FirstChildElement("module"); module;
    ...
    std::string exception_handler = GetParamValue(module, "exception_handler");
    ...
    if (process_type == "binary") {
      ...
      pe.binary_path = process_name;  // first token used by StartProcess for binary
      if (StartProcess(pe) <= 0) {
        LogError("Start manager [" + process_name + "] failed. Stop all!");
        StopAll(SIGINT);
        return 2;
      }
    } else {
      std::vector<std::string> dag_list = GetParamList(module, "dag_conf");
      if (dag_list.empty()) {
        LogError("module [" + module_name + "] library dag conf is null.");
        continue;
      }
      ...
      if (extra) {
        ...
        size_t pos = 0;
        while (pos < ex.size()) {
          while (pos < ex.size() && (ex[pos] == ' ' || ex[pos] == '\t')) pos++;
          if (pos >= ex.size()) break;
          size_t end = ex.find_first_of(" \t", pos);
          if (end == std::string::npos) end = ex.size();
...
void PrintUsage() {
  std::cout << "usage: autolink_launch [-h] {start,stop} ...\n"
            << "autolink_launch is a launcher for autolink modules.\n\n"
            << "  start    start modules from a launch file (default: autolink.launch)\n"
            << "  stop     stop all launchers or those matching a launch file\n\n"
            << "  start [file]   launch file (under AUTOLINK_LAUNCH_PATH or /autolink)\n"
            << "  stop [file]   if given, stop only processes matching this launch file\n";
}
