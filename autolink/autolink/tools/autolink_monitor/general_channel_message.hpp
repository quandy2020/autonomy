...
private:
    explicit GeneralChannelMessage(const std::string& node_name,
        ...
        : GeneralMessageBase(parent),
          ...
          message_type_(),
          frame_counter_(0),
          last_time_(autolink::Time::MonoTime()),
    ...
    static void DoDelete(std::vector<std::string>* vec,
    ...
    std::shared_ptr<autolink::message::RawMessage> CopyMsgPtr(void) const {
    ...
    autolink::Time last_time_;
    autolink::Time msg_time_;
    autolink::Time time_last_calc_ = autolink::Time::MonoTime();

    std::unique_ptr<autolink::Node> channel_node_;
    ...
    friend class AutolinkTopologyMessage;
