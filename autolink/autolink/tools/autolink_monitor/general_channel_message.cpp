...
using autolink::record::kGB;
using autolink::record::kKB;
...
double GeneralChannelMessage::frame_ratio(void) {
    ...
    auto time_now = autolink::Time::MonoTime();
...
GeneralChannelMessage* GeneralChannelMessage::OpenChannel(
    ...
    channel_node_ = autolink::CreateNode(node_name_);
    ...
    channel_reader_ =
        channel_node_->CreateReader<autolink::message::RawMessage>(channel_name,
                                                                   callback);
