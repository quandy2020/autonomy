...
#include "autolink/tools/autolink_monitor/general_message.hpp"
...
GeneralMessage::GeneralMessage(GeneralMessageBase* parent, const google::protobuf::Message* msg,
...
int GeneralMessage::Render(const Screen* s, int key) {
  ...
  int line_no = 0;
  {
    ...
    out_str << std::fixed << std::setprecision(FrameRatio_Precision) << channel_msg_ptr->frame_ratio();
    ...
    if (message_ptr_ && reflection_ptr_) {
      ...
      if (is_folded_) {
        GeneralMessageBase::PrintField(this, *message_ptr_, &jump_lines, s, &line_no, 0, reflection_ptr_, field_,
                                       indices[item_index_]);
      } else {
