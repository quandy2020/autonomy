...
Recorder::Recorder(const std::string& output, bool all_channels, const std::vector<std::string>& white_channels,
                   const std::vector<std::string>& black_channels)
    : output_(output), all_channels_(all_channels), white_channels_(white_channels), black_channels_(black_channels) {
  header_ = HeaderBuilder::GetHeader();
}

Recorder::Recorder(const std::string& output, bool all_channels, const std::vector<std::string>& white_channels,
                   const std::vector<std::string>& black_channels, const proto::Header& header)
    : output_(output),
...
bool Recorder::Start() {
  for (const auto& channel_name : white_channels_) {
    if (std::find(black_channels_.begin(), black_channels_.end(), channel_name) != black_channels_.end()) {
  ...
  auto get_patterns_func = [](const std::vector<std::string>& channels, std::vector<std::regex>* channel_patterns) {
  ...
  std::string node_name = "autolink_recorder_record_" + std::to_string(getpid());
...
void Recorder::FindNewChannel(const RoleAttributes& role_attr) {
  ...
  auto channel_match_func = [&role_attr](const std::vector<std::string> channels,
                                         std::vector<std::regex>& patterns) -> bool {
    if (std::find(channels.begin(), channels.end(), role_attr.channel_name()) != channels.end()) {
      return true;
    }
  ...
  if (!all_channels_ && !channel_match_func(white_channels_, white_channel_patterns_)) {
  ...
  if (channel_match_func(black_channels_, black_channel_patterns_)) {
    ADEBUG << "New channel '" << role_attr.channel_name() << "' was found, but it appears in the blacklist.";
    return;
  }

  if (!writer_->WriteChannel(role_attr.channel_name(), role_attr.message_type(), role_attr.proto_desc())) {
...
bool Recorder::InitReadersImpl() {
  ...
  auto topology_callback = [this](const autolink::proto::ChangeMsg& change_msg) {
    autolink::Async([this, change_msg] { this->TopologyCallback(change_msg); });
  };
...
bool Recorder::InitReaderImpl(const std::string& channel_name, const std::string& message_type) {
  try {
    ...
    config.pending_queue_size = gflags::Int32FromEnv("AUTOLINK_PENDING_QUEUE_SIZE", 50);
...
void Recorder::ShowProgress() {
  while (is_started_ && !is_stopping_) {
    std::cout << "\r[RUNNING]  Record Time: " << std::setprecision(3) << message_time_ / 1000000000
              << "    Progress: " << channel_reader_map_.size() << " channels, " << message_count_ << " messages";
    std::cout.flush();
