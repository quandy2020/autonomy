...
#include "autolink/tools/autolink_recorder/player/play_task_consumer.hpp"
  ...
  while (!is_stopped_.load()) {
    ...
    if (base_msg_play_time_ns_ == 0) {
      ...
      if (base_msg_play_time_ns_ > begin_time_ns_) {
        sleep_ns = static_cast<uint64_t>(static_cast<double>(base_msg_play_time_ns_ - begin_time_ns_) / play_rate_);
      ...
      ADEBUG << "base_msg_play_time_ns: " << base_msg_play_time_ns_ << "base_real_time_ns: " << base_real_time_ns;
    }

    uint64_t task_interval_ns =
        static_cast<uint64_t>(static_cast<double>(task->msg_play_time_ns() - base_msg_play_time_ns_) / play_rate_);
    uint64_t real_time_interval_ns = Time::Now().ToNanosecond() - base_real_time_ns - accumulated_pause_time_ns;
