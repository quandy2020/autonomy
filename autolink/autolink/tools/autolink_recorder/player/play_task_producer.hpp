...
#include "autolink/tools/autolink_recorder/player/play_task_buffer.hpp"

namespace autolink {
namespace record {

using ::autolink::proto::RecordInfo;
...
class PlayTaskProducer {
 public:
  ...
  PlayTaskProducer(const TaskBufferPtr& task_buffer, const PlayParam& play_param, const NodePtr& node = nullptr,
 ...
 private:
  ...
  bool CreateWriters();
  bool CreatePlayTaskWriter(const std::string& channel_name, const std::string& msg_type);
  void ThreadFunc();
  ...
  // When this value is true, it means that we preload the buffer before
  // playing we use it when dv play record under nohup process,all record
  // player related to the same node,so when this value is true,we pass
  // parameter node to assign value to node_
