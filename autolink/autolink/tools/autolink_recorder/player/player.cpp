...
Player::Player(const PlayParam& play_param, const NodePtr& node, const bool preload_fill_buffer_mode)
    : is_initialized_(false), is_stopped_(true), consumer_(nullptr), producer_(nullptr), task_buffer_(nullptr) {
  task_buffer_ = std::make_shared<PlayTaskBuffer>();
  consumer_.reset(new PlayTaskConsumer(task_buffer_, play_param.play_rate));
  producer_.reset(new PlayTaskProducer(task_buffer_, play_param, node, preload_fill_buffer_mode));
}
...
bool Player::ThreadFunc_Play_Nohup() {
  ...
  const double total_progress_time_s = static_cast<double>(play_param.end_time_ns - play_param.begin_time_ns) / 1e9 +
  ...
  while (!is_stopped_.load() && autolink::OK()) {
    ...
    if (consumer_->last_played_msg_real_time_ns() > 0) {
      progress_time_s +=
          static_cast<double>(consumer_->last_played_msg_real_time_ns() - consumer_->base_msg_play_time_ns() +
                              consumer_->base_msg_real_time_ns() - producer_->play_param().begin_time_ns) /
          1e9;
    }
...
void Player::NohupPlayRecord() { nohup_play_th_.reset(new std::thread(&Player::ThreadFunc_Play_Nohup, this)); }
...
bool Player::Start() {
  ...
  while (!is_stopped_.load() && autolink::OK()) {
    ...
    double last_played_msg_real_time_s = static_cast<double>(consumer_->last_played_msg_real_time_ns()) / 1e9;
    ...
    std::cout << std::setprecision(3) << last_played_msg_real_time_s << "    Progress: " << progress_time_s << " / "
