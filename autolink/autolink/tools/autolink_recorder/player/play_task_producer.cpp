...
#include "autolink/tools/autolink_recorder/player/play_task_producer.hpp"
...
PlayTaskProducer::PlayTaskProducer(const TaskBufferPtr& task_buffer, const PlayParam& play_param, const NodePtr& node,
...
void PlayTaskProducer::Start() {
  ...
  if (preload_fill_buffer_mode_) {
    produce_th_.reset(new std::thread(&PlayTaskProducer::ThreadFuncUnderPreloadMode, this));
  } else {
...
bool PlayTaskProducer::ReadRecordInfo() {
  ...
  for (auto& file : play_param_.files_to_play) {
    ...
    for (auto& channel_name : channel_list) {
      ...
      if (!play_param_.is_play_all_channels && play_param_.channels_to_play.count(channel_name) > 0) {
        total_msg_num_ += record_reader->GetMessageNumber(channel_name);
      }
    ...
    auto begin_time_str = common::UnixSecondsToString(static_cast<int>(begin_time_s));
    auto end_time_str = common::UnixSecondsToString(static_cast<int>(end_time_s));

    std::cout << "file: " << file << ", chunk_number: " << header.chunk_number()
              << ", begin_time: " << header.begin_time() << " (" << begin_time_str << ")"
              << ", end_time: " << header.end_time() << " (" << end_time_str << ")"
              << ", message_number: " << header.message_number() << std::endl;
  }

  std::cout << "earliest_begin_time: " << earliest_begin_time_ << ", latest_end_time: " << latest_end_time_
...
bool PlayTaskProducer::UpdatePlayParam() {
  ...
  if (play_param_.start_time_s > 0) {
    play_param_.begin_time_ns =
        play_param_.base_begin_time_ns + static_cast<uint64_t>(static_cast<double>(play_param_.start_time_s) * 1e9);
  }
  ...
  if (play_param_.begin_time_ns >= play_param_.end_time_ns) {
    AERROR << "begin time are equal or larger than end time"
           << ", begin_time_ns=" << play_param_.begin_time_ns << ", end_time_ns=" << play_param_.end_time_ns;
    return false;
  }
...
void PlayTaskProducer::WriteRecordProgress(const double& curr_time_s, const double& total_time_s) {
...
void PlayTaskProducer::Reset(const double& progress_s) {
  ...
  record_viewer_ptr_ = nullptr;
  record_viewer_ptr_ = std::make_shared<RecordViewer>(record_readers_, play_param_.begin_time_ns,
                                                      play_param_.end_time_ns, play_param_.channels_to_play);
  record_viewer_ptr_->set_curr_itr(record_viewer_ptr_->begin());
}

bool PlayTaskProducer::CreatePlayTaskWriter(const std::string& channel_name, const std::string& msg_type) {
  ...
  if (writer == nullptr) {
    AERROR << "create writer failed. channel name: " << channel_name << ", message type: " << msg_type;
    return false;
  }
...
bool PlayTaskProducer::CreateWriters() {
  if (node_ == nullptr && !preload_fill_buffer_mode_) {
    std::string node_name = "autolink_recorder_play_" + std::to_string(getpid());
  ...
  return CreatePlayTaskWriter(record_info_channel, "autolink.proto.RecordInfo");
}
...
void PlayTaskProducer::FillPlayTaskBuffer() {
  ...
  for (; itr != record_viewer_ptr_->end(); ++itr) {
    ...
    auto task = std::make_shared<PlayTask>(raw_msg, search->second, itr->time, itr->time);
  ...
  while (!is_stopped_.load()) {
    ...
    while (itr != itr_end && !is_stopped_.load()) {
      while (!is_stopped_.load() && task_buffer_->Size() > preload_size) {
        std::this_thread::sleep_for(std::chrono::nanoseconds(avg_interval_time_ns));
      }
...
void PlayTaskProducer::ThreadFunc() {
  ...
  double avg_freq_hz = static_cast<double>(total_msg_num_) / (static_cast<double>(loop_time_ns) * 1e-9);
  ...
  while (!is_stopped_.load()) {
    ...
    while (itr != itr_end && !is_stopped_.load()) {
      ...
      for (; itr != itr_end && !is_stopped_.load(); ++itr) {
        ...
        auto raw_msg = std::make_shared<message::RawMessage>(itr->content);
        auto task = std::make_shared<PlayTask>(raw_msg, search->second, itr->time, itr->time + plus_time_ns);
        task_buffer_->Push(task);
      }
    }
