...
Recoverer::Recoverer(const std::string& input_file, const std::string& output_file)
...
bool Recoverer::Proc() {
  ...
  if (reader_.ReadIndex()) {
    ...
    FOR_EACH(i, 0, index.indexes_size()) {
      ...
      if (std::find(channel_vec_.begin(), channel_vec_.end(), chan_cache->name()) == channel_vec_.end()) {
  ...
  while (!reader_.EndOfFile()) {
    ...
    switch (section.type) {
      case SectionType::SECTION_CHANNEL: {
        ...
        if (std::find(channel_vec_.begin(), channel_vec_.end(), chan.name()) == channel_vec_.end()) {
