...
Spliter::Spliter(const std::string& input_file, const std::string& output_file,
                 const std::vector<std::string>& white_channels, const std::vector<std::string>& black_channels,
...
bool Spliter::Proc() {
  ...
  if (begin_time_ >= end_time_) {
    AERROR << "begin time larger or equal than end time, begin_time_: " << begin_time_ << "end_time_: " << end_time_;
    return false;
  }
  ...
  if (begin_time_ > header.end_time() || end_time_ < header.begin_time()) {
    AERROR << "time range " << begin_time_ << " to " << end_time_ << " is not include in this record file.";
    return false;
  }
    ...
    switch (section.type) {
      case SectionType::SECTION_CHANNEL: {
        ...
        if (white_channels_.empty() ||
            std::find(white_channels_.begin(), white_channels_.end(), chan.name()) != white_channels_.end()) {
          if (std::find(black_channels_.begin(), black_channels_.end(), chan.name()) == black_channels_.end()) {
      ...
      case SectionType::SECTION_CHUNK_BODY: {
        ...
        for (int idx = 0; idx < cbd.messages_size(); ++idx) {
          ...
          if (std::find(black_channels_.begin(), black_channels_.end(), cbd.messages(idx).channel_name()) !=
              black_channels_.end()) {
            continue;
          }
