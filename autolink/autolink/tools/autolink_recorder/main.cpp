  ...
  for (char option : options) {
    switch (option) {
      ...
      case 'd':
        ...
        break;
      case 'p':
        std::cout << "\t-p, --preload <seconds>\t\t\t" << command << " after trying to preload n second(s)"
                  << std::endl;
        break;
      case 'i':
      ...
      case 'h':
        ...
        break;
      case 'H':
        std::cout << "\t-H, --heap-profule\t\t\t\tprofile heap info" << std::endl;
        break;
      case 'C':
...
int main(int argc, char** argv) {
    ...
    switch (opt) {
      ...
      case 'f':
        if (!autolink::common::PathIsAbsolute(std::string(optarg))) {
          auto opt_file_abs_path = autolink::common::GetEnv("PWD") + "/" + std::string(optarg);
        ...
        for (int i = optind; i < argc; i++) {
          if (*argv[i] != '-') {
            if (!autolink::common::PathIsAbsolute(std::string(argv[i]))) {
              auto opt_file_abs_path = autolink::common::GetEnv("PWD") + "/" + std::string(argv[i]);
              if (autolink::common::PathExists(opt_file_abs_path)) {
                opt_file_vec.emplace_back(opt_file_abs_path);
              } else {
      ...
      case 'o':
        if (!autolink::common::PathIsAbsolute(std::string(optarg))) {
          auto opt_output_file_abs_path = autolink::common::GetEnv("PWD") + "/" + std::string(optarg);
      ...
      case 'r':
        ...
        break;
      case 'b':
        opt_begin = StringToUnixSeconds(std::string(optarg)) * 1000 * 1000 * 1000ULL;
        break;
      case 'e':
        opt_end = StringToUnixSeconds(std::string(optarg)) * 1000 * 1000 * 1000ULL;
        break;
      case 's':
        try {
      ...
      case 'p':
        try {
          ...
          std::cout << "Argument is out of range: -p/--preload " << std::string(optarg) << std::endl;
          return -1;
        }
        break;
      case 'i':
        try {
          ...
          std::cout << "Invalid argument: -i/--segment-interval " << std::string(optarg) << std::endl;
          return -1;
        } catch (const std::out_of_range& e) {
          std::cout << "Argument is out of range: -i/--segment-interval " << std::string(optarg) << std::endl;
          return -1;
        }
        break;
      case 'm':
        try {
          ...
          if (size_mb < 0) {
            std::cout << "Argument is less than zero: -m/--segment-size " << std::string(optarg) << std::endl;
            return -1;
          }
          ...
          std::cout << "Argument is out of range: -m/--segment-size " << std::string(optarg) << std::endl;
          return -1;
        }
        break;
      case 'h':
  ...
  if (command == "info") {
    ...
    if (!autolink::common::PathIsAbsolute(file_path)) {
      auto file_path_abs = autolink::common::GetEnv("PWD") + "/" + std::string(file_path);
  ...
  if (command == "play") {
    ...
    if (opt_output_vec.empty()) {
      std::string default_output_file =
          autolink::common::GetEnv("PWD") + "/" + UnixSecondsToString(time(nullptr), "%Y%m%d%H%M%S") + ".record";
      opt_output_vec.push_back(default_output_file);
    }
    ::autolink::Init(argv[0]);
    auto recorder =
        std::make_shared<Recorder>(opt_output_vec[0], opt_all, opt_white_channels, opt_black_channels, opt_header);
    ...
    ::autolink::Init(argv[0]);
    Spliter spliter(opt_file_vec[0], opt_output_vec[0], opt_white_channels, opt_black_channels, opt_begin, opt_end);
