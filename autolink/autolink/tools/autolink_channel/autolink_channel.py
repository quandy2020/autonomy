...
from autolink.python.autolink_py3 import autolink_time
from autolink.python.autolink_py3 import autolink   

def print_channel_type(channel_name):
    msgtype = autolink.ChannelUtils.get_msgtype(channel_name)
    print(channel_name, " type is [", msgtype, "]")


def _channel_cmd_type(argv):
    """
    Command-line parsing for 'autolink_channel type channelname' command.
    """
    args = argv[2:]
    from optparse import OptionParser
    parser = OptionParser(
        usage="usage: autolink_channel type channelname")
...
class AutolinkChannelBw(object):
...
def channel_bw(channel_name, window_size):
    rt = AutolinkChannelBw(window_size)
    node_bw = autolink.Node("listener_node_bw")
...
def _channel_cmd_bw(argv):
    """
    Command-line parsing for 'autolink_channel bw' command.
    """
    args = argv[2:]
    from optparse import OptionParser
    parser = OptionParser(
        usage="usage: autolink_channel bw channelname")
...
class AutolinkChannelHz(object):

    """
    AutolinkChannelHz receives messages for a topic and computes frequency stats
    """
    ...
    def callback_hz(self, m):
        with self.lock:
            curr_time = autolink.time.Time.now().to_sec()      
...
def channel_hz(channel_name, window_size):
    rt = AutolinkChannelHz(window_size)
    node_hz = autolink.Node("listener_node_hz")
...
def _channel_cmd_hz(argv):
    """
    Command-line parsing for 'autolink_channel hz' command.
    """
    args = argv[2:]
    from optparse import OptionParser
    parser = OptionParser(
        usage="usage: autolink_channel hz channelname")
    parser.add_option("-w", "--window",
                      dest="window_size", default=-1,
                      help="window size for calculating rate, max size is 50000")
...
def channel_info(channel_name):
    channlesinfo_dict = autolink.ChannelUtils.get_channels_info()
    time.sleep(1)
...
def _channel_cmd_info(argv):
    """
    Command-line parsing for 'autolink_channel info' command.
    """
    args = argv[2:]
    from optparse import OptionParser
    parser = OptionParser(
        usage="usage: autolink_channel info channelname ")
...
def print_channel_list():
    channels = sorted(autolink.ChannelUtils.get_channels())    
    print("The number of channels is: ", len(channels))
    for channel in channels:
        print(channel)
...
def _channel_cmd_list(argv):
    """
    Command-line parsing for 'autolink_channel list' command.
    """
    args = argv[2:]
    from optparse import OptionParser
    parser = OptionParser(
            usage="usage: autolink_channel list")
    (options, args) = parser.parse_args(args)
    if len(args) > 0:
        parser.error("param is too much")
    print_channel_list()
...
class AutolinkChannelecho(object):
    ...
    def callback(self, raw_data):
        ...
        msgtype = autolink.ChannelUtils.get_msgtype(self.channel_name, 0).decode('utf-8')
        print(autolink.ChannelUtils.get_debugstring_rawmsgdata(msgtype, raw_data).decode('utf-8'))
...
def channel_echo(channel_name):
    ...
    Reader message.
    """
    node_echo = autolink.Node("listener_node_echo")
    echo_cb = AutolinkChannelecho(channel_name)
...
def _channel_cmd_echo(argv):
    """
    Command-line parsing for 'autolink_channel echo' command.
    """
    args = argv[2:]
    from optparse import OptionParser
    parser = OptionParser(
        usage="usage: autolink_channel echo channelname")
    (options, args) = parser.parse_args(args)
    if len(args) > 1:
        parser.error("you may only specify one input channel")
    elif len(args) == 0:
...
def _printallusage():
    print("""autolink_channel is a command-line tool for printing information about Autolink Channels.
...
Type autolink_channel <command> -h for more detailed usage, e.g. 'autolink_channel echo -h'
""")
