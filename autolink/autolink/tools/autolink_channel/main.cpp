...
int main(int argc, char* argv[]) {
  ...
  if (command == "list") {
    ...
    if (argc < 3) {
      std::cerr << "usage: autolink_channel type channelname" << std::endl;
      ret = 64;
    } else if (argc > 3) {
    ...
    if (argc < 3) {
      std::cerr << "usage: autolink_channel echo channelname" << std::endl;
      ret = 64;
    } else if (argc > 3) {
      std::cerr << "you may only specify one input channel" << std::endl;
      ret = 64;
    ...
    while (bw_argc > 0 && (c = getopt_long(bw_argc, bw_argv, "w:h", bw_opts, nullptr)) != -1) {
      ...
      if (c == 'h') {
        std::cout << "usage: autolink_channel bw channelname [-w|--window N]\n";
        return 0;
      }
    }
    if (optind < bw_argc) {
    ...
    while (hz_argc > 0 && (c = getopt_long(hz_argc, hz_argv, "w:h", hz_opts, nullptr)) != -1) {
      ...
      if (c == 'h') {
        std::cout << "usage: autolink_channel hz channelname [-w|--window N]\n";
        return 0;
      }
    }
    if (optind < hz_argc) {
