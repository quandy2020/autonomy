...
namespace autolink {
namespace event {
PerfEventCache* PerfEventCache::Instance() {
    static PerfEventCache instance;
    return &instance;
}
...
void PerfEventCache::AddSchedEvent(const SchedPerf event_id,
    ...
    auto e = std::make_shared<SchedEvent>();
    ...
    event_queue_.Enqueue(std::static_pointer_cast<EventBase>(e));
}
...
void PerfEventCache::AddTransportEvent(const TransPerf event_id,
    ...
    auto e = std::make_shared<TransportEvent>();
    e->set_eid(static_cast<int>(event_id));
...
void PerfEventCache::Run() {
    EventBasePtr event;
    int buf_size = 0;
    while (!shutdown_ && !autolink::IsShutdown()) {
...
void PerfEventCache::Start() {
    auto now = Time::Now();
    std::string perf_file = "autolink_perf_" + now.ToString() + ".data";
