...
inline bool py_init(const std::string& module_name) {
    ...
    if (inited) {
        AINFO << "autolink already inited.";
        return true;
    }

    if (!Init(module_name.c_str())) {
        AERROR << "autolink::Init failed:" << module_name;
        return false;
    }
    inited = true;
    ADEBUG << "autolink init succ.";
    return true;
}
...
inline void py_shutdown() {
    return Clear();
}

inline bool py_is_shutdown() {
    return IsShutdown();
}
...
public:
    PyReader(const std::string& channel, const std::string& type, Node* node)
        ...
        if (data_type_.compare(RAWDATATYPE) == 0) {
            ...
            // Set message type for PyMessageWrap to enable proper message type
            // matching
            proto::RoleAttributes role_attr;
            role_attr.set_channel_name(channel_name_);
            role_attr.set_message_type(message::PyMessageWrap::TypeName());
            reader_ = node_->CreateReader<message::PyMessageWrap>(role_attr, f);
        } else {
...
public:
    ...
    PyClient* create_client(const std::string& service,
                            const std::string& type) {
        if (node_) {
...
public:
    ...
    // Used in autolink_channel echo command
