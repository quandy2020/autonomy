...
bool MultiValueWarehouse::Search(const RoleAttributes& target_attr, RoleAttributes* first_matched_role_attr) {
  RETURN_VAL_IF_NULL(first_matched_role_attr, false);
  RolePtr role;
...
bool MultiValueWarehouse::Search(const RoleAttributes& target_attr, std::vector<RolePtr>* matched_roles) {
  RETURN_VAL_IF_NULL(matched_roles, false);
  bool find = false;
  ReadLockGuard<AtomicRWLock> lock(rw_lock_);
  for (auto& item : roles_) {
...
bool MultiValueWarehouse::Search(const RoleAttributes& target_attr, std::vector<RoleAttributes>* matched_roles_attr) {
  RETURN_VAL_IF_NULL(matched_roles_attr, false);
  bool find = false;
  ReadLockGuard<AtomicRWLock> lock(rw_lock_);
  for (auto& item : roles_) {
