...
void Time::SleepUntil(const Time& time) {
    Time now = Now();
    if (time <= now) {
        // Already past the target time, don't sleep
        return;
    }
    // Calculate the duration to sleep
    Duration sleep_duration = time - now;
    // Use sleep_for with the calculated duration
    std::this_thread::sleep_for(
        std::chrono::nanoseconds(sleep_duration.ToNanosecond()));
}
