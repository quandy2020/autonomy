...
bool Component<M0, NullType, NullType, NullType>::Initialize(
    ...
    auto func = [self, role_attr](const std::shared_ptr<M0>& msg) {
        ...
        // TODO: Statistics methods not implemented yet
        // sampling proc latency and autolink latency in microsecond
        // uint64_t process_start_time;
        // statistics::Statistics::Instance(true)->SamplingProcLatency<uint64_t>(
        //     role_attr->channel_name(), end_time - start_time);
        // if (statistics::Statistics::Instance(true)->GetProcStatus(
        //         role_attr->channel_name(), &process_start_time) &&
        //     (start_time - process_start_time) > 0) {
        //     statistics::Statistics::Instance(true)->SamplingCyberLatency(
        //         role_attr->channel_name(), start_time - process_start_time);
        // }
        (void)end_time;
        (void)start_time;
        (void)role_attr;
        ...
        auto func = [self, blocker1,
            ...
            if (ptr) {
                if (!blocker1->IsPublishedEmpty()) {
                    ...
                    //     ->SamplingProcLatency<uint64_t>(role_attr->channel_name(),
                    //                                     end_time -
                    //                                     start_time);
                    // if
                    // (statistics::Statistics::Instance(true)->GetProcStatus(
                    //         role_attr->channel_name(), &process_start_time)
                    //         &&
                    //     (start_time - process_start_time) > 0) {
                    //     statistics::Statistics::Instance(true)
                    //         ->SamplingCyberLatency(
                    //             role_attr->channel_name(), start_time -
                    //             process_start_time);
                    // }
                    (void)end_time;
                    (void)start_time;
...
#define AUTOLINK_REGISTER_COMPONENT(name) \
    CLASS_LOADER_REGISTER_CLASS(name, autolink::ComponentBase)
