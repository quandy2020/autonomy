...
bool Init(const char* binary_name, const std::string& dag_info) {
  ...
  if (global_data->IsMockTimeMode()) {
    ...
    auto cb =
        [](const std::shared_ptr<const autolink::proto::Clock>& msg) {
          if (msg->clock() != 0) {
  ...
  if (dag_info != "") {
    ...
    if (dag_info.length() > 200) {
      ...
      dump_path = common::GetEnv(
        "AUTOLINK_ENV_WORKROOT", "/autolink") + "/dumps/" + truncated;
    } else {
      dump_path = \
        common::GetEnv("AUTOLINK_ENV_WORKROOT", "/autolink") + "/dumps/" + dag_info;
    }
