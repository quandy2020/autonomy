...
void InitLogger(const char* binary_name) {
  ...
  // Init async logger
  async_logger = new ::autolink::logger::AsyncLogger(google::base::GetLogger(FLAGS_minloglevel));
...
bool Init(const char* binary_name, const std::string& dag_info) {
  ...
  if (dag_info != "") {
    ...
    if (dag_info.length() > 200) {
      ...
      dump_path = common::GetEnv("AUTOLINK_ENV_WORKROOT", "/autolink") + "/dumps/" + truncated;
    } else {
      dump_path = common::GetEnv("AUTOLINK_ENV_WORKROOT", "/autolink") + "/dumps/" + dag_info;
    }
