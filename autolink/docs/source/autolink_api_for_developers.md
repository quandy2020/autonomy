# Autolink Cyber RT API for Developers
...
```cpp
std::unique_ptr<Node> autolink::CreateNode(const std::string& node_name, const std::string& name_space = "");
```
...
int main(int argc, char *argv[]) {
  ...
  // create talker node
  std::shared_ptr<autolink::Node> talker_node(
      autolink::CreateNode("talker"));
  // create talker
  auto talker = talker_node->CreateWriter<Chatter>("channel/chatter");
  Rate rate(1.0);
  while (autolink::OK()) {
    static uint64_t seq = 0;
    auto msg = std::make_shared<autolink::proto::Chatter>();
...
void MessageCallback(
    const std::shared_ptr<autolink::proto::Chatter>& msg) {
...
int main(int argc, char *argv[]) {
  ...
  // create listener
  auto listener =
      listener_node->CreateReader<autolink::proto::Chatter>(
...
syntax = "proto2";
package autolink.examples.proto;
message Driver {
...
int main(int argc, char* argv[]) {
  ...
  std::shared_ptr<autolink::Node> node(
      autolink::CreateNode("start_node"));
...
- Examining result: you should see content below in autolink/data/log/service.INFO
...
All parameters passed through autolink are `autolink::Parameter` objects, the table below lists the 5 supported parameter types.

| Parameter type                            | C++ data type | protobuf data type |
| :---------------------------------------- | :------------ | :----------------- |
| autolink::proto::ParamType::INT      | int64_t       | int64              |
| autolink::proto::ParamType::DOUBLE   | double        | double             |
| autolink::proto::ParamType::BOOL     | bool          | bool               |
| autolink::proto::ParamType::STRING   | std::string   | string             |
| autolink::proto::ParamType::PROTOBUF | std::string   | string             |
| autolink::proto::ParamType::NOT_SET  | -             | -                  |
...
Parameter a("int", 10);
a.Name();  // return int
a.Type();  // return autolink::proto::ParamType::INT
...
```
export GLOG_log_dir=/autolink/data/log
...
```cpp++
autolink::autolink::Init(argv[0]) is initialized.
...
**autolink_launch** is the launcher of the Cyber RT framework. It starts multiple mainboards according to the launch file, and loads different components into different mainboards according to the dag file.
...
using ::autolink::record::RecordReader;
using ::autolink::record::RecordWriter;
using ::autolink::record::RecordMessage;
using autolink::message::RawMessage;

const char CHANNEL_NAME_1[] = "/test/channel1";
const char CHANNEL_NAME_2[] = "/test/channel2";
const char MESSAGE_TYPE_1[] = "autolink.proto.Test";
const char MESSAGE_TYPE_2[] = "autolink.proto.Channel";
