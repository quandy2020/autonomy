# Autolink Cyber RT 快速开始

## 如何使用 Cyber RT 创建新的组件

Autolink 的 Cyber RT 框架是基于组件概念来构建的。每个组件都是 Cyber RT 框架的一个特定的算法模块， 处理一组输入并产生其输出
数椐。
...
下面的例子展示了如何创建、编译和运行一个组件。想更深入地探索 Cyber RT 框架，了解其各种功能，可参考`autolink/examples/`目录
下的更多示例。

_Note: 这些例子必须运行在 Autolink Docker 环境内， 且需要通过 Bazel 来编译。_

### 初始化组件的目录结构

以`autolink/examples/common_component_example/`目录下的样例程序为例：
...
- 使用`AUTOLINK_REGISTER_COMPONENT`宏定义把组件类注册成全局可用。
...
module_config {
module_library : "/autolink/bazel-bin/autolink/examples/common_component_example/libcommon_component_example.so"
components {
...
<autolink>
    <component>
        <name>common</name>
        <dag_conf>/autolink/autolink/examples/common_component_example/common.dag</dag_conf>
...
source autolink/setup.bash
export GLOG_alsologtostderr=1
/autolink/bazel-bin/autolink/examples/common_component_example/channel_test_writer
```
...
source autolink/setup.bash
export GLOG_alsologtostderr=1
/autolink/bazel-bin/autolink/examples/common_component_example/channel_prediction_writer
```
