...
function print_usage() 
{
    ...
    echo "${TAB}-c,--clean      Build image without docker cache and exit"
    echo "${TAB}-f,--dockerfile Build image with dockerfile and exit"
    echo "E.g.,"
    echo "${TAB}${prog} -f autonomy.x86_64.dockerfile"
    echo "${TAB}${prog} -c -f autonomy.x86_64.dockerfile"
}
