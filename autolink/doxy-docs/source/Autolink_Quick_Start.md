# How to Create and Run a new Component in Autolink

Apollo Autolink framework is built upon the concept of components. As the
building block of Autolink, each component is a specific algorithm module which
...
named `CommonComponentExample`. To explore more about Autolink, you can find a
couple of examples showing different functionalities of Autolink under the
`autolink/examples` directory.
...
Take the sample component under `autolink/examples/common_component_example` for
example:
...
- Register the sample component class to be globally visible using the
  `AUTOLINK_REGISTER_COMPONENT` macro.
...
```cpp
#include "autolink/examples/common_component_example/common_component_example.h"
...
module_config {
module_library : "/apollo/bazel-bin/autolink/examples/common_component_example/libcommon_component_example.so"
components {
...
<autolink>
    <component>
        <name>common</name>
        <dag_conf>/apollo/autolink/examples/common_component_example/common.dag</dag_conf>
...
```bash
autolink_launch start autolink/examples/common_component_example/common.launch
```

- Launch with the DAG file

```bash
mainboard -d autolink/examples/common_component_example/common.dag
```
...
source autolink/setup.bash
export GLOG_alsologtostderr=1
/apollo/bazel-bin/autolink/examples/common_component_example/channel_test_writer
```
...
source autolink/setup.bash
export GLOG_alsologtostderr=1
/apollo/bazel-bin/autolink/examples/common_component_example/channel_prediction_writer
```
