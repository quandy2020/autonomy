...
The core functions of Autolink are developed in C++. We also provide more python interfaces to help developers build their own utilities for specific projects.

## 2. Autolink Python Interfaces

The python interfaces of Autolink are wrapper the corresponding C++ interfaces. The implementation doesn't rely on other third-party tools, e.g. swig, which makes it easier to maintain.

## 3. Overview of Python Interfaces in Autolink
...
### 4.1 Read from Channel (in autolink/python/examples/listener.py)

```python
import sys
sys.path.append("../")
from autolink_py import autolink
...
### 4.2 Write to Channel(in autolink/python/examples/talker.py)
...
### 4.3 Read and Write Messages from/to Record File(in autolink/python/examples/record.py)
...
sys.path.append("../")
from autolink_py import autolink
from autolink_py import record
