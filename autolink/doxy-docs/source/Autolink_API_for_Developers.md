Autolink API tutorial
=============================

This document provides an extensive technical deep dive into how to create, manipulate and use Autolink's API.
...
The first part of demonstrating Autolink API is to understand the Talker/Listener example. Following are three essential concepts: node (basic unit), reader(facility to read message) and writer(facility to write message) of the example.

### Create a node

In the Autolink framework, the node is the most fundamental unit, similar to the role of a `handle`. When creating a specific functional object (writer, reader, etc.), you need to create it based on an existing node instance.
The node creation interface is as follows:

```cpp
std::unique_ptr<Node> apollo::autolink::CreateNode(const std::string& node_name, const std::string& name_space = "");
```
...
- Error Conditions - when `autolink::Init()` has not called, the system is in an uninitialized state, unable to create a node, return nullptr

### Create a writer

The writer is the basic facility used in Autolink to send messages. Every writer corresponds to a channel with a specific data type.
...
The reader is the basic facility used in autolink to receive messages. Reader has to be bound to a callback function when it is created. When a new message arrives in the channel, the callback will be called.
...
#### Talker (autolink/examples/talker.cc)

```cpp
#include "autolink/autolink.h"
#include "autolink/proto/chatter.pb.h"
#include "autolink/time/rate.h"
#include "autolink/time/time.h"
using apollo::autolink::Rate;
using apollo::autolink::Time;
using apollo::autolink::proto::Chatter;
int main(int argc, char *argv[]) {
  // init autolink framework
  apollo::autolink::Init(argv[0]);
  // create talker node
  std::shared_ptr<apollo::autolink::Node> talker_node(
      apollo::autolink::CreateNode("talker"));
  // create talker
  auto talker = talker_node->CreateWriter<Chatter>("channel/chatter");
  Rate rate(1.0);
  while (apollo::autolink::OK()) {
    static uint64_t seq = 0;
    auto msg = std::make_shared<apollo::autolink::proto::Chatter>();
...
#### Listener (autolink/examples/listener.cc)

```cpp
#include "autolink/autolink.h"
#include "autolink/proto/chatter.pb.h"
void MessageCallback(
    const std::shared_ptr<apollo::autolink::proto::Chatter>& msg) {
...
int main(int argc, char *argv[]) {
  ...
  auto listener_node = apollo::autolink::CreateNode("listener");
  // create listener
  auto listener =
      listener_node->CreateReader<apollo::autolink::proto::Chatter>(
          "channel/chatter", MessageCallback);
  apollo::autolink::WaitForShutdown();
  return 0;
}
```

#### Bazel BUILD file(autolink/samples/BUILD)
...
cc_binary(
    ...
    deps = [
        "//autolink",
        "//autolink/examples/proto:examples_cc_proto",
    ],
)
...
- Build: bazel build autolink/examples/…
- Run talker/listener in different terminals:
  - ./bazel-bin/autolink/examples/talker
  - ./bazel-bin/autolink/examples/listener
...
All messages in autolink are in the `protobuf` format. Any protobuf message with serialize/deserialize interfaces can be used as the service request and response message. `Driver` in examples.proto is used as service request and response in this example:

```protobuf
// filename: examples.proto
syntax = "proto2";
package apollo.autolink.examples.proto;
message Driver {
...
```cpp
// filename: autolink/examples/service.cc
#include "autolink/autolink.h"
#include "autolink/examples/proto/examples.pb.h"

using apollo::autolink::examples::proto::Driver;

int main(int argc, char* argv[]) {
  apollo::autolink::Init(argv[0]);
  std::shared_ptr<apollo::autolink::Node> node(
      apollo::autolink::CreateNode("start_node"));
...
- Build service/client: bazel build autolink/examples/…
- Run: ./bazel-bin/autolink/examples/service
...
All parameters passed through autolink are `apollo::autolink::Parameter` objects, the table below lists the 5 supported parameter types.

Parameter type | C++ data type | protobuf data type
:------------- | :------------- | :--------------
apollo::autolink::proto::ParamType::INT    |   int64_t |   int64
apollo::autolink::proto::ParamType::DOUBLE | double | double
apollo::autolink::proto::ParamType::BOOL   | bool |bool
apollo::autolink::proto::ParamType::STRING  | std::string | string
apollo::autolink::proto::ParamType::PROTOBUF  | std::string | string
apollo::autolink::proto::ParamType::NOT_SET | - | -
...
Parameter d("string", "autolink");
Parameter e("string", std::string("autolink"));
...
Parameter a("int", 10);
a.Name();  // return int
a.Type();  // return apollo::autolink::proto::ParamType::INT
...
```cpp
#include "autolink/autolink.h"
#include "autolink/parameter/parameter_client.h"
#include "autolink/parameter/parameter_server.h"

using apollo::autolink::Parameter;
using apollo::autolink::ParameterServer;
using apollo::autolink::ParameterClient;

int main(int argc, char** argv) {
  apollo::autolink::Init(*argv);
  std::shared_ptr<apollo::autolink::Node> node =
      apollo::autolink::CreateNode("parameter");
...
- Build: bazel build autolink/examples/…
- Run: ./bazel-bin/autolink/examples/paramserver

## Log API

### Log library

Autolink log library is built on top of glog. The following header files need to be included:

```cpp
#include "autolink/common/log.h"
#include "autolink/init.h"
```

### Log configuration

Default global config path: autolink/setup.bash
...
```cpp++
apollo::autolink::autolink::Init(argv[0]) is initialized.
...
```cpp
ADEBUG << "hello autolink.";
AINFO  << "hello autolink.";
AWARN  << "hello autolink.";
AERROR << "hello autolink.";
AFATAL << "hello autolink.";
...
The component is the base class that Autolink provides to build application modules. Each specific application module can inherit the Component class and define its own `Init` and `Proc` functions so that it can be loaded into the Autolink framework.

#### 2. Binary vs Component

There are two options to use Autolink framework for applications:

- Binary based: the application is compiled separately into a binary, which communicates with other autolink modules by creating its own `Reader` and `Writer`.
- Component based: the application is compiled into a Shared Library. By inheriting the Component class and writing the corresponding dag description file, the Autolink framework will load and run the application dynamically.
...
- **module_library**: If you want to load the .so library the root directory is the working directory of autolink (the same directory of `setup.bash`)
...
#### Common_component_example(autolink/examples/common_component_example/*)
...
#include "autolink/class_loader/class_loader.h"
#include "autolink/component/component.h"
#include "autolink/examples/proto/examples.pb.h"

using apollo::autolink::examples::proto::Driver;
using apollo::autolink::Component;
using apollo::autolink::ComponentBase;
...
AUTOLINK_REGISTER_COMPONENT(Commontestcomponent)
```

Cpp file implementation(common_component_example.cc)

```cpp
#include "autolink/examples/common_component_smaple/common_component_example.h"
...
#### Timer_component_example(autolink/examples/timer_component_example/*)
...
#include "autolink/class_loader/class_loader.h"
#include "autolink/component/component.h"
#include "autolink/component/timer_component.h"
#include "autolink/examples/proto/examples.pb.h"

using apollo::autolink::examples::proto::Driver;
using apollo::autolink::Component;
using apollo::autolink::ComponentBase;
using apollo::autolink::TimerComponent;
using apollo::autolink::Writer;
...
AUTOLINK_REGISTER_COMPONENT(TimertestComponent)
```

Cpp file implementation(timer_component_example.cc)

```cpp
#include "autolink/examples/timer_component_example/timer_component_example.h"
...
- Build: bazel build autolink/examples/timer_component_smaple/…
- Run: mainboard -d autolink/examples/timer_component_smaple/timer.dag
...
```cpp
AUTOLINK_REGISTER_COMPONENT(DriverComponent)
```
...
**autolink_launch** is the launcher of the Autolink framework. It starts multiple mainboards according to the launch file, and loads different components into different mainboards according to the dag file.
autolink_launch supports two scenarios for dynamically loading components or starting Binary programs in a child process.
...
int main(int argc, char** argv) {
    ...
    autolink::Timer timer(100, [](){
        std::cout << autolink::Time::Now() << std::endl;
    }, false);
    timer.Start()
    sleep(1);
    timer.Stop();
...
**RecordReader** is the component used to read messages in the autolink framework. Each RecordReader can open an existing record file through the `Open` method, and the thread will asynchronously read the information in the record file. The user only needs to execute ReadMessage to extract the latest message in RecordReader, and then get the message information through GetCurrentMessageChannelName, GetCurrentRawMessage, GetCurrentMessageTime.

**RecordWriter** is the component used to record messages in the autolink framework. Each RecordWriter can create a new record file through the Open method. The user only needs to execute WriteMessage and WriteChannel to write message and channel information, and the writing process is asynchronous.

### Demo - example(autolink/examples/record.cc)
...
#include "autolink/autolink.h"
#include "autolink/message/raw_message.h"
#include "autolink/proto/record.pb.h"
#include "autolink/record/record_message.h"
#include "autolink/record/record_reader.h"
#include "autolink/record/record_writer.h"

using ::apollo::autolink::record::RecordReader;
using ::apollo::autolink::record::RecordWriter;
using ::apollo::autolink::record::RecordMessage;
using apollo::autolink::message::RawMessage;

const char CHANNEL_NAME_1[] = "/test/channel1";
const char CHANNEL_NAME_2[] = "/test/channel2";
const char MESSAGE_TYPE_1[] = "apollo.autolink.proto.Test";
const char MESSAGE_TYPE_2[] = "apollo.autolink.proto.Channel";
...
- Build: bazel build autolink/examples/…
- Run: ./bazel-bin/autolink/examples/record
- Examining result:
...
auto CreateReader(const ReaderConfig& config,
    ...
    -> std::shared_ptr<autolinktron::Reader<MessageT>>;
