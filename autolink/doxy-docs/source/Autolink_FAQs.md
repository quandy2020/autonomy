...
## What is Apollo Autolink?

Apollo's Autolink is an open source runtime framework designed specifically for autonomous driving scenarios. Based on a centralized computing model, it is highly optimized for performance, latency, and data throughput
...
- The industry is evolving, so does Apollo. Going forward, Apollo has already moved from development to productization, with volume deployments in the real world, we see the demands for the highest robustness and high performance. That’s why we spent years of building Apollo Autolink, which addresses that requirements of autonomous driving solutions.
...
## Will Apollo Autolink affect regular code development?

If you have not modified anything at runtime framework layer and have only worked on Apollo's module code base, you will not be affected by the introduction of our new runtime framework as most of time you would only need to re-interface the access of the input and output data. Additional documents are under [autolink](https://github.com/ApolloAuto/apollo/tree/master/docs/autolink/) with more details.

---

## Recommended setup for Apollo Autolink
...
- Fork and clone the Apollo repo with the new framework code which can be found at [apollo/autolink](https://github.com/ApolloAuto/apollo/tree/master/autolink/)

## How to enable SHM to decrease the latency?

To decrease number of threads, the readable notification mechanism of shared memory was changed in Autolink. The default mechanism is UDP multicast, and system call(sendto) will cause some latency.

So, to decrease the latency, you can change the mechanism, The steps are listed as following:
1. update the Autolink to the latest version;
2. uncomment the transport_conf in https://github.com/ApolloAuto/apollo/blob/master/autolink/conf/autolink.pb.conf;
3. change **notifier_type** of **shm_conf** from "multicast" to "condition";
4. build Autolink with opt like `bazel build -c opt --copt=-fpic //autolink/...`;
...
The message types supported by Autolink include both serializable structured data like protobuf and raw sequence of bytes.
You can refer the sample code:
- apollo::autolink::message::RawMessage
- talker: https://github.com/gruminions/apollo/blob/record/autolink/examples/talker.cc
- listener: https://github.com/gruminions/apollo/blob/record/autolink/examples/listener.cc
...
You just need to modify `AUTOLINK_IP` of `/apollo/autolink/setup.bash`

```bash
export AUTOLINK_IP=127.0.0.1
```

Suppose you have two hosts A and B，the ip of A is `192.168.10.6`, and the ip of B is `192.168.10.7`. Then set `AUTOLINK_IP` to `192.168.10.6` on host A, and set `AUTOLINK_IP` to `192.168.10.7` on host B. Now host A can communicate with host B.
