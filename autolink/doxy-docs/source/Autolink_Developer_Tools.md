Apollo Autolink Developer Tools
==================================

Apollo Autolink framework comes with a collection of useful tools for daily development, including one visualization tool autolink_visualizer and two command line tools autolink_monitor and autolink_recorder.

*Note: apollo docker environment is required to use the tools, please follow apollo wiki to enter docker*

All the tools from Apollo Autolink rely on Apollo Autolink library, so you must source the setup.bash file for environment setup before using any Apollo Autolink tools, shown as below:

```bash
username@computername:~$: source /your-path-to-apollo-install-dir/autolink/setup.bash
```
...
`autolink_visualizer` is a visualization tool for displaying the channel data in Apollo Autolink.
...
- When data flow through channels in Apollo Autolink, the list of all channels are displayed under `ChannelNames` as seen in the figure below. For example, you can use the record tool(autolink_recorder) of Apollo Autolink to replay data from another terminal, then `autolink_visualizer` will receive information of all active channels(from replay data) and display it.
...
The command line tool `autolink_monitor` provides a clear view of the list of real time channel information Apollo Autolink in the terminal.
...
After launching the command line tool, you will notice it is similar to autolink_visualizer. It automatically collects the information of all the channels through the topology and displays them in two columns (channel name, channel data type).
...
`autolink_recorder` is a record/playback tool provided by Apollo Autolink. It provides many useful functions, including recording a record file, playing back a record file, splitting a record file, checking the information of record file and etc.
...
```bash
$ source /your-path-to-apollo-install-dir/autolink/setup.bash
$ autolink_recorder
...
`rosbag_to_record` is a tool which can convert rosbag to recorder file provided by Apollo Autolink. Now the tool support following channel:
