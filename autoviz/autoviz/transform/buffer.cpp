...
geometry_msgs::TransformStamped Buffer::ToTf2Message(
  ...
  geo_msg.header.stamp = commsgs::TimeToNanoseconds(trans.header().stamp());
...
automsgs::msgs::geometry_msgs::TransformStamped Buffer::lookupTransform(
  ...
  uint64_t tf2_time_ns = commsgs::TimeToNanoseconds(time);
...
bool Buffer::canTransform(const std::string& target_frame,
  ...
  const uint64_t requested_ns = commsgs::TimeToNanoseconds(time);
