...
TwistStampedDisplay::TwistStampedDisplay(std::string channel)
    : ChannelDisplay<automsgs::msgs::geometry_msgs::TwistStamped>(
          "TwistStamped", std::move(channel),
          "automsgs.msgs.geometry_msgs.TwistStamped") {
  setProperties({});
}

std::vector<common::DisplayPropertySpec> TwistStampedDisplay::propertySpecs()
    const {
...
void TwistStampedDisplay::processMessage(
    const automsgs::msgs::geometry_msgs::TwistStamped& message) {
  if (context_ == nullptr) {
    return;
  }
  ...
  const QVector3D local_linear = ProtoVector3(message.twist().linear());
  const QVector3D local_angular = ProtoVector3(message.twist().angular());
  try {
