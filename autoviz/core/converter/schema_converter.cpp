...
bool SchemaConverter::RegisterMessage(const google::protobuf::Descriptor* descriptor) {
  return ConvertMessageRecursive(descriptor);
}

bool SchemaConverter::RegisterMessageByName(
...
const FoxgloveMessageSchema* SchemaConverter::FindByProtoFullName(
...
std::vector<FoxgloveMessageSchema> SchemaConverter::ExportAll() const {
...
bool SchemaConverter::ConvertMessageRecursive(
  ...
  schema.foxglove_type_name = ToFoxgloveTypeName(full_name);
  ...
  for (int i = 0; i < descriptor->field_count(); ++i) {
    ...
    const FoxgloveFieldSchema field_schema = ConvertField(*field);
    if (!field_schema.message_full_name.empty()) {
      deps.insert(field_schema.message_full_name);
      ConvertMessageRecursive(field->message_type());
    }
...
FoxgloveFieldSchema SchemaConverter::ConvertField(
    const google::protobuf::FieldDescriptor& field) const {
  FoxgloveFieldSchema out;
  out.name = field.name();
  out.type = ConvertFieldType(field);
...
FoxgloveValueType SchemaConverter::ConvertFieldType(
