...
class Recorder {
 public:
  ...
  // Register topic schema/channel metadata once before writing messages.
  bool RegisterTopic(const std::string& topic,
                     const std::string& msg_type,
                     const std::string& proto_desc);

  // Write one serialized protobuf message to the topic.
  bool WriteMessage(const std::string& topic,
                    const void* data,
                    std::size_t size,
