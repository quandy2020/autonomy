...
`joy.enable: true` 且 `bluetooth_connect: true` 时，正常启动 `autodriver` 就会连接手柄。首次配对或要清掉旧绑定时，再跑一次性命令（不启传感/底盘）：
...
详见 [使用 · DualSense](usage.md#21-dualsense-遥操) 与 [配置 · joy](configuration.md#41-手柄遥操joy默认索尼-dualsenseps5)。
...
| 手柄无 `js*` / 需重新配对 | Create + PS 后启动，或 `autodriver --pair-joy` / `--pair-mode usb`；见 [使用](usage.md#21-dualsense-遥操) |
