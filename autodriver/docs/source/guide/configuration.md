...
`profile: dualsense` 时，若 `device` 打不开且 `bluetooth_connect: true`，进程会用 `bluetoothctl` 连接 DualSense（已配对则直接连接，否则扫描），连上后把速度发到 `cmd_vel`。断连会发零速并重试连接。

| YAML 键 | 默认 | 说明 |
|---|---|---|
| `enable` | `false` | `false` 时 JoyTeleop 为 no-op |
| `profile` | `dualsense` | `dualsense`/`ps5` 应用轴、速度与蓝牙预设；`generic` 不改 |
| `device` | `/dev/input/js0` | 优先打开的 `js` 节点；蓝牙连上后会改成实际节点 |
| `joy_channel` | `/joy` | Joy 消息 |
| `cmd_vel_channel` | 空 | 空则使用 `chassis.cmd_vel_channel` |
| `publish_hz` | `50` | 发布频率 |
| `max_linear` / `max_angular` | `1.5` / `1.5` | 满杆 m/s、rad/s |
| `max_linear_acc` / `max_angular_acc` | `0` / `0` | 加速度限制；`0` 为立即跟随摇杆 |
| `deadzone` | `0.05` | 摇杆死区，区外重新拉伸到满量程 |
| `require_enable` | `false` | `true` 时须按住 `enable_button` |
| `bluetooth_connect` | profile 为 `true` | 节点缺失时蓝牙连接 DualSense |
| `bluetooth_timeout_sec` | `15` | 设备列表里没有手柄时的扫描秒数 |
