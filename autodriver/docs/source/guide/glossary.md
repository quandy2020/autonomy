...
| **手柄遥操** | joy teleop | `JoyTeleop`：默认 **DualSense（PS5）** 左摇杆前后、右摇杆转向 → `/joy` + `/cmd_vel`；默认不需按住使能键 |
| **手柄配对** | `--pair-joy` | 一次性 CLI：`bluetooth`（`bluetoothctl`）或 `usb`/`driver`（`hid-playstation` + 等待 `js*`）；不启传感/底盘 |
