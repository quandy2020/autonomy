...
本文描述 `autonomy/navigator` 的逻辑架构、核心组件关系与运行时数据流。

## 5.1 设计目标

Navigator 模块遵循以下设计原则：

1. **与 nav2 对齐**：`BehaviorTreeNavigator`、`NavigatorMuxer`、BT XML 结构与 Navigation2 `nav2_bt_navigator` 保持一致
2. **编排不实现算法**：路径搜索、局部控制、定位均委托下游模块，Navigator 仅做 BT 调度
3. **配置驱动**：Lua → Protobuf 管线，`common.lua` 三模块共享帧与容差
4. **插件化 BT 节点**：52 个独立 `.so`，支持 Groot 可视化编辑与运行时 XML 替换
5. **定位韧性**：`navigate_to_pose.xml` 在 TF 丢失时进入局部生存模式（Autonomy 相对 nav2 的增强）
6. **进程内优先**：`BtActionServer` 内存 Goal/Feedback，降低 autolink 序列化开销
...
### 5.3.2 引擎层 — `BehaviorTreeNavigator<ActionT>`
...
详见 [08_bt_plugins.md](08_bt_plugins.md)。
