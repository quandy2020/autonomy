...
| 轨迹层 | 几何跟踪 + `slowdown_radius` 等**启发式 time-scaling**（非显式 $\Delta T_k$ 优化） |
...
| 轨迹层 | 固定 horizon 网格 $\{x_k,u_k\}$；离散时空轨迹 |
| 与 MPPI | 显式 NLP/QP vs 路径积分采样 |
| Autonomy | ❌ 未实现 |

## 5.7 控制器对比

| 维度 | Graceful | MPPI | RPP | DWB | MPC |
|------|----------|------|-----|-----|-----|
| 显式时间 | ❌ | 隐式 $\Delta t$ 固定 | ❌ | 隐式 | 固定网格 |
| 计算量 | 低 | 高 | 低 | 中 | 中–高 |
| 动态避障 | 中 | 强 | 弱 | 强 | 强* |
| 参数数量 | 少 | 多 | 中 | 多 | 多 |
| 路径跟踪 | 高 | 高 | 中 | 中 | 高 |
...
| NMPC | NMPC | Stopped（推荐） | Pose（若多原地转） |
...
4. **Phase 4**：按场景评估 DWB / **NMPC**；对接类任务配 StoppedGoalChecker
