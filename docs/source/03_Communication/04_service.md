...
| 本文 §4 | 相关文档 |
|---------|----------|
| RPC | [§0 指南](00_guide.md) · [§2 Node](02_node.md) · [§6 Parameter](06_parameter.md) |

---

## 4.1 设计

Service 提供**同步请求–响应 RPC**：Client 阻塞至 Server 回调填好 `resp` 或超时。与 Channel 区别：Channel 为持续流、无应答；Service 适合地图查询、一次性配置读取等短事务。长时、可取消任务用 [Action](05_action.md)。
...
<table class="comm-flow-legend">
  ...
  <tr><td>与 Channel</td><td>Channel 持续流、无应答；Service 为短事务 RPC</td></tr>
...
| 现象 | 处理 |
|------|------|
| `SendRequest` 返回 null | Server 未启动；service 名不一致；类型不匹配 |
| 阻塞过久 | 检查 Server 回调是否死锁；设置超时 |
| 同进程冲突 | Server/Client 用不同 Node 名 |

---

**导航**：[← §3 Channel](03_channel.md) · [§0 指南](00_guide.md) · [§5 Action →](05_action.md)
