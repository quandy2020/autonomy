...
> 完整数学推导见 [Planning 指南 · 数学公式](guide.md#planning-math) §6。
...
$$
L_{\text{grid}} \geq \|q_a - q_b\|_2
$$

等号仅当路径与坐标轴对齐。Theta* 通过**视线捷径**使路径逼近欧氏直线：

$$
L_{\text{Theta*}} \approx \sum_k \|q_{k+1} - q_k\|_2 \approx \|q_g - q_s\|_2
$$
...
**若 $\text{LOS}(\text{par}, n) = \text{true}$**（视线畅通）：

$$
g(n) = g(\text{par}) + w_e \| \text{par} - n \|_2 + w_t \cdot \tau(n)
$$
...
$\text{LOS}(a, b)$ 用 Bresenham 算法遍历线段上所有栅格。任一格阻塞则返回 false：

$$
\text{blocked}(i,j) \Leftrightarrow
c_{ij} \in \{253, 254\} \lor (c_{ij}=255 \land \neg\text{allow\_unknown})
$$
