...
gRPC 插件是 Bridge 模块当前**唯一可用**的传输实现，位于 `autonomy/bridge/plugins/grpc/`。基于 Cartographer 衍生的 `async_grpc` 框架，提供异步、流式 RPC 服务。

> 协议数学模型见 [03_math.md](03_math.md)；架构关系见 [05_architecture.md](05_architecture.md)。
...
## 6.4 AutonomyService 接口

完整定义见 `external_command_service.proto`。

### 6.4.1 SendNavigationCommand
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `timestamp` | `Time` | 客户端时间戳 |
| `command` | `NavigationCommand` | 指令枚举 |
| `poses` | `PoseArray` | `NAV_CMD_START` 时的目标点序列 |

**响应字段**：

| 字段 | 类型 | 说明 |
|------|------|------|
| `success` | `bool` | 本条响应是否表示成功 |
...
| 指令 | 参数 |
|------|------|
| `EXPLORATION_CMD_SET_AREA` | `geometry_msgs.Polygon area` |
| `EXPLORATION_CMD_SAVE_MAP` | `string map_name` |
...
### 6.4.3 ReceiveBotStates / ReceiveBotEvents
...
> `RobotState` / `RobotEvent` 消息体当前为空，需在 `vehicle_msgs.proto` 中补充字段后实现 Handler。
