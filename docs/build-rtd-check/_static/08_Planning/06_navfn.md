...
> 完整数学推导见 [Planning 指南 · 数学公式](03_math.md)。
...
### 6.3.1 Dijkstra 模式（`use_astar = false`）
...
当 $\phi(q_g) \geq \phi_{\mathrm{HIGH}}$ 时，在 $\|q - q_g\|_\infty \leq \varepsilon$ 内搜索最近可达点：
...
默认 $\varepsilon = 0.1\,\mathrm{m}$。$\phi_{\mathrm{HIGH}}$ 对应常量 `POT_HIGH`。
