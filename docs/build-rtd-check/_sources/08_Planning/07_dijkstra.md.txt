...
> 完整数学推导见 [Planning 指南 · 数学公式](03_math.md) §3.4。
...
NavFn 将图松弛替换为**平面波更新**（见 [NavFn 文档](06_navfn.md) §6.2.1），并用桶队列按 $\phi$ 值分桶，阈值为 $T,\; T + 2C_n,\; T + 4C_n,\; \ldots$（$C_n = 50$）。
...
| 终止 | $d(q_g)$ 确定 | $\phi(q_s) < \phi_{\mathrm{HIGH}}$ |

### 7.2.3 提前终止

`atStart=true` 时，一旦 $\phi(q_s) < \phi_{\mathrm{HIGH}}$ 即停止（`POT_HIGH`）：

$$
|\{n : \phi(n)\ \mathrm{updated}\}| \ll |V| = M \times N
$$

实际复杂度接近 $O(|V_{\mathrm{reach}}|)$，而非全图 $O(N)$。其中 $V_{\mathrm{reach}}$ 为可达区域。
...
设 $N = M \times N_{\mathrm{cells}}$，$L$ 为路径长度：
